...
Uso (PowerShell o cualquier shell):
    python cruzar_catalogo_y_mapa.py <catalogo_csv> <mapa_csv> [<carpeta_corpus>] [<salida_csv>]

Ejemplo:
    python cruzar_catalogo_y_mapa.py catalogo_v14.csv mapa_paginas.csv ..\\markdowns_v2 fallos_localizados.csv
...
Status posibles:
  ...
  ultimo_del_tomo          — último fallo del tomo, linea_fin = última línea del .md
...
def cruzar(catalogo, mapa_pagina, n_lineas_por_archivo):
    """
    ...
    for fila in catalogo:
        ...
        if pg_fin is None:
            ...
            if archivo_ini in n_lineas_por_archivo:
...
def procesar(ruta_catalogo, ruta_mapa, carpeta_corpus, salida_csv):
    ...
    # Cruce
    resultados = cruzar(catalogo, mapa_pagina, n_lineas_por_archivo)
    ...
    for status in ['ok', 'pagina_no_en_mapa', 'ultimo_del_tomo',
                   'ultimo_del_tomo_sin_fin', 'pagina_fin_no_en_mapa',
    ...
    with open(resumen_path, 'w', encoding='utf-8', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=[
            'tomo', 'total', 'ok', 'pagina_no_en_mapa',
    ...
    # CSV de huérfanos: solo los que no tienen status='ok'
    huerfanos_path = salida_csv_path.with_name(salida_csv_path.stem + '_huerfanos.csv')
    huerfanos = [r for r in resultados if r['status'] != 'ok']
...
def main(argv):
    ...
    procesar(ruta_catalogo, ruta_mapa, carpeta_corpus, salida_csv)
