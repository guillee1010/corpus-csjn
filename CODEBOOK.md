...
### Coding note — `partyWinning` / `parte_ganadora` (M20 de-interleaved axes)

> **Scope.** Full documentation of the orthogonal axes introduced in the M26 refactor (`admisibilidad`, `disposicion`, `es_revision_fondo`, `parte_ganadora`) and their inter-axis Cohen's κ reliability is deferred to the next codebook rewrite (this version still describes the pre-refactor `outcome` schema). This note records the `partyWinning` coding decision now, for transparency, because it affects the reported κ.

`parte_ganadora` follows the SCDB `partyWinning` convention: it is **binary and petitioner-centric** — the *recurrente* either obtains a favorable disposition (`recurrente_gana`) or does not (`recurrente_pierde`). Following SCDB, a **partial victory** (e.g. an "affirmed in part, reversed in part" disposition, or a reversal-and-remand) is coded as a win, and there is **no separate `parcial` value**. Reference: SCDB Online Codebook, *Winning Party*.

The M20 hand-coded validation gold initially used an informal three-value scheme (gana / parcial / pierde). To conform it to the binary target schema, the 8 cases originally tagged `parcial` were **re-read blind against the source ruling** (the case `.md`, not the parser output) and recoded: genuine partial victories → `recurrente_gana`; cases with no favorable element to the *recurrente* that had been loosely tagged `parcial` → `recurrente_pierde` (a correction under the binary rule). The direction was determined by the procedural role and the operative disposition read from the text, independently of the pipeline's prediction. Coding is restricted to merit-review cases (`es_revision_fondo = "si"`), consistent with the axis being undefined off the merits.

This recoding raises the `parte_ganadora` κ relative to a three-value treatment, because cases the prediction structurally cannot match (the binary pipeline has no `parcial`) are removed as a schema artifact rather than counted as a detector failure. Two safeguards keep the change auditable: (a) the original three-value gold (`planilla_M20_codificar-56xlsx.xlsx`) is retained in the repository, so the recoding is fully inspectable and reproducible against the source; (b) the κ remains capped below 1.0 by **7 residual role-inversions** — cases where the operative verb favors one party but the *recurrente* stands on the opposite side — which are preserved as genuine disagreements, confirming that the recoding conformed the schema without bending the gold toward the pipeline. When the de-interleaved reliability table is added, κ will be reported both ways (three-value and SCDB-binary) so the effect of the collapse is visible.
