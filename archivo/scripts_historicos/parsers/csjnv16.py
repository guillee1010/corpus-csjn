"""
CSJN Fallos Parser v16
======================
Cambios respecto a v15:
  1. Fix de extracción de fecha. v15 buscaba la fecha solo en las primeras
     8 líneas del bloque. Como en v14/v15 el bloque arranca con sumarios y
     dictamen del Procurador, "Buenos Aires, [fecha]" del fallo cae mucho
     más adelante (tras "FALLO DE LA CORTE SUPREMA"). El resultado: solo
     5.6% de los fallos tenían fecha extraída en v15.

     v16 busca la fecha así:
       (a) Si se detectó marcador clásico de apertura: busca en las 10
           líneas siguientes al marcador (ahí está la fecha real del fallo).
       (b) Si no hay marcador: busca la ÚLTIMA fecha "Buenos Aires, ..."
           del bloque, excluyendo líneas marcadas como dictamen — la última
           fecha fuera del dictamen es la del fallo.
       (c) Fallback: la primera fecha del bloque, igual que antes.

  Toda la lógica analítica restante (detección de fin real, dispositivo,
  firmas, votos, originaria, outcomes) es idéntica a v15. Solo cambia la
  extracción de fecha.

Uso:
  python csjnv16.py --localizados fallos_localizados.csv \
                    --mapa mapa_paginas.csv \
                    --corpus ../markdowns_v2 \
                    --output csjn_casos_v16.csv

Compatibilidad: schema idéntico a v15 (mismas columnas).
"""
