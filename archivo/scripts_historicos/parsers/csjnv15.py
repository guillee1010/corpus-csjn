"""
CSJN Fallos Parser v15
======================
Cambios respecto a v14:
  1. Detección de FIN REAL del fallo (no solo del bloque del catálogo).
     v14 usaba el linea_fin del catálogo como fin del bloque, pero el
     contenido decisorio del fallo X puede:
       (a) terminar antes (caso "Migraciones": el bloque del catálogo
           cubre múltiples fallos).
       (b) terminar después (caso "Carmona": la firma de la Corte cae en
           la página que el catálogo asignó al fallo siguiente).
     v15 detecta la frontera con el fallo siguiente usando pistas en
     cascada (carátula del siguiente, header de sumario nuevo, marcador
     de apertura, firma del actual) y búsqueda bidireccional alrededor
     del linea_fin del catálogo.

  2. NUEVA columna linea_fin_real: línea donde termina el contenido
     decisorio (incluyendo todas las disidencias y concurrencias).
     linea_fin (la del catálogo) se mantiene como referencia.

  3. NUEVA columna status_fin ∈ {fin_dentro_bloque,
     fin_extendido_pag_compartida, fin_por_firma_actual, fin_no_detectado}.
     Indica si la frontera quedó dentro del bloque del catálogo, se
     extendió a la página compartida con el siguiente, etc.

  4. NUEVA columna pista_fin ∈ {caratula_siguiente, sumario_siguiente,
     marcador_apertura_siguiente, firma_actual, fallback_catalogo}.
     Identifica qué pista permitió detectar la frontera.

  5. word_count, wc_mayoria, wc_votos, wc_considerando se calculan ahora
     hasta linea_fin_real, NO hasta linea_fin del catálogo. Esto elimina
     la contaminación por basura editorial al final del bloque.

  6. extraer_textos_votos recorta el último voto en linea_fin_real, no en
     len(bloque). El texto de la última disidencia/concurrencia ya no se
     contamina con sumarios del fallo siguiente — crítico para análisis
     comparativo de votos.

Toda la lógica analítica interna (detección de dispositivo, classify_outcome,
parse_firma, originaria, etc.) es idéntica a v14.

Uso:
  python csjnv15.py --localizados fallos_localizados.csv \
                    --corpus ../markdowns_v2 \
                    --mapa mapa_paginas.csv \
                    --output csjn_casos_v15.csv

Compatibilidad: schema mayormente compatible con v14 más cuatro columnas
nuevas (linea_fin_real, status_fin, pista_fin, en casos; reusa
caso_id_canonico en votos). Análisis previos que usan v14 pueden migrarse
sin cambios — los nombres de columnas existentes mantienen su semántica
(salvo word_count y wc_*, que ahora son del cuerpo decisorio puro).
"""
...
def main():
    ap = argparse.ArgumentParser(description="CSJN Fallos Parser v15 (con mapa + fin real)")
    ...
    ap.add_argument("--output", default="csjn_casos_v15.csv",
