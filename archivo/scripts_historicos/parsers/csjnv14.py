"""
CSJN Fallos Parser v14
======================
Cambios respecto a v12:
  1. Reemplazo de la detección de inicios de fallo. v12 buscaba la línea
     literal "FALLO DE LA CORTE SUPREMA" o "SENTENCIA DE LA CORTE SUPREMA"
     como marcador de apertura. Si el marcador estaba ausente o roto por
     OCR, el fallo entero se perdía; si el siguiente fallo tampoco tenía
     marcador detectado, el actual se "comía" el cuerpo del siguiente.

     v14 toma los inicios y fines de cada fallo desde fallos_localizados.csv
     (producto del cruce catalogo + mapa de páginas). El catálogo editorial
     define qué fallos hay y dónde están; el parser ya no decide eso.

  2. caso_id pasa a ser caso_id_canonico del catálogo (formato 329_p5).
     Estable entre corridas, vinculado al catálogo. v12 usaba tomo_lineaApertura,
     que cambiaba si el formato del .md se regeneraba.

  3. NUEVA columna case_name_indice — la carátula tal como la indexó el
     editor (puede contener varias variantes separadas por "|"). El
     case_name del cuerpo (que el parser extrae heurísticamente) sigue
     existiendo como case_name_cuerpo, para comparación.

  4. NUEVA columna status_localizacion ∈ {ok, pagina_fin_no_en_mapa,
     fallo_cruza_archivos, ultimo_del_tomo, sin_marcador_apertura}.
     Indica con qué precisión están definidos los límites del bloque.

  5. Los fallos del catálogo con status='pagina_no_en_mapa' (no localizables
     en el corpus de .md) NO se procesan — quedan registrados solo en el
     catálogo pero no aparecen en el output del parser.

  6. apertura_tipo conserva su semántica (fallo / sentencia / vacío) pero
     ya no es requisito para procesar el fallo. Los fallos sin marcador
     clásico ahora se procesan igual y quedan con apertura_tipo="" y
     status_localizacion incluyendo "sin_marcador_apertura".

Toda la lógica analítica interna (dispositivo, classify_outcome, parse_firma,
extracción de votos, clasificación de tipo de voto, originaria, etc.) es
idéntica a v12. Solo cambia cómo se determina el bloque de cada caso.

Uso:
  python csjnv14.py --localizados fallos_localizados.csv \
                    --corpus ../markdowns_v2 \
                    --output csjn_casos_v14.csv

Compatibilidad: schema mayoritariamente compatible con v12, salvo:
  - caso_id ahora es caso_id_canonico (formato distinto)
  - se agregan case_name_indice y status_localizacion
  - se elimina line_apertura (ya no es la coordenada de identificación)
  - se agregan linea_inicio y linea_fin (coordenadas físicas en el .md)
"""
...
def detectar_apertura_en_bloque(bloque):
    """
    v14: busca el marcador clásico FALLO/SENTENCIA DE LA CORTE SUPREMA en las
    primeras líneas del bloque. Devuelve (apertura_tipo, apertura_idx_relativo)
    donde apertura_idx_relativo es 0-indexado dentro del bloque, o
    (None, None) si no encuentra marcador.

    Para bloques del catálogo, la apertura del fallo suele estar dentro de las
    primeras 30-50 líneas. Buscamos hasta 80 por seguridad.
    """
    horizonte = min(len(bloque), 80)
    for k in range(horizonte):
        m = RE_APERTURA.match(bloque[k].strip())
        if m:
...
def procesar_archivo(filepath, fallos_del_archivo):
    """
    v14: procesa un archivo .md y devuelve dos listas (casos, votos).
    ...
    El parser ya no busca aperturas: toma cada (linea_inicio, linea_fin)
    como bloque predefinido. apertura_tipo se detecta dentro del bloque
    para mantener la columna, pero su ausencia no descarta el fallo.
    """
    ...
    for fallo_meta in fallos_del_archivo:
        ...
        # Fecha
        fecha_str = ""
        for k, ln in enumerate(bloque[:8]):
...
def main():
    ap = argparse.ArgumentParser(description="CSJN Fallos Parser v14 (con mapa)")
    ...
    ap.add_argument("--output", default="csjn_casos_v14.csv",
    ...
    for filepath in sorted(grupos.keys(), key=lambda p: p.name):
        ...
        try:
            casos, votos, descon = procesar_archivo(filepath, fallos_arch)
    ...
    if all_casos:
        fieldnames = [
            ...
            "source_file", "linea_inicio", "linea_fin", "status_localizacion",
        ]
