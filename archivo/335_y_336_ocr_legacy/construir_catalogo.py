...
__version__ = "1.01"  # H080: ruta de índice general tipo 336 (Tesseract)
...
# ============================================================================
# ÍNDICE GENERAL TIPO 336 (H080)
# ----------------------------------------------------------------------------
# Tomos digitalizados con Tesseract (336) no traen el índice canónico
# "INDICE POR LOS NOMBRES DE LAS PARTES". Solo hay un índice general alfabético
# por carátula, encabezado por "Índice"/"Indice" suelto, sin ancla ": p.".
# Dos sub-formatos según el volumen:
#   - trailing: "Carátula. 448"             (página = entero final, puede wrapear)
#   - leaders:  "Carátula s/ tipo. ... 1477" (página tras líderes de puntos)
# Solo se usa como fallback si no se detectó el índice canónico. Cero regresión.
# ============================================================================

RE_HEADER_INDICE_336 = re.compile(r'^[ií]ndice$', re.IGNORECASE)
RE_RUNNING_HDR_336 = re.compile(r'FALLOS DE LA CORTE SUPREMA', re.IGNORECASE)
RE_ANCLA_336_LEADERS = re.compile(r'\.{2,}\s*(\d{1,4})(?=\s|$)')
RE_ANCLA_336_TRAILING = re.compile(r'(.*?)[.\s]\s*(\d{1,4})\s*$')


def _localizar_indice_336(lines):
    """Índice 0-based del header 'Índice'/'Indice' del índice general (mitad
    final del archivo), o None."""
    n = len(lines)
    cand = [i for i, l in enumerate(lines)
            if RE_HEADER_INDICE_336.match(l.strip()) and i > n * 0.5]
    return cand[0] if cand else None


def _bloque_indice_336(lines, inicio_0):
    """Líneas del índice general, desde después del header hasta la próxima
    sección de índice canónica o EOF."""
    fin = len(lines)
    for i in range(inicio_0 + 1, len(lines)):
        if any(rx.match(lines[i].strip()) for rx in RE_HEADERS_SECCION.values()):
            fin = i
            break
    return lines[inicio_0 + 1:fin]


def _parsear_336_leaders(bloque):
    """Sub-formato con líderes de puntos. Página = entero tras la corrida."""
    utiles = []
    for raw in bloque:
        l = raw.strip()
        if not l:
        ...
        if RE_CABECERA_ALFA.match(l):
            continue
        if RE_RUNNING_HDR_336.search(l):
            continue
        if re.fullmatch(r'\d{1,4}', l):   # número suelto = running header impreso
            continue
        utiles.append(l)
    texto = ' '.join(utiles)
    entradas = []
    pos = 0
    for m in RE_ANCLA_336_LEADERS.finditer(texto):
        car = limpiar_caratula(texto[pos:m.start()].rstrip(' .'))
        if len(car) > 3:
            entradas.append((car, [int(m.group(1))]))
        pos = m.end()
    return entradas


def _parsear_336_trailing(bloque):
    """Sub-formato sin líderes. Página = entero final del bloque de entrada,
    que puede wrapear a la línea siguiente."""
    entradas = []
    acc = []
    for raw in bloque:
        l = raw.strip()
        if not l or RE_CABECERA_ALFA.match(l) or RE_RUNNING_HDR_336.search(l):
            continue
        if re.fullmatch(r'\d{1,4}', l):           # página de una entrada wrapeada
            if acc:
                entradas.append((limpiar_caratula(' '.join(acc)), [int(l)]))
                acc = []
            continue
        m = RE_ANCLA_336_TRAILING.search(l)
        if m and re.search(r'[A-Za-zÁÉÍÓÚÑáéíóúñ]', m.group(1)):
            car = limpiar_caratula((' '.join(acc) + ' ' + m.group(1)).strip())
            entradas.append((car, [int(m.group(2))]))
            acc = []
        else:
            acc.append(l)
    return entradas
...
def parsear_indice_336(lines):
    """Fallback 336. Retorna (entradas, inicio_0) con entradas=[(caratula,
    [paginas])], o ([], None) si no hay índice tipo 336."""
    inicio_0 = _localizar_indice_336(lines)
    if inicio_0 is None:
        return ([], None)
    bloque = _bloque_indice_336(lines, inicio_0)
    n_lead = sum(1 for l in bloque if re.search(r'\.{3,}', l))
    if n_lead > len(bloque) * 0.2:
        return (_parsear_336_leaders(bloque), inicio_0)
    return (_parsear_336_trailing(bloque), inicio_0)
...
def procesar_archivo(path, verbose=False):
    ...
    if rango_nombres is None:
        # Fallback H080: índice general tipo 336 (Tesseract, sin header canónico)
        entradas_336, inicio_336 = parsear_indice_336(lines)
        if entradas_336:
            if verbose:
                print(f"  [336] {filename}: {len(entradas_336)} entradas vía índice "
                      f"general (L{inicio_336 + 1})", file=sys.stderr)
            secciones = secciones + [('nombres', inicio_336 + 1, len(lines))]
            return (tomo, secciones, entradas_336)
        if verbose:
