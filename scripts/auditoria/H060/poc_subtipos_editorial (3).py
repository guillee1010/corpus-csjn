...
def detectar_fin_toc(lines, pos_ig, linea_fin):
    """
    Encuentra la última línea de indice_general buscando hacia atrás
    el último marcador fuerte de TOC/fin-de-archivo.

    Marcadores fuertes: HOJA COMPLEMENTARIA, líneas con puntos
    suspensivos (entries del TOC), headers de volumen, "INDICE GENERAL".
    NO incluye números sueltos ni líneas vacías (esos aparecen también
    en contenido no-TOC como el acumulativo de 330.4).
    """
    ...
    for i in range(pos_ig, min(linea_fin + 1, len(lines))):
        ...
        if not s:
            continue
        # Marcadores fuertes: inequívocamente TOC o fin de archivo
        if (RE_TOC_DOTS.search(s)
                or RE_TOC_HOJA.match(s)
