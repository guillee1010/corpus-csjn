...
RE_TOC_HOJA = re.compile(r"^(?:HOJA COMPLEMENTARIA|Hoja incorporada|SE TERMINO)", re.I)
...
def detectar_fin_toc(lines, pos_ig, linea_fin):
    """
    Desde la posición de INDICE GENERAL, encuentra dónde termina el TOC.
    Retorna la última línea que es parte del TOC.
    Criterio: 3+ líneas consecutivas no-TOC = el TOC ya terminó.
    """
    consecutivas_no_toc = 0
    ultima_toc = pos_ig

    for i in range(pos_ig, min(linea_fin + 1, len(lines))):
        if _es_linea_toc(lines[i]):
            consecutivas_no_toc = 0
            ultima_toc = i
        else:
            consecutivas_no_toc += 1
            if consecutivas_no_toc >= 3:
                return ultima_toc
...
def clasificar_editorial(lines, linea_ini, linea_fin, tomo, source_file):
    ...
    for idx, (li, subtipo) in enumerate(boundaries):
        ...
        # Truncar indice_general: solo el TOC, no el acumulativo
        if subtipo == "indice_general":
            lf_toc = detectar_fin_toc(lines, li, lf)
            if lf_toc < lf:
                descartado = lf - lf_toc
                if descartado > 100:
                    print(f"  TRUNCADO {source_file}: indice_general truncado "
                          f"en L{lf_toc} ({descartado} líneas descartadas "
                          f"= probable acumulativo)")
                lf = lf_toc
