...
RE_TOC_DOTS = re.compile(r"\.{4,}\s*$")              # "Fallos ........" (puntos al final)
RE_TOC_PAGE = re.compile(                             # "5", "(1)", "(VII)"
    r"^\(?(?:\d+|[IVXLCDM]+)\)?\s*$")
...
def detectar_fin_toc(lines, pos_ig, linea_fin):
    ...
    """
    # Buscar la última línea con patrón de puntos
    ultima_dots = None
    ...
    # Sin puntos (Era 2) → no truncar, todo es TOC
    if ultima_dots is None:
        return linea_fin

    # Con puntos → incluir la línea de página que sigue
    fin = ultima_dots
