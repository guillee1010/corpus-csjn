#!/usr/bin/env python3
"""
PoC B070 v3 — Validación de NO-texto-corriente en Pista 1 forward.

Fix: cuando Pista 1 forward encuentra un match, verificar que la línea
NO sea texto corriente (continuación de oración). Es texto corriente si:
  (a) empieza con minúscula, O
  (b) la línea anterior (no vacía, no page header) termina con guión, O
  (c) la línea anterior (no vacía, no page header) no termina en punto/
      dos puntos/cierre de paréntesis y la línea actual empieza con 
      minúscula o artículo/preposición.

Si es texto corriente, seguir buscando el próximo match.

Uso:
  python scripts/auditoria/H048/poc_b070_v3.py
"""
...
def _es_texto_corriente(lines, k):
    ...
    # (b) Línea anterior termina con guión → palabra partida
    if prev_line.endswith('-'):
        return True

    # (c) Línea anterior no termina en cierre de oración → continuación
    cierre = prev_line[-1] if prev_line else ''
    if cierre not in ('.', ':', ')', '"', ';'):
        return True
...
def detectar_fin_real_b070v3(lines, linea_inicio, linea_fin_catalogo,
...
P.detectar_fin_real = detectar_fin_real_b070v3
...
pd.DataFrame(diff_rows).to_csv(out_dir / "poc_b070v3_diff.csv", index=False, encoding="utf-8")
