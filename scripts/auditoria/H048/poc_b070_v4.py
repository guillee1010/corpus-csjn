#!/usr/bin/env python3
"""
PoC B070 v4 — Validación de NO-texto-corriente en Pista 1 forward.

Fix: cuando Pista 1 forward encuentra un match, verificar que la línea
NO sea texto corriente. Es texto corriente si:
  (a) empieza con minúscula (continuación de oración), O
  (b) la línea anterior significativa termina con guión (palabra partida).

Condición (c) de v3 (anterior no termina en cierre) eliminada: causaba
13 regresiones porque carátulas pueden seguir a firmas sin punto.

Uso:
  python scripts/auditoria/H048/poc_b070_v4.py
"""
...
def detectar_fin_real_b070v4(lines, linea_inicio, linea_fin_catalogo,
...
P.detectar_fin_real = detectar_fin_real_b070v4
...
pd.DataFrame(diff_rows).to_csv(out_dir / "poc_b070v4_diff.csv", index=False, encoding="utf-8")
