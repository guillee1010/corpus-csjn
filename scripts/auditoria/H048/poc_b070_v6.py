#!/usr/bin/env python3
"""
PoC B070 v6 — Pista 1 forward: validación texto-corriente + normalización tildes.

Dos fixes combinados:
1. B070: cuando Pista 1 forward encuentra un match, verificar que la línea
   NO sea texto corriente:
   (a) empieza con minúscula (excepto "c/" y "s/"), O
   (b) línea anterior termina con word-split genuino (letra + guión).
2. B071: normalizar tildes en el regex de Pista 1 para que "Administración"
   matchee "ADMINISTRACION" en carátulas ALL CAPS.

Uso:
  python scripts/auditoria/H048/poc_b070_v6.py
"""
...
# ── Helper: normalizar tildes ────────────────────────────────────────────

def _strip_accents(s):
    """á->a, é->e, ñ->n, etc. Para matching tilde-insensitive."""
    nfkd = _unicodedata.normalize('NFKD', s)
    return ''.join(c for c in nfkd if _unicodedata.category(c) != 'Mn')
...
def detectar_fin_real_b070v6(lines, linea_inicio, linea_fin_catalogo,
    ...
    # ── Pista 1 con validación no-texto-corriente + tildes ────────────────
    if primer_token_siguiente and len(primer_token_siguiente) >= 5:
        token_norm = _strip_accents(primer_token_siguiente)
        pat = _re.compile(r"\b" + _re.escape(token_norm) + r"\b", _re.I)

        desde = lfc + 1
        while desde <= limite_adelante:
            # Buscar match con tildes normalizadas
            k = None
...
P.detectar_fin_real = detectar_fin_real_b070v6
...
pd.DataFrame(diff_rows).to_csv(out_dir / "poc_b070v6_diff.csv", index=False, encoding="utf-8")
