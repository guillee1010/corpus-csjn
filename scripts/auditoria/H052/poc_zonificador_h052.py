"""
PoC H052 — Re-medición concordancia dictamen (fix type mismatch H051).

El PoC H051 comparaba dictamen_presente del CSV contra "1", pero parser.py
escribe Python bool → el CSV tiene "True"/"False". La concordancia de 41.4%
era un artefacto del type mismatch, no una discrepancia real.

Este script es el mismo PoC H051 con la comparación corregida:
  caso.get("dictamen_presente","") == "1"  →  str(...).lower() in ("1","true")

Uso (PowerShell, desde raíz del repo):
  python scripts/auditoria/H052/poc_zonificador_h052.py --todos
  python scripts/auditoria/H052/poc_zonificador_h052.py --caso 329_p1554
  python scripts/auditoria/H052/poc_zonificador_h052.py --verbose

Salida:
  stdout: resumen de concordancia corregido
  scripts/auditoria/H052/zonificacion_h052.csv
  scripts/auditoria/H052/zonificacion_h052_reporte.md
"""
...
OUTPUT_CSV = Path(__file__).resolve().parent / "zonificacion_h052.csv"
OUTPUT_MD = Path(__file__).resolve().parent / "zonificacion_h052_reporte.md"
...
def comparar_con_parser(caso, zonas):
    ...
    # H052 fix: parser.py escribe bool (True/False) al CSV, no "1"/"0".
    parser_dictamen = str(caso.get("dictamen_presente", "")).lower() in ("1", "true")
...
def main():
    ...
    print("=" * 70)
    print("PoC H052 — Concordancia dictamen corregida (fix type mismatch)")
    print("=" * 70)
    ...
    for caso in muestra:
        ...
        if args.verbose:
            ...
            print(f"  Parser: firma={caso['voting_pattern']}"
                  ...
                  f"  dictamen={'sí' if str(caso.get('dictamen_presente','')).lower() in ('1','true') else 'no'}")
    ...
    md = []
    md.append(f"# Zonificación H052 — Reporte (fix concordancia dictamen)\n")
    ...
    # ── H052: Diagnóstico de 4 cuadrantes para dictamen ───────────────
    md.append(f"\n## Dictamen: 4 cuadrantes\n")
    def _parser_dice_dictamen(r):
        return str(r["parser_dictamen"]).lower() in ("1", "true")
    q_ambos_si   = [r for r in resultados if     _parser_dice_dictamen(r) and     r["zonif_dictamen"]]
    q_ambos_no   = [r for r in resultados if not _parser_dice_dictamen(r) and not r["zonif_dictamen"]]
    q_zonif_only = [r for r in resultados if not _parser_dice_dictamen(r) and     r["zonif_dictamen"]]
    q_parser_only= [r for r in resultados if     _parser_dice_dictamen(r) and not r["zonif_dictamen"]]
    md.append(f"| Cuadrante | N | % |")
    md.append(f"|---|---|---|")
    total = len(resultados) if resultados else 1
    for label, lst in [("ambos_sí (parser=True, zonif=True)", q_ambos_si),
                        ("ambos_no (parser=False, zonif=False)", q_ambos_no),
                        ("zonif_only (parser=False, zonif=True)", q_zonif_only),
                        ("parser_only (parser=True, zonif=False)", q_parser_only)]:
        md.append(f"| {label} | {len(lst)} | {100*len(lst)/total:.1f}% |")
    md.append("")
    if q_zonif_only:
        md.append(f"### Muestra zonif_only (primeros 20)\n")
        for r in q_zonif_only[:20]:
            md.append(f"- `{r['caso_id']}` (t{r['tomo']}, span={r['span']}, "
                      f"n_dictamen={r['n_dictamen']})")
        md.append("")
    if q_parser_only:
        md.append(f"### Muestra parser_only (primeros 20)\n")
        for r in q_parser_only[:20]:
            md.append(f"- `{r['caso_id']}` (t{r['tomo']}, span={r['span']}, "
                      f"n_dictamen={r['n_dictamen']})")
        md.append("")
    ...
    print(f"  Dictamen cuadrantes — ambos_sí: {len(q_ambos_si)}"
          f"  ambos_no: {len(q_ambos_no)}"
          f"  zonif_only: {len(q_zonif_only)}"
          f"  parser_only: {len(q_parser_only)}")
