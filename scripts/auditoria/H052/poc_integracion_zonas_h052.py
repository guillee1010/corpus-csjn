"""
PoC H052 — Integración de zonas + CSV zona-centered.

Valida la integración del zonificador como fuente canónica de fronteras
de zona. Genera `csjn_casos_zonas.csv` (tercer artefacto canónico) y
compara `wc_dictamen` derivado de las zonas vs el valor del parser.

Algoritmo:
  1. Carga csjn_casos.csv para referencia (wc_dictamen del parser).
  2. Reconstruye cada bloque (misma lógica que procesar_caso).
  3. Corre zonificar_bloque() → (zonas: list[str], anclas: list[tuple]).
  4. Extrae segmentos contiguos de cada zona con linea_ini/linea_fin.
  5. Calcula wc por segmento.
  6. Compara wc_dictamen_zonas vs wc_dictamen_parser.
  7. Exporta csjn_casos_zonas.csv + reporte de concordancia.

Uso (PowerShell, desde raíz del repo):
  python scripts/auditoria/H052/poc_integracion_zonas_h052.py

Salida:
  scripts/auditoria/H052/csjn_casos_zonas_h052.csv
  scripts/auditoria/H052/integracion_zonas_h052_reporte.md
...
OUTPUT_ZONAS = OUTPUT_DIR / "csjn_casos_zonas_h052.csv"
OUTPUT_MD = OUTPUT_DIR / "integracion_zonas_h052_reporte.md"
...
def main():
    print("=" * 70)
    print("PoC H052 — Integración zonas + CSV zona-centered")
    print("=" * 70)
    ...
    # ── Escribir reporte ──────────────────────────────────────────────
    md = []
    md.append("# H052 — Integración zonas: reporte\n")
