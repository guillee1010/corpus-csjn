...
MATERIA_CSV = REPO_ROOT / "output" / "parser" / "csjn_casos_materia.csv"
...
def load_casos():
    ...
    # materia (sidecar H112): left-join 1:1 por caso_id_canonico. Graceful: si el
    # sidecar no existe, el explorador sigue andando sin las columnas de materia.
...
def render_sidebar(df_full: pd.DataFrame):
    ...
    with sb.expander("🏛️ Proceso / jurisdicción", expanded=False):
        ...
        df = _multiselect(st, df, "materia", "Materia (capa 1)", "f_materia")
        df = _multiselect(st, df, "materia_capa",
                          "Cobertura de materia (capa)", "f_materia_capa")
