...
PLAN  = sys.argv[1] if len(sys.argv)>1 else "planilla_M20_codificar.xlsx"
CLAVE = sys.argv[2] if len(sys.argv)>2 else "M20_clave_parser_n300.csv"
Z = 1.959963985

def leer(path):
    """Lee .xlsx/.xlsm o .csv (csv tolera BOM de Excel 'CSV UTF-8')."""
    if path.lower().endswith((".xlsx",".xlsm")):
        try:
            return pd.read_excel(path, dtype=str).fillna("")
        except ImportError:
            sys.exit("Falta openpyxl para leer .xlsx. Corré:  pip install openpyxl\n"
                     "O en Excel: Guardar como → CSV UTF-8 y pasá ese .csv como argumento.")
