#!/usr/bin/env python3
"""
pipeline_h038_forward_firma.py — Forward + validación de firma

Lógica: buscar dispositivo forward (como hoy), pero solo aceptar un
match si tiene linea_es_firma_de_juez en las 40 líneas siguientes.
Si no tiene firma, guardar como fallback y seguir buscando.
Si ningún match tiene firma, usar el primero (= comportamiento actual).

Correr:
  $env:PYTHONIOENCODING = "utf-8"
  python scripts/diagnostico/H038/pipeline_h038_forward_firma.py
"""
...
def buscar_dispositivo_forward_firma(bloque, apertura_rel, lineas_dictamen, inicio_votos_indiv):
    """
    Forward con validación de firma:
    - Primer match CON firma en 40 líneas gana.
    - Si ninguno tiene firma, usa el primero (fallback = comportamiento actual).
    """
    ...
    for k in range(inicio, fin):
        ...
        if es_disp:
            candidate_text = extraer_por_ello_text(bloque, k)
            # Guardar primer match como fallback
            if fallback_idx is None:
                fallback_idx = k
                fallback_text = candidate_text
            # ¿Tiene firma en las próximas 40 líneas?
            tiene_firma = any(
                linea_es_firma_de_juez(bloque[j])
                for j in range(k + 1, min(k + 41, len(bloque)))
            )
            if tiene_firma:
                return k, tipo_disp, candidate_text, "forward_con_firma"

    # Ningún match con firma → fallback al primero
    if fallback_idx is not None:
        return fallback_idx, None, fallback_text, "fallback_sin_firma"
...
def main():
    ...
    for source_file, casos in sorted(por_archivo.items()):
        ...
        for caso in casos:
            ...
            new_idx, new_tipo, new_pet, metodo = buscar_dispositivo_forward_firma(
    ...
    csv_path = OUTPUT_DIR / "forward_firma_cambios.csv"
    if cambios:
    ...
    resumen_path = OUTPUT_DIR / "forward_firma_resumen.txt"
    with open(resumen_path, "w", encoding="utf-8") as f:
        ...
        w("=" * 70)
        w("H038 — Forward + validación firma (full 5702)")
