#!/usr/bin/env python3
"""
pipeline_h038_reversa_firma.py — Reversa desde votos + validación de firma

Lógica:
  - Con votos post-apertura: reversa desde inicio_votos_indiv hacia
    apertura_rel. Solo acepta match si hay linea_es_firma_de_juez en
    las 40 líneas siguientes. Si ningún match tiene firma, fallback
    al primer match forward (= comportamiento actual, sin regresión).
  - Sin votos: forward (comportamiento actual).

Correr:
  $env:PYTHONIOENCODING = "utf-8"
  python scripts/diagnostico/H038/pipeline_h038_reversa_firma.py
"""
...
def tiene_firma_cerca(bloque, k, ventana=40):
    """¿Hay firma de juez en las próximas `ventana` líneas?"""
    for j in range(k + 1, min(k + ventana + 1, len(bloque))):
        if linea_es_firma_de_juez(bloque[j]):
            return True
    return False


def buscar_dispositivo(bloque, apertura_rel, lineas_dictamen, inicio_votos_indiv):
    """
    Estrategia combinada:
    1. Con votos post-apertura: reversa desde votos, acepta solo con firma.
       Si ninguno tiene firma → fallback al primer match forward.
    2. Sin votos: forward (= comportamiento actual).
    """
    ...
    if votos_post_apertura:
        # ── REVERSA desde votos, con validación de firma ──
        for k in range(inicio_votos_indiv - 1, inicio - 1, -1):
            ...
            if es_disp and tiene_firma_cerca(bloque, k):
                return k, tipo_disp, extraer_por_ello_text(bloque, k), "reversa_con_firma"

        # Fallback: primer match forward (comportamiento actual, sin regresión)
        ...
        for k in range(inicio, fin):
            ...
            es_disp, tipo_disp = detectar_apertura_dispositivo(stripped)
            if es_disp:
                return k, tipo_disp, extraer_por_ello_text(bloque, k), "fallback_forward"
...
def main():
    ...
    for source_file, casos in sorted(por_archivo.items()):
        ...
        for caso in casos:
            ...
            new_idx, new_tipo, new_pet, metodo = buscar_dispositivo(
    ...
    csv_path = OUTPUT_DIR / "reversa_firma_cambios.csv"
    if cambios:
    ...
    resumen_path = OUTPUT_DIR / "reversa_firma_resumen.txt"
    with open(resumen_path, "w", encoding="utf-8") as f:
        ...
        w("=" * 70)
        w("H038 — Reversa desde votos + validación firma")
        ...
        # Fallbacks: ¿cuántos cayeron al forward por no tener firma?
        fallbacks = [c for c in cambios if c["metodo"] == "fallback_forward"]
        w(f"\n── Fallbacks al forward: {metodo_stats.get('fallback_forward', 0)} total ──")
        if fallbacks:
            w(f"  De esos, con cambio: {len(fallbacks)}")
            fb_tipos = Counter(c["tipo_cambio"] for c in fallbacks)
            for k, v in fb_tipos.most_common():
                w(f"    {k}: {v}")
