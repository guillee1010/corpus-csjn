...
# Extraer apellidos de JUECES_CONOCIDOS para clasificación de firmas
# con apellido solo (sin nombre de pila)
APELLIDOS_JUECES = set()
for _pat, nombre in JUECES_CONOCIDOS:
    apellido = nombre.split("(")[0].strip().split()[-1].lower()
    APELLIDOS_JUECES.add(apellido)
...
def clasificar_linea(linea):
    """Clasifica una línea que matcheó en una función pero no en la otra."""
    s = linea.strip()
    if linea_es_firma_de_juez(linea):
        return "firma_juez"
    # Firma con apellido solo (sin nombre de pila) + calificador
    # Cubre "ARGIBAY (en disidencia).", "LORENZETTI (según su voto).", etc.
    if RE_CALIFICADOR.search(s):
        return "firma_calificador"
    # Apellido de juez solo en línea corta (firma sin calificador)
    s_lower = s.lower()
    for apellido in APELLIDOS_JUECES:
        if apellido in s_lower and len(s) <= 100:
            return "firma_apellido_solo"
    if es_header_pagina(linea):
        return "header_pagina"
    if es_marcador_apertura(linea):
        return "marcador_apertura"
    # Carátula: tiene "c/" o "s/" o "|"
    if "c/" in s or "s/" in s or "|" in s:
        return "posible_caratula"
    # Línea mayúsculas pura corta
...
def diagnostico_linea_por_linea():
    ...
    clasificacion_solo_auditor = Counter()
    # Detalle para CSV
    detalle_rows = []
    detalle_auditor_rows = []
        ...
        for fallo in fallos:
            ...
            for k in range(mitad_bloque, lfc + 1):
                ...
                if mp and ma:
                    ...
                    cls = clasificar_linea(linea)
                    clasificacion_solo_auditor[cls] += 1
                    if len(detalle_auditor_rows) < 500:  # cap para no explotar
                        detalle_auditor_rows.append({
                            "caso_id": fallo["caso_id"],
                            "tomo": fallo["tomo"],
                            "linea_abs": k,
                            "zona": "atras",
            ...
            for k in range(lfc + 1, limite_adelante + 1):
                ...
                if mp and ma:
                    ...
                    cls = clasificar_linea(linea)
                    clasificacion_solo_auditor[cls] += 1
                    if len(detalle_auditor_rows) < 500:
                        detalle_auditor_rows.append({
                            "caso_id": fallo["caso_id"],
                            "tomo": fallo["tomo"],
                            "linea_abs": k,
    ...
    if clasificacion_solo_auditor:
        print("Clasificación de 'solo auditor' (40K líneas):")
        print(f"  {'tipo':<25} {'count':>7}")
        print("  " + "-" * 35)
        for cls, cnt in clasificacion_solo_auditor.most_common():
            print(f"  {cls:<25} {cnt:>7}")
    print()

    # ── Guardar detalle CSV (ambas fuentes) ──────────────────────────────────
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    all_detail = detalle_rows + detalle_auditor_rows
    out_path = OUT_DIR / "pista2_sumario_detalle.csv"
    if all_detail:
        with open(out_path, "w", encoding="utf-8", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=[
                "caso_id", "tomo", "linea_abs", "zona", "fuente",
                "clasificacion", "texto"
            ])
            writer.writeheader()
            writer.writerows(all_detail)
        print(f"Detalle guardado en: {out_path}")
        print(f"  ({len(detalle_rows)} solo-parser + {len(detalle_auditor_rows)} solo-auditor [cap 500])")
    else:
        print("Sin divergencias detectadas (nada que guardar).")

    # ── Muestra de las primeras 20 líneas solo-parser ────────────────────────
    if detalle_rows:
        print()
        print("Muestra solo-parser (primeras 20):")
        print("-" * 70)
        for row in detalle_rows[:20]:
            print(f"  [{row['clasificacion']:<20}] T{row['tomo']} L{row['linea_abs']:>5} | {row['texto'][:70]}")

    # ── Muestra de las primeras 20 líneas solo-auditor ─────────────────────
    if detalle_auditor_rows:
        print()
        print("Muestra solo-auditor (primeras 20):")
        print("-" * 70)
        for row in detalle_auditor_rows[:20]:
            print(f"  [{row['clasificacion']:<20}] T{row['tomo']} L{row['linea_abs']:>5} | {row['texto'][:70]}")
