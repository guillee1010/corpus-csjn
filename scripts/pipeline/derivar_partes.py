...
# B145 (v0.18): gramática v2 portada de RE_PIE_START (extraer_epilogos v0.4 /
# parser 26.1) — verbos ensanchados (inte\w*rpu\w+ OCR / presentad / articulad /
# fundad), tolerancia «y fundado» y «por:» (dos puntos, formato viejo, el grueso:
# 35/50 del flip-set). «por» sigue ADYACENTE al verbo (el «por libre» de la rama
# 5 quedó DIFERIDO: mueve el borde de captura; ver DEUDA sub-clase «a favor de X
# por»). GUARD «contra» pre-verbo (tempered dot): divergencia DELIBERADA respecto
# de RE_PIE_START — acá el marcador CAPTURA el clause y el FP lo corrompe
# (testigo 340_p437, cita de queja OIT «Queja contra el Gobierno ... presentada
# por el Sindicato», sub-clase M44 «contra pre-verbo»); superficie del guard
# medida corpus-wide en el PoC = exactamente 1 línea (el FP), 0 matches genuinos
# afectados.
RE_MARK_REC = re.compile(
    r"^(?:Recursos?|Queja)\b(?:(?!\bcontra\b)[^\n])*?"
    r"(?:inte\w*rpu\w+|deducid\w+|presentad\w+|articulad\w+|fundad\w+)"
    r"(?:\s+y\s+fundad\w+)?\s+por[:\s]\s*"
