...
from clasificador_disposicion import es_de_fondo   # B136 (H169): mérito de la originaria (fuente única, compartida con el deriver)
...
def procesar_archivo(filepath, fallos_del_archivo, headers_archivo, primer_token_por_caso, siguiente_caso):
    ...
    for fallo_meta in fallos_del_archivo:
        ...
        # B136 (H169): la originaria resuelve la demanda directamente (no revisa un
        # inferior) → su mérito NO se lee del outcome apelativo sino del verbo que
        # resuelve la demanda (es_de_fondo, clasificador_disposicion). Reemplaza el
        # hard-0 de B119 #1. Mismo detector que es_revision_fondo del deriver → los
        # ejes coinciden en la originaria. No-originaria: sin cambio de comportamiento.
        if is_originaria:
            is_merit = int(es_de_fondo(considerando_text, por_ello_text))
        else:
