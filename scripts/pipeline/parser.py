...
RE_DISPOSITIVO_VARIANTES = [
    ...
    # ── H203 (B166 fix parcial): fórmula de cierre estricta de 344_p1102 ──
    # Superficie corpus-wide medida (superficie_b166, H203): forma amplia
    # «^En orden a lo» = 16 matches, 14 argumental/dictamen («opino que»,
    # «entiendo que V.E. puede desestimar») → MUERTA; forma estricta = 2/2 TP
    # (dispositivos de mayoría + voto Rosatti de 344_p1102, leídos en extracto).
    # INCONDICIONAL: el performativo wrapea a la línea siguiente y el resolutor
    # no peekeaba (constancia H194; desde H207 el resolutor SÍ peekea vía
    # _barrer, pero la variante queda incondicional: 2/2 TP y 0 FP corpus-wide
    # medidos en H203 — un perf-gate no compra precisión y re-abriría 1102).
    ("en_orden_expuesto", re.compile(r"^En orden a lo que hasta aqu[íi] se ha expuesto\b", re.I)),
]
...
# H207 (B166 b1-A): el peek de cola_wrap llega TAMBIEN desde el resolutor
# (_barrer lo computa y los candidatos estructurales lo forwardean) — el
# residual H194 «los wrapeados recuperan ZONA, no outcome» queda CERRADO;
# flip-set sellado 4 = 4 TP (poc_b166_b1a_peek v0.1, H207).
...
def detectar_apertura_dispositivo(stripped_line, cola_wrap="", zonif=False):
    ...
    H207 (B166 b1-A): el peek llega TAMBIEN desde el resolutor — _barrer
    computa cola_wrap y los candidatos estructurales la forwardean (cierra
    el residual H194 «los wrapeados recuperan ZONA, no outcome»). Con
    cola_wrap="" el performativo debe estar en la MISMA linea.
...
# B168 (H216): vocabulario ensanchado a las clases selladas del universo
# H216 (b168_universo_sellado.csv, 61 casos). Clase E (header partido por
# wrap ANTES del cargo) queda FUERA por diseño: el tercer grupo se exige
# en la MISMA linea. Calibracion poc_b168_vocab v0.1: 40/40 clases A-D+F,
# 0 FP nuevos, 0 perdidas de las 2836 formas ya cubiertas.
RE_VOTO_HDR  = re.compile(
    r"^(Voto(\s+Concurrente)?|Ampliaci[oó]n\s+de\s+Fundamentos)\s+"
    r"(del?|de\s+el|de\s+l[ao]s?|la|el|los|las)\s+"
    r"(Se[nñ]or(?:es|as|a)?|Vicepresident[ea]s?|President[ea]s?|Ministr[ao]s?)",
    re.I
)
RE_DISID_HDR = re.compile(
    r"^Disidencia\s+(Parcial\s+)?"
    r"(del?|de\s+el|de\s+l[ao]s?|la|el|los|las)\s+"
    r"(Se[nñ]or(?:es|as|a)?|Vicepresident[ea]s?|President[ea]s?|Ministr[ao]s?)",
    re.I
)

# B168-E (H217): match de header con PEEK DE WRAP. La clase E del universo
# sellado trunca el header ANTES del cargo («DISIDENCIA PARCIAL DE LA ⏎
# SEÑORA…»); ningun consumidor peekeaba. Discriminador = el JOIN contra las
# regex reales (superficie_b168e v0.1: 0 FP corpus-wide — los candidatos
# no-header no joinean; headers completos intactos, el peek ni corre).
# Cola estricta k+1, espejo H207. Devuelve ("voto"|"disidencia", via_wrap)
# o None.
def _hdr_tipo_wrap(stripped, cola_wrap=None):
    if RE_VOTO_HDR.match(stripped):
        return ("voto", False)
    if RE_DISID_HDR.match(stripped):
        return ("disidencia", False)
    if cola_wrap:
        joined = stripped + " " + cola_wrap
        if RE_VOTO_HDR.match(joined):
            return ("voto", True)
        if RE_DISID_HDR.match(joined):
            return ("disidencia", True)
    return None
...
# H219 (frente C, Z_caption_anidado): el cierre de la caratula se busca por
# CLASE PAR de la comilla de apertura, no por "cualquier comilla del set".
# Causa raiz: con caption anidado (convencion de ACUMULADOS: cada causa entre
# comillas simples dentro del caption en dobles — 5 tomos, no es OCR; el OCR
# agrega ruido ENCIMA: cierre doble impreso como apertura en 330_p2414, simple
# de cierre perdida en 337_p1006, doble duplicada en 339_p841, apostrofos
# impresos como \u2019 en "D\u2019 Alessandro") el search viejo encontraba la
# comilla simple en posicion 0 -> acumulado[:0] == "" y la formula perfecta
# devolvia nada. La clase doble tolera la \u201C swapeada como cierre; el
# guard de captura-no-vacia cura la duplicada.
_COMILLAS_DOBLES  = '\u201C\u201D"'
_COMILLAS_SIMPLES = "\u2018\u2019'"

# Recorte a PRIMERA CAUSA (decision del operador H219: el caso es la causa
# lider, como lo denomina la SJ de la CSJN — "Cherquis" y no el tren de 5
# acumuladas). GATEADO: solo corre si la captura ARRANCA con comilla, cosa
# imposible en la rama v1 sana publicada (si arrancara con comilla, el codigo
# previo devolvia "" y la fila era Z) -> la rama sana queda bit a bit intacta.
# Corte: cierre simple seguido de separador; fallback a separador + patron de
# expediente (viejo "D.459.XLVIII" / nuevo "CIV 111084/2008") para la simple
# que el OCR perdio (337_p1006).
_RE_CORTE_CAUSA = re.compile(r"[\u2019']\s*(?:;|,|\u201D|\"|$|\s+y\s)")
_RE_SEP_EXPTE = re.compile(
    r"\s*(?:;|\s+y)\s+(?:el\s+recurso\s+\w+\s+)?"
    r"(?:[A-Z]\.\s?\d+\.\s?[CLXVI]+|[A-Z]{2,4}\s?\d+/\d+)")


def _acotar_primera_causa(cap):
    """H219: post-proc gateado del retorno de V1. Solo actua si la captura
    arranca con comilla (firma del acumulado / comilla duplicada); la rama
    v1 sana retorna intacta."""
    if not cap or cap[0] not in COMILLAS_CIERRE:
        return cap
    if cap[0] in _COMILLAS_SIMPLES:
        cuerpo = cap[1:]
        cortes = [m.start() for m in (_RE_CORTE_CAUSA.search(cuerpo),
                                      _RE_SEP_EXPTE.search(cuerpo)) if m]
        corte = min(cortes) if cortes else len(cuerpo)
        return cuerpo[:corte].strip(COMILLAS_CIERRE + ' ')
    return cap.strip(COMILLAS_CIERRE + ' ')


def _extraer_caratula_v1_legado(bloque, apertura_rel, max_lineas=8):
    """Comportamiento pre-H219 VERBATIM. Queda como FALLBACK del extractor
    nuevo (patron guard-con-fallback D2/H201): cuando no aparece cierre de
    CLASE en la ventana, se devuelve exactamente lo que devolvia v42.0 —
    todo flip del fix tiene, por construccion, un cierre genuino."""
...
def extraer_caratula_v1(bloque, apertura_rel, max_lineas=8):
    """
    v18: extrae la caratula desde el patron V1 (`Vistos los autos: "X"`).
    Itera el bloque desde apertura_rel hacia adelante y reconstruye la
    caratula hasta cerrar la comilla, manejando wrap por silabacion.

    H219 (v42.1): el cierre es el de la CLASE PAR de la apertura — las
    comillas simples anidadas de los acumulados y los apostrofos impresos
    como \u2019 se vuelven invisibles — con guard de captura no vacia y
    fallback al comportamiento viejo verbatim si no hay cierre de clase en
    la ventana. Espec y flip-set sellados: poc_z_caption v0.2 (H219/,
    baseline poc_z_caption_baseline.csv, 73 = 58 reparaciones + 15 Z).
    Devuelve la caratula (primera causa si el caption es acumulado), o ""
    si no hay V1.
    """
    inicio = apertura_rel if apertura_rel is not None else 0
    for i in range(inicio, len(bloque)):
        ...
        if not m:
            continue
        comilla_apert = m.group(1)
        cierre = (_COMILLAS_DOBLES if comilla_apert in _COMILLAS_DOBLES
                  else _COMILLAS_SIMPLES)
        pos_apertura = linea.index(comilla_apert)
        acumulado = linea[pos_apertura + 1:].rstrip()
        # cierre en la misma linea: primer cierre DE CLASE con captura no
        # vacia (salta la comilla duplicada en posicion 0)
        for mm in re.finditer(f'[{re.escape(cierre)}]', acumulado):
            if mm.start() > 0:
                return _acotar_primera_causa(acumulado[:mm.start()])
        # wrap: trigger y rfind DE CLASE
        for j in range(i + 1, min(i + max_lineas, len(bloque))):
            sig = bloque[j].rstrip()
            if acumulado.endswith('\u00AD') or acumulado.endswith('-'):
                acumulado = acumulado.rstrip('\u00AD-') + sig.lstrip()
                ...
                acumulado = acumulado + ' ' + sig.lstrip()
            if re.search(f'[{re.escape(cierre)}]', sig):
                pos_cierre = max(acumulado.rfind(c) for c in cierre)
                if pos_cierre > 0:
                    return _acotar_primera_causa(acumulado[:pos_cierre])
        # sin cierre de clase en la ventana -> comportamiento viejo VERBATIM
        return _extraer_caratula_v1_legado(bloque, apertura_rel, max_lineas)
    return ""
...
# B168/F (H216): calificador «(con) ampliación de fundamentos» — consumo
# SOLO en parse_firma (panel). RE_CALIFICADOR de arriba queda INTACTA:
# tiene consumidores en frontera y nucleo B148 cuyo radio no se midio;
# la unificacion queda como deuda (H216).
RE_CALIFICADOR_FIRMA = re.compile(
    r"\(\s*(en\s+disidencia|seg[úu]n\s+su\s+voto|por\s+su\s+voto"
    r"|(?:con\s+)?ampliaci[oó]n\s+de\s+fundamentos)"
    r"(\s+parcial)?\s*\)",
    re.I
)
...
def collect_firma_lines(bloque, idx_start, max_lines=None):
    ...
    for k in range(idx_start, techo):
        ...
        if RE_PAGE_HEADER.match(line) or line.startswith("Recurso de"):
            break
        _cola_b168e = bloque[k + 1].strip() if k + 1 < techo else None
        if _hdr_tipo_wrap(line, _cola_b168e):
            break
...
def parse_firma(firma_raw):
    ...
    #    (ventanas sobre la lista POSICIONAL completa — el dedup va DESPUÉS:
    #    dedup temprano extendería la ventana del juez anterior a través del
    #    match borrado y robaría el calificador del duplicado. Espec:
    #    poc_b168_dedup v0.1, H217, flip-set sellado {329_p2063}.)
    for i, (ini, fin, nombre) in enumerate(matches_dedup):
        ...
        cm = RE_CALIFICADOR_FIRMA.search(ventana)   # B168/F (H216)
        calificador = None
        if cm:
            ...
            elif "ampliaci" in cal_text:
                # B168/F (H216): ampliacion de fundamentos = concurrencia
                calificador = "según su voto"
    ...
    # 2b. B168-dedup (H217): dedup por NOMBRE preservando orden y PRIMERA
    #     entrada (calificador incluido — decisión del operador, PROMPT_H217).
    #     Cierra n_jueces = len(jueces) contando duplicados posicionales
    #     (único caso post-v39: 329_p2063, Fayt×2/Zaffaroni×2). panel_cprima
    #     ya dedupeaba para su gate (nj_dedup) → selección c′ invariante.
    _vistos = set()
    _dedup_out = []
    for j in jueces_out:
        if j["nombre"] in _vistos:
            continue
        _vistos.add(j["nombre"])
        _dedup_out.append(j)
    jueces_out = _dedup_out
    ...
    # Flags desde la lista FINAL deduped (consistencia posiciones↔voting_pattern:
    # un calificador de entrada descartada no deja huella en vp — edge vacío
    # hoy, candado VP 0 flips corpus-wide, poc_b168_dedup v0.1).
    has_voto  = any(j["calificador"] in ("según su voto", "por su voto")
                    for j in jueces_out)
    has_disid = any(j["calificador"] == "en disidencia" for j in jueces_out)
...
# ── B166 (c) fix (H205): consumidor del panel por evidencia de zonas ─────────

_MIN_JUECES_PANEL = 2

def panel_cprima(bloque, zonas_linea):
    """B166 (c) — H205: consumidor c′ de la evidencia de zonas para el panel.

    El zonificador etiqueta la firma temprana del fallo (firma_linea, Pasada 1)
    pero el camino del panel era forward-only desde el pick del dispositivo
    (collect_firma_lines(pick+1)): cuando el pick cae en un voto/disidencia o
    la firma colectiva esta POST voto concurrente (344_p441), el panel
    publicado era el del voto, no el del caso.

    Regla c′ (H204, predicado VERBATIM de poc_b166_c_regla v0.2; espec del
    cableado = poc_b166_c_replica v0.1, H205): primera zona 'firma' (run
    contiguo de zonas_linea, post-relabel A1) con inicio estrictamente
    posterior al primer indice de zona 'apertura' (-1 si no hay apertura),
    cuyo collect_firma_lines(bloque, zona_ini) parsea PANEL: nj deduped por
    nombre >= _MIN_JUECES_PANEL, o algun calificador. El filtro post-apertura
    mata las firmas del bleed/residuo del caso anterior (defecto v0.1 del PoC
    H204, 11 casos medidos).

    Devuelve (firma_raw, firma_parsed) o (None, None) si ninguna candidata
    califica — el caller cae al comportamiento previo (pick+1 / A001) VERBATIM.
    """
    ap_ini = next((k for k, z in enumerate(zonas_linea) if z == "apertura"), -1)
    n = len(zonas_linea)
    for k in range(n):
        if zonas_linea[k] != "firma":
            continue
        if k > 0 and zonas_linea[k - 1] == "firma":
            continue  # no es inicio de run
        if k <= ap_ini:
            continue
        raw = collect_firma_lines(bloque, k)
        if not raw:
            continue
        parsed = parse_firma(raw)
        nj_dedup = len({j["nombre"] for j in parsed["jueces"]})
        tiene_cal = any(j["calificador"] for j in parsed["jueces"])
        if nj_dedup >= _MIN_JUECES_PANEL or tiene_cal:
            return raw, parsed
    return None, None
...
def detectar_votos_disidencias(bloque, lineas_excluir):
    ...
    for k, bl in enumerate(bloque):
        ...
        _cola_b168e = (bloque[k + 1].strip() if k + 1 < len(bloque) else None)
        _ht = _hdr_tipo_wrap(stripped, _cola_b168e)
        if _ht:
            tipo = _ht[0]
            if tipo == "voto":
                n_votos_svoto += 1
...
def detectar_fin_real(lines, linea_inicio, linea_fin_catalogo,
    ...
    v34 (B147/D2, H201): Pista 1 con guard-con-fallback — prefiere el primer
    match del token CON forma de carátula (_es_caratula_v2 ∧ no-firma); si
    ningún candidato de la ventana tiene forma, ancla en el primer match a
    secas (= v33.0, sin caída de pista inducida). Ver comentario in situ.
    """
    n = len(lines)
    ...
    if (primer_token_siguiente and len(primer_token_siguiente) >= 5
        ...
        #
        # B147/D2 (H201): guard-con-fallback — primer-match-CON-forma.
        # k_plain = primer match que pasa los guards viejos (= ancla v33.0);
        # k_forma = primer match que ADEMÁS no es firma (incondicional) y
        # pasa _es_caratula_v2 (fuente única D1). Ancla = k_forma si existe,
        # si no k_plain (comportamiento v33.0 INTACTO). El rechazo DURO quedó
        # MUERTO con dato (H201, 6 testigos .md): cuando ningún candidato
        # tiene forma, la caída a Pistas 3/4/firma_actual amputa contenido
        # PROPIO (341_p868/339_p599: Pista 4 ancla el FALLO propio;
        # 334_p763: firma de mayoría amputa el voto de Highton; 343_p2135 y
        # ~10 hermanos: epílogo huérfano). Causa de esos FP: carátula genuina
        # wrapeada con conector en línea 2 (geometría D1b) o mixta
        # institucional. Espec = poc_b147_d2_v02.py VERBATIM (H201/).
        desde = lfc + 1
        k_plain = None
        k_forma = None
        while desde <= limite_adelante:
            ...
            # (Bajo D2 queda subsumida por el no-firma de k_forma, pero
            # sigue filtrando al candidato k_plain — se conserva verbatim.)
            ...
            if k_plain is None:
                k_plain = k
            if (not linea_es_firma_de_juez(lines[k])
                    and _es_caratula_v2(lines[k])):
                k_forma = k
                break
            desde = k + 1
        ancla_p1 = k_forma if k_forma is not None else k_plain
        if ancla_p1 is not None:
            return (retroceder_frontera(lines, li, ancla_p1 - 1, tokens_nombre_propio),
...
def zonificar_bloque(bloque):
    ...
    for k in range(n):
        ...
        _cola_b168e = (bloque[k + 1].strip() if k + 1 < len(bloque) else None)
        if _hdr_tipo_wrap(s, _cola_b168e):
            _en_sumario = False
...
def _cand_estructural(stripped, cola_wrap=""):
    """Tiers 1 y 3: apertura estructural (detectar_apertura_dispositivo).
    H207 (B166 b1-A): recibe cola_wrap (linea siguiente stripeada, la pasa
    _barrer) y la forwardea — las perf-variantes toleran el performativo
    wrapeado tambien en el RESOLUTOR (cierra el residual H194)."""
    es_disp, _ = detectar_apertura_dispositivo(stripped, cola_wrap)
    return es_disp


def _cand_t2(stripped, cola_wrap=""):
    ...
    cola_wrap se acepta por uniformidad de interfaz (H207) y se ignora."""
    for pat in _T2_PATS:
...
def _cand_t3b(stripped, cola_wrap=""):
    """Tier 3b: apertura estructural + guarda argumental propia (_T3B_ARG_RE).
    H207 (B166 b1-A): forwardea cola_wrap a detectar_apertura_dispositivo
    (mismo peek que _cand_estructural)."""
    es_disp, _ = detectar_apertura_dispositivo(stripped, cola_wrap)
    if not es_disp:
        return False
...
def _cand_t4(stripped, cola_wrap=""):
...
def _barrer(bloque, rango, lineas_dictamen, *,
    ...
    for k in range(inicio, fin):
        ...
        if not stripped:
            continue
        # H207 (B166 b1-A): peek de wrap — espejo VERBATIM del call-site del
        # zonificador (Pasada 1): cola_wrap = linea siguiente stripeada. Las
        # perf-variantes (RE_DISPOSITIVO_VARIANTES_PERF) del resolutor toleran
        # asi el performativo wrapeado por el OCR («En virtud de lo expuesto,
        # … por ⏎ …se hace lugar»). Monotono por construccion: solo AGREGA
        # candidatos (el resto de linea ya estaba en la cola), nunca quita.
        # Flip-set sellado 4 = 4 TP (poc_b166_b1a_peek v0.1, H207; testigos
        # H206 344_p575/340_p1940/347_p2160 + recuperacion 344_p2779).
        cola_wrap = bloque[k + 1].strip() if k + 1 < len(bloque) else ""
        if not es_candidato(stripped, cola_wrap):
            continue
...
def procesar_archivo(filepath, fallos_del_archivo, headers_archivo, primer_token_por_caso, siguiente_caso):
    ...
    for fallo_meta in fallos_del_archivo:
        ...
        # B172 (H218): el `or` se ELIMINA. case_name_cuerpo = V1 y punto.
        #
        # find_case_name escanea HACIA ATRAS desde la apertura, o sea hacia
        # adentro del dictamen, y devuelve la primera linea que contenga la
        # cadena "c/" (o "s/" en el segundo tramo) SIN ningun guard de forma.
        # Lo que vive ahi son las CITAS de precedentes y causas conexas: en la
        # rama medida (1160 casos, superficie_c_v1.csv H218/) el 44,6% de los
        # strings pertenece a OTRO fallo. El campo pasa a significar «autos
        # completos cuando el fallo los trae con la formula ritual» —
        # auto-descriptivo y sin ruido. El identificador lo sigue dando
        # case_name_indice, con 0 vacias en las 5894.
        #
        # Lo que la rama NO compra, medido: 0 filas con «Recurso de hecho
        # deducido por» (imposible por construccion — si estuviera, V1 habria
        # disparado) · 0,0% de RE_CARAT_QUEJA · el indice cubre MAS conector
        # c/ que el fallback (54,7% vs 43,6%) · derivar_partes rinde 1 fila
        # sobre 1160 (347_p357, unico costo declarado del ciclo).
        #
        # case_name_cuerpo_fuente: columna de procedencia. Antes la rama solo
        # era reconstruible por el ACCIDENTE `cuerpo == legacy`, particion que
        # H218 midio CONTAMINADA: 11 de esas 1171 filas eran V1 que coincidia
        # con el fallback (el dictamen habla de la propia causa). Sin esta
        # columna la rama no es auditable.
        #
        # case_name_cuerpo_legacy SE CONSERVA: su borrado es decision propia
        # y posterior — hoy es lo unico que deja leer QUE agarro el fallback.
        if apertura_rel is not None:
            case_name_cuerpo_legacy = find_case_name(lines, apertura_idx)
            case_name_cuerpo_v1 = extraer_caratula_v1(bloque, apertura_rel)
            case_name_cuerpo = case_name_cuerpo_v1
            if case_name_cuerpo_v1:
                case_name_cuerpo_fuente = "v1"
            elif case_name_cuerpo_legacy:
                case_name_cuerpo_fuente = "fallback"
            else:
                case_name_cuerpo_fuente = "vacio"
        else:
            case_name_cuerpo_legacy = ""
        ...
        # ── B166 (c) fix (H205): consumidor c′ de la evidencia de zonas ──
        # Corre ANTES del camino forward-only e INDEPENDIENTE de por_ello_idx
        # (los sin_firma->panel del flip-set no tienen pick util). _zonas_linea
        # llega post-relabel A1 (L4089). Si c′ no encuentra candidata, el
        # bloque de abajo corre VERBATIM v35.0 — cero regresion por
        # construccion. Flip-set sellado 34: poc_b166_c_replica_out.csv (H205).
        _cp_raw, _cp_parsed = panel_cprima(bloque, _zonas_linea)
        if _cp_raw is not None:
            firma_raw = _cp_raw
            firma_parsed = _cp_parsed
        ...
        # ── M58 paso 1 (H211): texto del dictamen para textos.csv ────────
        # Misma fuente que dictamen_presente/wc_dictamen (lineas_dictamen,
        # H052, derivado del zonificador post-relabel). Convención de armado
        # VERBATIM de extraer_considerando: strip, drop vacías, join " ".
        # Sin _unhyphenate (el persistido de considerando_text tampoco lo
        # tiene: L626-629 son locales a classify_outcome).
        # Invariante: dictamen_text != "" <=> dictamen_presente, salvo zona
        # dictamen all-blank (esperado 0; medir en el ciclo).
        dictamen_text = " ".join(
            ln for ln in (bloque[k].strip()
                          for k in sorted(lineas_dictamen)
                          if 0 <= k < len(bloque))
            if ln
        )
        ...
        caso = {
            ...
            "case_name_cuerpo_fuente": case_name_cuerpo_fuente,
            "date":                   fecha_str,
