...
def detectar_apertura_dispositivo(stripped_line, cola_wrap=""):
    """
...
def _firma_nucleo(s):
    ...
    while True:
        ...
        for m in list(_RE_PAREN_BALANCEADO_FIRMA.finditer(t)):
            if not any(pat.search(m.group(0)) for pat, _n in JUECES_CONOCIDOS):
                t = t[:m.start()] + " " + t[m.end():]
                cambio = True
                break
        if not cambio:
...
def linea_es_firma_de_juez(linea):
    ...
    if not any(pat.search(core) for pat, _nombre in JUECES_CONOCIDOS):
...
def zonificar_bloque(bloque):
    ...
    for k in range(n):
        ...
        es_disp, _ = detectar_apertura_dispositivo(s, _cola_wrap)
        if es_disp:
            _en_sumario = False
