...
**Unidad de análisis: parte × recurso, no caso (hallazgo H106, B109).** La descripción de arriba asume implícitamente una vía de acceso y un resultado por caso. Eso es falso en una minoría de casos: queja y recurso concedido NO se autoexcluyen a nivel caso, solo respecto de la MISMA parte. Una parte cuyo REX fue denegado va por queja; otra parte del mismo expediente cuyo REX fue concedido va por el recurso concedido — y la Corte puede resolverlos en sentido opuesto. Ejemplos reales (de los flips de B109): 342_p1017 (dos quejas, desestima una por art. 280 y declara procedente la otra → revoca la sentencia); 331_p530 (REX concedidos declarados inadmisibles + queja desestimada, distintas partes, mismo expediente). Hoy `outcome` único fuerza a elegir un "ganador" del campo (B109 los deja en `desestima`, que es la mejor aproximación a nivel caso, pero pierde la disposición de fondo de la otra parte). Consecuencia de diseño: la unidad correcta del eje disposición/parte es **(parte recurrente × recurso × caso)**, análoga a cómo `csjn_casos_votos` es (juez × caso). El refactor robusto agrega una tabla candidata **`csjn_casos_recursos`** (una fila por parte recurrente, con vía de acceso, disposición y parte_ganadora), y `csjn_casos.outcome` queda como vista agregada o disposición dominante. Marginal en frecuencia (~pocos casos), pero taxonómicamente correcto y necesario para que el modelo no mienta en los multi-parte. Conecta con `certReason`/`jurisdiction` del SCDB (la vía de acceso vive a nivel parte, no caso). En M20: codificar `cod_disposicion`/`cod_parte_ganadora`/`cod_via` sobre los n=300 decidiendo PRIMERO la unidad (caso vs parte×recurso); si se elige parte×recurso, la planilla de validación necesita filas por recurrente, no por caso.
...
4. **Refactor outcome→disposición+parte (M20)** — frente propio, grande, posterior a B109/B105; diseñar equivalencia + re-titular. DECISIÓN PREVIA: unidad de análisis (caso vs parte×recurso, ver §1 «Unidad de análisis»); si parte×recurso, tabla nueva `csjn_casos_recursos`.
