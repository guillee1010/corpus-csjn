...
- **Corpus:** 5862 casos (5669 fallos + 160 sumario_con_link + 33 sumario_editorial). Sin cambios de cardinalidad.
- **Sin firma:** 16 / 5669 fallos.
...
**Validación:** PoC A/B (OLD=parser real, NEW=RNR último) = 12 transiciones SIN_CAUSAL→RNR, cero robo. check_regresion column-aware: exactamente esas 12 celdas, votos/zonas/editorial [OK] byte-idénticos; producción = 12 (la watch-list de 26 no sumó nada). Re-golden consciente, [CLEAN] 4/4.
...
### H105 — Addendum: evaluación REE + diseño del Frente B (materia) y deshyphenate selectivo

Cierre de sesión con dos discusiones de diseño que conviene no perder.

**1. Solidez REE del proyecto a esta altura.** Lo que distingue al corpus de un parser cualquiera es que el error está MEDIDO contra ground truth (M19). Robustez: alta para lo medido (red check_regresion + titular con IC de Wilson + PoC A/B), con techo de OCR (M15, running-heads B104) y dos campos flojos (causa_inadmisibilidad 50% ralo, tipo_cf 70,8%); el hueco más serio NO es un bug suelto sino que el kappa (confiabilidad del gold) sigue sin medir → el titular descansa sobre un codificador único. Elegancia: buena disciplina de refactor (M13, R5) pero la cascada `OUTCOME_PATTERNS_DISPOSITIVO` es una pila de parches con patterns duplicados en zona alta/fallback — pragmática, no platónica; métrica útil = atomicidad del cambio (B113 tocó exactamente 12). Escalabilidad: cómoda en volumen, limitada en variedad lingüística (techo regex → M18 LLM para holding/obiter) y en dimensiones nuevas (falta materia). Métricas propuestas para calificar: tasa de `otro` como termómetro de cobertura (9,3% de fallos hoy), costo marginal por tomo, cobertura de dimensiones vs SCDB.

**2. Frente B / materia — el ángulo que destraba «no sé cómo sacarla».** Hallazgo: la materia NO se saca del texto libre del fallo (de ahí la sensación de variedad inmanejable) sino de la señal YA estructurada de `tribunal_origen`. Evidencia sobre v18.20: para fuero nacional especializado, tribunal→materia es casi determinístico (CA, Seguridad Social→previsional, CNAT→laboral, Casación Penal, Civil/Comercial, Electoral). Diseño por capas (ver DEUDA, Frente B): capa 1 lookup tribunal_origen→fuero; capa 2 provinciales multi-materia + SIN_TRIBUNAL (1791) vía normas citadas/partes; capa 3 originaria (477). Molde de producto = SCDB issue/issueArea (2 niveles, reglas de scope), validándolo contra los n=300 ya codificados (cod_materia, precision/recall por valor). Proyección a secretaría = terreno del usuario, con dos alertas del corpus: la estructura de secretarías cambió 2006–2025 → inferencia temporal; los provinciales multi-materia son punto ciego también para secretaría. SCDB codifica a mano: da taxonomía y reglas, no el método de extracción.
