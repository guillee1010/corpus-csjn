...
### M52 — Performance del parser: prefiltro de alternación única para JUECES_CONOCIDOS (~55% del runtime en juego)

**Origen:** H196 (apertura de sesión: cProfile sobre la corrida completa v30.0, 46 archivos / 5894 casos).
**Medición:** 208,5 s totales / 301M llamadas. `zonificar_bloque` 136,1 s cumulative (65%). Hot spot dominante: `linea_es_firma_de_juez` = 1.701.762 llamadas / 114,7 s (**55% del runtime total**); el caso dominante es el NEGATIVO — cada línea sin juez paga las ~74 `re.search` de JUECES_CONOCIDOS completas (≈134M searches / 93,1 s tottime). Segundo escalón un orden abajo: `detectar_apertura_dispositivo` 17 s / `re.match` 16,8 s / `re.sub` 11,7 s. `parser_editorial` fuera del top-30.
**Diseño candidato:** prefiltro de alternación única compilada desde el MISMO CSV (`_meta/jueces/jueces_csjn.csv`): una sola `re.search` que decide «¿hay algún juez acá?» antes del loop de identificación — semánticamente equivalente (la unión de searches independientes es la disyunción) SI ningún patrón del CSV trae construcciones que rompan la unión (verificar los 74 patrones al implementar). El predicado es fuente única con 10+ call-sites: el prefiltro rinde en todos.
**Contrato del ciclo (REEL de refactor):** sha byte-idéntico de los 5 CSV o FRENAR — el orquestador default lo verifica solo. No prometer cifra; medir post.
**Estado:** pendiente (diseño candidato; no unidad de la sesión que lo midió, por decisión).
**Referencias cruzadas:** M46 (la tabla-dato es la fuente del prefiltro), B148/B153 (historia del predicado), M40 (backlog REE). H196.
