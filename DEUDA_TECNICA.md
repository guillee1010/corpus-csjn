...
| B129 | falso inoficioso: `result\w+ inoficioso` (dictamen PGN) marca revocación como `abstracto`/`is_merit=0` (testigo 334_p1272) | **fix validada H141** (lookahead, 25 flips); **H142: el de-interleave la recupera ESTRUCTURALMENTE** (los 25 caen en los 49 `abstracto`→fondo vía verbo, sin el parche); el lookahead queda redundante salvo que se quiera el outcome plano correcto |
...
### B129 — Falso inoficioso: `result\w+ inoficioso` (dictamen PGN) clasifica revocación como `abstracto`
...
**Estado del fix:** diseñado y validado, **NO aplicado** — diferido a M26 (el bug escaló a refactor); disponible como commit standalone de ~4 líneas.
**Referencias cruzadas:** M26 (lo subsume), B119. H141.
