...
**Estado del fix:** no diseñado. Patrón candidato: guard tipo `_en_sumario` extendido (si la línea es larga / tiene verbo / no cierra en `.` de firma → no es firma) o techo de wc del tramo. MAJOR (mueve bordes de zona → `wc_*` + sidecars epilogo/partes; posible ripple a considerando por `lineas_residuo`). Requiere leer `linea_es_firma_de_juez` (no leída aún, solo referenciada).
**Referencias cruzadas:** M45 (origen), B076 (guard precedente), B117 (familia zona-que-come-cuerpo), M44 (vista normalizada de Pasada 1). H188. Sin ID histórico.
...
**Estado de verificación:** `confirmado_caso_testigo` (1 testigo leído; cardinalidad no aislada del resto de BUG-2).
...
### B154 — Conjueces/variantes residuales destapados por el ciclo H191 (Müller, Leal-«M.», candidatos del top de desconocidos)
