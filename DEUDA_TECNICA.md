...
**Diagnóstico / evidencia:** `hipotesis_no_verificada` — cardinalidad medida (483 en disco, parser v19.0), composición sin triar.
**Validador propuesto:** muestrear los 483 (estratificar por longitud de `por_ello_text`, presencia de dispositivo, zona), extraer una muestra con `extraer_caso.py` y leer el `.md` real; clasificar las sub-causas y dimensionar cuáles son recuperables (vocabulario nuevo, nueva categoría) vs genuinamente sin-dispositivo. Cruzar con el gold donde haya solape (validación, no tuneo). Es DEUDA por pedido explícito (cierre H126).
**Estado del fix:** no diseñado.
**Referencias cruzadas:** M21 (cerró la fracción truncada-por-banner), B120 (merit-recall), B118. H126. Sin ID histórico.
