...
### B159 — Zona `dictamen` sin cierre engulle el fallo entero (dispositivo y firma quedan dentro de `dictamen`)

**Componente:** parser (`zonificar_bloque`, cierre de la zona dictamen — la guarda H052 solo cierra con apertura/fecha).
...
**Causa raíz (hipótesis por forma, NO leída caso a caso):** la apertura del fallo posterior al dictamen no dispara ancla de cierre (RE_APERTURA/fecha ausentes o degradadas por OCR) → todo el bloque post-dictamen queda `dictamen`, incluida la firma. El `por_ello_idx` del resolutor es CORRECTO en los 7 (Tier 3b existe exactamente para esto: no excluye dictamen), pero re-etiquetar ahí exige re-anclar también firma/epílogo — cirugía distinta.
**Diagnóstico / evidencia:** 7 casos exactos: 332_p2418, 334_p109, 337_p1006, 337_p166, 339_p662, 344_p1952, 344_p2123 (`poc_b149_a1_flipset_v29_baseline.csv` filas zona=dictamen == `poc_b149_a1_flipset_postfix.csv` completo — el flip-set post-fix ES esta clase, doblando como su regresión).
**Estado de verificación:** `confirmado_cuantificado` (7/7 por forma mecánica; mecanismo del no-cierre por leer).
**Validador propuesto:** extraer los 7 (`extraer_caso.py`) y leer por qué la apertura no ancla; el fix probable toca el CIERRE de dictamen, no el detector de dispositivo.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B149 (origen del flip-set; su fix A1 los excluye por diseño), H052 (guarda dictamen), B148 (333_p385 era la variante dictamen-como-firma, reparada por otro camino). H194. Sin ID histórico.
