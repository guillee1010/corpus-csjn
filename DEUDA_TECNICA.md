...
**Prioridad H152 — documentación de repo + higiene in/out CERRADA (sin cambios canónicos).** 6 READMEs por directorio colocados; κ promovido a `scripts/validacion/` (demo en disco idéntica, parte 0,784); `.gitignore` reescrito (intermedios conservados, forense/`docs` fuera del tracking). **Teed up, en orden:** (a) **triage de los 4 de H148/H149** (`clasificador_causa.py`/`csjn_casos_recursos.csv`/`_manifest.json`/`CHANGELOG.md` modificados sin stagear) → su commit con **re-sello de `_manifest.json`** (invariante de procedencia); (b) **reescritura del CODEBOOK** (sigue v1.3/`outcome` legacy → ejes M26 como variables + crosswalk SCDB contra nombres en español + tabla κ desintercalada reemplazando accuracy M19 + bump de versión); (c) **refresh de Dataverse** (publicado pre-M26, `doi:10.7910/DVN/TJTVKW`); (d) **B130/reenvia** (codear 42 del universo de reversión leyendo `scripts/validacion/reenvia_42.md` → recomputar → reportable sobre ~113); (e) `auditar_fallo.py` carve-out (tool vs `validacion/`). 335-336 siguen excluidos.
