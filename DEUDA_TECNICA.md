...
**Diagnóstico / evidencia:** `confirmado_caso_testigo` (1 leído entero); cardinalidad corpus-wide SIN medir.
**Validador propuesto:** superficie corpus-wide de matches de header de voto/disidencia cuya línea ANTERIOR está ABIERTA (sin «.») o termina en paréntesis/«(Fallos:» colgante — patrón espejo del guard `_es_continuacion_wrap` (H190-b). Superficie ANTES de discriminador.
**Estado del fix:** no diseñado.
...
**Causa raíz:** el catálogo no cubre al menos dos fórmulas frecuentes: «a cuyos términos corresponde remitir» (la rama anafórica solo acepta «al que/cual cabe remitir») y «comparte el dictamen» a secas (la rama comparte exige «los fundamentos»).
**Diagnóstico / evidencia:** testigos leídos 342_p186, 344_p2639, 339_p1683 («a cuyos términos…») y 333_p530 («comparte el dictamen»); en el pool sin_ancla la señal midió 438/1104 con la regex vigente = COTA INFERIOR. Cardinalidad corpus-wide del subconteo SIN medir.
**Estado de verificación:** `confirmado_caso_testigo`.
**Validador propuesto:** superficie corpus-wide de las variantes candidatas sobre los textos de voto ANTES de ensanchar (patrón B067: medir FP argumentales primero — «corresponde remitir» aparece también en remisiones a precedente, no solo a dictamen).
**Estado del fix:** no diseñado.
