...
**Última actualización:** 2026-07-02 (H172 — **B135 (a)+(b) IMPLEMENTADO y validado en disco; paso 1 de M39 EJECUTADO; adjudicación de los pendientes H171**. Mañana: 329_p5115 (Civitarreale, doble conforme) adjudicado FONDO / gate ACIERTO (decisión Spaeth-compatible: vacated&remanded = mérito, consistente con el 41% de deja_sin_efecto del corpus); 329_p5261 (ADC/imagen religiosa) confirmado ÚNICO FP de B140a (Munsingwear-style vacatur por mootness, 0 adjudicación) → **B140a cardinalidad real = 1**. B138: 2 ordinarios leídos (334_p1302 Mezzadra, 342_p1524 Deutsche Rück — este último con «rechaza»≡«confirma» textual entre mayoría y votos) → ambos ACIERTO → **B138 = 17 FP + 2 aciertos ordinarios**; guard futuro debe excluir via=ordinario además de barrer por objeto. Tarde: B135(a)+(b) cableado a parser (v23.0→23.1). (b) mask RE_RUNNING_HEAD antes de _unhyphenate — miss por guionado corpus-wide medido = 1 (337_p234 Credicoop). (a) 5ª señal «competencia originaria» pelada con 4 guards por-match (local/apelada/precedente/provincial W=120), calibrada por PoC read-only `scripts/diagnostico/H172/poc_b135_flips.py` v0.1→v0.3 (anclas A1-A6 [OK]): flip-set 43 = 39 TP + 4 FP-F5 aceptados y documentados (349_p163, 347_p2146, 347_p2286 Ferrari-Levinas, 334_p1842 Sorrento — historia procesal narrada/doctrina, 0,07%). Ensanche de RE_ART_117_CN medido y RECHAZADO (marginal 0 TP/1 FP: 348_p841). M1 corregido 15→14 (337_p901=Duarte, FP por cita CIDH «competencia originaria local»; gemelo 342_p2389). Validación `verificar_b135_post.py` v0.2 [P1-P4 OK]: is_originaria 546→589, ripple de is_merit BIDIRECCIONAL vía es_de_fondo (B136) 3003→3006 (+6 rechaza-de-fondo / −3), confinado a los 43 IDs × {is_originaria, is_merit_decision, tribunal_origen_status}. Golden re-sellado (csjn_casos sha b8a21dcd1169), recursos re-derivado (v0.6), manifest [CLEAN] 64. **Divergencia M39 234→219**; M1-del-parser = 0 (el «1» que reporta consistencia_merito v0.3 es 337_p901 = ruido de la regex amplia sin guards, no un miss del parser). PENDIENTE H173: housekeeping de schema (ver entrada M41), eyeball 329_p3894, sub-audits M2B/M2C abiertos.)
...
**Diagnóstico / evidencia:** `confirmado_caso_testigo` — `diagnostico/_extraidos/H171/` (~10 archivos) y `diagnostico/_extraidos/H172/` (13+ extraídos + 3 `b135_flips*.csv`) en la raíz. `check_allowlist_paths` (gate H166) no incluye la raíz `\diagnostico\` en su lista de rechazo.
...
**Estado del fix:** no aplicado — **primer ítem de H173** (migración de `_extraidos/H171` + `H172` al destino canónico vía `git mv`, actualización de referencias en BITACORA con nota de que las rutas viejas son testimonio, + wiring del gate). NO se hizo en H172 para no mezclar un move de archivos con la validación pendiente del golden.
**Referencias cruzadas:** `check_allowlist_paths` (H166), M38 (disparo de los gates de cierre — mismo problema de enforcement no automático), skills apertura/cierre-sesion-corpus, directriz de schema del system prompt del proyecto. H172. Sin ID histórico.
