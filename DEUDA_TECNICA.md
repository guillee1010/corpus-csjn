...
**Última actualización:** 2026-06-16 (H136–H137 — **B127 CERRADO (alcance capa-deriver) + M21 Fase 2 DIAGNOSTICADA y parser v21.0 PATCHEADO pero NO corrido/validado (EN CURSO).** **B127:** `clasificador_disposicion.py` v1.02→v1.04 (NO en la cadena del parser, NO sellado). v1.03 = `OBJ_es` plural -es ('resoluciones'/'decisiones') SOLO en revoca/deja_sin_efecto (NO en nulidad/invalidez/confirma → el -es arrastra FP); disco 5890: 6 flips, 0 regresiones, gold 0/300 tocado. v1.04 = variante 'FALLOS DE LA CORTE SUPREMA' en `RE_HEADER` + chequeo movido DESPUÉS de GRANT → 18 no_fondo→`por_ello_cortado` (relabel honesto del dispositivo truncado por banner; **STOPGAP**, NO recupera parte_ganadora). El **'skip bifásico' de la deuda H132 era ERRÓNEO** (verificado en texto de producción: la cláusula de admisibilidad NO bloquea el match); el bucket `no_fondo ∧ es_revision_fondo=si` (76) se descompone en ~19 banner-truncados (territorio M21/parser), ~3-15 cobertura OBJ, 1 misclasificación parser (B129), 42 `no_fondo` legítimos (parser sobre-dispara `es_revision_fondo`). **Magnitud (corrige la justificación de B127):** `parte_ganadora` del universo fondo ≈ **81,4%** (2353/2892); recuperar TODOS los ~22 recuperables mueve ~+0,1pp → el valor NO es la métrica, es **DATO LIMPIO** (463 banners fuera del por_ello). **M21 Fase 2 (B122/B118):** root cause confirmado en código — `RE_PAGE_HEADER` (línea-sola, `^...$`) saca casi todos los headers split-form pero NO la TERNA 'número FALLOS DE LA CORTE SUPREMA número'; `_barrer` saltea vacías (Fase 1, H126) pero APPENDEA el banner → ensucia el `por_ello` y agota el presupuesto del chunk → trunca el verbo de fondo (testigo 330_p1907). **Fix parser v21.0:** `RE_RUNNING_HEAD` (terna substring, ambas orientaciones, número obligatorio a un lado → 0 FP sobre la frase legítima 'Corte Suprema de Justicia de la Nación') enmascarada en `_barrer` ANTES de armar el chunk (saca el banner + si la línea era solo banner queda vacía → no cuenta presupuesto → recupera el truncado, igual que las vacías). Verificado SOLO en sandbox sobre el sidecar: **463 banners / 0 FP.** **NO corrido en disco** → recuperación-por-presupuesto y regresión SIN medir. MAJOR (afecta outcome/por_ello_text/votos denormalizados) → re-golden + re-sello. **PENDIENTE PRÓXIMA SESIÓN:** correr parser v21.0 en disco + `validar_m21_fase2.py` (Gate A 0 banners + Gate B 0 regresiones `real→otro` → si aparece `real→otro` PARAR) + `check_regresion` (FAIL esperado) → re-golden consciente + `generar_manifiesto` (re-sello) + re-correr `derivar_recursos`; después **colapsar clasificador a v1.03** (original recuperado en sesión = `clasificador_disposicion_v103.py`; el relabel v1.04 queda moot) + merge rama `refactor/m21-fase2-banner`→main. **B129 NUEVO:** 334_p1272 — el parser marca una revocación de fondo (salvataje hipotecario, leyes 25.798/26.167) como `es_revision_fondo=no`/`is_merit=0`/`outcome=abstracto`; fondo real mal clasificado (no diseñado). **Estado de cierre H136–H137 (deliberado):** SOLO docs commiteados (DEUDA + BITACORA). **parser v21.0 patcheado en disco SIN validar; clasificador v1.04 SIN downgrade** — para no ensuciar commits, el código se corre/valida/commitea la próxima. capa-deriver NO sellada en el manifest. Lección REE reconocida: el mecanismo de header ya existía (`RE_PAGE_HEADER`); el error fue construir un parche aguas abajo (relabel en el clasificador) en vez del fix en el ORIGEN, reinventar detección que el parser ya tenía, y desviar con magnitud cuando se lo recordó → fuente > parche; reusar > reinventar; pedir el archivo > adivinar; cerrar con dato en disco. **Previo H135** — **merge `refactor/m20-disposicion` → main (FF `a2ac106..622bfa6`, parser v20.01, harness [CLEAN] 5/5 sobre main).** main es ahora la línea principal con M20 + capa-deriver (disposición + vía). **Persistencia tardía de H132:** se commiteó el estado que el cierre de H132 declaró pero NO guardó (commit `964359c`): `derivar_recursos` v0.1→v0.2 (vía: `via_recurso`/`multi_recurso`, vía 0,956 vs gold n=136) + `clasificador_via` v0.1 (untracked→trackeado) + regen `csjn_casos_recursos.csv` (5891 filas, +2 cols vía, NO canónico) + clave/`build_m20`/`validar_H120` (renombre `no_fondo`); git vuelve a coincidir con la bitácora. **B128 CERRADO** (commit `0333379`): `extraer_caso.py` v2.1→v2.3 — lee `considerando_text`/`por_ello_text` del sidecar `csjn_casos_textos.csv` por join (revive `--md`/`--blind`/sanity-check) + **default cambiado a CRUDO** (el `.md` tal cual; la herramienta de diagnóstico es deliberadamente INDEPENDIENTE del parser para no heredar sus errores de normalización) + perillas ortogonales `--norm`/`--deshifen`/`--colapso`; validado en disco (332_p663 POR_ELLO poblado + firmas partidas B126 visibles en crudo; 329_p142 POR_ELLO con running-head embebido). **Parser v20.01 INTACTO** — NO se tocó la cadena, 5 CSV canónicos [CLEAN] vs golden, manifest NO re-sellado (la cadena no cambió). **Lección de proceso:** el cierre de corpus debe garantizar que git == bitácora (commitear el estado declarado); H132 lo declaró pero no lo commiteó, de ahí la persistencia tardía. **Previo H134** — **sesión DIAGNÓSTICA: recalibración de `tipo_cuestion_federal` + seed de `causal_arbitrariedad` (B111). Parser v20.0 INTACTO — NO se tocó la cadena, NO re-sello; regression/golden siguen [CLEAN] por construcción.** Root cause del sobre-disparo de `mixto` (parser `classify_cuestion_federal`, ~884-914): la regla de co-ocurrencia dispara `has_arb` con `\barbitrariedad\b` pelado y `has_cf` con `ley 48` pelado —ambos casi ubicuos en REX— → `mixto`=677 (parser) vs **1** (gold humano) vs 230 con voz literal de la Secretaría. **Hallazgo central: la señal de arbitrariedad está DISTRIBUIDA en tres zonas** —sumario editorial (voz de la Secretaría: precisa pero ~24% recall) / considerando (palabra suelta, sobre-dispara) / **dictamen** (la causal vive ahí cuando la Corte remite: de 28 misses-sin-`arbitrar`, 21 con `cod_dictamen=remite`, 23 considerando <400 chars)— ninguna zona completa → caso empírico de **weak supervision**. **Taxonomía de causales (voz de la Secretaría verbatim, leaf tras "Procedencia del recurso"):** falta de fundamentación suficiente, defectos en la consideración de extremos conducentes, excesos u omisiones en el pronunciamiento, defectos en la fundamentación normativa, exceso ritual manifiesto, apartamiento de constancias, contradicción, valoración de prueba (gravedad institucional = flag ORTOGONAL, no causal). **Seed de dos zonas (voz + dictamen):** recall vs gold-arbitrariedad **18/45 (voz sola) → 38/45 (84%, voz+dictamen)**; precisión muestreada ~80% (8/10; FP = marcador ancho en contexto legítimo, NO discurso reportado: el gate de discurso reportado descarta solo 10 casos / 0 cambio de recall = **techo de la regex**). **Codebook de CF anclado en art. 14 Ley 48** (descomposición pretoriana = dirección de fix): inc. 1 + inc. 3 = CF **simple**, inc. 2 = CF **compleja** (prelación art. 31), arbitrariedad = doctrina pretoriana **ORTOGONAL** (flag aparte, NO tipo de CF); "ley 48"=21% del corpus → NO discrimina, art. 15 / "relación directa" = boilerplate de admisibilidad. **Retractación honesta:** el "85% over-fire" inicial se corrigió — solo 230/1560 tienen voz literal "Sentencias arbitrarias", pero la voz SUB-CUENTA (11/45 gold-arb con voz literal; **14 gold-arb son FN del parser**). **B111 avanza de "lever" a diagnosticado + seed** (NO clasificador limpio: el campo pide ML/weak-supervision — voz = LF de alta precisión, dictamen = LF de alto recall). **B128 NUEVO:** `extraer_caso.py` stale post-H113 (lee `considerando_text`/`por_ello_text` de `csjn_casos.csv` que ya NO los tiene —migrados al sidecar `csjn_casos_textos.csv`—; `ancla`/`pe` vacíos → modos `--md`/glob/`--blind`/sanity-check muertos, char-count reporta 0/TRUNCADO mal; la extracción por rango de líneas sobre el `.md` SÍ funciona). Script diagnóstico nuevo `scripts/diagnostico/H134/harvestear_sumarios.py` v0.1 → `sumarios_arbitrariedad.csv` (1560 filas, voz + dictamen). Mejora anotada: el parser CALCULA `sumario_text` (~parser:3535-3543) y `lineas_dictamen` pero NO los persiste (harvest los recupera del cuerpo; bug del split del harvest: `RE_DICTAMEN` matchea la línea de atribución "–Del dictamen de la Procuración–" del sumario y misubica voces). Paso 0 (merge M20→main + B127) sigue pendiente. **Previo H132** — **capa-deriver disposición/vía: construida y validada en disco, parser v20.0 INTACTO.** Tres módulos en `scripts/pipeline/` (NO en la cadena del parser, NO en el manifest): `clasificador_disposicion.py` **v1.02** (des-hifenado del soft-hyphen del OCR → disposición 0,887→0,930, 0 regresiones; + renombre `sin_disposicion_legible`→**`no_fondo`**, inocuo: 1860 casos idénticos, accuracy sin cambio), `clasificador_via.py` **v0.1 (sin cambios)** (vía recursiva fuente-única, 0,956 vs gold n=136), `derivar_recursos.py` **v0.2** → `csjn_casos_recursos.csv` (5890 filas, NO canónico, NO sellado). **Diagnóstico del bucket `no_fondo` (1860, 31,6% del corpus): NO es OCR ni cobertura, es taxonómico** — el `por_ello` se lee perfecto; son resoluciones SIN disposición de fondo (competencia/liquidación/deserción/queja/honorarios). **El gold CONCUERDA:** de los 89 del bucket en el n300, 88 vacío + 1 `deja_sin_efecto` (98,9%); el gold se diseñó binario (fondo fino / no-fondo todo "no"), así que el bucket hace lo que el gold quiere y solo se renombró. **Hallazgo → B127 NUEVO (dispositivo bifásico):** ~26 FN candidatos (1 confirmado gold = 330_p1907) donde el clasificador agarra el verbo de ADMISIBILIDAD ("se declara procedente el recurso extraordinario") y no salta al de FONDO tras la "y"; 14 son quejas. Impacto = COBERTURA del corpus (~26 victorias de fondo subcontadas → sesga `parte_ganadora`), NO la métrica (gold sub-representa el patrón, cuesta 1 caso). **Vía → M23 NUEVO (reevaluación):** per saltum (art. 257 bis) diseñado y validado (16 detectados) pero **NO aplicado** — decisión H132: la "primacía" es innecesaria para algo <1% y un per saltum ES un extraordinario. **El tipo de recurso NO es un fin analítico** sino feature para clasificar el dispositivo (originaria/competencia/ordinario/extraordinario tienen dispositivos distintos); el eje que importa es concedido-vs-queja (anuario CSJN), ya en `es_queja`. Pendientes anotados: 253 quejas sin vía (imputar extraordinario, art. 285 — incierto si se captura), originarios sin-vía (conecta is_originaria). M20 sigue defendible sobre el gold; el bifásico es frente aparte. **Parser v20.0 sin cambios, 5 CSV canónicos intactos, golden y manifest sin tocar — NO se re-sella (la cadena del parser no se tocó).** **Previo H130** — **B124 CERRADO: regla P (`RE_PERF v2`) integrada en `_barrer`, parser v20.0.** Auditado en disco el universo "resuelve sin se" ANTES de tocar la regex (`audit_resuelve_sin_se.py`, 5890 casos): únicas formas sin-`se` entre candidatos-con-firma de la ventana = "el Tribunal resuelve" (300) + "resuelve:" (23 = "Tribunal" hifenado por el wrap del OCR / palabras interpuestas / "esta Corte Suprema" / "la mayoría del Tribunal"); `OTRO_RESUELVE`/`ESTA_CORTE`/`LA_CORTE`/`RESUELVE_UP` = **0** → el over-match de instancia inferior NO se materializa, "resuelve:" pelado es seguro en la ventana. `RE_PERF v2` = v1 (`se <verbo>` clítico) | `(el Tribunal|esta Corte|la Corte) resuelve` | `resuelve:`. `_barrer` con regla P: devuelve el PRIMER performativo-con-firma; fallback al primer-con-firma (= comportamiento v19) si ninguno lo es. **Validación en disco:** outcome **+121 recuperaciones** (otro→real) / 29 real→real (dom. `inadmisible_280`→merit) / **0 regresiones a otro**; **`scan_concurrencia` 0 sospechosos** (check NUEVO de pick-de-concurrencia que generaliza `scan_disidencia` a concurrencias según-su-voto — `RE_VOTO_HEAD v2` agarra "Voto la señora/el señor ministro" que el regex H129 perdía; mis-pick 331_p1028 cerrado, ninguno nuevo); `es_queja` +8 recup / 2 correcciones de FP (329_p1703 "quejas de las partes" = agravios; 329_p1998 "queja por casación denegada" = cita de instancia inferior) / 1 FN conocido (340_p114); votos net +1 (342_p1170 1→5 recupera panel, 348_p1435 dedup Alcalá; **332_p663 6→4 = exposición de B126, frente aparte, NO regresión de B124**). Re-golden [CLEAN] 5/5 + manifest re-sellado **[CLEAN] 61 artefactos**, parser **v20.0**. Scripts H130 (`scripts/diagnostico/H130/`): `audit_resuelve_sin_se.py`, `scan_concurrencia.py`, `outcome_delta_P.py`. **B126 sigue ABIERTO = próximo frente** (extractor de firma, nombres partidos por línea; testigo 332_p663 Salas/BOSQUES confirmado sobre el `.md` crudo). **Previo H129** — **B124 REABIERTO: regla P implementada (v20.0) y ROLLBACK a v19.0 por mis-pick.** P (`RE_PERF` = primer performativo c/firma) se implementó en `_barrer` con clítico opcional (`se (lo/la/los/las/le/les)? <verbo>`). Validación en disco: outcome corpus-wide **+147 corrige / 0 rompe** (119 otro→real + 28 real→real), gold n300 **7 mejoras / 0 regresiones**, `es_queja` **+6 neto** (7 quejas reales recuperadas — el `por_ello` mislocalizado ocultaba el "se hace lugar/desestima la queja": 329_p3617, 337_p590, 338_p1258, 340_p1913, 344_p3431, 346_p347, 348_p437; −2 correcciones de FP "quejas de las partes/recurso de queja citado"; 1 FN nuevo 340_p114, queja con dispositivo "se revoca la resolución" sin token). **PERO** `check_regresion` destapó **−7 filas en `csjn_casos_votos`** (4 casos), auditadas con `extraer_caso` sobre `.md` real: **331_p1028 = MIS-PICK de P** — la mayoría cierra "Por ello, **el Tribunal resuelve**: I)… II)…" (sin `se` → `RE_PERF` NO matchea) seguida de Argibay según-su-voto "Por ello, **se confirma**…" (sí matchea) → P salta a la concurrencia: outcome=`confirma` (de Argibay, NO el holding procesal de la mayoría "declarar recaudos + cúmplase cons. 7º"), firma = solo Argibay (9→1). **Blind spot de la validación H128/H129:** el scan de disidencia buscaba *votos perdedores*, no *concurrencias* (Argibay concurre, mismo resultado) → el A/B contó ese `otro→confirma` como corrección. Otros 3: 342_p1170 (1→5) = MEJORA de P (recupera el panel de 5); 348_p1435 dedup (Alcalá ×2→×1); **332_p663 (6→4) NO es mis-pick** → `RE_PERF` ancla bien ("se resuelve") pero `collect_firma_lines`/`parse_firma` dropea nombres partidos por salto de línea ("Carlos\nS. Fayt", "E.\nRaúl Zaffaroni") = **B126 NUEVO** (extractor de firma; caso originaria/no-fondo). **Decisión: ROLLBACK a v19.0** — parser + outputs restaurados al estado committeado, golden v19 intacto, sin re-golden ni re-sello. B124 sigue ABIERTO; el fix necesita **`RE_PERF v2`** (matchear performativos de mayoría sin `se`: "(el Tribunal|esta Corte|la Corte) resuelve", "resuelve:", "RESUELVE:") + auditoría del universo "resuelve sin se" ANTES de tocar la regex + re-validación con **check de pick-de-concurrencia** (no solo disidencia). Scripts diagnóstico entregados: `votos_delta_P.py`, `esqueja_flips_P.py` (`scripts/diagnostico/H129/`). Sesión sin cambios netos de pipeline — parser **v19.0 intacto** tras rollback. Previo H128 — **B124 RESUELTO en diseño (regla P), pendiente auditar 31 rotos antes de implementar.** Descartado el diseño "último-con-firma" (A1/A2) de H127: rompe 86/93 casos en corpus y 14 en gold (mete accesorios posteriores). Probadas 6 reglas de selección de candidato (actual, A1=último-c/firma, A2=último-s/firma, B=último-c/verbo, C=primer-c/verbo, **P=primer performativo c/firma**) sobre gold M20 n300 + corpus 5890, en disco local. **Ganadora = P:** no es la posición (primero/último) ni el verbo semántico, sino la **PERFORMATIVIDAD con firma** — `RE_PERF` = `\bse\s+(resuelve|decide|declara|revoca|confirma|hace lugar|deja sin efecto|rechaza|desestima|tiene por|admite|anula|hace saber|intima)\b`. El argumental no-performativo ("En consecuencia, no discutida…", "Por ello, si bien…", "Por lo expuesto, habida cuenta…") se saltea; el dispositivo de fondo es el PRIMER performativo c/firma; los accesorios posteriores (acordada 47/91, art. 94 CPCC) son no-performativos o performativos-posteriores y no se eligen. **Corpus:** P rompe **31** (mínimo, vs A1=86/A2=93/B=69/C=39), recupera **119 reales** (sin pérdida a `sin_dispositivo`; B/C inflan +295 a vacío = espejismo), **neto +88** (el mejor). **Gold:** P cambia solo **7 fuente-OK y los 7 son MEJORAS** (argumentales→dispositivo: 332_p2625, 334_p941, 334_p1081, 344_p2393, 345_p583, 346_p44, 348_p405); los **5 accesorios** que metían A1/A2 **desaparecieron** (331_p2913, 332_p979, 332_p1280, 339_p1530, 340_p1554); 0 pérdidas a vacío; no rompe ambiguos de competencia. Gates de fidelidad: G1 reconstrucción 99,4% (37 mismatch, 15 de t345), G2 réplica de la cascada **0 fails**, firma confiable (95% de las "fuera de zona" son inocuas en sumario, fuera del rango de búsqueda). **Pendiente antes de tocar `parser.py`:** auditar los **31 rotos de P en corpus** (el gold solapa ~5%, NO los cubre) para confirmar corrección vs regresión — script `autopsia_rotos_P.py` entregado (autodetecta los 31, imprime paisaje de candidatos). Patrón esperado: correcciones de `inadmisible_280` espurio→dispositivo real (338_p1032, 340_p1380, 341_p242, 342_p259); revisar 330_p1759 (único P→`sin_dispositivo`). Hallazgo colateral → **B125 NUEVO** (t345: `por_ello_text` vacío en CSV donde la reconstrucción extrae dispositivo). Sesión diagnóstico/diseño — sin cambios de pipeline, parser **v19.0 intacto**, sin re-golden ni re-sello. Previo H127 — escrutinio B123: triage de los 483 `otro` sobre v19.0 (lectura de 77 `.md`, seed=127). **Reframe: dos frentes** — **186 (38%) mislocalización del `por_ello`** (→ B124 NUEVO: `_barrer` ancla el PRIMER "Por ello" no-final → considerando truncado L1298 + outcome `otro`; testigo Sosa 332_p2425 = `revoca` MERIT perdido) vs **297 (61%) dispositivo capturado, verbo no en vocab** (frente taxonómico). Categorías verificadas contra `.md`: `aclaratoria`/`regulacion_honorarios`/`suspension` reales, `recusacion` muerta (B095), `caducidad` ya en vocab. Fix B124 "último-antes-de-la-firma" diseñado, no implementado. Sesión diagnóstico/diseño — sin cambios de pipeline, parser v19.0 intacto, sin re-golden ni re-sello. H126 — INTEGRADO el skip M21/B122 a `parser.py` (v18.26→v19.0): skip de líneas vacías en el presupuesto del chunk de `_barrer`. 50 flips de outcome (otro→competencia 33, acceso→fondo el resto; competencia 877→910, otro 528→483), 487 `por_ello` con la cola de la disposición recuperada (todo extensión, 0 pérdida), votos cambia solo en columnas denormalizadas (identidad del voto intacta), considerando/firma/zonas/editorial byte-idénticos. Re-golden [CLEAN] + manifest re-sellado v19.0. **B122/B118 CERRADOS**, **M21 Fase 1 integrada** (Fase 2 masking gated, pendiente). Fix de infra: `csv.field_size_limit` en `check_regresion`. Previo H125 — diagnóstico del mecanismo + PoC de impacto del fix M21/B122; sesión sin cambios de pipeline, parser v18.26 intacto, sin re-golden ni re-sello. Lever confirmado en disco = skip de vacías en el presupuesto del chunk de `_barrer`; el masking es secundario. PoC corpus-wide: +masking 0 / +skip 50 / +masking+skip 64 flips, otro→competencia 37, 0 regresiones. Fix validado, integración pendiente H126. Previo H123: diagnóstico zona/normalización + verificación de contaminación del gold). **M20 — validación formal CERRADA** (branch `refactor/m20-disposicion`, sin merge). Generador de la clave (`build_m20.py`) **recuperado y versionado** (causa raíz: estaba perdido, nunca commiteado); el clasificador reproduce la clave previa **300/300**; clave regenerada del `csjn_casos.csv` actual y `validar_H120.py` corrido formal. **Distinción honesta de defendibilidad:** gate **0,953 in-sample post-B119** (B119 se tuneó sobre el n300) / **0,907 held-out pre-B119**; disposición **0,857 blind** (regex congelada pre-gold), parte_ganadora 0,794, reenvía 0,773 — los blind son los números fuertes; cuestión federal 0,676, dictamen 0,930 / adhesión 71%; certiorari criollo queja 95% vs concedido 68% (H1). **Disposición productivizada (v0.1):** `clasificador_disposicion.py` v1.0 (módulo fuente-única, importado por validador y producción → sin drift) + `derivar_recursos.py` v0.1 → `csjn_casos_recursos.csv` (5890 filas, cobertura merit **92,8%** = H118; consistencia vs clave 300/300). **Hallazgo:** 166 casos no-merit con verbo de disposición = banco cuantificado de candidatos gate-FN para B120. **DEUDA metodológica (M20) — VERIFICADA H123:** ledger de contaminación del gold resuelto. **Único contaminado: el gate** (B119 se tuneó sobre el n300 → 0,953 in-sample / 0,907 pre-B119; no hay held-out limpio del gate post-B119 dentro del n300). **Limpios:** disposición 0,857 / parte 0,794 / reenvía 0,773 (regex congelada pre-gold), cuestión federal 0,676 y dictamen 0,930 (medidos, fix no aplicado), **materia v3.2 (medida contra `cod_materia`, refinamientos nunca aplicados)**. El split dev/test sigue pendiente solo para el tuneo FUTURO (B120, fix B111, refinamientos materia) — NO para lo ya congelado. Parser **v18.26 sin cambios**, 5 CSV canónicos intactos, golden sin tocar, **manifest NO re-sellado**; `csjn_casos_recursos.csv` NO sellado aún (pendiente al estabilizar deriver + merge). **Pendientes M20:** split dev/test del gold (solo para tuneo futuro — verificación de contaminación HECHA H123); **M21 Fase 1 (skip en `_barrer`) — INTEGRADA H126, parser v19.0 (cierra B122-dispositivo/B118)**; M21 Fase 2 (masking del banner) — pendiente/gated; **escrutinio de los ~483 `otro` residuales (post-skip) — DEUDA H126**; **reenvío desde el dispositivo (validar vs gold, recall mejora post-skip) — frente H126**; **re-evaluación de qué más extraer del gold + revisar el "bloqueo del gold" (validar ≠ tunear) — H126**; v0.2 `derivar_recursos` (via_recurso/admisión/eje queja-concedido); B120 (banco=166); `derivar_dictamen.py`; fix B111. **Pendientes previos vigentes:** CODEBOOK materia; csjn_analisis_v4 left-join; refinamiento CA; test-retest/kappa (en pausa).
...
| B119 | `is_merit` sobre-incluye NO-fondo (capa disposición) | CERRADO H121: gate 0,907→0,953; FP 19→5; capa disposición pre-cascada (competencia/cautelar/nulidad_concesion/inoficioso) + #1 originaria + #2 des-hifenado |
...
- *Por verificar:* materia (`cod_materia` figura como "semilla validada" — ¿`derivar_materia` se calibró contra ella?) y cortes B118 (¿fixes desarrollados mirando los flags ZONA/notas del gold?). Define si esos números son clean o in-sample.
...
**Estado:** decisión pendiente — BLOQUEA todo fix que tunee sobre el gold (B120, B122, B111).
...
**Referencias cruzadas:** H121, H122. M19 (estudio madre). B119 (in-sample), B120, B122, B111. Capa disposición/`derivar_recursos` (blind). Sin ID histórico.
...
### B127 — Dispositivo bifásico (admisibilidad + fondo): el clasificador agarra la admisibilidad y no salta al verbo de fondo
...
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B123 (mismo frente taxonómico, distinta causa: B123 = verbo no en vocab; B127 = verbo de fondo presente pero precedido por admisibilidad), `clasificador_disposicion.py`, parte_ganadora, M23 (vía/quejas, capa-deriver hermana). H132. Sin ID histórico.
