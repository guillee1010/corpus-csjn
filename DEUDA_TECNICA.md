...
**Última actualización:** 2026-07-01 (H169 — **B136 CERRADO**. `is_merit` de la originaria de fondo. La premisa H165 («condicionar el guard #1 de B119, 1 palanca, los verbos ya están») era **FALSA**: el eje vive en DOS capas (parser `is_merit` + deriver `es_revision_fondo`), y la originaria **resuelve la demanda** (verbos hacer_lugar/rechazar), no revisa un inferior (confirma/revoca). Fix = detector nuevo **`es_de_fondo(considerando, por_ello)`** en `clasificador_disposicion.py` (v1.09→**v1.10**, módulo hoja), importado por `parser.py` (v22.0→**v23.0**, `is_merit`) y usado por `es_revision_fondo` (`derivar_recursos.py` v0.5→**v0.6**) → **ejes CONSISTENTES**. Medido en disco: **133 originarias-de-fondo** (89 grant + 44 reject); `is_merit` **2870→3003**, `es_revision_fondo` **2816→2949**; 0 no-originarias tocadas; consistencia `is_merit`⟺`es_revision_fondo` en 546 orig = True. `check_regresion [CLEAN]` 5/5 (re-golden); `confirmar_scope_b136 [OK]`; `--verify [CLEAN] 64`. Commit `10f2c5c` (re-selló el manifest; código+data en el commit previo). **Hallazgo → B137 (bug nuevo, corpus-wide):** el cascade de `is_merit` reclasificó **13 votos** de fondo-originarias `indeterminado→D` vía el fallback `is_merit AND wc>=2500` de `clasificar_tipo_voto` (error SILENCIOSO pre-existente). 2 dudosos flagueados: 333_p1088 Argibay, 343_p1944 Rosenkrantz. **Pendiente:** κ ciego nuevo de `es_de_fondo` (antes de Dataverse/M35-④); audit B137 (H170).)
