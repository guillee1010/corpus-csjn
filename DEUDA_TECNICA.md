...
### B114 — `tribunal_origen` fragmentado por OCR (corte de línea / running-head) y no normalizado
...
### B116 — `pagina_no_en_mapa`: entradas del catálogo sin header de página en el mapa (familia B009, lado cuerpo)
...
**Causa raíz:** ciertas páginas que el índice SÍ lista (y `construir_catalogo` cataloga bien) no tienen su header de página detectado en `mapa_paginas.csv`. El cruce las marca `pagina_no_en_mapa` (no las puede localizar) y el parser las descarta en `cargar_localizados`. Es el lado-CUERPO de la familia B009 (B115 es el lado-índice): la entrada existe en el catálogo, falta el ancla del cuerpo.
**Diagnóstico / evidencia:** cruce H109 con el catálogo v1.01: tomos 331/332/333/334 = **11 `pagina_no_en_mapa` cada uno** (44 total), cluster sistemático y acotado a 329-334. Caso testigo: Astiz (334_p1063) — recuperado al catálogo por B115 pero no localizable (a/d=0/1, `sin_mapa`). El Pérez tragado de 343_p1987 (el FP de `es_queja` que NO cayó con B115) es del mismo lado-cuerpo (343 no estaba recortado en el índice). 335-336 son un caso límite distinto: `mapa_paginas.csv` no cubre 335 en absoluto (255 `pagina_no_en_mapa`) ni 336 → detectar_paginas/mapa nunca corrió sobre ellos tras la incorporación al corpus.
**Estado de verificación:** `confirmado_cuantificado` (44 en 331-334; +255 de 335 si se cuenta el gap de mapa).
**Validador propuesto:** diff por tomo de páginas-del-índice (catálogo) vs headers-del-mapa; inspeccionar por qué `detectar_paginas` no emite esos headers (variante de running-head / ruido OCR en el número de página). Re-correr detectar_paginas → mapa sobre 335-336 para cerrar el gap de esos tomos.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** H109. Familia B009 (lado cuerpo; complementa B115, que es el lado índice). 335-336 (mapa pendiente; ver H079/H080/H088). Sin ID histórico.
...
- **Capa 1 (alta precisión, barata):** `tribunal_origen` → fuero → materia, vía diccionario sobre la columna ya parseada (requiere B114 resuelto primero). Para fuero nacional especializado es casi determinístico: Contencioso Administrativo → CA; Cám. Fed. Seguridad Social (217) → previsional; CNAT (~130+) → laboral; Cám. Fed. Casación Penal (118) → penal; Civil/Comercial → civil-comercial; Cám. Nac. Electoral (29) → electoral.
- **Capa 2 (provinciales + SIN_TRIBUNAL):** los tribunales provinciales (SCJBA 131, Mendoza 31, STJ varios) son MULTI-MATERIA — el tribunal NO desambigua; necesitan señal secundaria (normas citadas en el considerando: ley 24.241→previsional, LCT/20.744→laboral, 11.683→tributario, Cód. Penal→penal; u objeto/partes). `SIN_TRIBUNAL_ORIGEN` (1791) es el bolsón más huérfano.
- **Capa 3 (originaria, 477):** materia propia (competencia entre Estados, CA federal); regla aparte. `outcome=competencia` (863) ya captura parte del eje.
...
**Estado:** DISEÑADO (H105), no implementado. Frente de sesión propia (taxonomía + capas + validación), no patch atómico. Candidato post re-titular M19.
**Referencias cruzadas:** H082, H083, H105. B114 (pre-requisito). M19 (validación). SCDB. is_originaria / tribunal_origen_status.
...
**Prioridad H108 — arco B115 (merge/caso-perdido por hueco de índice).** Arco propio. Conseguir el índice fuente, resolver el fork (parseo de índice acotado vs body-scan supplement), dimensionar fino el bucket de 71, diseñar y aplicar el fix. Al cerrarlo, Arriola/Acosta reaparecen como casos propios y los 3 FP de `es_queja` caen solos.
