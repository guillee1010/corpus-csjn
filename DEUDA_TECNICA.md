...
**Última actualización:** 2026-07-04 (H175 — **paso 2 de M39 sustancialmente ejecutado: B138 CERRADO (guard lista-positiva, clasificador v1.11→v1.12) + B140b CERRADO (ensanche `RE_NULIDAD_CONCESION`, v1.12→v1.13) + B140a resuelto-sin-guard (documentado)**. B138: el diseño v1 (lista negativa, 17 flips) se DESCARTÓ pre-instalación al leer testigos — «se rechaza el REX» es HETEROGÉNEO: 330_p3801 (Minaglia) es fondo real con dispositivo mixto (→ evidencia B142) y 331_p2567 borderline-fondo, vs 330_p1205/348_p747/331_p2621/330_p4891 acceso puro → guard final = lista POSITIVA (queja/recurso de hecho/recurso de queja/reposición), 11 flips exactos, 4 FP residuales documentados, contabilidad 19/19 cerrada. B140b: 10 flips de VERBO (nulidad→nulidad_concesion, 41 totales), ripple completo adjudicado: parte_ganadora→no_aplica, gate→no, y ripple NO PREDICHO a `admisibilidad` (los 10 salen de `admite`: 9→sin_marcador coherentes con los 31 veteranos B131, 1→inadmite+CUESTION_ABSTRACTA = Paccagnini) — lección: el contrato de ripple de un flip de verbo debe enumerar TODOS los ejes que consumen `disposicion` (parte, gate, admisibilidad, causa). Divergencia M39: 219→208 (B138) →216 (B140b: +9 invisibles EXPUESTOS = residuo lado parser [su copia de la regex, parser L470, NO se tocó — orden lockeado], −1 Paccagnini converge; se absorben en el paso 3). Gold n300: 0 flips en la clave; `build_m20` con v1.13 regenera clave byte-idéntica (0 celdas diff en 300) → blind 0,930 intacto por construcción. Verificadores BIMODALES (pre/post rederivación) en `scripts/diagnostico/H175/` (poc_b138_flips, poc_b140b_flips) + backup v1.12. **M43 NUEVA:** paquete de re-validación pre-Dataverse (re-κ ciego n300 + κ es_de_fondo). Menor: docstring L6 del clasificador apunta al path viejo de build_m20 (hoy `scripts/validacion/`) — corregir en el próximo bump. Lección ops (2 unidades tropezaron en instalación vía Downloads): protocolo fijo = versión en ORIGEN → copia con DESTINO EXPLÍCITO → versión en DESTINO → Downloads limpio; los candados de versión en los PoC frenaron bien las 3 veces. PENDIENTE H176: B139 (re-estratificar los 59 contra el corpus post-B141), luego paso 3 de M39.)

**Anterior:** 2026-07-03 (H174 — **B141 CERRADO (parser v23.2 + clasificador_disposicion v1.11) → B142 y M42 NUEVOS**. Causa raíz de B141 re-atribuida y verificada en código: NO era `es_de_fondo` (inocente — PoC sobre texto completo da `True` con los 3 candidatos de H173 descartados) sino el chunk de `_barrer` cortando en FALSO TERMINADOR: inicial anonimizada / numeral romano a fin de línea (L3102, `s.endswith(".")`). Familia de 16 adjudicada contra fuente: 14 truncados reales + 2 fines genuinos en inicial (332_p238, 340_p397). Fix = `RE_FALSO_TERMINADOR` + peek `_proxima_linea_es_firma` (reusa `linea_es_firma_de_juez`); ACOPLE embarcado primero: v1.11 guard in-limine en `es_de_fondo` (330_p3777, rechazo in limine = umbral por falta de caso — sin guard el fix fabricaba 1 FP; A/B 0 flips pre-fix). Mérito recuperado: is_merit 3006→3008 ({329_p3894, 341_p1148} 0→1, coherente bicapa). Bonus 334_p1047: el «: I.» a fin de línea escondía «Se revoca» → chunk no-performativo → regla P caía al fallback argumental; el fix sana el PICK + devuelve el considerando robado (wc 2471→2629). Divergencia M39 = 219 SIN cambio, explicada caso a caso. Diff contra golden 100% adjudicado (13 por_ello, outcome {3894✓, 4526, 1047, 1148}, zonas/editorial byte-idénticos); golden re-sellado (`--make-golden` + [CLEAN] 5/5), recursos + materia re-derivados (el considerando de 1047 cambió), manifest [CLEAN] 64; determinismo del pipeline verificado (2 corridas completas, sha idénticos). **B142 NUEVO:** sesgo bicapa en dispositivos MIXTOS — testigo 1047 (`inoficioso` le gana a `revoca` en AMBAS capas → coinciden en el error, invisible a la divergencia; + 1148 rechaza-vs-hace_lugar, 4526 caducidad-vs-desistimiento). **M42 NUEVO:** orquestador del pipeline. Residuales presupuesto/banner-partido → M21 Fase 3 (1951, 2080, 1237, 563, medida 3 de 274). +3 candidatos FN is_originaria → post-B010 (329_p3403, 330_p4526, 338_p699). Fix infra: stdout/stderr `errors="replace"` (crash cp1252 bajo stdout redirigido, latente en TODAS las versiones, reproducido sobre v23.1 intacta). Lecciones ops: backup verificado en disco ANTES de instalar; pausar Drive en corridas largas (transitorio ×46 NOT FOUND en H174, causa formal no cerrada); PYTHONUTF8=1 para corridas logueadas de scripts sin el fix. PENDIENTE H175: paso 2 de M39 (guards del gate), B135(c) opcional, sub-audits M2B/M2C, docs/ de M41.)
...
**Referencias cruzadas:** M19/M20 (gold y frame), B136 (κ es_de_fondo pendiente), M39 paso 3 (conviene medir DESPUÉS del paso 3, con el eje bicapa unificado). H175. Sin ID histórico.
