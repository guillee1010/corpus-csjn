...
| M29 | capa de partes (petitioner/respondent): recurrente/recurrido + rol del epílogo editorial | **capa 1 (epílogo) CERRADA H154** — `extraer_epilogos.py` v0.2 + `derivar_partes.py` v0.2 (NUEVOS, capa-deriver; parser intacto). `csjn_casos_epilogo.csv` 5697 fallos 1:1 (ok 4345/76,3%) + `csjn_casos_partes.csv` recurrente_ok **3633 (63,8% fallos / 84% epílogos)**, rol {penal 444 / demandada 368 / actora 343 / solo_letrado 45 / querellante 21 / codemandada 18 / por_derecho_propio 9 / coactora 2 / sin_rol 2383}; multi 149. Hallazgo: DOS ejes — Eje B recurrente/recurrido (=petitioner, del epílogo) ≠ Eje A actor/demandado (del índice `c/`). Manifest 8→10 canónicos (v1.7). **Pendiente: frente 2 formato viejo (~365) + frente 3 arrastre de zona (~103)**. Insumo de M25 (7 inversiones de rol 7/7 legible) |
...
**Estado del fix:** **capa 1 (epílogo) APLICADA Y VALIDADA H154**; frente 2 (formato viejo) + frente 3 (arrastre de zona) pendientes.
