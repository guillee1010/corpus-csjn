...
| B129 | falso inoficioso: `result\w+ inoficioso` (dictamen PGN) marca revocación como `abstracto`/`is_merit=0` (testigo 334_p1272) | **fix validada H141** (lookahead, 25 flips / 0 regresiones); DIFERIDA a M26 (refactor); disponible standalone |
...
| M26 | refactor admisión/mérito: de-interleave de `outcome` en dos canales canónicos (`admision` + `disposicion` multiclass + eje coarse fondo/procedimiento/originaria) | **DISEÑADO H141 (LOCKED)**; multi-sesión (3–5); Fase 1 = A/B en disco [próxima]; reabre κ + republica Dataverse; handoff en `PROMPT_H142_refactor_admision_merito.md` |
...
**Estado del fix:** diseñado (no aplicado). Plan por fases (3–5 sesiones) en `PROMPT_H142_refactor_admision_merito.md`.
**Decisiones abiertas:** `outcome` A (dura) vs B (blanda); value-set de `admision` vs `causa_inadmisibilidad`; ubicación de `competencia` (admisión / procedimiento / eje aparte).
...
**Diagnóstico / evidencia:** hipótesis derivada del corte de nulidad; NO cuantificado aún.
**Estado de verificación:** `hipotesis_no_verificada`.
