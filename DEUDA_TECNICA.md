...
**Última actualización:** 2026-06-03 (H112: **Frente B / materia capa 1 IMPLEMENTADA** — `scripts/pipeline/derivar_materia.py` v1.0 (módulo de derivación standalone, NO en el parser) escribe el sidecar nuevo `output/parser/csjn_casos_materia.csv` keyed por `caso_id_canonico` (cols materia/materia_capa/materia_fuente; LF; join 1:1, 5890 filas, 0 huérfanos). Cobertura sobre 5697 fallos: capa1 2474 (43,4%) [civil_comercial 746, contencioso_administrativo 618, penal 414, laboral 374, previsional 292, electoral 30], pendiente_capa2 2732 (48,0%), pendiente_capa3 477 (8,4% = originaria exacto), sui_generis 8, residual 6, no_aplica 193. Decisiones: `status==originaria` corta a capa 3 ANTES de las reglas; `tributario` confirmado capa 2 (0 en capa 1 — sube por Cám. Cont. Adm. Fed.); sui_generis (Jurado Enjuiciamiento + Consejo Magistratura) agrupados como categoría terminal sin label (pendiente lectura). Capa 2 bloqueada por `csjn_casos_textos`; capa 3 pendiente. **Pendiente de integración:** sumar el 6º output a la allow-list de `generar_manifiesto.py` (bump v1.0→1.1) + CODEBOOK. Verificado reproducible en máquina del usuario. **Apertura de sesión:** saldada la deuda de procedencia de H111 — regenerado `_manifest.json` (parser 18.24, sha256 post-B114 + LF), fixup commit; agregado el paso de re-sello del manifest al skill de cierre `cierre-sesion-corpus`. NO toca el parser (sigue v18.24) ni los 5 outputs canónicos previos. | H111: **B114 CERRADO** — `find_tribunal_origen` v11→v12: el nombre del tribunal venía cortado por el fin de línea del OCR (intra-palabra con guión/soft-hyphen + inter-palabra en preposición); v12 une hasta la línea que cierra en `.`, breaks estructurales + `_parece_caratula` (≥60% tokens MAYÚS) + `_unhyphenate` al persistir. PoC A/B sobre `.md` completo: 0 violaciones de invariante, FP de carátula L1144 resuelto. parser v18.23→18.24; **1129 celdas `tribunal_origen`** recuperadas (column-aware: 0 en otras columnas, 5890/5890, `tribunal_origen_status` intacto 3889/1331/477); votos/zonas/editorial [CLEAN]; re-golden consciente + [CLEAN] 4/4. Habilita la capa 1 del Frente B (tribunal→fuero→materia). **Addendum infra:** `lineterminator="\n"` en los 4 `DictWriter` — `csv.DictWriter`+`newline=""` escribía CRLF pero golden/prod estaban en LF (git) → check_regresion daba [FAIL] espurio en los 4 CSV (byte-diff, 0 diffs de celda) desde un checkout post-H110; ahora salida LF determinística, contrato de regresión estable. | H110: **B116 CERRADO** — páginas-apertura-de-sección (mes/Acordadas) suprimen el running-head superior `NNNN / DE JUSTICIA / 33X`; `detectar_paginas` keyaba solo en línea==tomo+entero vecino → no emitía header → `pagina_no_en_mapa`. Fix `detectar_paginas` v1.0→v1.01: interpola headers sintéticos guiados por catálogo (`cargar_paginas_catalogo`, solo `pagina_inicio` esperadas) + anclados al banner de sección (`RE_BANNER_SECCION`, `linea_header`=banner ⇒ `linea_inicio==header`, título en +1) + región file-local (hueco interior o abridor de chunk); CSVs de trazabilidad `mapa_paginas_inferidas.csv`/`_sin_banner.csv`; schema del mapa intacto → `cruzar` sin tocar. Validación: 44 inferidas [331:11/332:11/333:11/334:11], sin_banner=0; `pagina_no_en_mapa` 299→255 (44 a 0; 335 intacto en 255); Astiz `334_p1063`→`ok`, linea_inicio 54 (banner OCTUBRE), outcome `improcedente` preservado; perímetro confinado a 331-334 (comparación por clave: 0 nuevos/0 desaparecidos/**84 modificados = 21/tomo** = 11 recuperados + ~10 vecinos con bleed-through corregido; **0 fuera de 331-334**; 15 tomos byte-idénticos como predijo el blast radius); editorial +1 solo en 331 (afloramiento de B115, golden pre-B115). Outputs: mapa 46936→46980, csjn_casos 84 mod, votos +24, zonas 141054→141451, editorial 151→152. Re-golden consciente + **baseline re-sellado** (resuelve de paso la deuda del re-sello de H109) → check_regresion [CLEAN] 4/4 con el estado B115+B116. NO toca 343_p1987 (tomo 343 sin `pagina_no_en_mapa`, era mis-scope del prompt — su FP de `es_queja` tiene otra causa, pendiente aparte) ni 335-336 (excluidos hasta fuente confiable). | H109: **B115 CERRADO** — recorte del INICIO del bloque del índice de partes en tomos 331-334. Leído el `.md` fuente: la entrada `Arriola, Sebastián y otros: p. 1963.` está limpia (sin «s/ causa 9080» — era reconstrucción del prompt); el cruce no machea por carátula (cruza por `(tomo,página)`, nunca dropea). Causa raíz: la portadilla repetida `INDICE POR LOS NOMBRES DE LAS PARTES` cae a mitad de la «A» (L34087) y `detectar_secciones` la toma como inicio; `extender_inicio_indice_nombres` no la rescata porque la Validación 1 exigía línea en blanco antes de la «A», pero el listado arranca con el header `Por nombre del actor` encima. Recortaba las «A» tempranas (Arriola, Acosta, Astiz, Alsogaray, Apablaza…); A-fracción 331-334 1-4% vs 8-19% normal. Fix `construir_catalogo` v1.0→1.01: `RE_SUBSECCION_NOMBRES` + Validación 1 acepta header de subsección + filtro anti-polución en `parsear_indice_nombres`. A/B real sobre LibroVol332.3: 481→515 entradas, 1963 capturado, 0 pérdidas. Re-golden: catálogo +283 ids (**28 B115** [331:12/332:8/333:2/334:6] + 255 tomo 335 incorporado, no-B115); `csjn_casos` 5862→**5890** (+28; 335 cae por `pagina_no_en_mapa`); swallow Massuh/Arriola roto (332_p1960 apertura 2→1, deja de ser Arriola; 332_p1963 Arriola caso propio 1/1; Pérez 332_p2043 intacto); QA 28: 27 ok + 1 Astiz `sin_mapa`→B116; FP es_queja 332_p1960/331_p856→0, 343_p1987 sigue→B116. check_regresion [FAIL] consciente; golden re-sellado sha256 casos `[PEND]`. **B116 NUEVO** (lado cuerpo: `pagina_no_en_mapa`, 44 en 331-334 + 255 de 335; detectar_paginas/mapa). 335-336: índice parsea (335 con el fix) pero mapa no los cubre → no localizables aún). | (H108: **B115 NUEVO** — merge/caso-perdido por hueco en índice de partes (familia B009): el índice SÍ lista esos fallos (Arriola «s/ causa 9080: p. 1963» está en el índice por nombre del actor) pero `construir_catalogo`/cruzador los pierde (rama (a) confirmada H108: bug de parseo/cruce, NO omisión estructural ni como YPF c/ Mercante) y el rango de la entrada anterior se estira fundiendo el fallo intermedio; el parser es inocente (extrae el rango del catálogo, NO es fin-de-bloque); set-diff catalogo vs casos = 0/0 (el merge ya está en el catálogo: `332_p1960 = pg 1960-2033 "Massuh"` traga Arriola); detector `apertura≥2` en zonas (normal=1): 103 casos → 71 con dispositivo≥2 (merge real) + 32 disp≤1 (acumulación); ~70 fallos perdidos ~1,2%; el span de páginas NO sirve como detector; fix no diseñado, fork índice-parseo vs body-scan pendiente del índice fuente. **Capa-fuente `es_queja` IMPLEMENTADA** (parser v18.22→18.23): `classify_queja(por_ello_text, caratula_text="")` detecta la vía en la carátula (`RE_CARAT_QUEJA` «recursos? de hecho (deducidos?|interpuestos?) por» + guard de cita) además del `por_ello`; solo ancla fuerte; es_queja 2056→2281 (+225, 0 flips 1→0); queja_resultado sin_clasificar 38→263; precisión de flip 222/225 = 98,7% validada leyendo 7 fallos (4 TP de tercero + 3 FP —332_p1960/343_p1987/331_p856— que son downstream de B115 y caen solos al cerrarlo); tail ancla débil ~11 + capa considerando DIFERIDOS; check_regresion [FAIL] solo es_queja en casos, votos/zonas/editorial [CLEAN] byte-idénticos; golden re-sellado sha256 casos 904bfdedeadc→e7e59ca36772. CODEBOOK v1.3 producido (auditoría: inadmisible_280 240→38, desestima 547→793) pero NO publicado —Dataverse descartado esta sesión— y ya stale en es_queja (reporta 2056, real 2281)). | (H107: **B110 PARCIAL** — sub-causa PLURAL cerrada (`\bqueja\b`→`\bquejas?\b` en `RE_ES_QUEJA` y `_SYN_Q` + pluralización de artículos/verbos en `QUEJA_RESULTADO_PATTERNS`; parser v18.21→18.22); el plural sangraba las quejas multi-recurrente en TODO el corpus, no los 20 FN del Marco A: `es_queja` 1993→2056 (+63, puramente aditivo, 0 flips 1→0), ~59 celdas de `queja_resultado` pobladas (57 de los flips + 2 toques en casos ya-queja: 332_p2441 desestima→hace_lugar y 343_p637 ''→hace_lugar, ambos consistentes con su `outcome`). Sub-causa SOFT-HYPHEN descartada (era falsa: `_unhyphenate` ya une «que­ ja»→«queja»). 1 FP residual aceptado y logueado: 329_p1703 («las quejas de las partes resultan inhábiles» = agravio/sustantivo común en un juicio de expropiación, no recurso de hecho; precisión del flip 98,4% > 96,9% global; sin guard — «inhábil» es ancla ruidosa, 39/40 son «días inhábiles»/feria, y «queja…inhábil» es hapax=1). Capa-fuente de B110 (carátula/considerando) SIGUE ABIERTA. M15 reconfirmada al revés: PoC sobre `por_ello` truncado a 300 del CSV dio 60, parser real sobre `.md` completo dio 63. check_regresion [FAIL] solo casos (es_queja+queja_resultado), votos [CLEAN] byte-idéntico a H106 (el patch no toca `outcome`), zonas/editorial [CLEAN]; golden re-sellado sha256 casos 904bfdedeadc, votos 5f5bc5171fe2 (=H106), zonas 5f4949c93a88, editorial e8fa2ccf1e8e). | H106: **B109 CERRADO** (sobre-trigger `inadmisible_280`/`ac4` en quejas desestimadas → el verbo dispositivo manda; parser v18.20→18.21; fix de DOS partes: (1) `desestima` sumado a `OUTCOMES_NO_FALLBACK_280` en `classify_outcome`, (2) detección TEXTUAL de 280/ac4 al inicio del bloque gate de `clasificar_causa_inadmisibilidad` reusando las regex de classify_outcome — antes la causal venía del mapa OUTCOME_A_CAUSA y se hubiera perdido al pasar a desestima; 246 `inadmisible_280/ac4`→`desestima`, causa preservada 0 caídas, invariante gate intacto 1056/ART_280 240/ACORDADA_4 50; conteos desestima 547→793, 280 240→38, ac4 50→6, conservación exacta; M15: PoC truncado 229 vs parser real 246, los 17 extra = 280 pasado el corte de 2000, parser clasifica sobre `.md` completo; check_regresion [FAIL] solo casos+votos, zonas/editorial [CLEAN], golden sellado sha256 casos f79679fe2ba1, votos 5f5bc5171fe2). **M20 NUEVO** (refactor `outcome`→disposición+parte molde SCDB; los mixtos de B109 —342_p1017, 331_p530— evidencian que la unidad de análisis es parte×recurso, no caso: tabla candidata `csjn_casos_recursos` una fila por parte recurrente con vía/disposición/parte_ganadora; ver DISENO_SCDB §1; frente propio grande, posterior a B105). **Ítem infraestructura NUEVO** (`considerando_text` = 55% del CSV, 47,6% truncado a 2000; evaluar separar columnas de texto pesado a tabla aparte `csjn_casos_textos` — baja el analítico a ~4 MB, elimina el truncado, escala lineal sano; NO subir el truncado de a poco —cambio masivo no atómico, no resuelve raíz—; Parquet nota futura ligada a publicación; frente propio, NO ahora). `output/parser` regenerado a v18.21 + golden sellado; `_manifest.json`+CODEBOOK+Dataverse PENDIENTES del batch M19. | H105: **B113 CERRADO** (`Declarar abstracta/o` infinitivo → `abstracto`; alternativa `\bdeclarar\s+abstract[oa]\b` en zona fallback, no se extendió el pattern alto para no robar mixtos; parser v18.19→18.20; 12 `otro`→`abstracto`, los 12 arrastran `causa_inadmisibilidad` ""→`CUESTION_ABSTRACTA`; `is_merit` sin cambio; conteo sellado en check_regresion = 12 exactos, 0 residuales; votos.csv espeja outcome por denormalización). `otro` 541→529, `abstracto` 89→101. `output/parser`+manifiesto+CODEBOOK+Dataverse PENDIENTES del batch M19. Addendum diseño H105: **B114 NUEVO** (`tribunal_origen` fragmentado por OCR — «Contencioso Administra­/Admi-/Administra-» son el mismo tribunal cortado; pre-requisito del Frente B; incluye la regla del deshyphenate selectivo: matchear≠persistir normalizado, no es atómico) y **Frente B/materia DISEÑADO** (molde SCDB issue/issueArea de 2 niveles; extracción por capas tribunal_origen→fuero→materia (capa 1), provinciales/normas (capa 2), originaria (capa 3); validación sobre n=300; proyección temporal materia→secretaría). | H104: B108 CERRADO del todo (decline-gap «no es de la competencia originaria» al core, parser v18.17→18.18, 16 `otro`→`competencia`) + **B112 CERRADO** (deprecación de `outcome=originaria` — era category error: el pattern estructural enumeración-«I.» robaba disposiciones de mérito; reemplazado por accept→`competencia`; parser v18.18→18.19; 165 `originaria`→0 redistribuidos a su disposición real: ~113 méritos rescatados + ~52 aperturas/procesales a `otro` + aceptaciones a `competencia`; `is_merit_decision` 0→1 en ~22; la dimensión proceso vive en `is_originaria`/`tribunal_origen_status`). B113 NUEVO (`Declarar abstracta` infinitivo → `otro` en vez de `abstracto`, 12 casos). Validación diff fino: proxy v19 = parser real 182/188 (6 diffs = override 280/ac4 + guard B107, parser correcto). Conteos: `originaria` 165→0, `competencia` 765→863, `otro` 530→541, `rechaza` 237→267, `hace_lugar` 1340→1355, `desestima` 543→547. `output/parser`+manifiesto+Dataverse PENDIENTES del batch M19. | H103: B107 CERRADO (29 celdas `outcome`, parser v18.16) y B108 CORE CERRADO (157 celdas `otro`→`competencia`, parser v18.17); fix tooling M19 #1 de `analizar_validacion.py` aplicado (v1.1); M19 kappa parqueada con submuestra de 60 sorteada (seed 20260601, `preparar_retest.py`); `output/parser`+manifiesto+Dataverse PENDIENTES del batch M19. otro 688→530, competencia 603→765. | H102: M19 — TITULAR recalculado sobre **n=300** tras integrar los 8 suplementarios recodificados a CIEGO en H101 (supera la codificación no-ciega de H100, que había dejado el n=300 contaminado). Cruce con `clave_supl_8.csv` posterior a la codificación: 8/8 limpio, sin faltantes ni duplicados, sin colisión con los 292; la regla `ok_` (1 si cod==parser; 0 si difieren; vacío si cod vacío o AMBIGUO) reproduce los 292 con 0 discrepancias → `planilla_consolidada_MARCO_A_v18_15_n300.csv`. **El titular vigente pasa de n=292 a n=300.** Resultados n=300 (AMBIGUO fuera del denominador): es_queja 92,3% [88,8–94,8] (n=300), outcome 86,6% [82,2–90,0] (n=291), queja_resultado 84,3% [76,2–89,9] (n=108), tipo_cuestion_federal 70,8% [61,1–79,0] (n=96), causa_inadmisibilidad 50,0% [29,0–71,0] (n=18, ralo); carátula 83,6% (250/299, 49 erradas, recomputada a mano), fecha 100% (289/289, cota ≤1,0%). Deltas vs n=292 no significativos (IC solapados en los 5 campos). El n=300 CONFIRMA B108–B111 (otro precision 14,8% / competencia recall 65,5%; 280 precision 25% / ac4 0%; es_queja recall 82,6%; mixto precision 33,3% / arbitrariedad recall 64,4%). Único error nuevo de outcome de los 8: 343_p595 (procedente vs otro, POR_ELLO truncado en el `.md`). Tres desempates de H101 resueltos: 329_p4150 (rechaza) y 329_p4503 (confirma) coinciden con el parser → no son error; 341_p560 carátula_ok=0 real por `case_name_cuerpo` truncada; AMBIGUO de 329_p4356 (causa) fuera del denominador. El fix de tooling (1) de `analizar_validacion.py` (cuenta `N`/`NO` con codificación `1`/`0`) se reconfirma sobre n=300 (reportó 0 carátulas mal habiendo 49) y SIGUE pendiente — ya documentado en H100, no es bug nuevo. Producto único: la consolidada n=300. NO toca pipeline ni outputs (parser v18.15, golden sin cambios). | H100: M19 — TITULAR de exactitud EJECUTADO (codificación ciega del Marco A vs salida del parser v18.15, golden 5862 = 5669 `fallo`). Resultado limpio sobre los **292 fallos ciegos** (AMBIGUO fuera del denominador): es_queja 92,1% [88,5–94,7] (n=292), outcome 86,6% [82,1–90,1] (n=283), queja_resultado 84,0% [75,8–89,7] (n=106), tipo_cuestion_federal 71,7% [61,8–79,9] (n=92), causa_inadmisibilidad 47,1% [26,2–69,0] (n=17, ralo, no concluyente); carátula 83,5% fiel (243/291 ok, 48 erradas), fecha 100% (281/281, cota superior ≤1,1%). Cuatro modos de falla del parser por precision/recall POR VALOR: (a) `competencia` (incompetencia originaria) tirada a `otro` — 21 de 26 `otro` mal clasificados, precision `otro` 14,8%, recall `competencia` 65,5% → B108 NUEVO; (b) sobre-trigger `inadmisible_280`/`inadmisible_acordada_4` en outcome de quejas que se desestiman (precision 280 14%, acordada_4 0%) → B109 NUEVO; (c) `es_queja` sub-detectado, recall 82,3% (20 FN, cascada a queja_resultado vacío) → B110 NUEVO; (d) `tipo_cf` sobre-usa `mixto` (precision 38%) y pierde `arbitrariedad` (recall 65%) → B111 NUEVO. El Marco A se CONFIRMÓ muestra aleatoria simple de los 5669 fallos (réplica del sorteo seed 20260531: el set codificado == subconjunto de fallos del draw de 300), así que el titular es estimación del corpus de fallos. Universo corregido: la población son los 5669 `fallo` (no los 5862); los 160 `sumario_con_link` + 33 `sumario_editorial` no son fallos (todos los scl en tomos 345-349). El sorteo planeaba 300; los 8 `sumario_con_link` que cayeron se excluyeron bien → quedaron 292. La completación a 300 (8 fallos suplementarios, seed 20260531 sobre pool 5377) se hizo NO-ciega (se miró la salida del parser antes de codificar) → INVÁLIDA; el n=300 queda contaminado y el titular vigente es **n=292**. Completar a 300 requiere recodificar los 8 a ciego en ventana fresca, sin la clave. Dos fixes de tooling de validación pendientes, NO tocan el parser de producción: `analizar_validacion.py` cuenta `N`/`NO` en el bloque estructural pero la codificación usa `1`/`0` → reporta 0 errores de carátula/fecha falsamente; `muestrear_validacion.py` sortea sobre los 5862 en vez de filtrar `tipo_entrada=='fallo'`. NO toca pipeline ni outputs (parser v18.15, golden sin cambios). | H098: M19 — codificación ciega del Marco A EJECUTADA (parte de campo del estudio de validación de exactitud). 292 fallos (los `sumario_con_link` fuera) codificados a mano contra el bloque de cada caso en los 7 campos `cod_*` (outcome/causa_inadmisibilidad/es_queja/queja_resultado/tipo_cuestion_federal/caratula_ok/fecha_ok); cruce 1-a-1 con los bundles, sin huecos. Productos: 8 `cod_marco_A_lote_01..08.csv`, planilla mergeada `planilla_codificacion_v18_15_MARCO_A_codificado.csv` (292 filas Marco A llenas), `HALLAZGOS_validacion_Marco_A_v18_15.md`. Distribución `outcome`: hace_lugar 60, competencia 55, procedente 42, desestima 41, revoca 19, confirma/rechaza 15, originaria 11, abstracto/deja_sin_efecto 6, otro 4, nulidad 3, desierto/desistimiento 2, inadmisible_280/mal_concedido 1; `es_queja=1` 113; `caratula_ok=0` 48/292 (16,4%); fecha vacía 11/292. AMBIGUO: outcome 9, causa 31, fecha 11, tipo_cf 1, carátula 1, queja_res 7. Hallazgos accionables: B105 NUEVO (`por_ello` capturó considerando/feria/oficio/remisión a precedentes en vez del dispositivo, 9 casos testigo), B106 NUEVO (`case_name_cuerpo` vacío teniendo «Vistos los autos: …» presente: 332_p2797 Autolatina, 346_p1241 C.J.M./Swiss Medical), B107 NUEVO (la cascada de `outcome` mal-buckea negación y excepción de incompetencia: «no hacer lugar al recurso»→hace_lugar, «hacer lugar a la excepción de incompetencia»→hace_lugar). El grueso de `caratula_ok=0` es residuo/cita en `case_name_cuerpo` (evidencia que amplía B043/B089/B096). TITULAR de exactitud (global + IC de Wilson + precisión/recall POR VALOR) PENDIENTE: falta correr `analizar_validacion.py` contra `csjn_casos.csv` (salida del parser); también pendiente doble codificación / inter-coder reliability y la sección «Reliability / known limitations» del CODEBOOK. Método: codificación a ciego, AMBIGUO donde el texto no decide; los scans de keyword para `causa`/`tipo_cf` se descartaron (falsos +/−); el atajo mecánico de carátula/fecha solo sirve limpio en el tomo 329 (el formato deriva del 330 en adelante). NO toca pipeline ni outputs (parser v18.15, golden sin cambios). | H097: publicación del dataset v2.0 a Harvard Dataverse (en review). Subidos los 5 CSV del golden v18.15, `CODEBOOK.md` v1.1, `README.md` publicable, `_manifest.json` y los scripts de pipeline + mapa; limpiado el upload anterior (archivos de test/auditoría + los `.tab` que generaba el ingest tabular de Dataverse). README del Dataverse subido. NO toca pipeline, parser ni outputs canónicos (sigue v18.15, golden sin cambios). M19 NUEVO — estudio de validación contra ground truth para cuantificar el error de medición del dataset; diseñado, agendado H098. Pendientes de la publicación (no de pipeline): (a) confirmar si los CSV ingestaron en Dataverse — el `.tab` archivable NO matchea el sha256 del manifiesto, la verificación va contra el «original format»; si ingestaron, aclararlo en el README publicable o bloquear el ingest; (b) release note pública 1.x→2.0 y confirmación de la versión previamente publicada (no se pudo leer la pestaña Versions). | H096: B103 CERRADO — guard EXCL en el bloque DEPOSITO de `clasificar_causa_inadmisibilidad`. Los 2 FP (330_p1025/343_p166): la frase del depósito describe una resolución ANTERIOR de la Corte («la resolución de fs. X que desestimó … no haberse … el depósito»), no un gate sobre el recurso presente. `RE_CAUSA_DEPOSITO_EXCL` anclado al considerando, sumado como `and not …` a la condición DEPOSITO (mismo discriminador holding-vs-antecedente que B100/B101, de nivel considerando porque 343_p166 dice «planteo», no «revocatoria»). PoC `scripts/diagnostico/H096/poc_b103_guard_deposito.py`: A/B OLD↔NEW sobre texto idéntico = 2 flips exactos; capa anti-M15 reconstruye el bloque completo del `.md` (reusa `construir_bloque_desde_localizacion`) y exige EXCL solo en los 2 FP — 348_p805 (TP truncado a 2000 en el CSV) da EXCL_full=False; universo de cambios ⊆ DEPOSITO del golden (el guard solo saca). check_regresion [FAIL] solo esas 2 celdas de `casos`, votos/zonas/editorial [OK]; re-golden consciente; parser v18.14→18.15 (DEPOSITO_PREVIO 4→2, INADMISIBLE_SIN_CAUSAL_EXPLICITA 413→415; gate total sin cambio 1036). Cierra la última inconsistencia conocida de la cola de `causa_inadmisibilidad`. Addendum mismo día: M16 PARCIAL — `scripts/explorador/exploradorv6.py` (filtros de las categorías nuevas + sidebar reorganizado + tabs Tabla/Resumen + descarga); CODEBOOK sincronizado a v1.1 (4 columnas nuevas documentadas: causa_inadmisibilidad/es_queja/queja_resultado/tipo_cuestion_federal; tabla de `outcome` actualizada, `otro` 26.6%→11.7%; ref parser v18.15). Todas las tablas (incl. auxiliares votos/zonas/editorial/indice_partes) re-medidas contra el golden v18.15: única corrección `tipo_voto_sep` (indeterminado 1342→1330, D 524→536, 48.8%→48.4%); resto byte-exacto. | H095: causal nueva `RESOLUCION_NO_RECURRIBLE` emitida (irrecurribilidad de las decisiones propias de la Corte, Fallos 316:1706; reposición/revocatoria/nulidad contra una decisión propia, rechazada porque sus decisiones no son susceptibles de recurso). Validados los 44 hits VISIBLE de la cola pendientes de H094: FALTA_SENTENCIA_DEFINITIVA 32/32 TP, FALTA_FUNDAMENTACION_AUTONOMA 9/9 TP, DEPOSITO_PREVIO 1 TP + 2 FP. Diseño: el ancla del dispositivo (por_ello, 29 candidatos) mal-etiquetaba ~80% (lo confirmó leer 16 `.md`); la causal se ancla en el FUNDAMENTO del considerando, chequeada ÚLTIMA en el bloque gate-genérico (por construcción no le roba a SD/FUND/DEPOSITO/FUERA) + guard de hace-lugar (excluye 344_p1904, que la Corte concede por excepción). Captura «doctrina invocada», NO distingue holding de obiter (límite del regex sobre OCR → M18). Decisiones taxonómicas: recusación NO es gatekeeping (incidente, no admisibilidad del recurso → residual); depósito-mérito ya vive en DEPOSITO_PREVIO (lo residual es revocatoria-sobre-intimación); competencia es eje materia (Frente B; `outcome=competencia` 603 ya la captura), no inadmisibilidad. PoC `scripts/diagnostico/H095/poc_resolucion_no_recurrible.py`; A/B = 12 filas exactas SIN_CAUSAL→RESOLUCION_NO_RECURRIBLE; check_regresion [FAIL] solo esas 12 celdas, votos/zonas/editorial [OK]; re-golden consciente; parser v18.13→18.14 (RESOLUCION_NO_RECURRIBLE 0→12, INADMISIBLE_SIN_CAUSAL_EXPLICITA 425→413; gate total sin cambio 1036). B103 ABIERTO (los 2 FP de DEPOSITO no los absorbe RNR — son mérito del depósito, van a SIN_CAUSAL en sesión futura). B104 NUEVO (running-heads OCR mid-palabra: 329_p5316 no entra; su hermano 329_p5138 sí). M17 saneado para FUERA (los 10 con ancla única intra-tomo, sin contaminación B102). M18 NUEVO (capa LLM para holding/obiter, futuro). | H094: cierre de calidad de la cola de `causa_inadmisibilidad` — validados a fondo los 12 «delta» de SENTENCIA_DEFINITIVA/FUNDAMENTACION contra `.md` reales con `extraer_caso.py`: 11 TP / 1 FP (los 48 hits VISIBLE quedan sin triar, a H095). (1) Bug en la propia herramienta de diagnóstico (B102, CERRADO): `extraer_caso.py` <v2.0 resolvía el `.md` por `glob(LibroVol{tomo}*.md)` + primer match del ancla y en tomos partidos en volúmenes solapados (338.1/338.2) con fallos hermanos de considerando casi idéntico leía el volumen equivocado — disfrazó a 338_p830 (ACUMAR/Mendoza, TP) de «label fantasma». Fix v1.01→v2.0: resuelve por `source_file` + rango `[linea_inicio, linea_fin_real]` reusando `construir_bloque_desde_localizacion` del parser (reproduce el bloque exacto que clasifica el parser); `--md` queda como override; sanity check de ancla en bloque. (2) Único FP real (B101, CERRADO): 334_p419, outcome `otro` — la frase «no constituye sentencia definitiva» venía del dictamen del Defensor y de la resolución de la Cámara citada, no del holding (la Corte declaró la nulidad y reenvió). Causa raíz: las 3 causales de cola se chequeaban ANTES del guard `otro→""`, mientras FUERA ya exigía gate-genérico. Fix: las 4 causales de cola gateadas bajo `if outcome in OUTCOMES_GATE_GENERICO:`. PoC `scripts/diagnostico/H094/poc_cola_gate.py`; A/B old↔new = 1 fila exacta (334_p419: FALTA_SENTENCIA_DEFINITIVA→""); re-golden consciente, check_regresion [CLEAN] 4/4. parser v18.12→18.13 (FALTA_SENTENCIA_DEFINITIVA 44→43, vacío 4632→4633). M17 (revalidar validaciones de cola previas sobre tomos partidos, contaminables por el bug de la herramienta). Pendiente: 48 hits VISIBLE de la cola sin triar; causal RESOLUCION_NO_RECURRIBLE para las 26 reposiciones-desestimadas; reposición hoy en DEPOSITO_PREVIO (posible mislabel). | H093: fix FP `FUERA_DE_TERMINO` (B100) — primer tramo de la validación de la cola de `causa_inadmisibilidad` contra `.md` reales (lo que H092 dejó pendiente). Corrección: producción emitía 12 FUERA, no los 10 del piso del PoC (`sub_gate.py` lee `considerando_text` truncado a 2000; los 2 hits faltantes tienen el `extempor` pasado el corte — parte de los 14 «delta» de la cola que H092 reportó como «validadas» pero nunca eyeballeó). FUERA cerrado a mano: 10 TP / 2 FP. Los 2 FP (`329_p5138`/`5316`) son reposiciones contra una resolución de la Corte: el `extempor` describe el antecedente atacado, no el recurso decidido (holding = «se desestima la reposición», Fallos 316:1706). Fix: `RE_CAUSA_FUERA_TERMINO_EXCL_DISP` anclado al `por_ello` (dispositivo de reposición/revocatoria/aclaratoria), inmune al truncado, sumado como condición al bloque FUERA de `clasificar_causa_inadmisibilidad`. Cambio de comportamiento: 2 casos `FUERA_DE_TERMINO`→`INADMISIBLE_SIN_CAUSAL_EXPLICITA` (FUERA 12→10, SIN_CAUSAL 441→443; votos/zonas/editorial byte-idénticos). PoC `scripts/diagnostico/H093/poc_excl_reposicion.py`; A/B old↔new sobre texto idéntico = 2 diffs exactos (aísla el patch del artefacto de truncado); check_regresion column-aware confirmó solo esas 2 celdas (fila 871/897); re-golden consciente. parser v18.11→18.12, commits 9c7fa7d (tooling) + 7bcac83 (fix). Herramienta canónica NUEVA `scripts/diagnostico/extraer_caso.py` (extrae considerando+por_ello completos del `.md` anclando en el prefijo del CSV; resuelve la validación de hits truncados, no ligada a sesión). M15 (regla: validar un patch A/B sobre texto idéntico, no contra la columna de producción; el truncado da vuelta veredictos). M16 (refacción del explorador Streamlit para auditoría a ojo). Pendiente: resto de la cola sin validar a fondo (SENTENCIA_DEFINITIVA 44, FUNDAMENTACION 12, DEPOSITO 4; solo spot-check 348_p494 = TP; los delta nunca eyeballeados); posible causal nueva RESOLUCION_NO_RECURRIBLE para las 26 reposiciones-desestimadas (1 hoy en DEPOSITO_PREVIO = posible mislabel a revisar). | H092: gate de admisibilidad — campo NUEVO y ADITIVO `causa_inadmisibilidad` (mitad de fondo del ítem 6: outcome como par gate+action; NO relabel del outcome de mérito). Vocabulario controlado de la Corte, con la causal atada al recurso decidido (cada término es polisémico como lo fue el art.280). Detección: 5 nativas derivadas del `outcome` (ART_280 238, ACORDADA_4_2007 50, CUESTION_ABSTRACTA 89, DESISTIMIENTO 10, CADUCIDAD_INSTANCIA 11) + 4 de cola VALIDADAS a mano sobre el universo gate-genérico (FALTA_SENTENCIA_DEFINITIVA 34, FUERA_DE_TERMINO 10, FALTA_FUNDAMENTACION_AUTONOMA 10, DEPOSITO_PREVIO 4; ancladas al recurso, sin solapes, bajo el techo del tablero) + residual bipartido (INADMISIBLE_REMITE_DICTAMEN 139 —la causal vive en el dictamen del Procurador, material H1/H4— y INADMISIBLE_SIN_CAUSAL_EXPLICITA 441). El catch-all `otro` NO entra al residual: queda `""` salvo causal explícita (no sobre-afirmar gatekeeping). Vacío (no-gate / fondo) 4633. Invariante: `causa_inadmisibilidad != "" ⇔ gatekeeping`. Rename `MERIT_OUTCOMES`→`OUTCOMES_NO_FALLBACK_280` en `classify_outcome` (sin cambio de valores; desambigua del `MERIT_OUTCOMES` de `procesar_archivo` que alimenta `is_merit`). PoC `scripts/diagnostico/H092/sub_gate.py`; validador column-aware `scripts/diagnostico/H092/check_columna.py`. Columna nueva = cambio de comportamiento → re-golden consciente de `casos` (column-aware: resto byte-idéntico al golden viejo, solo +1 columna tras `outcome`; votos/zonas/editorial [OK] byte-idénticos). parser v18.10→18.11, commit ee0a62d; manifiesto regenerado (csjn_casos.csv sha256 4cd12105cf30, corpus digest sin cambios 9fdd4726ce6d), --verify [CLEAN] 54. Pendiente: validar la cola contra `.md` reales (eyeball de los ~58 hits + recall); candidatas de cola SIN emitir aún (SALTO_DE_INSTANCIA da 5 en el PoC pero algunos solo discuten el per saltum; FALTA_DENEGACION_REX / FALTA_RELACION_DIRECTA / FALTA_INTRODUCCION_OPORTUNA_CF / TRIBUNAL_SUPERIOR_CAUSA dan 0 —ausencia genuina o fórmula muy estricta—); columna `causa_inadmisibilidad_parcial` (los 70 de la frontera 280, ítem 3) diferida; reclasificación de `is_merit` (rechaza/competencia/originaria) diferida. | H091: M13 CERRADO — extraídas las dos últimas piezas inline de `procesar_archivo`: el bucle de votos/disidencias → `detectar_votos_disidencias(bloque, lineas_excluir)` (movimiento puro, se descartó el `pass` muerto de H085) y el refinamiento de status → `refinar_status_localizacion(status_loc, apertura_rel, ancla_inicio)` (colapsa el `if/else` muerto cuyas dos ramas producían el mismo sufijo). `procesar_archivo` 513→458 líneas, ahora orquestador puro de funciones nombradas. Refactor con red (extracción pura, sin re-golden): poc_votos (13 dirigidos × 4 máscaras + 80k fuzz = 0 discrepancias), poc_status (105 combinaciones exhaustivas = 0), check_regresion [CLEAN] 4/4 (5862/27463/140956/151 byte-idénticos al golden, 0-delta), manifiesto regenerado a 18.10 (sha256 de los CSV sin cambios), --verify [CLEAN] 54. parser v18.09→18.10. | H090: R2 — dedup de la lógica 280/ac4 unificada en `classify_outcome` como sede única (gate+action, ítem 6 del roadmap). El bloque inline de `procesar_archivo` (caso `sin_dispositivo` con fallback 280/ac4 sobre el considerando) colapsó a una línea; `classify_outcome` maneja ahora `por_ello_text` vacío con base `sin_dispositivo`. Refactor con red: equivalencia old↔new validada por triplicado (probe de whitespace sobre los 25 sin_dispositivo = 0 flips; old↔new sobre los 5862 con patrones verbatim = 0; vs la `classify_outcome` ya escrita en el archivo = 0) → check_regresion [CLEAN] 4/4 (5862/27463/140956/151), 0-delta. parser v18.08→18.09, commit d0ced2b; manifiesto regenerado a 18.09 (sha256 de los CSV sin cambios), --verify [CLEAN] 54. Diagnóstico nuevo: `scripts/diagnostico/H090/extraer_por_outcome.py` (extractor parametrizado --outcome/--exclude-outcome/--considerando-regex) → recortes 280 (238), ac4 (50), frontera_280 (70). Hallazgos relevados (NO aplicados): la detección del 280 NO está contaminada por disidencia ni residuo (165/238 280s tienen disidencia pero 0 filtran texto de disidencia al considerando —corte en `por_ello_idx`—; 4 casos arrancan con encabezado propio por fallback `inicio_cons=0`, no residuo ajeno, sin misclasificar); y la **frontera del 280 parcial** son 70 casos (art.280 en el considerando pero outcome merit): 28 con 'parcial' explícito (concede parte + 280 al resto) + 42 sin 'parcial', y dentro de estos el `art.280` es polisémico (referencias al 2º/último párrafo = memorial/traslado, no el rechazo discrecional). El short-circuit de merit los mantiene bien como merit. Material para el Frente 280 (ítem 3). | H088: M14 ext — sellado del corpus crudo en el manifiesto, último eslabón de la cadena que faltaba fijar. `generar_manifiesto.py` v1.0→v1.1, `schema_version` 2→3: nueva capa `corpus` DERIVADA de la columna `source_file` de csjn_casos.csv (no glob, no lista a mano) → sella exactamente los 46 `.md` que produjeron el dataset; los parqueados 335/336 (4 archivos) se excluyen solos y entran al desparquear sin tocar código. Por archivo sha256+n_bytes; arriba un `digest` rolled-up (huella única, smoke detector) mientras el chequeo por-archivo localiza cuál cambió; fail-loud si el dataset declara una fuente ausente. `entrada_artefacto()` generalizada a raíz del repo (sirve output/ y corpus/). Validado en máquina real: generar OK (corpus 46, digest 9fdd4726ce6d), check_regresion [CLEAN] 4/4, --verify [CLEAN] 54 artefactos. NO toca CSV → golden [CLEAN] por construcción, parser sigue v18.07. Commit f8233ff. Hallazgos del frente datos (relevados, NO aplicados): B037 reclasificado no-bug (el `'0'` de dictamen_presente está solo en los 193 sumarios, nunca en un fallo → centinela N/A deliberado); recalibración de `is_originaria` descartada por marginal a la tesis (~1% de casos); `inadmisible_280` resultó no apto para recalibración simple — la señal de art.280 está contaminada (montos, fojas, citas de Fallos t.280) y hay casos de **280 parcial** que exigen decisión taxonómica antes que regex (ver ítem 3 del roadmap). | H087: M14 — manifiesto sidecar de procedencia IMPLEMENTADO. `scripts/pipeline/generar_manifiesto.py` v1.0 (standalone, último paso del pipeline) escribe `output/parser/_manifest.json` de tres capas: (A) git commit + flag dirty —fija todo el código del repo cuando dirty=false—; (B) `__version__` de los 5 scripts de la cadena (detectar_paginas/construir_catalogo/cruzar/parser/parser_editorial), leído estáticamente vía ast sin importar el módulo; (C) sha256 + filas + bytes de los 3 intermedios (mapa_paginas/catalogo/fallos_localizados) y los 5 outputs canónicos. Decisiones: standalone, no hook en parser.py (cubre el 5º CSV de parser_editorial, se re-corre sobre outputs viejos, no engrosa procesar_archivo); allow-list explícita, no glob (excluye BASELINE y el propio _manifest.json, falla ruidoso si falta un canónico); sha256 = 2ª red de integridad además del golden, usable vía `--verify`. NO toca CSV → golden [CLEAN] por construcción; parser sin bump (sigue v18.07). Validado en máquina real: generar OK, check_regresion [CLEAN] 4/4, --verify [CLEAN] 8/8. Conteos sellados: outputs 5862/27463/140956/151/11445, inputs 46936/5862/5862. Pendiente menor: digest del corpus crudo (LibroVol*.md), único eslabón de la cadena aún sin fijar. | H086: R5 — colapso de la cascada de dispositivo Tier 1→2→3→3b→4 a un motor `_barrer()` único + 4 detectores `_cand_*` + 5 llamadas configuradas (M13). NO es extracción pura: reescribe la lógica de las 5 capas (el mismo bucle ×5, difería solo en rango/exclusión de dictamen/detector/fallback); riesgo medio, sobre función ya aislada por R1 con red M12. `_T2_PATS`/`_T3B_ARG_RE`/`_RE_ASI` subidos a nivel de módulo (compilaban en cada llamada); los 12 literales raw quedan byte-idénticos. La cascada de 5 llamadas NO se data-ificó (rangos dependen de runtime, el orden codifica prioridad). resolver_dispositivo 223→63 líneas; archivo 3650→3603. Validación: PoC de equivalencia original↔parcheado (21 dirigidos + 9 adversariales + 60k fuzz idénticos; 5644/5644 dispositivos reales reconocidos por las closures) → check_regresion [CLEAN] (4/4 CSV idénticos al golden); parser v18.06→18.07. M14 (manifiesto sidecar) sigue diferido. | H085: primer refactor del parser bajo la red (M12). R1 — cascada de dispositivo Tier 1→2→3→3b→4 extraída de procesar_archivo a `resolver_dispositivo(bloque, apertura_rel, lineas_dictamen, inicio_votos_indiv) → (por_ello_idx, por_ello_text)` (M13). Extracción pura: 0 cambios de heurística; procesar_archivo 757→543 líneas. check_regresion [CLEAN] (4/4 CSV idénticos al golden); parser v18.05→18.06. Decisión de diseño: trazabilidad de versión NO va en columna de los CSV (rompería el golden y cada bump daría regresión espuria) sino en manifiesto sidecar — diferido a H086 (M14). | H084: diagnóstico de deuda estructural sobre parser v18.05 (set mínimo: parser + DEUDA + BITACORA + csjn_casos.csv + árbol). Recomendación: frente A (refacción REE), primer paso obligado = red de regresión (no había). Construido `scripts/tests/check_regresion.py` + golden de los 4 CSV del parser (casos/votos/zonas/editorial); verificado [CLEAN] (el golden se reproduce a sí mismo). Cierra el agujero "refactor sin red" (ver M12). Hallazgos de datos: dictamen_presente con 3 valores True/False/'0' (B037); 58 outcome=originaria con is_originaria=0 (valida frente D); outcome=otro 688/5862. NO toca pipeline ni outputs. | H083: módulo `estadisticas/` nuevo — extractor Playwright de los tableros Tableau del Anuario CSJN (sortea el AWS WAF vía navegador real); CSV de referencia 2024/2025 (secretaría, materia, admitidos); cruce voto×ministro 2025 extraído; HALLAZGO: voto propio de Lorenzetti 2025 = 12.899 ≈ total art. 280 (12.546), repartido transversalmente entre secretarías → suscripción individual del 280, no fragmentación doctrinaria (material H1/H3). Sesión de frente analítico, NO toca pipeline ni outputs canónicos. Decisión de fondo de H082 (variable materia) sigue postergada. | H080: diagnóstico refinado de tomos 335/336 (Tesseract); ruta de índice 336 diseñada y VALIDADA en construir_catalogo v1.01 (138 entradas, 0 regresión) pero PARQUEADA pendiente tomos papel; main revertido a baseline limpio pre-335 (056c31e); worklist de 62 firmas a escanear generado; infra: repo sacado del sync de Google Drive que corrompía .git. | H079 cont.: 4 minor outcomes
...
**Estado:** Capa 1 IMPLEMENTADA (H112). Capas 2 y 3 + validación PENDIENTES. **Pendientes de integración de capa 1 (para que el sidecar sea canónico):** (1) `generar_manifiesto.py` — sumar `output/parser/csjn_casos_materia.csv` como 6º output a la allow-list y bump v1.0→1.1; SIN esto, `--verify` ignora el nuevo canónico (o rompe la guarda de allow-list). (2) CODEBOOK — documentar `materia` / `materia_capa` / `materia_fuente` (con la nota de que `tributario` es capa 2). (3) `csjn_analisis_v4.py` — left-join por `caso_id_canonico` cuando se use materia.
