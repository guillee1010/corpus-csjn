...
**Última actualización:** 2026-06-27 (H166 — sesión de tooling/auditoría, **NO toca la cadena de parseo** → sin re-sello de manifest, sin entrada de CHANGELOG. **6 gates de cierre** en `scripts/tests/` (allowlist de paths · append-only docs · version-bump por VALOR · wrapper del manifiesto + el `check_regresion`/golden ya existentes), probados en ambos sentidos. **`MAPA.md`** del pipeline (grafo consume→produce de los 15 scripts, verificado en disco: clasificadores = librerías de `derivar_recursos`, `parser_editorial` = librería de `parser.py`). **`csjn_editorial_indice_partes.csv` caracterizado como FÓSIL** (productor real = `scripts/auditoria/H061/crosscheck_indice_partes.py`, muerto; manifest L115 mal atribuido a `parser_editorial`) — democión DISEÑADA y MEDIDA, no aplicada: MISS=0, 478 EXTRA todos en `catalogo.csv` con `nombres_indice` poblado → redundante e incompleto → **M35**. **Limpieza raíz/pipeline:** baja `extraer_lote_M20.py`; `335 y 336/`+`diagnostico/`+`epilogo_muestra/`→`archivo/`; `extraer_recuperados_H109`→`diagnostico/H109`; gold H163 (`partes_adjudicacion_manual.csv`, 30 overrides) PRESERVADO en `diagnostico/H163`; `check_regresion [CLEAN]` post-limpieza (cadena intacta, 5890 casos byte-idénticos). Deuda nueva: **M35** fósil · **M36** derivers sin golden · **M37** BITACORA atrasada (H164–H165) · **M38** disparo de gates. Allowlist `[CLEAN] 412`. *El detalle de H165 (B136, `derivar_partes` v0.17) vive en la fila B136 del Tablero y en M20; su narrativa va al backfill de M37.*)
...
| M35 | `csjn_editorial_indice_partes.csv` = output **FÓSIL** (productor muerto H061; manifest L115 mal atribuido a `parser_editorial`) — demover | **NUEVO H166** — confirmado_cuantificado: MISS=0; 478 EXTRA todos en `catalogo.csv` con `nombres_indice` poblado → redundante e incompleto. Fix diseñado (sacar de `output/parser/`, borrar L115 manifest → 64 artefactos, re-sellar, revisar Dataverse), NO aplicado. **Requiere OK** |
| M36 | harness de regresión cubre solo los 5 CSV del parser; los 5 derivers (partes/materia/recursos/epilogo/indice) **SIN golden** | **NUEVO H166** — confirmado. Extender golden a derivers o englobar en M24 |
...
**Diagnóstico / evidencia (medido en disco H166, no testimonio):** vs `catalogo.csv` (extracción robusta del MISMO índice oficial) — **MISS = 0** (el fósil no aporta ningún caso que el catálogo no tenga); **478 EXTRA**, los 478 EN `catalogo.csv` por página, **478/478 con `nombres_indice` poblado** en el catálogo. Conclusión: el fósil es redundante E INCOMPLETO frente al catálogo (478 casos que el fósil perdió y el catálogo cubre con sus partes). NO es falso positivo del parser — si lo fuera, no estarían en el catálogo.
**Estado de verificación:** `confirmado_cuantificado`.
**Validador propuesto:** demover — sacar `csjn_editorial_indice_partes.csv` de `output/parser/`; borrar la **línea 115** del manifiesto (→ 9 outputs / 64 artefactos); re-sellar con `generar_manifiesto.py`; revisar/actualizar el dataset publicado en Dataverse (doi:10.7910/DVN/TJTVKW), que contiene el fósil. Caracterización conservada en `scripts/auditoria/H166/caracterizar_extra_indice.py`.
**Estado del fix:** diseñado y medido, **NO aplicado** (toca el manifiesto sellado y el Dataverse → commit/sesión propia). **Mueve un artefacto publicado → requiere OK explícito de Guillermo.**
