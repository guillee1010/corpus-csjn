...
### B108 — `competencia` (incompetencia originaria) cae a `outcome=otro` — CORE CERRADO H103
...
**Estado del fix:** CORE aplicado (CERRADO H103). PENDIENTE la frontera: (a) **19** «la presente causa corresponde a la competencia originaria» (la Corte ACEPTA) → decisión taxonómica `competencia` vs `originaria` (toca Frente D / is_originaria); (b) **9** «remítanse las actuaciones al juzgado [específico]» → eyeball, probable `competencia`; (c) los **5** «remítanse al tribunal de origen/procedencia» DESCARTADOS (devoluciones procesales, quedan `otro` bien); (d) **48** `otro` de remisión pura al dictamen («por los fundamentos del dictamen…», sin verbo de competencia en el `por_ello`) → van a B105.
