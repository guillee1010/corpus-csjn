...
**Estado del fix:** **diseñado H123, NO aplicado.** El fix local (saltar banner en el chunk de `_barrer`) queda **SUBSUMIDO por M21** (`normalizar_bloque`) → NO se shippea suelto. Banco de prueba: los 42 + los 25. Validación: A/B + check_regresion corpus-wide bajo el harness del blueprint.

### M21 — Pre-pasada de normalización del bloque (`normalizar_bloque`) — DISEÑADO H123
...
**Estado del fix:** diseñado (blueprint `BLUEPRINT_normalizar_bloque.md`); PoC pendiente (sesión dedicada H124+, posiblemente >1).
