...
**Anterior:** 2026-07-26 (H215 — **Sesión de MEDICIÓN + CIERRE RETROACTIVO DEL BACKLOG H211. Pipeline INTOCADO: 0 scripts de la cadena, 0 `__version__`, 0 outputs canónicos; parser v38.0 y `derivar_materia` v4.1 sin cambio; manifest [CLEAN] 65 VERIFICADO en disco, sin re-sello — Paso 2 del cierre N/A, candado blind N/A.** **B172 RE-MEDIDA y FIX REDIRIGIDO (A)→(B).** (1) **La carátula NUNCA se pierde:** `case_name_indice` tiene 0 vacías en las 5894 filas → el problema de identificador no existe y las variantes «repoblar desde el índice / desde el encabezado de página» quedan SIN OBJETO. Lo que el fallback intenta recuperar no es la carátula sino los AUTOS COMPLETOS. (2) **El fallback NO compra cobertura:** `is_originaria` por rama = V1 165/3795 (4,3%) · fallback 263/1171 (**22,5%**) · vacío 168/928 (**18,1%**) — con el campo VACÍO la detección funciona igual; y DENTRO del fallback las filas dañadas dan 24,5% contra 20,9% las sanas (indistinguible). La tasa alta es de la POBLACIÓN (las competencias abren «Autos y Vistos:»), no del campo. **Corrección de constancia a H214, que había conjeturado lo contrario.** (3) ★ **Y sí inyecta dependencias falsas: 34 filas cuyo string contaminado contiene la palabra «Originario» —porque citan un precedente originario— tienen `is_originaria=1` en 34/34 (100%), contra 20,1% de base en la misma rama; V1 y vacío tienen CERO filas con esa palabra en el campo.** Un 100% contra un 20% no es efecto de población: es dependencia MECÁNICA de una subcadena de OTRO caso. Perfil de las 34: `outcome` competencia 23 / otro 8 / sin_dispositivo 1 / desestima 1 / hace_lugar 1; `is_merit_decision=1` en 0; 16/34 conservan marcador de cita visible (`in re`, `L. NNN`, romano). **CAVEAT OBLIGATORIO: no son 34 FP declarados** — `330_p2610` (Mendoza c/ Estado Nacional, Riachuelo) es originaria genuina, y en las 23 con `outcome=competencia` declarar la propia competencia originaria es el resultado esperable. **La procedencia está rota en las 34; cuántas están además materialmente mal exige adjudicarlas una por una.** (4) `es_queja` por rama = 57,1% / 8,0% / 4,2%; dentro del fallback las dañadas dan 4,8% contra 10,4% las sanas — el string contaminado tampoco ayuda acá. (5) **FIX REDIRIGIDO: (B) ELIMINAR EL FALLBACK** + columna de procedencia `case_name_cuerpo_fuente` ∈ {`v1`,`fallback`,`vacio`}, en vez de (A) guard-con-fallback. Deja de ser MAJOR sobre `find_case_name` y pasa a ciclo chico; **flip-set de adjudicación = las 34**. (6) **B106 acotada:** las 928 desagregadas — `word_count=0` 191 · `apertura_tipo` vacío 236 (`apertura_rel is None`, ni V1 ni el fallback corrieron) · `tipo_entrada` de sumario 191 (34 editorial + 157 con_link, que NO son fallos y correctamente no tienen autos) · `case_name_indice` vacío 0. La cota superior de 928 BAJA sustancialmente. (7) **TRAMPA DE INSTRUMENTO:** hoy la rama solo es identificable por el accidente `cuerpo == legacy`, y L4101 marca `case_name_cuerpo_legacy` como «eliminable en una corrida posterior» — si esa columna se borra, la partición V1/fallback/vacío deja de ser reconstruible y las filas dañadas se vuelven invisibles. La columna de procedencia va ANTES que cualquier borrado. (8) **CIERRE RETROACTIVO DEL BACKLOG H211:** los commits (3) y (4) declarados en la entrada H211 nunca se ejecutaron — verificado en `git log` (tras `15e415f` no hay commit que toque `derivar_materia`; HEAD arrastraba el manifest de las 20:57 con v4.0). Gate previo al commit, los tres PASS: `__version__` 4.1 · sha materia `92630d63` · manifest `[CLEAN] 65`. **No era brecha de validación** — los candados N1/M1 y la muestra seed 211 (24 TP / 1 FP) estaban adjudicados desde H211. Segundo cierre retroactivo del proyecto (el primero fue H210 por el backlog H207-H209) → **M63 nueva**. (9) Constancia: el docstring de `derivar_materia.py` conserva «rinde esperado ~215», número que el propio diseño tiered descartó — corregir cuando se toque el script. (10) **H213 CERRADA RETROACTIVAMENTE** (sesión del 19-jul, que había quedado abierta y que H214 explícitamente no absorbió). Los tres frentes se cierran con su estado real y lo no hecho migra a entrada propia, para que el cierre no sea una amnistía: **Frente A (Anuario)** — decisión sellada MAPEAR (no adoptar, no ignorar), pero `anuario_mapeo.csv` nunca se formalizó y quedan 4 categorías sin resolver → **M64 nueva**. **Frente B (7 casos gate-kw)** — sigue SIN adjudicar; se congeló esperando el sistema de gestión y post-feria no se retomó. **Frente E (DJA)** — medido, con estructura de `dja_mapeo.csv` borradoreada; su continuidad ya vive en **M61**, no necesita entrada nueva. **Constancia de numeración:** el número H213 estaba RESERVADO para el relabel terminal de `competencia_dirimente`/`procesal_propio` (decisión taxonómica H209); la sesión del 19-jul tomó el número y trabajó otra cosa, así que **el relabel terminal sigue pendiente y sin número**. **Tensión sin resolver que se documenta en vez de zanjarse:** en H212 el operador rechazó usar la secretaría de radicación como criterio de verdad para adjudicar materia (circularidad metodológica), y en H213 la señaló como la vía correcta para los casos ambiguos (los dos FP de `aduanero`); H214 agregó que la secretaría es una TRAYECTORIA y no un valor único. Las tres constancias no son necesariamente incompatibles —criterio de verdad del clasificador vs. evidencia sobre un caso puntual— pero la distinción no está sellada y bloquea el Frente B. Pendientes siguen: B168 · B169 · B170 · B171 · **B172 (ciclo (B), flip-set 34)** · B106 (cota acotada) · **Frente B (7 gate-kw, sin adjudicar)** · relabel terminal (sin número) · M57 · M61 · M62 · **M63 (nueva)** · **M64 (nueva)** · M43 ÚLTIMA.)
...
### B168 — Colector de firma traga el VOTO entero: header de voto invisible → firma_raw inflada, n_jueces con duplicados, posiciones PISADAS — **VOCABULARIO CERRADO H216 (parser v39.0)**; residual = clase E wrap-truncado (21) + dedup por nombre (2063)
