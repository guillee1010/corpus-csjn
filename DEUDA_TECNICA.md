...
**Última actualización:** 2026-07-01 (H171 — **Cardinalidad corpus-wide de los bugs del gate (read-only, 0 cambios canónicos)**. Baseline D1 reproducido exacto (234 / 0 residuales, taxonomía M1-M5) y medición corpus-wide en una pasada (`scripts/diagnostico/H171/cardinalidad_gate.py` v0.1→v0.2, importa detectores reales; anclas cruzadas contra D1 [OK]; sanity: `disposicion()` recomputada == CSV en los 5697). Resultados: **B138 = 19 CERRADO, invisibles a D1 = 0** (el mecanismo siempre diverge; 2 testigos REX nuevos leídos: 330_p1205 art. 280 disfrazado de «rechaza», 348_p747 remisión/art. 15 «mal concedido» — ambos FP del gate; residuo sin leer: 2 ordinarios + 4 REX). **B139a = 59 brutos** estratificados (FN material real — 3er testigo 330_p1927 «confirmar el acto administrativo» — · procesal-correcto · truncados M21/B122 — 329_p4634 confirmado contra fuente, sale del universo — · ruido del verbo pelado). **B139b = 13/178 nrd**: la vía derivada captura los 13 incl. todos los art. 16; señal restringida a NO-originarias (FP confirmado 331_p1262: única mención REX = cita de Fallos 324:2153). **B140a**: población gate=si = 27 [OK ancla]; 3 hits fuertes en `pe` → 340_p1973 adjudicado ACIERTO (art. 16 in fine invocado expresamente), 329_p5261 ADC único FP confirmado, 329_p5115 PENDIENTE decisión doctrinal (¿arbitrariedad-remand = fondo?); los 29 hits en `co` son boilerplate débil. **B140b = 10 CERRADO, 9 invisibles a D1**, fórmula única «nulidad de la resolución por la que se concedió el recurso» (sin «extraordinario») — testigo 329_p120 leído + guard `RE_NULIDAD_CONCESION` verificado no-match sobre el `pe` de producción. Pendientes H172: decisión doctrinal 329_p5115, leer 334_p1302/342_p1524 (ordinarios B138), luego diseño de guards según orden LOCKEADO de M39 (B135 primero).)
...
| B138 | `RE_RECHAZA_REC` sobre-dispara en denegatoria de acceso pura (queja/recurso de hecho) → `es_revision_fondo=si` + `recurrente_pierde` espurios | **NUEVO H170** — confirmado_cuantificado parcial: 19 en la divergencia D1 (testigos 329_p4083/330_p826 leídos, 0 fondo); cardinalidad corpus-wide PENDIENTE (donde outcome=confirma ambas capas fallan juntas, invisible a D1). Fix preferido: guard en `es_revision_fondo` (patrón B119/B131), verbo lockeado intacto |
| B139 | FN de `disposicion()`: verbo de fondo con objeto MATERIAL («confirmar el reajuste») + «rechaza la demanda» en sede recursiva (art. 16 in fine ley 48) | **NUEVO H170** — confirmado_caso_testigo (332_p731, 340_p411, 331_p100 leídos). M2B adjudicado MIXTO (2 FN gate + 2 outcome equivocado sobre 4). Cardinalidad corpus-wide pendiente; sub-causa (b) probablemente resoluble cruzando `via` |
| B140 | FP del gate: revoca-FORMAL en abstracto (doctrina «sin perjuicio de revocar…no importa abrir juicio») + nulidad-de-concesión fuera de la ventana del guard B131 | **NUEVO H170** — confirmado_caso_testigo (329_p5261 ADC, 343_p2098 Paccagnini). El tercero del sub-audit (337_p850 Malossi) resultó fondo real con aside «inoficioso…recursos de hecho» (variante B129 lado parser). Cardinalidad pendiente |
...
| M39 | eje de mérito BICAPA divergente: `is_merit` ⟺ `es_revision_fondo` — 234/5697 mismatches (4,1%), 0 en originarias, taxonomía M1-M5 cerrada 100% (19 testigos leídos) | **NUEVO H170 (entrada madre de D1)** — confirmado_cuantificado. Desglose: M1 15 (→B135) · M2A 98 (gate ok) · M2B 18 (MIXTO: →B139 + outcome) · M2C 13 (4 originarias confirmadas + B139b) · M3 44 (gate ok) · M4 24 asides B129 exactos + 3 (→B140 ×2 + variante aside) · M5 19 (→B138). ORDEN DE FIX LOCKEADO: B135 → guards del gate (B138/B139/B140) → extender patrón B136 corpus-wide. Orden inverso consagra los FP como canon. Próximo: cardinalidad corpus-wide de los 3 bugs del gate en una pasada |
...
### B138 — `RE_RECHAZA_REC` sobre-dispara en la denegatoria de acceso pura → `es_revision_fondo=si` espurio
...
**Validador propuesto:** ~~medición corpus-wide~~ **EJECUTADO H171** (`scripts/diagnostico/H171/cardinalidad_gate.py` v0.2). Falta: leer los 2 ordinarios antes de fijar el guard.
**Estado del fix:** no diseñado. Restricción dura: `disposicion()` está LOCKEADA (blind 0,930) → **preferir el patrón B119/B131: guard nuevo en `es_revision_fondo`** (denegatoria-de-acceso → no), verbo intacto, sin re-validar el blind. **Restricción nueva (H171): el guard debe discriminar POR OBJETO — queja/recurso de hecho/reposición → no es fondo; REX/ordinario requieren discriminador propio (280/remisión-mal-concedido vs. REX tratado al fondo), NO barrer todo `RE_RECHAZA_REC`.** Si se toca la regla misma → re-validación held-out completa obligatoria. Impacta `parte_ganadora` (re-κ parte).
**Referencias cruzadas:** M39 (M5), M25/κ-parte, B119/B131 (patrón guard-en-gate). H170. Sin ID histórico.

### B139 — FN de `disposicion()`: verbo de fondo con objeto MATERIAL, y «rechaza la demanda» en sede recursiva (art. 16 in fine ley 48)
...
**Estado del fix:** no diseñado. `disposicion()` LOCKEADA → guard / segunda pasada, NO edición del verbo.
...
### B140 — FP del gate: revoca-FORMAL en abstracto (sin juicio de mérito) y nulidad-de-concesión fuera de la ventana del guard
...
**Validador propuesto:** ~~(a) outcome=abstracto ∧ disp∈fondo + detector; (b) nulidad+conce* sin guard~~ **EJECUTADO H171** (`cardinalidad_gate.py` v0.2). Falta: decisión doctrinal sobre 329_p5115 (cierra el universo de (a)).
**Estado del fix:** no diseñado. Candidatos: (a) **si la arbitrariedad-remand se cuenta como fondo, el universo es 1 caso (ADC) → evaluar documentar como FP conocido en vez de guard**; si no, guard `abstracto+revoca-formal` en `es_revision_fondo` mirando SOLO el `pe`; (b) **cubrir la conectiva «(resolución|auto|decisión) … por (la|el) que se concedió el recurso» SIN exigir «extraordinario»** — ensanche del guard `RE_NULIDAD_CONCESION` o segunda pasada; los 9 comparten fórmula literal → fix quirúrgico de bajo riesgo, validar 0 regresiones sobre los matches actuales del guard. Verbo lockeado intacto en ambos.
**Referencias cruzadas:** M39 (M4_ABSTRACTO_OTRO), **B131** (el guard cuya ventana falla), B129 (familia aside; la variante «inoficioso…recursos de hecho» del lado parser queda anotada en M39), B133 (mootness — colindante, no causa). H170. Sin ID histórico.
