...
**Última actualización:** 2026-07-15 (H210 — **CIERRE RETROACTIVO H209 VERIFICADO EN DISCO Y COMMITEADO (backlog git H207–H209 pusheado; corrección de constancia: los «Commits: 2» declarados en H208 nunca se habían ejecutado) + M49 APLICADO: `derivar_materia` v3.3→v4.0 (MAJOR) — `materia_capa` renombrada a lectura_tribunal / lectura_tribunal_refinada / lectura_texto / sin_clasificar; candado `verificar_m49` PASÓ (cross-tab 1:1 exacta, 0 pares fuera, materias/fuentes/orden idénticos); sha materia 89eb50fe…→607793de…; resto de la cadena byte-idéntico; manifest re-sellado [CLEAN] 65.** Detalle en Prioridad H210 y entrada M49. Constancias: M51 segunda evidencia de ergonomía (ABORT corpus-drift) · M60 nueva (csjn_analisis reescritura post-madurez) · explorador v8.2 no canónico (ID clickeable).)

**Anterior:** 2026-07-15 (H209 — **M59 PASO 1 APLICADO: `extraer_normas` v1.0 etapa canónica (4 ámbitos: caratula/considerando/dispositivo/voto) → `csjn_casos_normas.csv` + `derivar_materia` v3.2→v3.3 refactor byte-idéntico (candado sha 89eb50fef37b PASÓ) + `correr_pipeline` v1.1 + `generar_manifiesto` v1.9 (10 CSV) + MAPA — manifest re-sellado [CLEAN] 65.** Baselines v0.2 reproducidos exacto (411 carátula / 5562 considerando / gap 265); primeras mediciones corpus-wide: dispositivo 161 pares / 78 normas / 145 casos · voto 1562 / 580 / 825 — extraídos INERTES para la cascada (v3.3 = refactor puro; habilitar cada ámbito = unidad con flip-set propio, M59 paso 2). **DECISIÓN TAXONÓMICA RESUELTA (ex-pendiente H208): diseño de DOS EJES** — competencia_dirimente y procesal_propio reciben categoría TERMINAL (espejo `originaria` v3.2) + columna futura `materia_subyacente` poblada vía voces JYC (ancla B163); dos fases, relabel terminal primero (más barato). **M49 RATIFICADO** (esquema sellado en la entrada; en H209 estuvo por duplicarse como «M60» — NO crear): implementación = unidad H210 (derivar_materia v4.0 MAJOR). Nota de registro: la sesión quedó sin cierre en el momento (cierre de browser durante la conversación de legibilidad); documentación reconstruida el mismo día desde el registro — VERIFICAR manifest con `--verify` antes del commit.)
...
**Referencias cruzadas:** M59 (sidecar de normas), B171 (señal de remisión), B163 (voces JYC — canal hermano), H113 (precedente del split de textos), M48 clase (1). H208. Sin ID histórico.
