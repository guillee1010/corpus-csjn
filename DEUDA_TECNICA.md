...
**Última actualización:** 2026-06-13 (H122 — validación formal M20 sellada + disposición productivizada). **M20 — validación formal CERRADA** (branch `refactor/m20-disposicion`, sin merge). Generador de la clave (`build_m20.py`) **recuperado y versionado** (causa raíz: estaba perdido, nunca commiteado); el clasificador reproduce la clave previa **300/300**; clave regenerada del `csjn_casos.csv` actual y `validar_H120.py` corrido formal. **Distinción honesta de defendibilidad:** gate **0,953 in-sample post-B119** (B119 se tuneó sobre el n300) / **0,907 held-out pre-B119**; disposición **0,857 blind** (regex congelada pre-gold), parte_ganadora 0,794, reenvía 0,773 — los blind son los números fuertes; cuestión federal 0,676, dictamen 0,930 / adhesión 71%; certiorari criollo queja 95% vs concedido 68% (H1). **Disposición productivizada (v0.1):** `clasificador_disposicion.py` v1.0 (módulo fuente-única, importado por validador y producción → sin drift) + `derivar_recursos.py` v0.1 → `csjn_casos_recursos.csv` (5890 filas, cobertura merit **92,8%** = H118; consistencia vs clave 300/300). **Hallazgo:** 166 casos no-merit con verbo de disposición = banco cuantificado de candidatos gate-FN para B120. **DEUDA metodológica destacada (M20):** el n300 se usó como set de ajuste (B119) Y de validación → in-sample; no puede ser dev y validación de la misma variable. Decisión que BLOQUEA B120/B122/B111: split dev/test del n300 + ledger de uso + verificar si materia/B118 se calibraron sobre el gold. Parser **v18.26 sin cambios**, 5 CSV canónicos intactos, golden sin tocar, **manifest NO re-sellado**; `csjn_casos_recursos.csv` NO sellado aún (pendiente al estabilizar deriver + merge). **Pendientes M20:** decisión del gold (bloquea); v0.2 `derivar_recursos` (via_recurso/admisión/eje queja-concedido); B120 (banco=166); B122 (banco=25 flagged); `derivar_dictamen.py`; fix B111. **Pendientes previos vigentes:** CODEBOOK materia; csjn_analisis_v4 left-join; refinamiento CA; test-retest/kappa (en pausa).
