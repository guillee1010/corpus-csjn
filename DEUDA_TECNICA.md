...
**Última actualización:** 2026-07-29 (H220 — **Sesión de SUPERFICIES read-only + cierre real de H219. Pipeline INTOCADO: 0 scripts de la cadena, 0 `__version__`, 0 outputs canónicos; parser v42.1 sin cambio; manifest [CLEAN] 65 vigente SIN re-sello.** **(0) Cierre real de H219 (M63, TERCERA ocurrencia):** los 2 commits declarados estaban SIN emitir (HEAD en docs-H218). Candados verificados en disco ANTES de emitir: casos 5894×40 · fuente v1 3821/fallback 1151/vacio 922 · golden==producción `59f2c4792262` · manifest [CLEAN] 65. BITACORA reparada: fence huérfano L20014 borrado (la entrada H219 entera renderizaba como bloque de código) + bloque H219-07 «Estado final» escrito CONTRA `git status` (regla M63), con constancia: adjudicación fina ⊆73 de los derivers SIN hacer — normas 13.908→13.911 (+3, sha `5adc4b9cdead`) · materia `56d5b601017a` · partes `df83ac2b7803`, movidos sin adjudicar. Commits emitidos, árbol limpio; push pendiente al cierre. **(1) B170 — validador EJECUTADO, pool SELLADO:** `superficie_ivi_b170.py` v0.3 (H220/; fuente única `_hdr_tipo_wrap`/`detectar_votos_disidencias` importados; `lineas_excluir` reconstruida de zonas.csv — invariante bajo relabel A1 por construcción; E0 5737/5737 · E0'' [OK] · E1 sin-zonas 191 == `sumario_con_link` 157 ∪ `sumario_editorial` 34): **2879 matches = prev_cerrada 2231 · prev_abierta 647 · prev_fallos 1** (la sub-clase LITERAL del testigo 348_p728 — «(Fallos:» colgante — es cardinalidad 1 corpus-wide); 648 sospechosas en 612 casos; **el ivi se mueve en 496 casos si la clase cae, 496/496 con techo T1/T2 activo** = cota superior, sin adjudicar. Consumidor extra leído en código: guard de vp L4504. **(2) B171 — validador EJECUTADO (nivel voto), pool SELLADO como COTA INFERIOR:** `superficie_remision_b171.py` v0.1 (H220/; clasificador real importado; cabeza=`texto_voto[:4000]` ⊆ el `[:5000]` publicado → réplica exacta desde votos.csv; E0 27.900/27.900): **75 matches** = F1 43 (32 hoy `indeterminado`; **riesgo B067 MEDIDO: 16/43 en contexto precedente/ambos**) + F2 16 + F3 16/15. Testigos 4/4 adjudicados con texto: CERO votos separados en los 4 — sus fórmulas viven en el considerando de MAYORÍA → **B171 tiene DOS NIVELES** (voto medido / mayoría SIN medir; consumidores M58, tesis-H2, `lectura_dictamen`). 339_p1683 = variante NUEVA «cuyos términos SE DAN POR REPRODUCIDOS» + **soft hyphen `\xad` que parte «dic­tamen»** y enmascara fórmulas YA cubiertas (la cabeza no se des-guiona, L1965). **Correcciones de constancia:** (a) al registro H219: los «stub 191 exentos» de `poc_z_caption` son `sumario_con_link` 157 ∪ `sumario_editorial` 34 — los 34 SÍ se evalúan en producción; (b) la cita de 339_p1683 en B171 era laxa. **Constancias de método (asistente, 5):** contador de líneas físicas por filas CSV · `csv.reader` sin `field_size_limit` · corrida pedida con el dato ya demostrado por sha · gate de stubs NO replicado en v0.1 (lección poc_z_caption/H219 REPETIDA) · E1 v0.2 postuló identidad falsa stubs==sin-zonas, corregida con dato. Candidatas para la próxima: **adjudicación estratificada de `prev_abierta` (647/612 — decide B170)** · **B171 nivel-mayoría** (hermana barata, + des-guionado) · unidad 3 del PROMPT_H220 (título de cabeza/Eje A) · B175+B176 · decisiones del operador: validación H208 (NO abandonada — verificar primero campos del baseline) y M62 · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-27 (H219 — **frente C, clase Z_caption_anidado CERRADA: parser v42.0→v42.1 (MINOR, precedente B135/H172), ciclo completo, manifest [CLEAN] 65; B173 validador (i) EJECUTADO: GAP HUÉRFANO.** `extraer_caratula_v1`: cierre por CLASE PAR de la comilla de apertura + guard de captura no vacía + fallback `_extraer_caratula_v1_legado` verbatim (patrón D2/H201: todo flip tiene cierre genuino) + `_acotar_primera_causa` GATEADA a capturas que ARRANCAN con comilla (decisión del operador: el campo lleva la CAUSA LÍDER, denominación SJ-CSJN — el expediente del titular ni figura en el caption; corte en ’+separador, fallback separador+patrón de expediente para la ’ perdida de 337_p1006). El anidamiento es CONVENCIÓN de acumulados (cada causa en simples dentro del caption en dobles, 5 tomos), no OCR — el OCR agrega ruido encima (” impresa como “ en 330_p2414, «““» duplicada en 339_p841). CONTRATO DEMOSTRADO celda a celda (`verificar_h219.py` v2, H219/, **[CLEAN] 7/7**, PRE de git HEAD extraído vía `cmd /c`): columnas tocadas EXACTAS {case_name_cuerpo: 73, case_name_cuerpo_fuente: 15} · casos con diff == baseline 73/73 · valores y transición de fuente 73/73 · conteo fuente 3806/1160/928 → **3821/1151/922** (v1 +15 = 9 del fallback + 6 del vacio) · barrido completo 5894×40 sin diffs fuera. FLIP-SET SELLADO 73 (`poc_z_caption.py` v0.2, H219/, baseline `poc_z_caption_baseline.csv`; E0/E0'' 5658/5658 con los DOS gates de producción replicados — stub 191 + apertura None 45 EXENTOS): **58 reparaciones-v1** (A prefijo de B 58/58 — la captura truncada en comilla simple interna se EXTIENDE al cierre real: apóstrofos D'Amore/O'Donnell/Moliné O'Connor/Brink's, apodos citados 'El Sol'/'El Día', acumulados rituales completos) + **9 Z_a_valor** + **6 Z_rama_vacio**. **CORRECCIÓN DE SUPERFICIE a H218: la clase Z real es 15, no 9** — la superficie solo clasificaba `cuerpo != ""`; los 6 de la rama vacio eran invisibles (firma inequívoca: fuente=vacio ∧ legacy==""). RIPPLE DE DECISIÓN VACÍO, medido (`poc_z_ripple.py` v0.1, brazos con es_originaria/classify_queja/es_revision_fondo REALES sobre los 73; candados E0 4× 73/73): is_originaria 0 · es_queja/queja_resultado 0 · tribunal_origen_status/is_merit/tipo_voto_sep 0 · **clave n300 INTACTA** (0 del flip-set en el n300; ausente de git status post-build_m20) → sin acople M43. Byte-idénticos DEMOSTRADOS POR SHA (== sello H218): textos `f2dcf8fa46f3` · votos `5df40a20cfd5` · zonas `0a5b1b650d47` · editorial `30a6da652e3a`; casos `46abe17fe666`→**`59f2c4792262`**. Derivers re-derivados con movimiento ⊆73 (dirección INVERSA a H218 — la concatenación carátula GANA contenido): materia cobertura 77,5%→**77,7%** · partes recurrente_ok 3897→**3899** (+62 vía carátula, mérito 90,6%) · normas ámbito carátula 393→**396 pares** (72 normas / 382 casos); epilogo/recursos byte-idénticos. Regresión propia post-fix: E0 5658/5658 + **flip-set VACÍO**; E0'' mismatch == los 73 exactos (la réplica A' es v42.0 — esperado, patrón H217-U2; la expectativa enunciada «E0'' limpio» estaba mal, constancia menor). **B173 SELLADO por dump L26315-26350 de 329.2: GAP HUÉRFANO COMPLETO, no bleed** — el Rojas genuino [26324-26347, 0-based] no lo posee ningún bloque (2063 fin_real 26323; Vignolo inicio 26348 INTACTO); trae V1 canónica ritual; bonus: el bloque de 2063 DUPLICA la cola de Rusillo que 2062 ya posee (ambos fin_real 26323, solape fin_extendido). Sigue PARKING. **POOL LATENTE anotado → B174 (nueva):** 6 fallos con apertura None gateados por L4193 donde V1 dispararía desde 0 (329_p988, 329_p1930/B165, 329_p4862, 331_p1592, 334_p1193, 346_p1241/testigo-B106). **TRES CONSTANCIAS DE MÉTODO (todas del asistente):** (1) el comando de extracción del PRE usó `git show | Out-File` en PowerShell → PowerShell re-decodifica el stream con la codepage de consola → mojibake de todo no-ASCII (C3/C5 espurios con miles de celdas «tocadas» en columnas con tildes; vía correcta `cmd /c "git show … > file"`); (2) `verificar_h219.py` v1 con ⊆ en prints sin `errors="replace"` → UnicodeEncodeError bajo redirección, clase H174 — entregado auto-testeado pero SIN test bajo redirección; v2 con patrón H174 + lectura utf-8-sig, auto-testeado 5/5 incl. control que reproduce el crash; (3) el candado C6 esperaba `vacio 928→913` — error aritmético: restaba los 15 migrantes completos del vacio cuando migran 9 desde fallback y 6 desde vacio (913 ni sumaba 5894); la cifra errada estaba TAMBIÉN en el changelog de `parser.py`; ambas corregidas pre-commit (hash post-fix `cda9b0fe0884bd42…` verificado por certutil). Pendientes siguen: B168 residual 329_p5741 · B173 (parking, gap adjudicado) · B169 · B170 · B171 · **B106 (6 recuperados H219, vacio 922; testigo 346_p1241 → B174)** · **frente C restante = 32 recuperables (B 15 · F1 10 · S 6 · E 1) sobre techo 1119 — Z CERRADA** · **B174 (nueva: gate L4193 apertura-None suprime V1, pool 6)** · **B175 (nueva: `_RE_SOBRE`/343_p1237)** · **B176 (nueva: `RE_CONECTOR_CARATULA_V` FP de inicial media)** · título-de-cabeza/Eje A (potencial 2578 sin medir) · borrado de `case_name_cuerpo_legacy` (decisión propia) · M57 · M61 · M62 · M63 · M64 · M65 · relabel terminal · validación operador H208 · M43 ÚLTIMA.)

**Anterior:** 2026-07-26 (H218 — **B172 CERRADO: parser v41.0→v42.0 (MAJOR, schema de casos.csv 39→40), ciclo completo, manifest [CLEAN] 65. Sale el `or` de L4169 — `case_name_cuerpo = extraer_caratula_v1` y punto — y entra la columna de procedencia `case_name_cuerpo_fuente` ∈ {v1, fallback, vacio}.** CONTRATO DEMOSTRADO celda a celda (`verificar_h218.py`, H218/, [CLEAN] en los 6 candados, PRE extraído de git HEAD): schema 39→40 con la única columna nueva · `fuente` v1 **3806** / fallback **1160** / vacio **928** · `case_name_cuerpo` vaciada en **1160 filas exactas, 0 a otro valor** · ese set == `fallback_real` de la superficie, exacto · **0 diffs en las otras 38 columnas viejas** · las 11 `v1_coincidente` conservan su carátula. Byte-idénticos DEMOSTRADOS POR SHA (mismos hashes que el cierre H217): textos `f2dcf8fa46f3` · votos `5df40a20cfd5` · zonas `0a5b1b650d47` · editorial `30a6da652e3a`; casos `d22dd411f633`→`46abe17fe666`. Clave n300 **byte-idéntica** (`build_m20` corrido, ausente de `git status`) → blind vigente, SIN acople con M43. (1) **Pendiente (g) de H215 respondido:** rinde de `derivar_partes` sobre la rama = «ninguna» 75,9% · «solo una» 20,6% · «ambas» 3,5%; los buckets de capa carátula dan 107 en v1, **1 en fallback**, 0 en vacío — y la causa es estructural: `RE_MARK_CARATULA` es `.match()` anclado en `^(?:Recursos?|Queja)` case-sensitive, imposible de satisfacer por un string de la rama. Único costo: `347_p357` (Gil Domínguez) pierde `recurrente` vía `_match_letrado_caratula` (L531), que no exige el marcador ritual y solo pide ` c/ ` — superficie de esa vía = 513 filas, disparó 1 vez, TP. (2) **CORRECCIÓN DE CONSTANCIA a la propagación de H214: no eran tres consumidores sino CINCO.** El grep sobre `scripts/{pipeline,validacion,tests}` suma `derivar_materia` (L91 columnas requeridas, L524 `caratula = case_name_cuerpo + " " + case_name_indice`) y `extraer_normas` (L17/L90/L136, ámbito `caratula`, idéntica concatenación). Ninguna variable de votación lo consume — eso sigue en pie. (3) **CORRECCIÓN DE CONSTANCIA a (c) de H215 — la enmienda ★ era FALSA.** El «34/34 = 100% contra 20,1% de base» se leyó como dependencia mecánica y es **correlación**: las 34 son originarias genuinas y el texto lo dice solo (señales 1/2/4 de `es_originaria` disparan en 33/34; la 34ª por la señal 5). Causa: en `es_originaria` las señales 1 y 2 se evalúan ANTES que la 3 (L1633-1639), y en esta población el considerando dice «competencia originaria» o cita el art. 117 — la carátula nunca llega a decidir. (4) **FLIP-SET DEL CICLO = VACÍO, medido** (`poc_b172_flipset.py` v0.1, H218/, brazos A/B con `es_originaria`/`classify_queja`/`es_revision_fondo` reales sobre el bloque reconstruido; candados E0 orig/queja/merit **1171/1171**): `is_originaria` 0 flips · `es_queja`/`queja_resultado` 0 flips · 0 ripple a `tribunal_origen_status` (columna que la entrada NO declaraba, L4309-4316), `is_merit_decision` ni `tipo_voto_sep` · **clave n300 INTACTA** (0 casos del flip-set en el n300, y `build_m20` proyecta columnas nombradas: la columna nueva es invisible por construcción) → sin acople con M43. (5) **PARTICIÓN VIEJA CONTAMINADA:** `cuerpo == legacy` daba 1171, pero correr `extraer_caratula_v1` real sobre el bloque sella **fallback_real 1160 + v1_coincidente 11** (V1 disparó y `find_case_name` devolvió la carátula del MISMO caso desde el dictamen). La columna de procedencia deja de ser burocracia: sin ella la rama no es auditable. (6) **Superficie del frente (C)** (`superficie_c_v1.py` v0.2, H218/, candado E0 V1-real==""): por qué V1 no dispara — `F2_otra_formula_SIN_caratula` **1089 (93,9%) con `ritual: 0`** (la apertura «Autos y Vistos: / Por los fundamentos del dictamen…» NO NOMBRA LOS AUTOS: techo estructural, no hay nada que ensanchar) · `G` 30 · `B_formula_sin_dos_puntos` 15 · `F1_otra_formula_CON_caratula` 10 · `Z_caption_anidado` 9 · `S_formula_singular_u_ocr` 6 · `E_caratula_sin_comillas` 1 → **recuperable ≈41, techo 1119**. `Z_caption_anidado` = bug del reconstructor: con caption anidado `Vistos los autos: "'X'; Y"` V1 abre en la comilla doble y encuentra la simple en posición 0 → `acumulado[:0] == ""`; la fórmula matchea perfecto y devuelve nada. (7) **Derivers medidos:** `normas.csv` 48 filas cambian / **18 pares desaparecen**, todos `ambito=caratula`, INERTE para la cascada (doble filtro L506-511 de `derivar_materia` solo admite considerando y dictamen) — y de paso queda medido que cambiar `extraer_normas` para leer solo el índice costaría **106 pares legítimos** de la rama v1: se arregla el campo, no el consumidor. `materia.csv` **87 filas** (`poc_b172_materia.py` v0.1, candado E0 5894/5894) = **61 reparaciones · 4 costos · 19 no-ops · 2 dudosos**; 45/53 de las que pierden materia tenían string de OTRO caso. **COSTOS ACEPTADOS 4:** `329_p1243` Blomqvist · `330_p1183` Pavicich · `331_p357` Lawny (los tres pierden `objeto(1)`: el `s/ objeto` vive solo en el cuerpo porque el índice está en forma invertida de catálogo) · `331_p125` Correa («ESTADO NACIONAL» en el cuerpo, «E.N.» en el índice). `343_p1386` Alessandria NO es costo sino **reparación**: el fragmento «PROVINCIA de BUENOS AIRES» sin el «Banco» delante burlaba `_RE_BANCO_PROV` (H114) y el fix restaura el comportamiento diseñado. `343_p1237` Hug = bug del índice (`Federals/` sin espacio rompe el `\b` de `_RE_SOBRE`), ticket propio. (8) **FRENTE NUEVO — título de cabeza como fuente del Eje A** (corrige mi propia lectura de que era redundante): de 141 títulos en versales visibles, **solo 2 son idénticos al índice**; el índice invierte nombres y separa con `|`, el título trae el **orden directo con `c/`** (o « V. » en tomos viejos). **2578/5894 casos (43,7%) no tienen `c/` en el índice** ⇒ orden de partes no derivable — y H163 ya lo había medido (convención «primer segmento = actora» 85,3%, 464 mismatch, por eso `derivar_partes` se niega a asumirlo). Demostrado 52; potencial 2578 SIN MEDIR. (9) **Derivers post-ciclo, medidos contra el sello H217:** `epilogo.csv` y `recursos.csv` byte-idénticos (ausentes de `git status`) · `partes.csv` 1 fila (347_p357) · `normas.csv` 13.926→**13.908** con ámbito carátula 411→**393** (−18 pares, todos inertes para la cascada) · `materia.csv` 87 filas = 61 reparaciones + 4 costos + 19 no-ops + 2 dudosos, con cobertura 78,5%→**77,5%** y `sin_clasificar` 1084→**1133** (+49 = 53 pierden − 4 ganan), `lectura_texto` 1472→**1420**, `lectura_dictamen` 77→**80**, `conflicto_capa2` 57→**53** — los cuatro deltas salen EXACTOS de las transiciones que selló el PoC. (10) **CONSTANCIAS DE MÉTODO (dos, ambas del asistente):** el `--regolden` se corrió ANTES de la adjudicación (el protocolo pide adjudicar y después congelar — familia del regolden-antes-del-consciente de H217/U2); recuperable sin costo porque no había commit y el golden viejo vivía en `HEAD` (extracción verificada por tamaño: 4.335.906 bytes == el `casos.csv` pre-fix exacto). Y `adjudicar_h218.py` se entregó SIN PROBAR: el flag `--golden` quedaba registrado en argparse pero `main()` seguía leyendo el default (el golden ya re-congelado) → comparaba producción contra sí misma y devolvía «+[] / -[]», un FRENAR sin significado; sucesor `verificar_h218.py` con paths posicionales y auto-testeado antes de entregarse. Constancia de instrumento: **`check_regresion` compara el header primero y ahí CORTA — nunca compara celdas**, así que su `[FAIL]` ante un cambio de schema no demuestra el contrato; cualquier ciclo MAJOR de schema necesita adjudicador propio. Pendientes siguen: B168 residual 329_p5741 · B173 (parking) · B169 · B170 · B171 · B106 · **frente C (ensanche de V1, ~41 recuperables sobre techo 1119)** · **título-de-cabeza/Eje A (NUEVO, potencial 2578 sin medir)** · **`_RE_SOBRE`/343_p1237 (NUEVO)** · **`RE_CONECTOR_CARATULA_V` FP de inicial media (NUEVO)** · borrado de `case_name_cuerpo_legacy` (decisión propia) · M57 · M61 · M62 · M63 · M64 · M65 · relabel terminal · validación operador H208 · M43 ÚLTIMA.)

**Anterior:** 2026-07-26 (H217 — **B168 CERRADO salvo residual único: dedup por nombre (parser v40.0) + peek de wrap clase E (parser v41.0), dos ciclos MAJOR con [CLEAN] 65; B173 NUEVA: 329_p2063 MAL LOCALIZADO (parking).** Camino de la sesión: (1) **U2 dedup:** superficie post-v39 primero (único dup restante = 2063, por dos instrumentos independientes — los 12 clase A absorbidos por el vocabulario H216) → diseño leído en código (dedup DESPUÉS del loop de calificadores para no robarle el calificador al duplicado; has_voto/has_disid desde la lista final; panel_cprima ya dedupeaba → c′ invariante) → PoC E0 5894/5894 · E0'' PASS · VP 0 flips corpus-wide · flip-set {2063} EXACTO → ciclo con FRENO reparado: el regolden se corrió ANTES del consciente (bloque de comandos del asistente sin el paso bloqueante — familia H191) y el candado anti-H178-2 ABORTÓ con «producción stale»; fix-forward → diff adjudicado contra git (1 fila casos / −2 votos → 27.900) · blind por construcción · gate M55 29→30 con alta única {2063} DEMOSTRADA por Compare-Object. (2) **B173 — destape del gate:** nj=2 imposible → lectura del corpus real (greps + dump L26280-26360 de 329.2): el bloque publicado [26309-26323] es ÍNTEGRAMENTE la cola de 329_p2062 (Rusillo: disidencia Fayt–Zaffaroni impresa en p. 2063, header wrapeado, + su epílogo). La carátula real de Rojas vive en L26325-26 (actor-primero, conector wrapeado a línea 2; el índice también wrapea «…(Ro-») y ancla_catalogo quedó con recorte 0 — todos los tiers de refinar fallaron. El caso genuino (desestima presentación postal de queja, 30-may-2006, firma de 5 Petracchi–Highton–Maqueda–Zaffaroni–Lorenzetti, unánime) NO lo posee ningún bloque — familia B012/B045/B152. **El dedup cierra el CONTEO, no el caso — y el caso resultó no ser el caso.** PARKING por protocolo, entrada B173 con validador. (3) **U1 peek clase E:** superficie corpus-wide (`superficie_b168e.py` v0.1): pool truncado 26 = 21 E + 5 FP; discriminador = JOIN contra las regex reales → 0 FP por construcción, 2.889 completos intactos, join 20/21 → **329_p5741 RESIDUAL SANCIONADO** (E+vocab «DISIDENCIA DE LOS ⏎ DOCTORES…»; ensanche a «Doctor» MUERTO con dato: 16 citas narrativas line-initial del grep del operador, y la zona no las saca — Pasada 1 pre-zonas + cuerpo no excluido en detectar_votos). Lectura Gate 2: el offset-loop de detectar_votos ya maneja headers multi-línea → fix mínimo = helper `_hdr_tipo_wrap` en los 3 call-sites (cola k+1 estricta, espejo H207). PoC A/B v0.3 (código real vs código real parcheado en memoria; harness espejo de main): E0 5894/5894 · **flip-set 20 = 20/20 ⊆ E20 · riesgo 1457 VACÍO CON DATO (0 flips de decisión)** · nd +1 los 20 · wc solo en 11 (la disidencia tragada se contaba como mayoría) · votos ±0 · **textos byte-idéntico → blind por construcción**. Ciclo: adjudicación fila a fila + conjunto completo por git (20/0 · ⊆20/0 · ⊆20/0, zonas +12) · regolden [CLEAN] 65 · **M55 pool 30 SIN movimiento** · post-fix «parche no aplica → convergencia declarada». (4) **Ripple normas adjudicado (cerró la hipótesis de U2):** 8 casos = 2063 (la fila dup de Zaffaroni con texto se extraía 2×) ∪ 7 de los 20 (texto_voto nuevo cita leyes), 0 fuera. (5) **Constancias de método:** regolden-antes-del-consciente (U2, asistente) · dos PoC quemados por schema inventado — `source_file` / clave de headers (Gate 1, asistente; reparado leyendo main() completo + verificación de headers CSV en disco; regla REE reafirmada por el operador). Pendientes siguen: **B168 residual = SOLO 329_p5741 (E+vocab)** · **B173 (nueva, parking, verificar gap-o-bleed del sucesor)** · B169 (decisión pendiente; dato fresco de sesión: 339_p270 nj=2 · 329_p3148 nj=3 · 329_p4446 nj=9 vigentes) · B170 · B171 · B172 (ciclo B) · B106 · M57 · M61 · M62 · M63 · M64 · M65 · relabel terminal (sin número) · validación operador H208 · M43 ÚLTIMA.)

**Anterior:** 2026-07-26 (H216 — **B168 fix VOCABULARIO CERRADO: parser v38.0→v39.0 (MAJOR), ciclo completo [CLEAN] 65, commit único pusheado.** Camino de la sesión: (1) **Daño re-dimensionado en el publicado ANTES de tocar nada:** `posiciones` es dict keyed-by-nombre con last-write-wins → el voto tragado (clase B168) BORRABA 11 pares (juez, posición) en 10 casos del pool — 3 disidencias reales publicadas como «mayoria» (337_p966 Highton+Fayt, 346_p629/931 Rosatti) → B168 no era cosmético: tocaba voting_pattern/posiciones, las columnas de H3. RE_VOTO_HDR gatea TRES consumidores (breaks colector L1180 · detectar_votos L1415 · zonificador L3380). (2) **Superficie corpus-wide** (`superficie_b168.py` v0.1→v0.3, H216/, fuente única = regex importadas; candados A0/A0b): universo SELLADO **61 líneas/61 casos** de header invisible en zona firma/epílogo = A orden-verbal 20 («Voto la señora…», «Disidencia el Señor…») · B «Voto Concurrente» 10 · C femenino «vicepresidenta» 3 · D OCR «de el» 1 · E wrap-truncado 21 · F «Ampliación de Fundamentos» 6. Pool 13 descompuesto por extracto: 10 header + 2 clase F (340_p1236 Gimbutas / 347_p1495 Riachuelo — la firma cierra «(…ampliación de fundamentos).», `_RE_FIRMA_COMPLETA` no dispara, el colector atraviesa el header y duplica al juez) + 1 conteo posicional (329_p2063, fuera del fix por diseño). Baseline: `b168_universo_sellado.csv`. (3) **Fix v39.0** = regex ensanchadas a A/B/C/D/F (E fuera por diseño: exigen el cargo en la MISMA línea) + rama calificador «(con) ampliación de fundamentos»→«según su voto» vía **RE_CALIFICADOR_FIRMA LOCAL** (la global INTACTA — consumidores frontera/núcleo B148 sin medir → M65 nueva) + **fila Antelo en jueces_csjn.csv** (conjuez CNACAF, extracto 345_p869; flip-set propio {332_p1124, 345_p869}). Espec = `poc_b168_vocab.py` v0.1 (brazos A/B/B2, harness real de main; E0 0/5894 casos + 0 votos; calibración 40/40 · 0 FP · 0 pérdidas). **FLIP-SET SELLADO 40 + 4 incrementales B2 ⊆ F** (563/1236 disidencia→mixed · 1717/1495 unanime→segun_su_voto) — 10 posiciones restauradas, 12 duplicados limpiados, nsv +34 / nd +6. (4) **COSTO ACEPTADO 1, adjudicado por extracto: 343_p1457 (Bertuzzi)** outcome revoca→otro + is_merit 1→0 — el revoca publicado era el dispositivo del VOTO de Highton (rel ~4325, pick sin techo); el techo nuevo (ivi=3411) lo mata TP, pero el pick cae en la cadena B166(d) de anclas argumentales (~rel 2380) ANTES del «Por ello… se resuelve:» genuino de mayoría (rel ~3352, item 2 = Revocar → **valor real COINCIDENTE, recuperable al cerrar B166(d)** — testigo sellado en esa entrada). (5) **Ciclo:** consciente `--ignorar-corpus-drift` → adjudicado → blind **n300 byte-idéntica** (ausente de git status) → regolden → manifest **[CLEAN] 65** → regresión propia `--esperar-version 39.0 --out propio`: E0 0/5894 + flip-set VACÍO → **votos 27.916→27.902 con aritmética exacta (−16 dup que el fix limpia + 2 filas Antelo)** → **gate M55 pool 29→29, 0 altas / 0 bajas DEMOSTRADO** (ningún tocado tenía nj<3; ningún miembro del pool fue tocado). (6) Adjudicación colateral de sesión: 345_p869 (Salvini) = control negativo de la superficie (sus headers son formas cubiertas; la línea flageada vive en el residuo) + destape Antelo. Pendientes siguen: **B168 residual = E wrap-truncado (21, peek con superficie propia) + dedup por nombre (2063)** · B169 (decisión) · B170 · B171 · B172 (ciclo B) · B106 · M57 · M61 · M62 · M63 · M64 · **M65 (nueva: unificación RE_CALIFICADOR/_FIRMA)** · relabel terminal sin número · validación operador H208 (SEXTA sesión diferida) · **M43 ÚLTIMA**.)


**Anterior:** 2026-07-25 (H214 — **Sesión de DIAGNÓSTICO. Pipeline INTOCADO: 0 scripts de la cadena, 0 `__version__`, 0 outputs canónicos; parser v38.0 sin cambio; manifest [CLEAN] 65 vigente SIN re-sello — Paso 2 del cierre N/A, candado blind N/A.** **B172 NUEVA — `find_case_name` (fallback de `case_name_cuerpo`) sin guards de forma.** Causa raíz LEÍDA en código (L4105 `v1 or legacy`; L1012 escanea HACIA ATRÁS al dictamen y devuelve la primera línea con `c/`|`s/`, sin guard). Medición por rama sobre las 5894 filas (la rama es identificable porque `cuerpo == legacy` ⇔ V1 devolvió vacío): **V1 3795 (64,4%) con 0,24% de daño · fallback 1171 (19,9%) con 423 (36,1%) · vacío 928 (15,7%)**. El daño NO está repartido: está concentrado en una rama identificada. Propagación medida: el campo alimenta SOLO `classify_queja` (L4179) y `es_originaria` (L4243, ripple a `is_merit`); ninguna variable de votación lo consume. **Hallazgo de cobertura mayor que el ruido:** `RE_CARAT_QUEJA` aparece en 54,0% de V1 y **0,0%** de las otras tres ramas — la capa carátula de `es_queja` (los +225 flips de H108) NO PUEDE contribuir en el 35,6% del corpus. **Nota de alcance:** la rama NO es muestra aleatoria (V1 dispara sobre la fórmula de fondo; las competencias abren «Autos y Vistos:») — `is_merit_decision` 74,7% en V1 vs 6,7%/6,6% en las otras → **el subcorpus de MÉRITO es el limpio**, reportar por rama en el CODEBOOK y no como tasa global. Fix DISEÑADO no implementado: `_es_caratula_v2` (L2458) + `linea_es_firma_de_juez` ya existen desde H200/D1 y `find_case_name` nunca los recibió → mismo guard, otra función, con fallback al comportamiento actual (patrón D2/H201, NO rechazo duro). **Dos correcciones de constancia:** (1) **B008** remitía a «ver B011» para el bug del fallback, pero B011 es el bug catalográfico de `344_p344`, sin relación — puntero corregido a B172, por eso el bug nunca tuvo ticket propio ni magnitud; (2) BITACORA L1622-24 concluyó sobre `330_p829` que «el daño no llega al CSV», lo que vale SÓLO cuando V1 dispara. **B106 pasa a `confirmado_cuantificado`:** la incidencia que pedía su validador está medida — 928 filas (15,7%), con el pendiente de separar los `apertura_rel is None` (L4106-4108), que no son ese bug. Frente CIRCULACIÓN (sistema de gestión, fuera del pipeline) trabajado en paralelo — ver BITACORA H214. Pendientes siguen: B168 · B169 · B170 · B171 · **B172 (PoC + fix)** · B106 · M57 · M43 ÚLTIMA.)

**Anterior:** 2026-07-15 (H212 — **FRENTE MATERIA: sesión READ-ONLY (parser v38.0, `derivar_materia` v4.1, manifest [CLEAN] 65 INTACTOS). CERRADA SIN CONCLUSIÓN: la validación del operador de los baselines H208 NO se produjo y sigue PENDIENTE.** Registro solo de hechos medidos y decisiones del operador; ninguna interpretación del frente se sella acá. **(1) Baselines H208 recuperados:** no estaban en `scripts/diagnostico/H208/` sino en `Downloads` — se entregaron en H208 y nunca se movieron al schema (H208/ es gitignoreado). `sin_ancla_muestra_adjudicada.csv` sha12 `590c5a7d11c4` (73 filas) y `dictamen_spotcheck_adjudicado.csv` (748 filas / 35 adjudicadas: 26 TP + 2 dudoso-TP + 3 dudoso + 2 FP + 1 FP-etiqueta + 1 dudoso-FP). Reconstrucción previa desde el transcript de H208: **sha idéntico, 0 diffs en 73 filas × todas las columnas**. **(2) Instrumento** `dump_gate_kw.py` v0.1 (H212/, read-only; invoca `extraer_caso.py` v2.3 por subprocess; raíz por marcador, no `parents[N]`): 7/7 rc=0 → `gate_kw_lectura.txt` (101.712 bytes) + `gate_kw_adjudicacion.csv` (sin adjudicar). **(3) Mediciones sobre el baseline:** 215 = norma 44 + kw+norma 29 + kw 138 + coocur 4; las 77 altas del tier = 44 + 29 + los 4 `kw(2)`; el pool kw es **134 `kw(1)` / 4 `kw(2)`**; **los 22 casos kw adjudicados en el spot-check son todos `kw(1)`**; `kw(1)` = **16/22** con los veredictos como fueron sellados, desagregado por materia asignada: `penal` 7/7 · `civil_comercial` 3/4 · `tributario` 0/2 · `lesa_humanidad` 0/1; el pool `kw(1)` por materia: penal 96 · cc 14 · laboral 12 · tributario 5 · lesa 3 · ambiental 2 · consumo 2; **64 de 134 casos del pool `kw(1)` tienen señal de contienda de competencia** en `dictamen_head` (heurística — el cruce firme es contra `outcome` de casos.csv), contra 9 de las 77 altas vigentes. **(4) DECISIONES TAXONÓMICAS DEL OPERADOR:** `aduanero` es **DUAL** — penal en lo delictivo, contencioso en todo lo que no es penal (clasificación de origen, multas, decomisos → `tributario`), y **el operador duda de que un regex/kw pueda dividir eso**; `penal_tributario` **es penal Y tributario** (dual); para contrabando ninguna etiqueta miente; **`materia` clasifica EL PLEITO y NO es representativa del trabajo de las secretarías**; la secretaría de radicación es **imposible de derivar del corpus** (el fallo publicado no la registra) — su fuente es el trabajo de campo, en producción (ya resuelto por esa vía: la **secretaría 6 agrupa salud + laboral + derechos humanos**); **se puede obtener información valiosa sobre temas y firmas sin saber por qué secretaría pasó cada caso**. **CANDIDATA:** adoptar la **taxonomía del Anuario de Estadísticas de la CSJN**, que tiene varios temas que no se corresponden con las secretarías. **(5) Constancia:** el baseline H208 se adjudicó sobre `dictamen_head` (700 chars); leídos 7 casos contra texto completo, Claude propuso cambiar 2 veredictos (329_p4234, 333_p1857 → `laboral`) — **propuestas no validadas por el operador**. Nada sellado, ningún flip-set, ningún re-sello.)

**Anterior:** 2026-07-15 (H211 — **M58 APLICADO COMPLETO (los dos pasos):** (1) parser v37.0→**v38.0** (MAJOR de schema de textos.csv): exporta `dictamen_text` (4ª columna, misma fuente H052 que `dictamen_presente`); candados C1/C2/C3 PASS post-hoc — 4 CSV byte-idénticos, columnas preexistentes byte-idénticas (drop de la col nueva == golden viejo `dc724eeb…`), cruce exacto texto≠''⇔dictamen_presente = **3.434**. (2) `extraer_normas` v1.0→**v1.1** (5º ámbito `dictamen`: 6.206 pares / 1.378 normas / 2.515 casos; candado N1 PASS: subset sin dictamen == sellado `8e838c49…`) + `derivar_materia` v4.0→**v4.1** (tier `lectura_dictamen`, gates norma-fuerte / kw≥2 / empate→None; candado M1 PASS: **77 altas** SOLO sin_ancla→lectura_dictamen, 0 clasificados tocados, conflicto_capa2 clavado en 57). Cobertura 77,0%→**78,5%** (4009/5107); sin_clasificar 1161→1084. Muestra M2 (25, seed 211) adjudicada por el operador contra texto real: **24 TP / 1 FP** (96%) — el FP es del canal NORMA (329_p5108, cita de rebote de la 24.241; primer FP del canal, acota el 13/13 de H208). WARN rinde 77 vs pin 215: diferencia = diseño tiered (kw-solitaria/empates/canales excluidos), pool recuperable si la validación del spot-check H208 lo habilita. Manifest [CLEAN] 65. **Método:** TRES violaciones de orden candado→sello en la sesión (regolden pre-candado ×1, commits con PASS/versión no demostrados ×2, uno pusheado 15e415f — corrección de constancia en BITACORA); dos bugs del verificador (paths, field_size_limit) lo dejaron sin correr → M62 nueva (candados como gate del orquestador). Constancias: acople relabel-terminal×tier (decisión taxonómica) · vocab_faltante con candidatas leídas 23.660/23.661/26.682→salud · M61 nueva (Digesto DJA como prior) · M51 tercera evidencia · textos.csv 62,39 MB (aviso GitHub, git-lfs a decidir).)

**Anterior:** 2026-07-15 (H210 — **CIERRE RETROACTIVO H209 VERIFICADO EN DISCO Y COMMITEADO (backlog git H207–H209 pusheado; corrección de constancia: los «Commits: 2» declarados en H208 nunca se habían ejecutado) + M49 APLICADO: `derivar_materia` v3.3→v4.0 (MAJOR) — `materia_capa` renombrada a lectura_tribunal / lectura_tribunal_refinada / lectura_texto / sin_clasificar; candado `verificar_m49` PASÓ (cross-tab 1:1 exacta, 0 pares fuera, materias/fuentes/orden idénticos); sha materia 89eb50fe…→607793de…; resto de la cadena byte-idéntico; manifest re-sellado [CLEAN] 65.** Detalle en Prioridad H210 y entrada M49. Constancias: M51 segunda evidencia de ergonomía (ABORT corpus-drift) · M60 nueva (csjn_analisis reescritura post-madurez) · explorador v8.2 no canónico (ID clickeable).)

**Anterior:** 2026-07-14 (H207 — **B166 (b1) sub-clase A CERRADA: fix PEEK DE WRAP en las perf-variantes del RESOLUTOR (parser v36.0→37.0, MAJOR) — `_barrer` computa `cola_wrap = bloque[k+1].strip()` (espejo VERBATIM del call-site del zonificador, Pasada 1) y la pasa a `es_candidato`; los 4 `_cand_*` ganan el parámetro (t2/t4 lo ignoran; `_cand_estructural`/`_cand_t3b` lo forwardean a `detectar_apertura_dispositivo`) → las perf-variantes (`en_virtud_perf`, `que_de_conf_perf`) toleran el performativo wrapeado TAMBIÉN en el resolutor. Cierra el residual documentado H194 («los wrapeados recuperan ZONA, no outcome»). Cero regex nueva; monótono por construcción (solo AGREGA candidatos — el resto de línea ya estaba en la cola). Distinción con el descarte H203: agrega peek a variantes YA perf-gated, no gatea incondicionales.** Espec = `poc_b166_b1a_peek.py` v0.1→v0.2 (scripts/diagnostico/H207/, A/B con réplica verbatim de `_barrer`+cascada — la interfaz `es_candidato(stripped)` no puede acarrear el peek sin réplica; detectores importados del parser; candados E0 canónico==publicado 0/5703 · E0'' réplica-sin-peek==canónico 0/5703). **FLIP-SET SELLADO 4 = 4 TP adjudicados** (baseline `poc_b166_b1a_peek_out.csv`): 344_p575 / 340_p1940 / 347_p2160 (sub-clase A H206, t3→t1 vía `en_virtud_perf`, exposición post-ivi 1→0, valor COINCIDENTE — el pe pasa al de mayoría, 0 flips de decisión, verificado en producción) + **344_p2779 RECUPERACIÓN adjudicada por extracto** (Caamaño c/ EN: la mayoría Rosenkrantz–Highton–Lorenzetti cierra «En virtud de lo expuesto y oída la señora Procuradora Fiscal, se ⏎ desestima…» [perf-wrap] y la disidencia de Maqueda cierra «Por lo tanto» [variante con constancia de muerte H194/H039] → publicado `sin_dispositivo`/tier none; pick nuevo 191 PRE-ivi (197) = dispositivo de mayoría → outcome sin_dispositivo→desestima). Riesgos medidos VACÍOS: 0 re-picks intra-T1 (la advertencia del PROMPT quedó vacía con dato) · anclas negativas sub-clase C (728/1084/1024) SIN flip (E4, lección H206 honrada). Ciclo: consciente adjudicado 100% en contrato — casos.csv 5 celdas ⊆{4} (wc_considerando ×4 a la baja + outcome SOLO 2779; is_merit/es_queja sin flip) · textos.csv SOLO {4} (pe+considerando) · votos.csv identidad INTACTA, solo denormalizado outcome ×4 filas de 2779 · **zonas y editorial BYTE-IDÉNTICOS** (relabel A1 no-op en ambos brazos, predicho exacto) → blind byte-idéntica (n300 ausente de git status) → regolden [CLEAN] → manifest **[CLEAN] 64** — sha casos 19028de5412d… / textos dc724eebd435… / votos b64f241085fe… / zonas 0fd3c38237e3… (sin cambio) / editorial 30a6da652e3a… (sin cambio) / recursos 433006ff0f68… (re-derivado del pe nuevo de 2779). **Gate M55 pool 29 SIN movimiento (0 altas / 0 bajas — el fix no toca panel, predicho).** Outcome `sin_dispositivo` 22→21. Regresión propia: `poc_b166_b1a_peek --esperar-version 37.0 --post-fix --out propio` → E0 0 · flip-set VACÍO · **E0'' == flip-set sellado {4}** (la réplica sin peek ES v36.0, patrón H200); radar `poc_b166_b_superficie --esperar-version 37.0 --out propio` → pool 13→**10** · t1 5646 · t2 13 · t3 10 · t3b 1 · t4 12 · none 21 — **los 6 números exactos de la predicción**; regresiones vivas H200-H205 ajustan pin a 37.0. Residual B166: **(b1)-B enumerada-con-clase** (fórmula de mayoría no catalogada/mid-línea, 7 casos, daño de decisión 4 = 0,07%, vocabulario no escala) · **(b2)→B161** (4140) · **(d)=B167**. Pendientes siguen: D1b · D3 · D4/B165 · B152 · B155 · B156-B158 · B160 · B161 (+4140) · B163 · B164 · **B166 (b1)-B / (b2)→B161 / (d)=B167** · B167 · B168 (pool 13) · B169 (costo sostenido) · B170 · M44 · M47-M51 · M53(a) · M54 · M56 (PAUSADA) · M57 · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1104 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-14 (H206 — **Sesión READ-ONLY triple (parser v36.0 SIN cambio; outputs/golden/manifest intactos): B169 superficie ADJUDICADA (clase pick = 3, costo sostenido, fix diferido) + B166 (b) superficie SELLADA (pool expuesto 13 = solo t3-post-techo; 13/13 adjudicados por extracto en sub-clases A/B/C) + M57 registrada + B170 nueva.** (i) `superficie_b169.py` v0.2 (H206/, candados E0t-tiling / E0c-vs-`panel_cprima`-real / E2-corpus-wide-vs-baseline-H205 = 0/0/0): 9.752 runs de zona firma · 5.689 picks c′ · picks con señal FP-prosa (chars≥300 ∧ frac≥0,30 ∧ sin dup) = 5, y los 2 no-anclas se adjudican con lo sellado H205 (348_p1751 = clase-A curada, firma real + cola; 329_p5317 = conjueces por-diseño B148) → **clase B169 sobre picks = EXACTAMENTE los 3 costos conocidos, 0 nuevos** (franja corta 150-299 barrida: 6, todas firmas legítimas con «según su voto» — el piso de frac de firma real con calificadores). DESTAPE cuantificado (prerequisito C/M57): runs firma NO-pick 4.063 → **1.243 califican panel** · 807 con raw≥300 (~86% clase header-de-voto-wrapeado + «Considerando:», heurística de instrumento SIN adjudicar) — pool latente inerte hoy que cualquier consumidor nuevo de bordes de zona firma tomaría por bueno. Bonus: dup_nombres en picks = **13 exactos == pool B168** (12 clase A + 2063; 419/1751 correctamente ausentes) — reconfirmación por instrumento independiente. (ii) `poc_b166_b_superficie.py` v0.1 (H206/, E0 pe==publicado 0/5703 · Etier réplica-con-tier==resolutor-canónico 0/5703): tiers t1 5642 / t2 13 / t3 13 / t3b 1 / t4 12 / none 22 → **pool expuesto (post-ivi ∨ zona voto_separado) = 13, TODOS t3-post-techo con pick en zona dispositivo; T3b/T4 aportan 0**. **E3: 4140 NO está en el pool y eso ADJUDICA — su pick es t1 DENTRO de la ventana (pick 140 < ivi 146)** → (b) se PARTE: **(b1)** t3-post-techo (pool 13) y **(b2)** sentencia-embebida-en-nota (4140; fix = des-intercalado B161, NO techo) — corrección de constancia: el testigo 4140 estaba en la clase equivocada. (iii) 13/13 leídos por extracto (6 nd≥1 + 7 nd=0), TRES sub-clases: **A** perf-wrap «En virtud de lo expuesto» (575/1940/2160 — valor COINCIDENTE, dispositivo calcado; candidato = peek de wrap en las perf-variantes del RESOLUTOR, flips de texto sin flips de decisión esperados, superficie propia obligatoria) · **B** fórmula-de-mayoría no catalogada/mid-línea (3141/3801/2237/343_p300/332_p5/204/757 — TODO el daño real acá: **2237 INVERTIDO** [publicado desestima; la mayoría DISPONE consulta al TPR — «Por lo tanto», variante con constancia de muerte H194] · **343_p300 INCORRECTO** [otro vs competencia real; pe = disidencia Rosenkrantz truncada en la habilitación de feria] · 3141 pe-mandato-INVERSO con etiqueta salvada · 332_p5 etiqueta divergente [improcedente-280 vs mal-concedido]; 204/757 honorarios coincidentes-mismas-sumas · 3801 coincidente en sentido → **daño de decisión = 4/5894 = 0,07%, enumerado con clase**) · **C** pick tardío CORRECTO (728 ivi-ESPURIO por cita wrapeada → **B170 nueva** · 1084 dispositivo CONSOLIDADO post-todos-los-votos · 1024 mayoría SIN dispositivo impreso) → **techo duro en T3 y chequeo-de-pertenencia naive MUERTOS con dato** (matarían 728/1084/1024). Housekeeping: M57 registrada (5 consumidores; prerequisito B169/B163 ahora cuantificado). Pendientes siguen: D1b · D3 · D4/B165 · B152 · B155 · B156-B158 · B160 · B161 (+4140 adjudicado acá) · B163 · B164 · **B166 (b1) pool 13 / (b2)→B161 / (d)=B167** · B167 · B168 (pool 13 reconfirmado) · **B169 (adjudicada: costo sostenido, fix diferido)** · **B170 (nueva)** · M44 · M47-M51 · M53(a) · M54 · M56 (PAUSADA) · **M57 (nueva H206)** · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1104 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-14 (H205 — **B166 (c) CERRADO: fix real PANEL POR EVIDENCIA DE ZONAS (parser v35.0→36.0, MAJOR) — consumidor c′ (`panel_cprima`) cableado en el camino del panel, ciclo completo [CLEAN].** Espec = `poc_b166_c_replica.py` v0.1 (scripts/diagnostico/H205/, A/B fuente única, candado E0 0/5894 == publicado); predicado VERBATIM de la regla H204. **FLIP-SET SELLADO 34** (vs candidato ≈41 de H204) **= 31 TP + 3 costos aceptados**, adjudicación completa demostrada keyed-by-caso (`adjudicar_h205.py`: diff EXACTO ⊆ {34}, solo columnas de panel, 0 flips de decisión corpus-wide). Hitos de la adjudicación: (i) gemelos C2 curados (441 nj1→5 disidencia · 575 nj1→4 mixed) + pool C2 casi entero; (ii) **329_p4140 adjudicado por extracto = familia nota-al-pie (B161):** la firma publicada (Belluscio/Boggiano, banco dic-2004) era de la sentencia EMBEBIDA «(*) Dicha sentencia dice así» (Ursina c/ Chianese) — c′ recupera el panel PROPIO 2006; outcome/pe de 4140 SIGUEN siendo los de la embebida (el pick no se toca — residual B166(b)/B161 con constancia); (iii) **CORRECCIÓN DE CONSTANCIA a clase A/B168: flipea solo 2/14** (331_p419 9→5, 348_p1751 5→3) — en los otros 12 c′ elige la MISMA zona que collect(pick+1) ya alcanzaba, output idéntico: el «nj baja al panel real» del PoC H204 era el DEDUP del contador del PoC, no un collect distinto → B168 conserva fix propio; (iv) sin_firma→panel ×4 (2794/378/1084/2063; 2063 con duplicados posicionales clase-B168 + vp vía B087, y su firma recuperada es la de la DISIDENCIA Fayt–Zaffaroni con posiciones=mayoria incorrectas — constancia); (v) **3 COSTOS con clase única = primera-zona-firma-FP de PROSA densa en nombres → B169 (nueva):** 339_p270 nj3→2 · 329_p3148 nj9→3 · 329_p4446 nj7→9. Ciclo: consciente adjudicado [OK] → regolden → blind byte-idéntica (n300 ausente de git status) → manifest **[CLEAN] 64** — sha casos 5faa27f472cf… / textos 390ace04abf1… / votos 73700c5ef7a8… / zonas 0fd3c38237e3… (sin cambio) / editorial 30a6da652e3a… (sin cambio) / recursos 676ca037a1d8… (sin cambio). Votos 27.850→**27.916** (+66 == predicción exacta de la réplica) · sin_firma 12→**8** · **gate M55 pool 45→29**: 17 bajas ⊆ flip-set + **1 alta ADJUDICADA = 339_p270** (el costo aceptado entra al pool — es el radar funcionando, no regresión desconocida). Regresión propia: réplica `--esperar-version 36.0 --post-fix` → E0 0/5894 + flip-set VACÍO; **las regresiones vivas H200-H203 ajustan pin a 36.0**. Pendientes siguen: D1b · D3 · D4/B165 · B152 · B155 · B156-B158 · B160 · B161 (+4140) · B163 · B164 · **B166 (b)/(d)** · B167 · **B168 (2/14 curados, 12 siguen inflados)** · **B169 (nueva)** · M44 · M47-M51 · M53(a) · M54 · M56 (PAUSADA) · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1104 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-13 (H204 — **Unidad B166 (c) ADJUDICADA COMPLETA (sesión read-only, parser v35.0 SIN cambio de código ni outputs): gemelos C2 adjudicados por extracto+publicado+código — DOS mecánicas DISTINTAS — y regla panel-por-evidencia-de-zonas medida corpus-wide en dos iteraciones: candado 5643/5687 (99,2% full match) · flip-set candidato del fix ≈ 41.** (i) **344_p575 (Grindetti)** = clase 1102 pero NO variante nueva de vocabulario: es el residual-wrap DOCUMENTADO de `en_virtud_perf` (H194-A2/B067) con caso real — la mayoría Rosenkrantz–Highton cierra «En virtud de lo expuesto, de conformidad con lo dictaminado por ⏎ la señora Procuradora Fiscal, se hace lugar…» (performativo wrapea; el resolutor no peekea, L135/L142, peek solo zonificador L3333) → zonas OK (dispositivo 759-763 + firma 764-765 = huella 1102 completa) pero T1 del resolutor vacío → T3 sin techo pesca el «Por ello» del voto de Maqueda (774-777 == `por_ello_text` publicado VERBATIM) → panel forward-only = «Juan Carlos Maqueda.» (778, 20 chars == `firma_raw` publicado exacto); ground truth: panel 4, mixed, Rosatti en disidencia parcial (1036-1041). La regla (c) original SÍ lo cubre (firma pre-primer-voto_separado). (ii) **344_p441 (Comunidad Mapuche Catalán)** = mecánica NUEVA firma-colectiva-post-voto: el pick de outcome es CORRECTO (T1 «Por ello, y conforme con lo dictaminado» 1091, revoca OK == pe publicado); lo roto es SOLO el panel — la mayoría NO firma tras su dispositivo; la firma colectiva del panel de 5 («Rosenkrantz (en disidencia) – Highton – Maqueda – Lorenzetti – Rosatti.», 1382-1384, ZONIFICADA) está POST voto concurrente de Rosatti (1121-1381) → `collect(pick+1)` se topa primero con el header del voto → nj=1 Rosatti «mayoria», firma_raw 1830 chars de basura; bonus: header de la disidencia WRAPEADO («…Don Carlos Fernando ⏎ Rosenkrantz» 1389-1390) rompe la extracción del juez del header (jueces_desconocidos «Señor Ministro Doctor Don»), nv/nd correctos → la regla (c) original NO cubre 441 → **ENMIENDA c′** (primera zona firma post-apertura cuyo collect parsea panel: nj≥2 ∨ calificador, en cualquier posición; fallback a c). (iii) **Regla medida** (`poc_b166_c_regla` v0.1→v0.2, H204/, read-only, fuente única parser importado): v0.1 mató DOS defectos propios con dato — c′ sin filtro post-apertura pescaba firmas del bleed (11 casos, cp_ini<<c_ini) y el panel leído del span VERBATIM de la zona no es la semántica del consumidor (134 mismatches = ARTEFACTO wrap-de-firma clase H190-b, pub nj6→zona nj5 en 329/330); v0.2 = c′ post-apertura + panel vía `collect_firma_lines(bloque, zona_ini)` → mismatches 155→44 · **full match 5643/5687** · flip-pool 21 · ancla 1102 match · tablero C2: 13/16 panel completo vía c, +441/1396 vía c′, 347_p520 y 348_p708 residuales con clase. (iv) **Clase A adjudicada con firma_raw publicado (query 24 casos): 14/14 = pub INFLADO por VOTO TRAGADO** — el colector no frena en el header «Voto la señora ministra doctora doña …» (variante invisible a los breaks de `collect_firma_lines`) → firma_raw = firma panel + VOTO ENTERO + firma del voto → `n_jueces = len(lista)` (L4236; parse_firma dedup por POSICIÓN, no por nombre) cuenta duplicados — testigo 331_p1028: pub nj9, columna jueces «…Argibay | Argibay | Argibay», panel real 7 → **B168 (nuevo)**; +4 pub-inflado por PROSA-CON-JUEZ al arranque del collect (330_p2799, 340_p1510, 347_p2154; 346_p965 = la expectativa B160 exacta, Figueroa contada desde prosa donde es PARTE; 347_p520 resuelto: la zona firma ARRANCA en línea-prosa «…Doctor Don Horacio Rosatti.» y la guarda `_RE_FIRMA_COMPLETA` corta a nj1 con los 4 jueces reales una línea abajo). (v) Residuo con clase = 6 (329_p3148/4426/4446 primera-zona-firma-FP en sagas conjuez · 329_p4140 dos firmas de 7 con sets distintos, leer · 339_p270 la zona arranca 1 línea tarde y pierde el juez wrapeado · 347_p520) + 348_p708 (única zona firma POST-primer-voto, nj1 conjuez Rabbi-Baldi Cabanillas — puede ser dato correcto, adjudicar por extracto). **FLIP-SET CANDIDATO ≈ 41** = pool 21 (11 C2 + 330_p3801 [de C1-no-M55] + familia pub-nj2 330_p3141/331_p1397/332_p5 + sin_firma recuperados 330_p2794/345_p378/347_p1084 + conjuez 329_p3649/329_p5317 + 340_p1993 [flageado 1C H190, leer]) + clase A 14 + prosa 4 + {441, 1396} vía c′. **PRÓXIMO (H205) = fix real:** cablear el consumidor (c) en el camino del panel (L4124-4144) + **PoC-réplica A/B con candado E0 y FLIP-SET SELLADO** — el PoC de hoy mide regla-vs-publicado, NO código-vs-código; los efectos de segundo orden (posiciones, n_titulares, is_full_bench, vp derivado, votos.csv ±filas — p.ej. el voto de Argibay tragado) solo los captura la réplica. Manifest [CLEAN] 64 y sha de H203 VIGENTES (nada tocado). Pendientes siguen: D1b · D3 · D4/B165 · B152 · B155 · B156-B158 · B160 (+4 casos medidos) · B161 (+1928) · B163 · B164 · **B166 (c) fix H205 / (b) después** · B167 · **B168 (nuevo)** · M44 · M47-M51 · M53(a) · M54 · M56 (PAUSADA) · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1104 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-13 (H203 — **B166 fix parcial VOCABULARIO CERRADO (parser v34.0→35.0, MAJOR): variante T1 incondicional `en_orden_expuesto`; el candado de decisión de 344_p1102 (Verbitsky ejecución) cumplido COMPLETO por vocabulario.** Mecánica REAL adjudicada con código+extracto (corrección total a la entrada B166): (i) hipótesis headers-wrapeados MUERTA con dato (los dos headers zonificados OK, seg2 con wrap absorbido wc 9) y «último performativo» MUERTA con código (`_barrer` devuelve el PRIMER performativo-con-firma del rango, L3654); (ii) causa real: mayoría y voto Rosatti cierran con «En orden a lo que hasta aquí se ha expuesto» — fórmula INVISIBLE a T1-T4 → T1/T2 vacíos en (apertura=601, inicio_votos_indiv=913) → **T3 SIN TECHO DE VOTOS** (L3781: rango hasta len(bloque), sin chequeo de pertenencia del pick) pesca el `por_todo_ello` GENUINO de la DISIDENCIA (rel 1388, regla P vía firma solitaria 1391) → **`collect_firma_lines(pick+1)` FORWARD-ONLY** (L4117) toma la firma de Rosenkrantz e ignora la firma de mayoría 910-912 que el propio zonificador etiquetó `firma` → el writer de votos (L4271) itera SOLO la firma pickeada (Rosatti sin fila pese a estar en marcadores; Rosenkrantz `posicion=mayoria` con texto de disidencia = la contradicción publicada). Superficie medida (superficie_b166, H203/): forma amplia «^En orden a lo» = 16 matches / 14 argumental-dictamen («opino que», «entiendo que V.E.») → MUERTA; forma estricta = 2/2 TP, ambos 1102, 0 FP corpus-wide; INCONDICIONAL obligada (el performativo wrapea a la línea siguiente y el resolutor no peekea, constancia H194 — perf-gate anularía el pick). Espec = `poc_b166_variante.py` v0.2 (H203/, A/B FUENTE ÚNICA: parser importado como módulo + harness de main() replicado, brazo B = tabla + variante parcheada en memoria): E0 0/5894 · FLIP-SET SELLADO {344_p1102} EXACTO (baseline poc_b166_flipset.csv). Ciclo: consciente adjudicado ⊆ {1102} — outcome rechaza→hace_lugar · vp unanime→mixed · nj/n_titulares 1→5 · is_full_bench 0→1 · is_merit 0→1 · queja_resultado→hace_lugar · posiciones = panel real (Rosenkrantz en disidencia / Rosatti según su voto) · wc_considerando −95 (**bonus TP: el golden traía dispositivo+firma de mayoría DENTRO del considerando; el pick nuevo lo descontamina**) — · votos 27.846→**27.850** (+4 = Highton/Maqueda/Lorenzetti/Rosatti; Rosatti con texto de su voto) · zonas 137.988→**137.990** (+2: dispositivo de mayoría 900-909 en 2 segmentos + dispositivo del voto Rosatti; **la zona FP 1328-1390 del wrap QUEDA → B167**) · recursos re-deriva (universo mérito 2969→2970, cobertura 90,6% sostenida) · blind byte-idéntica (n300 ausente de git status) · regolden [CLEAN] · invariante (c) 5/5 · manifest **[CLEAN] 64** — sha casos 457cdc863dbb… / textos 988a089b0094… / votos 2e662dc8e5ea… / zonas 0fd3c38237e3… / editorial 30a6da652e3a… (sin cambio) / recursos 676ca037a1d8…. **Gate M55 pool 46→45, baja exacta {344_p1102}, 0 altas** (diff contra baseline 48: {1847, 3070, 1102}). Regresión propia post-fix: E0 0/5894 · flip-set VACÍO. DESTAPES dimensionados H203: **B167 (nuevo)** — wrap-FP de RE_POR_ELLO en minúscula, pool 76 líneas corpus-wide · tablero **C2 = 16 ⊂ pool M55** (nj=1 ∧ hay votos; gemelos 344_p441/344_p575; 4 C-sin-huella-de-zonas incl. 441 → mecánica posiblemente DISTINTA de 1102) · **C1-no-M55 = 8** (unanime∧nd≥1 con nj≥3 — clase blanda, vp mal / panel quizás bien) · constancia: instrumento H de huella-de-zonas NO discriminante (2096 = layout normal multivoto; la clase no es visible en zonas solas, requiere el pick). RESIDUAL ESTRUCTURAL B166 ABIERTO: (b) T3/T3b/T4 sin techo de votos · (c) panel forward-only sin consumidor de la firma_linea temprana. Pendientes siguen: D1b · D3 · D4/B165 · B152 · B155 · B156-B158 · B160 · B161 (+1928) · B163 · B164 · **B166 (b)/(c)** · **B167 (nuevo)** · M44 · M47-M51 (M51 destacada) · M53(a) · M54 · M56 (PAUSADA) · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1104 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-12 (H202 — **Tándem M55 + B154-bis CERRADOS: instrumento de quórum + ciclo de DATO (tabla 87→91, parser v34.0 SIN cambio de código) — la unidad se auto-validó: medición → fix → re-medición con gate estrenado.** **(i) M55 instrumento:** `check_m55_quorum.py` v0.1 (`scripts/validacion/`, read-only, `--out` obligatorio regla H201) sobre casos.csv sha-verificado contra el manifest; **población medida 48** (nj=1: 19 · nj=2: 29; subclase unanime∧nj<3 = 47/48, excepción 339_p323) — los 2 testigos eran cota inferior ×24; baseline sellado `output/validacion/m55_pool_baseline.csv`. **(ii) B154-bis adjudicado por lectura de 5 extractos .md:** hipótesis firma-estampada de 1847 MUERTA con dato (mundo (a): la mayoría Rivera—Botana—Torres—Caballero—García Lema es texto tipografiado, wrapeada en 3 líneas); grep saga corpus-wide destapó el CLUSTER INVERTIDO «Apellido Nombre» sin acentos (344_p2752 firma real con panel de 4 publicado sin_firma/nj=0; 344_p3070 panel de 5 con Méndez/Montesi/Leal de Ibarra invertidos, publicado nj=1 solo Cossio — cuya fila YA tenía la alternación invertida); 345_p12 = interlocutoria de 4 titulares (nj=4 CORRECTO, los 5 conjueces en prosa del considerando — control de guards B148); 329_p1092 García Lema y Méndez YA contados (control no-flip). **CORRECCIÓN DE CONSTANCIA a H201/B154-bis: García Lema fue contado vía su FILA propia (migrada verbatim, existía desde <=27.1) matcheando la firma de cierre de su disidencia — NO «vía colector de desconocidos» (desconocidos de 1847 publicaba VACÍO); el ciclo real = 4 filas nuevas + 4 ensanches invertidos, no «sumar los 5».** Ciclo: PoC `poc_b154bis_conjueces.py` v0.1 (H202/, adaptación post-externalización de poc_b153: A/B = CSV-vigente vs CSV-propuesto, réplica parse_firma L1191-1204 + loader espejo L865-874 LEÍDOS) — E0 0/5703 · E1 == espec · flip-set 1 (3070 vía Leal de Ibarra invertido en firma_raw; **límite declarado patrón H191: 1847/2752 recuperan vía FRONTERA, no PoC**). Consciente adjudicado COMPLETO: casos.csv SOLO {1847 nj 1→5 unanime→mixed calificadores TP · 2752 nj 0→4 sin_firma→unanime · 3070 nj 1→5}, **0 flips outcome/decisión corpus-wide**; votos 27.834→**27.846** (+12 = 4+4+4 exacto); zonas 137.979→**137.988** (+9, contabilidad COMPLETA vía diff alineado H192: 6 casos = 3 TP testigos [mayoría 1074-1076 wc20 + Caballero 1337 en 1847 · firma 473-474 en 2752 · extensión 284 en 3070] + **3 FP zona-label clase letrado-Rivera(h)** [334_p38 «Dr. Julio César Rivera (h).» LEÍDO · 339_p194 idx237 · 344_p782 idx339, líneas del grep verbatim + contención de rango verificada] aceptados con constancia patrón H192 — daño canónico CERO: casos/textos intactos en los 3, firma real intacta, marker de epílogo queda, partes sostuvo 3897; mecanismo = título+nombre-de-juez+«(h)» sobrevive el residuo-guard B148, engorda pool B148/B155, discriminador candidato «Dr./Dres. pre-nombre» = unidad futura). Blind byte-idéntica (M20 ausente de git status) · regolden [CLEAN] · manifest **[CLEAN] 64** re-sellado — sha casos 265883cb5cd0… / textos 463172ca4c04… / votos 7da36587ba5b… / zonas 30ce7cdc76ee… / editorial 30a6da652e3a… (sin cambio). Regresión propia post-regolden: **convergencia total** (E1 0/0/0 · E0 0 diffs · flip-set VACÍO; baseline renombrado). **(iii) Paso 5 — gate M55 ESTRENADO: pool 48→46, bajas exactas {1847, 3070}, 0 altas.** **(iv) DESTAPE GRAVE — corrección al candado de decisión H201: el flip de 344_p1102 «outcome lee el dispositivo real (desestima→rechaza)» leyó el dispositivo de la DISIDENCIA de Rosenkrantz** (último performativo del bloque, pegado a su firma solitaria que da nj=1); la MAYORÍA (firma de 5 titulares L928-930, Rosatti concurrente en igual sentido) dice «se hace lugar a la queja… se revoca» → outcome publicado INCORRECTO, ambos valores del flip lo eran → **B166 nuevo** (clase mayoría-vs-disidencia; 1102 NO sana con B154-bis, queda en pool M55 adjudicado). Micro-destape: filtro anti-dup de `desconocidos` (parse_firma L1238) usa nombre_canonico como substring → canónicos con inicial («Rivera J.C.», «Torres V.N.») dejan pasar el token completo como desconocido (ruido cosmético 1847/2752, columna de auditoría — nota en B155). Pendientes siguen: D1b · D3 · D4/B165 · B152 · B155 · B156-B158 · B160 · B161 (+1928) · B163 · B164 · **B166 (nuevo)** · M44 · M47-M51 (**M51 destacada: operador fatigado del flag --ignorar-corpus-drift**) · M53(a) · M54 · M56 (PAUSADA) · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1104 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-12 (H201 — **B147/M45 sub-frente D, unidad D2 CERRADA (parser v33.0→34.0, MAJOR): guard-con-fallback en Pista 1 de `detectar_fin_real` (vista FIN de la frontera); B018 comp. 2 cerrado en AMBOS lados con ella.** Camino en dos PoC con el patrón H200: **v0.1 = rechazo duro (espejo literal de D1) MUERTO con dato** (patrón M52/B067: el flip-set adjudicado mata el diseño antes de implementar) — 96 flips / 46 caídas de pista; 6 testigos `.md` leídos: la caída amputa contenido PROPIO (341_p868/339_p599: Pista 4 ancla el «FALLO DE LA CORTE SUPREMA» propio pasado el lfc → sentencia+firma afuera; 334_p763: firma_actual en la firma de mayoría → el VOTO entero de Highton afuera; 343_p2135 y ~10 hermanos: epílogo propio huérfano; causa de los FP: la carátula genuina del siguiente no pasa `_es_caratula_v2` en la línea del token — wrapeada con conector en línea 2 [geometría D1b] o mixta institucional). Adjudicaciones colaterales de v0.1: 330_p5251 = TP del duro perdido con constancia (token wrapeado en prosa del dictamen ajeno) · 340_p660 sin flip CORRECTO → **corrección de constancia al diag H199: la sub-clase solape<0 mezcla truncamientos con GAPS DE BANNER benignos** (660 = gap 4 líneas). **v0.2 = fallback:** k_plain (ancla v33.0 exacta) + k_forma (no-firma incondicional ∧ `_es_caratula_v2`); ancla = k_forma ∨ fallback k_plain — cero caída de pista inducida; candados E0 5894/5894 + **E1 = 0** (todo flip c→c con k_forma, por construcción). **FLIP-SET SELLADO 50** (`poc_b147_d2_v02_baseline.csv`), **50/50 TP-o-mejora adjudicados desde el propio CSV** (columnas ancla_plain/ancla_forma): 33 ancla-en-EPÍLOGO-propio (el fin avanza +3..+13 y el epílogo entra al bloque) + 16 prosa (incl. 330_p1135 el testigo H199 y 341_p2015 fin a mitad del dispositivo) + 1 firma versal (329_p1301 Boggiano +32); deltas +1..+32, delta<0 = 0; sub-clase acoplada **D1b-en-FIN = 3** (1762/6071/1259): 2 con bleed CERO (línea 1 all-caps la come `retroceder_frontera`), 1259 absorbe 1 línea mixta — sana con D1b. X-ref solape<0 9/14 + 5 misses adjudicados (660 gap · 4811 interleave/D5 · 1303/3218/3221 Boggiano-D4). **Corrección de constancia al PROMPT_H201: el costo-bleed de las caídas-a-catalogo de D1 NO es Pista 1** — los previos reales (329_p2731, 331_p570, 333_p1784) anclan por `sumario_siguiente` (Pista 3; 570/1784 con solape POSITIVO +4/+11) → fuera del alcance de D2 por diseño, residual con clase identificada. **Ciclo consciente con perímetro COMPLETO programático** (`diff_perimetro_d2` v0.2, field_size_limit + invariante fino `_ancla_*`): casos 50/50 ⊆ set 0 fuera · **candado de decisión 22 flips: 21/22 TP + 1 regresión de metadata aceptada (1847)** — 330_p4365 sin_dispositivo→desestima con panel 0→4 · **333_p1442 `sumario_editorial`→`fallo` (espejo INVERSO del 345_p715 de H200: un fallo entero dado de baja vuelve al corpus — hace_lugar, unánime, full bench 5, is_merit 0→1)** · 341_p2015 firma de 5 recuperada · 344_p1102 outcome lee el dispositivo real (desestima→rechaza) · 342_p1847 frontera TP + **regresión de metadata aceptada con constancia y sucesor** (patrón H178/B165; corrección intra-sesión de la adjudicación, PDF oficial leído: García Lema es CONJUEZ del tribunal íntegro de conjueces de la saga art. 110 — panel real 5/mixed, golden 0/sin_firma y v34 1/unanime ambos mal → **ciclo de dato B154-bis**, ver B154) · textos 6 ⊆ set · votos 27.818→**27.834** (+16 = 4+5+5+1+1, tres fallos mudos ganan panel completo) · zonas 137.905→**137.979** (+74) ⊆ set, 0 vecinos-por-solape. `status_localizacion`/`tipo_entrada` adjudicadas como derivadas del contenido del bloque (no freno; el invariante fino es el componente `_ancla_*` + `linea_inicio`, ambos 0 flips). Constancia B161: 329_p1928 absorbe la nota «(*) Dicha sentencia dice así» (Halper) al avanzar el fin — variante hermana del marcador. Ripple derivers: partes recurrente_ok 3885→**3897** · mérito **2690/2969 = 90,6%** (universo +4). Candado BLIND: clave n300 **byte-idéntica** (git status vacío). Regolden [CLEAN] · invariante (c) sostenido · manifest **[CLEAN] 64** — sha casos eb688bf81bb7… / textos 6afc4bce6d14… / votos 0039201b129f… / zonas cff2f828af56… / editorial 30a6da652e3a… (sin cambio). Regresión propia post-regolden con `--out` propio (regla nueva ESTRENADA, baseline intacto): E0 5894/5894 / E1 0 / **flip-set VACÍO** (ambos brazos convergen — la réplica guarded ES v34.0). Micro-items H200 saldados: cierre constatado (`git log`: 3 commits en origin, sin `a51a912`) · baseline-pisado documentado + regla al skill (texto entregado) · micro-fix `--out` de poc_d1 encolado para cuando se toque ese .py. Pendientes siguen: **D1b · D3 (dato nuevo: «Autos» 334_p763 confirma la clase) · D4/B165** · B152 · B155 · **B154-bis (conjueces saga art. 110, 5 nombres sellados + 54 causas gemelas — ciclo de dato chico, rinde potencial alto)** · B156-B158 · B160 · B161 (+1928) · B163 · B164 · M44 · M47-M51 · M53(a) · M54 · **M55 (invariante quórum n_jueces≥3, 2 testigos)** · M56 (overlay curado — PAUSADA, último recurso) · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1104 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-12 (H200 — **B147/M45 sub-frente D, unidad D1 CERRADA (parser v32.1→33.0, MAJOR): guards de forma en los tiers exact/prefix de `refinar_inicio_por_titulo`; B018 componente 2 lado-CABEZA cerrado con ella.** Camino de sellado en dos PoC: v0.2 (calibración del discriminador: flip-set candidato 84, testigos 11/12, destapó que CAE_DE_BASE era artefacto del instrumento — el fix real cae a Tier 4/vistos/catalogo — y el bug regex «c/» a fin de línea en 339_p1048) → **v0.3 = réplica VERBATIM de la función completa con y sin guards** (candados E0 real==publicado y E0'' réplica==real, ambos **5894/5894**): **FLIP-SET SELLADO 93** (base→base 84 / →t4a 4 / →catalogo 4 / →vistos 1), fall-through medido salva title-case (MARINCCIONI, TELECOM SpA caps-ratio 0.5) y 2 de 3 Boggiano juez-parte (re-anclan idéntico vía T4 sin guard firma). Adjudicación: clase dominante = ancla publicada en EPÍLOGO del previo → carátula real con conector (el veredicto «962 BENIGNA» de H199 era sub-muestreo: ~13 TP latentes ahí) + clase a (firmas versales: 1898/2478 → ZAVALIA/SAN JUAN) + clase c (mixtas/anonimizadas vía conector) + 7 cabezas leídas en dump (`dump_cabezas_h200`): 329_p5691 «FISCAL» = carátula propia (no rubro), catalogo-falls 2733/583/1789 = recuperación de cabeza propia (ARGÜELLO/CAJAL/carát-wrapeada) con costo de bleed conocido (enfermedad D → D2), 330_p2584 corregido: era V-wrap TP (el «considerando propio» de H199 era bleed del previo), 330_p224 = familia B161 nota-al-pie (explica la fecha imposible de B153/H191). **Ciclo consciente adjudicado con perímetro COMPLETO programático** (`diff_perimetro_d1` v0.1, patrón H196/H197): casos 93/93 ⊆ set, 0 fuera · votos identidad INTACTA (27.818, 0 filas ±; 44 denormalizadas ⊆ set) · zonas 93 ⊆ set (137.979→**137.905**, −74 merges) · textos 13 ⊆ set. **Candado de decisión: 2 flips, ambos TP adjudicados** — 329_p4815 outcome otro→rechaza (la caída a catalogo recuperó el «Por ello» completo) · 345_p715 fallo→sumario_editorial con outcome/voting a ∅ (sus 337 palabras eran ÍNTEGRAMENTE bleed B161: dejó de fabricar un fallo). **Balance 91/93 TP-o-mejora / 2 regresiones de metadata ACEPTADAS con constancia y sucesor (patrón H178) → B165:** 329_p1303 (guard firma mata la carátula «ANTONIO BOGGIANO» del juez-PARTE y una cita posterior con «s/» pasa el conector → Δ32, date/apertura/status degradan) y 329_p1930 (flip a «Vistos» saltea apertura/fecha). Candado BLIND: `build_m20` → clave n300 modificada en **1 caso ⊆ flip-set** (intersección esperada — textos cambiaba en ⊆{93}; nota para M43). Regolden [CLEAN] · invariante (c) sostenido · manifest **[CLEAN] 64** — sha casos f1ffc9ec6af0… / textos c9fc2dea9bd5… / votos cbee28f6e566… / zonas b65f77520cd4… / editorial 30a6da652e3a… (sin cambio). Regresiones propias post-regolden: `poc_b147_d1_v03 --esperar-version 33.0` → E0 5894/5894 / E0'' mismatch = **93 EXACTOS** (réplica unguarded ES v32.1 — esperado) / flip-set VACÍO; `poc_m52_literales` E1-E4 sin FRENOS (5.4x). Destapes menores: la corrida de regresión PISÓ el baseline del flip-set (restaurado como `poc_b147_d1_v03_baseline.csv`; lección → skill cierre: regresiones post-fix con `--out` propio) · residual **D1b** = ancla en línea 2 de carátula wrapeada, 6 casos (ver B147) · **M54** nueva (auditoría firmas × vigencia: 9 anomalías Boggiano/Bossert/Nazareno en tomos 329-330). Micro-items H200: `__pycache__/` ya estaba en .gitignore (append duplicado revertido) · explorador v8 adjudicado (duplicado de raíz idéntico, borrado; commit del explorador PENDIENTE). Pendientes siguen: **D1b · D2 (vista FIN) · D3 (tokens B018-1) · D4/B165 (juez-parte)** · B152 · B155-B158 · B160 · B161 · B163 · B164 · M44 · M47-M51 · M53(a) · M54 · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1102 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-12 (H199 — **Sesión de DIAGNÓSTICO: sub-frente D de B147/M45 adjudicado (vista CABEZA) — B018 RESUCITADA con el dato que le faltaba; fix NO implementado por decisión, partido en unidades D1-D5** (ver entrada B147). Pipeline INTOCADO (0 scripts de la cadena, 0 `__version__`; parser v32.1 sin cambio; único artefacto tocado = re-sello de `_manifest.json`). **Micro-items H198 saldados:** (1) invariante de procedencia: `_manifest.json` registraba `parser.py: 32.0` con disco en 32.1 (el ciclo H198 fue byte-idéntico y el orquestador no re-selló) → re-sello [OK] con sha **5/5 IDÉNTICOS al sello H197** (casos 71586cb1fd95… / textos 7bac60b39d64… / votos bac640565cfd… / zonas dc1522e5c7f1… / editorial 30a6da652e3a…), derivers verificados (materia v3.2 / recursos v0.6 / epilogo v0.4 / partes v0.18), commit propio; (2) `_tmp/` de H196 adjudicado (los 5 CSV de la corrida de perfil H198, vía 1 del contrato triple; sha ya sellados en la constancia) y PURGADO; (3) cierre H198 constatado: 3 commits en origin/main (`git log` a la vista). **Mecanismo del sub-frente D adjudicado por lectura de 7 extractos `.md` + código:** el tier base de `refinar_inicio_por_titulo` (parser L2470-2478/2486-2491) toma el PRIMER match del token del título SIN ningún guard — `_es_texto_corriente`, stoplist y retry existen SOLO en el Tier 4 — → **falso-match-temprano** sobre el material del previo en la cabeza: «Banco» en «Banco de la Ciudad de Bue-» = cola de Pontoriero (329_p2645), «Mendoza» en «Provincia de Mendoza» = dispositivo de Lavado c/ Mendoza (330_p1158, **el Mendoza REAL del pool — saga Riachuelo, resolución de terceros**), «Nuevo» re.I en «un nuevo fallo» (329_p2179, disidencia entera de Argibay de Hendi en la cabeza, confirma H194). **El MISMO token rompe el lado FIN:** la Pista 1 de `detectar_fin_real` matchea «Mendoza» dentro del dispositivo del PREVIO y lo trunca a mitad de oración (330_p1135 lfr=44304 «…a la Suprema Corte de la ⏎»; `_es_texto_corriente` no atrapa el arranque en mayúscula «Provincia…») → la sub-clase solape<0 (14/32 del diag de apertura) = **previo truncado + cola huérfana heredada**, NO orden OCR revuelto. Sub-clase divergencia índice/cuerpo en la CABEZA (espejo «Hurting/HURTIG» H190): «Melchiori»→«MELCHORI» (OCR pierde una i) → todos los tiers fallan → `_ancla_catalogo`, recorte 0 (329_p3444). **CORRECCIÓN DE CONSTANCIA H194: 329_p3444 NO es Mendoza/Riachuelo — es Melchiori c/ Provincia de Santa Fe**; su cabeza trae la cola de Ferrari de Grand (= 329_p3403, el de B135c), cuyo propio epílogo quedó FUERA de su rango; las «~46 líneas antes de la carátula» son ~21 (el bloque entero mide 46). **329_p4815 (Lago Espejo) = clase INTERLEAVE OCR real** (familia B152/329_p4140): el cuerpo del caso vive físicamente ANTES del banner de su página, en un gap de 64 líneas que ningún bloque posee (tras el fin correcto de García 329_p4811); bloque publicado = 8 líneas de cola de dispositivo (`refinar` ancló en el «Lago» del wrap) — fuera del alcance de frontera-fina. **Convergencia: B018 componentes 1+2** — la entrada dormía desde H068 por «sin señal medible desde el CSV»; la señal existía en la gramática de zonas (el D-87 de H194). **PoC corpus-wide** (`poc_b147_d_cabeza` v0.1, scripts/diagnostico/H199/; importa refinar/cargar_localizados/construir_bloque/_parece_caratula REALES): **candado E0 5894/5894** (li_catalogo + offset == li_publicado; clave: reusar `cargar_localizados`, que MUTA linea_inicio en los `pagina_no_en_mapa`); anclas: titulo 5850 / catalogo 40 / vistos 4; **flip-set candidato FLIP_TITULO_PROSA = 105 casos** (∩ D-frontera 19/32; delta_recorte 1/7/114 min/p50/max; tokens top: Restitución 12, Banco 7, Luis 6, Asociación 6, Aires 5, Cámara 4…). **Adjudicación por muestra seed-199 (13 D-sin-flip + 10 + 10 + 8):** los 13 D sin flip = 3 sub-clases explicadas — (a) ancla en FIRMA-versales del previo que `_parece_caratula` acepta, con «Santiago»/«Juan» matcheando NOMBRES DE JUECES (329_p1898, 330_p2478 — el MISMO FP que reventó el PoC v0.2 de H190, 958 casos); (b) vecinos HOMÓNIMOS (4 Boggiano consecutivos: el token no discrimina la carátula propia de la hermana — la clase que rompió extraer_caso v1 con «los Mendoza de ejecución»); (c) carátula propia mixta/anonimizada que el detector de forma del PoC no reconoce (334_p965, 345_p905…). Flips nuevos ≈6 TP (anclas en EPÍLOGO del previo: «Tribunal de origen: Cámara…», «interpuesto por la Asociación…») / ≈4 FP del instrumento: ancla en la carátula REAL anonimizada («R. P., C. A. c/ M. O., N. D. R. s/ Restitución…») con k_forma = RUBRO temático en caps — **un fix ingenuo que avance al k_forma amputaría la carátula verdadera**; peor aún 330_p2584 (ancla en considerando propio «7º) Sentado…»). Clase TITULO_PROSA_SIN_FORMA_POST (962) adjudicada **BENIGNA** (carátula real mixta/anonimizada, ancla correcta) — descartada como bug, con constancia: **`_parece_caratula` NO sirve como discriminador de carátula** (fue diseñado para otra cosa, H111). CAT_FORMA_SIN_TOKEN = 37 (clase Melchiori dimensionada: inicio sin refinar, ancla en el banner de página). **Plan D1-D5 sellado en la entrada B147.** Pendientes siguen: **frontera-D → D1 (H200)** · B152 · B155-B158 · B160 · B161 · B163 · B164 · M44 · M47-M51 · M53(a) · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1102 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-12 (H198 — **M52 CERRADO (parser v32.0→32.1, MINOR): prefiltro de literales obligatorios para JUECES_CONOCIDOS — refactor byte-idéntico, la etapa parser baja de ~208 s a 107,8 s.** FRENO al diseño candidato de DEUDA adjudicado con dato ANTES de implementar: la unión de alternación única rinde **1.0x** en el caso negativo dominante (`re` stdlib no optimiza alternaciones grandes — mismo O(87·n) que el loop; sandbox, equivalencia 0 mismatches). Diseño enmendado: literal obligatorio por patrón derivado mecánicamente del árbol de parseo (`re._parser`, fallback `sre_parse`) del MISMO CSV — fuente única, cero drift; `hay_juez_conocido()` = etapa 1 substring-en-C → etapa 2 loop intacto; 6 sitios de existencia pura reemplazados, 3 de extracción de nombre intactos; patrón futuro sin literal ≥4 → chequeo incondicional (hoy 0/87); literal genérico único «ctor» (Méndez) documentado. Verificación en capas: sandbox (v32.0 vs v32.1 importados contra CSV real: 0 mismatches en 9933 líneas sobre `hay_juez_conocido`/`linea_es_firma_de_juez`/`collect_firma_lines`/`_firma_nucleo`; anclas H190-b preservadas) → disco `poc_m52_literales` v0.1 **E3: 0 mismatches / 2.207.636 líneas / 50 `.md`** (positivos 26.356 ≈ 1,2%) → contrato byte-idéntico TRIPLE: 5/5 sha del perfil `_tmp` == producción · gate [CLEAN] · invariante (c) golden==producción, sha idénticos al sello H197 (casos 71586cb1fd95… / textos 7bac60b39d64… / votos bac640565cfd… / zonas dc1522e5c7f1… / editorial 30a6da652e3a…) · manifest [CLEAN] 64. **Perf medida:** `re.Pattern.search` 133,8M→10,6M (−92%) · `linea_es_firma_de_juez` 162,4→66,1 s cum · perfil 218,8→195,6 s · etapa parser del orquestador **107,8 s**. Correcciones de constancia: el «~74» de M52/H196 estaba stale desde H192/B154 (tabla=87); margen futuro documentado en M52: genexpr del prefiltro 109,7M llamadas / 21 s (micro-opt loop plano, unidad propia). **Micro-items de la cola SALDADOS:** (1) housekeeping H195 → unificado en H196/ (7 extractos B159 + parser.prof v30 + poc_b159_{superficie,flipset} .py/.csv, verificado con `dir`; H195/ eliminado); (2) `dir archivo\` corrido — destapó bak huérfano `parser_v27.1_bak.py` viviendo en `scripts/pipeline/`, movido → archivo/ queda {v27.1, v31.0, v32.0}; (3) cierre H197 constatado: 3 commits pusheados a origin/main (`git log` a la vista). Constancia de método: DOS corridas de perfil quemadas contra v32.0 porque el copy Descargas→scripts/pipeline no se había ejecutado — el pin del orquestador (`[ABORT] pin de versión`) y el E1 del PoC frenaron exactamente como debían; verificación barata pre-corrida (`Select-String __version__` en DESTINO) adoptada como reflejo. Pendientes siguen: B147-1C + frontera-D (30+Mendoza) · B152 · B155-B158 · B160 · B161 · B163 · B164 · M44 · M47-M51 · M53(a) · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1102 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-12 (H197 — **B162 CERRADO (parser v31.0→32.0, MAJOR): {en_consecuencia, atento_a} salen de T1 incondicional a `RE_DISPOSITIVO_VARIANTES_ZONIF_PERF` — perf-gated (RE_PERF en cola+wrap) SOLO en el zonificador (`detectar_apertura_dispositivo` +1 kwarg `zonif=False`, único call-site `zonif=True` = Pasada 1); en el RESOLUTOR siguen INCONDICIONALES.** Corrección de constancia sobre el alcance: el micro-item p1952 L923 (leído sobre el `.md`) adjudicó que el ancla del segmento 923-972 era **atento_a** (wrap del OCR «atento el carácter nacional…», minúscula, prev abierta), NO en_consecuencia — la clase es «variantes T1 FP-narrativas», con constancia madre **H041** (BITACORA L4028/L4055: trío {de_conformidad, en_consecuencia, atento_a} ya excluido del Tier 2 mid-line en mayo; línea-inicial quedó incondicional hasta hoy). Superficie corpus-wide (`poc_b162_superficie` v0.2, 4607 matches / 16 variantes, testigos 4/4): en_consecuencia 1411 matches / 471 anclas / 10 picks + atento_a 74/30/5 = perfil B067; **gate de resolutor MUERTO con dato** — el performativo vive ADENTRO del chunk, no en la línea del ancla (por_los_fund 342/342 picks sinPerfL, de_conformidad 212; 5 extractos leídos: 341_p774 revoca CORRECTO con perf wrapeado perfW=1/perfL=0 — falsifica la arquitectura H194-A2 pura; 330_p2520 «lo que así se ⏎ resuelve» perf a 5 líneas; 329_p2985 «hágase saber» ENCLÍTICO fuera de RE_PERF + doble pick = colisión del proxy 40-chars [el sumario duplica el dispositivo]; 346_p1117 costas genuino; 347_p109 → B164). Diseño adjudicado: zonificador-solo + red A1 de H194 (los genuinos PICKEADOS que pierden el ancla recuperan la zona vía el relabel del pick — medido: 4 a1_fired + 9 ancla==pick sin flip, incl. 329_p3493 = FN del proxy adjudicado por construcción). Flip-set SELLADO (`poc_b162_flipset` v0.1→v0.2, réplica fiel v31 CON relabel A1, candado E0 **5703/5703**, contrato leído del dump de superficie, no hardcodeado): **423 casos, 0 flips outcome/por_ello_idx/por_ello_text**, transiciones dispositivo→cuerpo/sumario/firma/residuo (máx 1428 líneas 329_p3235); **1 flip vecino-por-solape adjudicado TP** (330_p2122: el ancla caída del vecino 330_p2115 vive en las 16 líneas compartidas por `fin_extendido_pag_compartida`; el dump atribuye la línea al primer rango que la contiene) → contrato real: flips ⊆ {423} ∪ vecinos-por-solape. Ciclo consciente adjudicado con **perímetro COMPLETO programático** (`diff_perimetro_b162` v0.1, patrón H196): casos.csv 17 casos, columnas SOLO {word_count, wc_mayoria}, candado de decisión (outcome/is_merit/is_originaria/es_queja/voting_pattern/n_jueces) **0 flips**; votos.csv 27.818 filas idénticas en número, SOLO la denormalizada wc_mayoria en los mismos 17 (corrección de constancia: el contrato decía «votos sin cambio» pensando en identidad de filas y olvidó la columna denormalizada — identidad de voto intacta); zonas 423 casos ⊆ flip-set, 138.078→**137.979** (−99, merge de segmentos espurios). Mecanismo del wc adjudicado y es TP puro: los 17 son la clase `dispositivo→residuo_caso_anterior` y el residuo se EXCLUYE del word_count (H055/B083) → el golden contaba bleed del caso anterior como mayoría (339_p704 wc 2210→583 con 164 líneas de bleed; 340_p549 408) = **clase M50 parcialmente autocorregida**. textos.csv **byte-idéntico** (sha 7bac60b39d64 sin cambio) → candado blind por construcción; recursos byte-idéntico (57dca1d14947); editorial sin cambio. Regolden + manifest **[CLEAN] 64** — sha casos 71586cb1fd95… / votos bac640565cfd… / zonas dc1522e5c7f1…. Regresión propia post-fix: `poc_b162_flipset` v0.2 detecta layout v32 → E0 5703/5703 / **flip-set VACÍO**. **Absorbe M53(b)**: header del módulo reescrito (estaba clavado en «v17 beta», 15 versiones stale; ahora sin número, no puede envejecer); M53(a) NO se toca. Destapes → **B164** (347_p109: bloque con comentario DOCTRINAL «podríamos decir, conforme Carrió», outcome=revoca fabricado de narración — clase de entrada, no de anclas) · constancia 329_p4815 (pick en b0: el bloque arranca a mitad del dispositivo del anterior — familia frontera B147/M45). Micro-item 2 de H196 PARCIAL: commit pusheado ✓, .py en H196/ ✓, pero H195/ sigue vivo (7 extractos + parser.prof + superficie.csv + _tmp) y el `dir archivo\` no se corrió — decisión del operador pendiente. Pendientes siguen: B147-1C + frontera-D (30+Mendoza) · B152 · B155-B158 · B160 · B161 · B163 · B164 (nuevo) · M44 · M47-M53(a) · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1102 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-12 (H196 — **B159 clase B CERRADO (parser v30.0→31.0, MAJOR): `RE_DICT_HDR` pasa de prefijo-abierto a FORMA-TÍTULO LAXA — el «dictamen sin cierre» de H194 era el síntoma; la causa era un FP NARRATIVO del detector.** Causa raíz LEÍDA sobre los 7 testigos `.md` extraídos: **6/7** = el wrap del OCR deja «dictamen de la/del Procura…» a inicio de línea y `re.I` lo matcheaba → `dictamen_inicio` espurio → guarda H052 engulle dispositivo/firma/votos (334_p109, 337_p166, 337_p1006, 339_p662, 344_p1952 [el wrap cruza el banner de página], 344_p2123); **1/7** = clase distinta → **B161** (dictamen genuino como NOTA AL PIE «(*) Dicho dictamen dice así», intercalado a pie de página, nunca cierra — 332_p2418 + 5 hermanos corpus-wide). Calibración del discriminador sobre el universo COMPLETO (`poc_b159_superficie` v0.1: 3804 matches / 50 archivos): **capitalización à la H139 MUERTA** (8 títulos genuinos en minúscula = versalitas-OCR de 336.1, tomo hoy excluido pero futuro) · **prev_abierta MUERTA** (565 títulos genuinos con línea previa abierta) · **forma-título laxa GANA: quedan 3789 / caen 15 / 0 pérdidas** (slot de 1 token tolera títulos OCR-dañados «Geberal» 336.1 / «Genera» 338.1). Los 15 FP = los 6 testigos + 9 nuevos (329_p1638, 338_p1009, 340_p1542, 342_p1735, 344_p2307, 344_p3249, 347_p1944, 348_p763) — los 9 = **NO-OP adjudicado con dato** (la línea FP cae DENTRO de dictamen genuino, `dictamen_inicio` exento en Pasada 2; mina latente desactivada gratis). Flip-set SELLADO (`poc_b159_flipset` v0.2, réplica fiel v30.0 CON relabel A1, **candado E0 5703/5703, 0 mismatch**): **7 casos, 7/7 en contrato, 0 flips outcome, 0 por_ello_idx**. Ciclo consciente adjudicado con perímetro COMPLETO (diff programático casos.csv y zonas.csv contra golden: 7/7 ids EXACTOS, ningún 8vo) → corolario medido: **radio fin_real/frontera VACÍO** (los FP no anclaban el fin de ningún vecino). Adjudicaciones puntuales: `tipo_cuestion_federal` arbitrariedad→mixto 334_p109 TP (considerando destapado con ambas señales) · `dictamen_presente` True→False ×2 TP (334_p109, 340_p691) · votos SIN +filas (27.818 idéntico) — se recupera CONTENIDO, no cardinalidad (texto_voto/wc_voto/tipo_voto_sep poblados: votos Petracchi-Argibay y Maqueda de Editorial Perfil) · **is_merit_decision 0 flips** (verificado contra golden v30.0 vía git) · blind n300 byte-idéntica · zonas 138.037→**138.078** (+41). Regolden + manifest **[CLEAN] 64** — sha casos b27ca03940dd… / textos 7bac60b39d64… / votos aae443cdd1fb… / zonas 71359e8fec79… / editorial 30a6da652e3a… (sin cambio). Regresión propia post-regolden: flip-set VACÍO / E0 0 mismatch (pre-regolden da e0_mismatch=7 == flip-set, esperado, no FRENO); `poc_b159_superficie` queda como baseline histórico (regex vieja hardcodeada por diseño). **Destapes → B162** (dispositivo sobre-extendido en votos: `en_consecuencia` T1 argumental SIN guard performativo — 344_p1952 L1586 ~360 líneas · 344_p2123 L1237 [wrap en minúscula] y L1596; p1952 L923 ancla sin identificar, lectura pendiente) · **B163** (334_p109: el dictamen real de Righi zonificado como `sumario` — título no línea-inicial en el OCR, preexistente, quedó a la vista en el diff). **Apertura de sesión: perfil de performance v30.0** (cProfile 208,5 s / 301M llamadas: `zonificar_bloque` 65%; `linea_es_firma_de_juez` 1.701.762 llamadas / 114,7 s = **55% del runtime**, ~74 `re.search` de JUECES_CONOCIDOS por llamada ≈ 134M searches / 93 s) → **M52** (prefiltro de alternación única) + **M53** (consistencia RUIDO_FIRMA-vs-M46 + docstring de cabecera clavado en «v17 beta»). Constancia de método: Gate 1 VIOLADO por el asistente en apertura (DEUDA/BITACORA/MAPA/correr_pipeline estaban en uploads desde el inicio y se pidieron igual — el `dir` de inventario no se corrió); detectado por el operador al cierre. Pendientes siguen: B147-1C + frontera-D (30+Mendoza) · B152 · B155-B158 · B160 · B161-B163 (nuevos) · M44 · M47-M53 · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1102 · CODEBOOK/Dataverse (espera M43) · **M43 ÚLTIMA**.)

**Anterior:** 2026-07-12 (H193 — **B148 CERRADO (parser v28.0→29.0, MAJOR): guards del detector de firma — `linea_es_firma_de_juez` deja de disparar sobre atribuciones editoriales de sumario y cuerpo denso en nombres.** Espec calibrada por PoC iterado a nivel línea corpus-wide (`poc_b148_flipset` v0.1→v0.3, importa el parser real, scripts/diagnostico/H193/): v0.1 (espec H192 tal cual) tenía DOS clases de FP sistemáticas que los controles atraparon ANTES del fix — 521 firmas reales con calificador partido por el wrap («…NOLASCO (según su ⏎ voto)») y 71 continuaciones de firma con «)» colgante («cia) — LORENZETTI…»): la clase FRENO v27.0 a escala. Espec v2/v3: (i) RE_ATRIBUCION_SUMARIO con plurales y palabra intermedia; (ii) NÚCLEO de firma = strip de paréntesis balanceados SIN juez (cubre calificadores fuera de catálogo: «según mi voto», «ampliación de fundamentos», soft-hyphen — RE_CALIFICADOR intacta) + corte de fragmentos parentéticos wrapeados, y el match de JUECES_CONOCIDOS debe SOBREVIVIR en el núcleo; (iii) residuo-prosa sobre el núcleo (conjueces desconocidos preservados; enumeración 5317 sobrevive por diseño). **Flip-set SELLADO 3799 líneas / 1303 casos** (g1 1671 / gU 1401 / g2 727); adjudicado: clase-1 15/15, clase-2 8/9, NO-B148 17/17 TP (el control «esperado 0» era expectativa errada: kills en bleed son TP), hdr_sumario 0 (L2029 sin efecto). Regresión propia post-fix: flip-set 0 / viejo=True 19930→16131 (−3799 EXACTO). **Ciclo consciente adjudicado 100% TP:** contrato 0 flips outcome/voting_pattern/is_originaria/is_full_bench/is_merit_decision · votos 27818 byte-idéntico en identidad · zonas −649 filas / 552 casos, **552/552 ⊆ flipset, 0 ripple externo**, 0 casos sin firma · casos.csv 94 (word_count/wc_mayoria a la BAJA = evidencia nueva M50: el sumario etiquetado firma inflaba el conteo) · textos 2 filas (considerando 340_p204/345 descontaminados) · **2 linea_fin_real = reparación de FRONTERA de rebote** (337_p505, 340_p1070, adjudicados en .md: soltaron carátula+sumario del caso siguiente cuya atribución era el ancla falsa del fin — sub-clase frontera-por-atribución de la familia B147, no encolada, reparada gratis). Blind [CLEAN] · golden re-congelado · manifest [CLEAN] 64 · sha casos 06054f05b274 / textos 226671eb4134 / votos 0e1270a8f71d / zonas fda8e6045c80. **Corrección de constancia H192:** firma_raw de 346_p965 NO se reparó (la línea semilla «1°) Declarar que la Dra. Figueroa…» no menciona juez del set → el veneno es del COLECTOR, no del detector; las zonas de 346_p965 sí quedaron reparadas) → evidencia a B149-familia. **Residuo del pool: 217/398 → 101/125** (A>200 48→5 · B 21→1 [5317 por diseño] · C 18→8 · D 130→87 [FP-A M45, fuera de alcance]); a leer H194: 329_p4140, 339_p270, 339_p323, 348_p1751 (A) + 329_p4446, 331_p2363, 331_p364 (C) — esa lectura diseña la unidad siguiente. Pendientes siguen: B147-1C · B149 (evidencia nueva) · B152 · B155-B158 · M44 · M47-M51 · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · materia sin_ancla 1102 · CODEBOOK/Dataverse · M43 ÚLTIMA.)

**Anterior:** 2026-07-12 (H192 — **B154 CERRADO (ciclo de DATO, parser v28.0 sin cambio de código): tabla de jueces 74→87** (+13 conjueces: Müller, Aranguren B.E., Irurzun, Lozano, Boldú, Sánchez A.G., Barreiro, Sotelo de Andreau, Bejas, Candisano Mera, Llorens, Moltini, González R.L.; +2 ensanches: Leal de Ibarra inicial «M.», Tazza OCR «Alejando»). La adjudicación por lectura de Aranguren/Irurzun destapó el cluster de paneles con conjueces t.348-349 (Tabacalera Sarandí) incl. órdenes INVERTIDOS «Apellido Nombre» (348_p563/569/716/755); descartes con constancia: Juan José Aranguren (ministro PEN, bleed 339_p1223, persona distinta de Beatriz Estela) y Barreiro Deymonnaz (letrado, bleed 330_p2527). Flip-set convergido en 3 iteraciones: **24 casos / +45 jueces / 0 pérdidas / 0 full_bench / 0 fuera del pool adjudicado**; contrato verificado con diff ALINEADO por caso_id (instrumentos nuevos `diff_casos_b154.py`/`diff_zonas_b154.py` — el diff del gate es posicional y se corta en 25): 1 flip de decisión TP (344_p1557 voting_pattern segun_su_voto→mixed = calificadores Irurzun/Sotelo recuperados). Ripple zonas +10 = 2 TP + 5 FP clase-B148 aceptados con constancia (329_p5317, 348_p569, 346_p1564, 348_p61, 348_p841 — daño canónico CERO verificado; pasan al pool B148). Corrección de constancia: el word_count +5 de 338_p1433 es cola de firma del caso ANTERIOR reclasificada residuo→firma (clase FP-A M45), no ripple TP. Candado blind intacto POR CONSTRUCCIÓN (textos.csv byte-idéntico al sello H191). **Además: DIAGNÓSTICO B148 COMPLETO** — `linea_es_firma_de_juez` (L1842-1876) leída ENTERA por primera vez + Pasadas 1-3 de `zonificar_bloque` + `linea_es_header_sumario` + `RE_HEADER_VOTO_DISIDENCIA`: dos causas raíz exactas verificadas en código (el `^` de RE_HEADER_VOTO_DISIDENCIA no tolera guión/paréntesis de las atribuciones editoriales; linea_es_header_sumario exige «./:» que las voces caps no tienen → `_en_sumario` inerte); pool medido `poc_b148_cardinalidad` v0.1 = 217 casos / 398 tramos firma-pre-dispositivo (A>200wc 48 ≈ los 49 BUG-1 de M45); adjudicación por lectura de 34 casos / 99 tramos (`dump_b148_tramos.md`): 15 SUMARIO-atribución + 9 CUERPO-denso + 5 no-B148; firma_raw mayormente DESACOPLADO del daño de zona (5/6; excepción 346_p965 envenenado). **Fix A diseñado** (guard de atribución + predicado residuo-forma-de-nombre, en el detector, causa raíz); **opción B (pasada correctiva por umbral wc) DESCARTADA como parche, no encolada**. Implementación → H193 (PROMPT armado). Destapes: evidencia B149 (330_p3801) · evidencia B155 GRAVE (348_p1553 cuenta al letrado Belluscio como juez) · M50 (wc incluye zona firma) · M51 (exclusiones corpus-drift como dato). Pendientes siguen: B147-1C · B148 (H193) · B149 · B152 · B155-B158 · M44 · M47/M48/M49/M50/M51 · residuo B089-B096 · B146 · Hotesur 348:18047-18076 · diagnóstico materia sin_ancla 1102 · CODEBOOK/Dataverse · M43 ÚLTIMA.)

**Anterior:** 2026-07-11 (H191 — **B153 CERRADO (parser v27.1→28.0, MAJOR): `JUECES_CONOCIDOS` externalizado a DATO (`_meta/jueces/jueces_csjn.csv`, 74 entradas: 56 migradas verbatim + 18 nuevas + 2 ensanches) + columna `jueces_desconocidos` RE-CONECTADA** (estaba muerta por construcción: `conocido=True` hardcodeado + filtro `not conocido` en el writer = vacía desde siempre; la lista real solo iba al print). Sesión doble: **(a) Unidad B — auditoría de derivers post-B147-1A (read-only):** 0 degradación — partes recurrente_ok 3882→3885/3886 (mérito 90,3% plano), materia 77,2→77,0% (composición: originaria 546→596 == is_originaria del parser, 0 mismatch), gate es_revision_fondo == is_merit 2967; headroom nuevo → M48; 2 discrepancias PROMPT-vs-disco documentadas (split tipo_entrada; votos sello 27724 vs golden 27712). **(b) B153:** PoC con triple candado (E0 réplica==publicado 0 diffs/5703 · E1 CSV==espec · E2 código instalado==espec, corrido sandbox + disco), flip-set 35 casos / +48 jueces / 0 pérdidas / 0 FP fuera del pool; adjudicación de 5 descartes con constancia (RICARDO LUIS=truncamiento upstream → B158; HIGTHON ya contada; ASOCIART/DARIMAR=bleed → B155). Ciclo consciente: contrato verificado RETROACTIVO (outcome 0 diffs distribución completa · is_merit 2967=2967 · is_originaria 596=596; voting_pattern 2 flips TP unanime→mixed = calificadores recuperados 329_p3221/3235); **ripple de frontera MAYOR que el PoC y adjudicado TP**: reconocer conjueces extiende `collect_firma_lines` → firma_raw recupera texto (1303 nj 1→5, 1305 2→6, 2830/3221/3235 ganan nombres; votos +61 vs cota +48; el «Otero sin LUIS» era artefacto de truncamiento, la firma completa dice LUIS CÉSAR OTERO). Destapes del ciclo → **B154** (Müller nuevo conjuez + Leal de Ibarra inicial «M.» + Aranguren candidata — la columna re-conectada ya detecta), **B156** (is_full_bench ==5 no period-aware; FP aceptado 330_p224), **B157** (330_p224 fecha imposible 2007/panel 1998-2003), **M47** (períodos de jueces en la tabla → validador n_jueces-mínimo + full_bench period-aware). golden re-congelado, manifest [CLEAN] 64, sha casos 031b5759912b… v28.0. Constancia de método: el regolden corrió pegado al consciente por bloque de comandos copiable (error de armado del asistente, no del operador); adjudicación ejecutada retroactiva ANTES del commit, contrato verificado. Candado blind PENDIENTE de verificar (textos.csv cambió en 5 filas firma_raw; correr build_m20 + git status — esperado byte-idéntico, la clave no lee firma_raw). Pendientes siguen: B147-1C · B148 · B149 · B152 · B154-B158 (nuevos) · M47/M48 (nuevos) · residuo B089-B096 · M44 · B146 · CODEBOOK/Dataverse N=5894 · M43 ÚLTIMA.)

**Anterior:** 2026-07-11 (H190 — **B147-1A CERRADO (parser v26.1→27.1): retroceso de frontera fina en `detectar_fin_real`, corrección de cardinalidad 51→1866.** Mecanismo adjudicado sobre 8 testigos `.md` que CORRIGE el modelo H188: Pista 1 corta en carátula−1 pero deja colgando el material PRE-carátula del siguiente (rótulos/banners/apertura de dictamen); Pistas 3/4 anclan en el DICTAMEN del siguiente y absorben carátula+sumario ENTERO (perfil causado por token corto «Lumi»<5 o divergencia OCR índice/cuerpo «Hurting»/«HURTIG» → fix independiente del token). PoC v0.1→v0.4 (`scripts/diagnostico/H190/`, flip-set 1866/5542, B 91/91 TP, anclas 5/5): guards firma-versales + header-voto + nombre-propio salidos de la adjudicación en disco. **FRENO intermedio documentado:** el consciente de v27.0 violó el contrato (votos −42, `firma_raw` truncada ~34: el fragmento final de firma wrapeada «LORENZETTI.» no matchea `JUECES_CONOCIDOS` nombre+apellido) → guard continuación-de-wrap (v27.1) verificado y re-ciclado; 329_p4506 (conjuez Fernández) reparado de rebote. Ciclo sellado: regolden + manifest [CLEAN] + regresión propia del PoC = 0 recortes corpus-wide. Colaterales: 3 FP de la bandera-1 del diag H188 verificados en `.md` (345_p683/330_p2892/330_p3126, frontera perfecta) · 3 candidatos apéndice-1C flageados (339_p1648/340_p1993/345_p1138, detector `b_descarto_propio`) · under-trim residual 345_p1219 (≤1 línea, carátula mixta wrapeada). Pendientes siguen: B147-1C · B148 · B149 · B152 · B153 · residuo B089-B096 · M44 · B146 · CODEBOOK/Dataverse a N=5894 · M43 ÚLTIMA.)

**Anterior:** 2026-07-11 (H189 — **B147 fase 2 ADJUDICADA + B151 CERRADO + M46 suplemento editorial: los 4 fallos fantasma RECUPERADOS, N 5890→5894.** Adjudicación de los 46 candidatos por lectura (lote extraído + cruce en disco): 43 FP (24 solape / 13 bleed editorial / 6 multi-resolución) — la cota «27-46 ausentes» se corrige a 4. Causa raíz de los 2 pipeline leída en código: RE_ANCLA exige ':p.' juntos y el OCR degrada de a un token → entrada pegada a la siguiente → página sin fila (B151, construir_catalogo v1.01→v1.04, PoC lado-índice 10/14.406=0,07%). Los 2 editoriales (Lombardi 333:1966, Mercante 349:43) NO están en el índice oficial → suplemento editorial curado (_meta/catalogo_suplemento_editorial.csv, M46). Dos ciclos --consciente/--regolden sellados; reparaciones colaterales: Tedesco/Parques/Décima descontaminados (F002 cerrado), Bergés 2218 panel 1→6, Chabán 2221 quimera resuelta (procedente→desestima adjudicado). Destapes nuevos: B152 (Figueiredo, colisión de página → schema sub-página) y B153 (conjueces Lozano/Rodríguez Basavilbaso no reconocidos en firma). Pendientes siguen: B147-1A/1C · B148 · B149 · residuo B089-B096 · M44 · B146 · M43 ÚLTIMA.)

**Anterior:** 2026-07-11 (H188 — **M45 fase 1 ADJUDICADA (sesión de diagnóstico, pipeline INTOCADO**: 0 scripts de pipeline, 0 outputs canónicos, 0 `__version__` de la cadena; manifest vigente [CLEAN] 64 sin re-sello — Paso 2 del cierre N/A; blind N/A). Instrumento validado en su **primer uso en disco** (pendiente H186/H187): las dos banderas de la gramática de zonas del explorador v8 reproducen EXACTO sobre `zonas.csv` real con la lógica textual de `compute_zona_metrics._flags` (154 y 179). Veredicto: **ninguna es detector limpio de mis-zonificación**. **Bandera 2 (firma-antes-de-dispositivo, 179):** FP-A 64 (firma del caso previo en la cola-residuo, wc≤30; confirmado 4/4 por lectura: 329_p2179, 333_p9, 337_p346, 341_p2019) + residuo-miss 14 (330_p1927) + BUG-1 49 (`residuo`+`firma` etiquetan la cabeza del propio caso: 334_p1461, 333_p385) + BUG-2 52 (`firma` dentro del cuerpo: excusaciones/nombres densos 329_p3649/339_p1371 · miss de dispositivo «Que por lo expuesto…» 329_p2596) → **mis-zonificación genuina 101/179 = 56%** (el pase estructural inicial sobre-estimaba el FP; el discriminador real es wc + posición del tramo respecto del residuo, salió de LEER). **Bandera 1 (epílogo-no-terminal, 154):** es un **detector de bleed de frontera de caso**, no de zonificación; partición reproducida en disco (`diag_frontera_b1.py` v0.3) = 51 carátula-suelta-1A + 27 fantasma-1B-fuerte + 19 fantasma-1B-ambiguo + 21 solape-1B' + 13 apéndice-1C + 23 epílogo-previo-FP. 4 findings del parser localizados en su capa real por LECTURA de `zonificar_bloque` (~2746-2969), `detectar_fin_real` (~2477-2650) y el acople en `procesar_archivo` (3572-3610). **Hallazgo mayor: la familia B012/B045 (fallos ausentes del corpus) dimensionada por primera vez = 27-46 candidatos (~0,5-0,8% de 5697), concentrados en tomos 329-331.** Findings nuevos → entradas B147/B148/B149 abajo; M45 marcado ADJUDICADO. Scripts: `scripts/diagnostico/H188/diag_frontera_b1.py` v0.3 (read-only, validador del finding 1). Pendientes encolados siguen vigentes: adjudicar 27+19 fantasmas → recuperación al catálogo · residuo B089-B096 · M44 · B146 · audit B137 · B144 · B126 · Hotesur real · M43 ÚLTIMA.)

**Anterior:** 2026-07-11 (H187 — **B145 CERRADO (derivar_partes v0.17→v0.18)**: `RE_MARK_REC` = gramática v2 portada de `RE_PIE_START` (extraer_epilogos v0.4 / parser 26.1) + guard «contra» pre-verbo. PoC escalera E0→E3 sobre epilogo.csv v0.4 real (poc_b145_mark_rec v0.1): universo ok 4381 == sello, E0 match 3722, fix recupera **50 / pierde 0 / 4 clause_movido** — la magnitud real no era «y fundado» (3) sino el **«por:» de tomos viejos (35/50)**, gap H161 nunca medido. Guard «contra» (tempered dot): superficie corpus-wide = 1 línea exacta (el FP 340_p437, cita queja OIT, sub-clase M44 «contra pre-verbo»), 0 genuinos afectados — divergencia DELIBERADA vs RE_PIE_START porque acá el marcador CAPTURA. «Por libre» (rama 5) DIFERIDO → **B146** (sub-clase «interpuesto a favor de X por su defensor», parte pre-«por»). Diff en disco adjudicado 52/52 (diff_partes_b145 v0.1): 34 GANA + 8 CAMBIA_FUENTE + 10 MOVIDO (todos mejoras/neutrales; eyeball 344_p645 contra el `.md`: el pie nombra a Omint = SUCESORA por fusión de la ejecutada — el pie más preciso que la carátula, doctrina de cascada vindicada) + **PIERDE 0**. Efecto: recurrente_ok 3848→**3882**, mérito **2677/2965 = 90,3%** (+25), epilogo_sin_marcador mérito 188→163. Colaterales documentados: 3 roles perdidos caratula→epilogo (población M33) + micro-gap RE_MULTI (señal plural en la línea del marcador, fuera del clause: 341_p1727 multi si→no). Parser [CLEAN] 5/5 sin re-golden (flip en deriver, fuera del harness — instancia M36); manifest re-sellado [CLEAN] 64 (partes.csv sha 6ef71c06167a… v0.18). Candado blind N/A. Pendientes encolados siguen vigentes: 18 outlier_epi · residuo B089-B096 · M44 · M45 · audit B137 · B144 · B126 · B146 (nuevo) · Hotesur real · M43 ÚLTIMA por decisión H185/H186.)

**Anterior:** 2026-07-06 (H186 — **Explorador v8 (M16 CERRADO)**: sesión de herramienta, pipeline INTOCADO (0 scripts de pipeline, 0 outputs canónicos, 0 `__version__`; manifest vigente [CLEAN] 64 sin re-sello — Paso 2 del cierre N/A). `scripts/explorador/exploradorv8.py` + README: joins graceful de los 4 sidecars del deriver sobre casos.csv (recursos con `causa_inadmisibilidad` recuperada en la UI — muerta en el explorador desde H148 —, partes, epilogo sin cargar texto, materia con capa/fuente), métricas por zona en namespace `wcz_*`/`nseg_*` (fix post-smoke: `wc_dictamen` del pivot colisionaba con la columna homónima del parser; namespace blinda contra futuras `wc_*`), **gramática de zonas** (instrumento NUEVO, 0 antecedentes por grep en DEUDA/BITACORA): secuencia colapsada por caso + 14 banderas calibradas sobre los CSVs reales H185 (sin firma 20 · sin dispositivo 49 · firma ≥4 seg 453 · firma >200 wc 142 [wc firma p50=20 / p95=71 / max=18.353]; la gramática modela el patrón multi-voto para no fabricar FP), tab Auditoría (banderas click-para-filtrar + muestreo reproducible por seed + marcado TP/FP/dudoso con export CSV) que cierra el pendiente M16 «modo auditoría-de-precisión», y panel inline de considerando/por_ello/firma completos desde textos.csv (H113) que cierra el otro pendiente M16. **Poblaciones NUEVAS SIN ADJUDICAR → M45 (nuevo):** epílogo-no-terminal = 154 · firma-antes-de-dispositivo = 179 (señal adjudicable, NO bug demostrado). **DECISIÓN DE SESIÓN: madurar el pipeline antes del siguiente κ** — reafirma la postergación de M43 decidida en H185. Los pendientes encolados de H185 (18 outlier_epi residuales · residuo B089-B096 · B145 · M44 · audit B137 · B144 · B126 · Hotesur real) NO se tocaron, siguen vigentes.)

**Constancia H184 (encabezado histórico):** B117 F2 EJECUTADA Y CERRADA (parser 25.0→26.0→26.1, MAJOR; golden re-congelado, manifest [CLEAN] 64) — el detalle completo vive en la constancia H184 de la entrada B117: `RE_EPILOGO_MARKER` nueva = escalera v2 consolidada, SOLO en el sitio del marker (`RE_DATOS_PARTES` INTACTA para A001) · regresión del fix EXACTA (markers vivos 9882+1, 34/34 testigos, cae 0) · **hallazgo del dump: clase «Queja/Causa» FUERA del universo F1** — la regex vieja no anclaba `Queja` y exigía `Causa:` pegado → 104 markers nuevos, adjudicados por lectura **104/104 espurios**: 103 narrativa «causa …(Fallos: …» matada en **26.1** (rama `(?i:^Causa\s*:)` anclada verbatim vieja, 0 pérdidas del universo F1 por construcción; 87/87 líneas únicas mueren, 21 controles sin diff) + 1 FP ACEPTADO documentado (340_p437 «Queja contra el Gobierno…» = cita de queja OIT caso 2240/informe 332, leída en .md; sub-clase M44 «contra pre-verbo») · ciclo --consciente x2 (26.0 y 26.1): **casos/textos/votos/editorial byte-idénticos → 0 flips de decisión/outcome/mérito, M43 no contaminada · materia/recursos hash-idénticos al pre-fix → 0 ripple** · único CSV del parser que cambia: zonas (−992 segmentos) · **corrección de constancia: el «920 (86+834)» de F1 era marker-level; efecto real zona-level = 728 casos (68+1 caída entera + 655 parcial), conciliado EXACTO en disco (conciliar_f1_h184: 920 − 19 todos_caen-sin-zona-por-guarda-dictamen − 173 mixtos-con-markers-redundantes = 728, anomalías 0)** · 470.005 wc de cuerpo recuperados / 655.469 wc de epílogo espurio descartado; Mazzeo 330_p3248 epi 19.962→56 · corrección de constancia 2: casos.csv NO tiene columnas wc_cuerpo/wc_epilogo/wc_residuo (viven en zonas.csv; el contrato del PROMPT sobre-predijo) · **residuo B089/B096 INDEPENDIENTE de B117 demostrado** (selección wc_residuo>300 recomputada = 793: 0 bajan de 300, 0 bajan algo) · epilogo ok 4402→4377 (via_firma 98→100) / partes recurrente_ok 3848 (67,5%; mérito 2652/2965 = 89,4%). Scripts: dump_diff_h184.py + conciliar_f1_h184.py + pocs postfix (scripts/diagnostico/H184/). Candado blind N/A (clasificador e insumos n300 intactos; casos/textos byte-idénticos). PENDIENTE H185: **M43** (re-κ ciego del eje unificado 2965, pre-Dataverse v2) · **residuo B089-B096** (adjudicar corrida gemela H182, con independencia de B117 probada) · RE_PIE_START ensanche con la gramática v2 (unidad downstream opcional, fuente única) · M44 (+sub-clase «contra pre-verbo», testigo 340_p437) · audit B137 · B144 · B126 · Hotesur real.)
...
(viejo comportamiento, sigue con bug — **ver B172**; la referencia
original a «B011» era ERRÓNEA: B011 es el bug catalográfico de
`344_p344`, sin relación. Corregido H214, con la magnitud medida).
...
**Diagnóstico / evidencia:** casos testigo (H098, ambos `cod_caratula_ok=0`): 332_p2797 (caption «Autolatina Argentina S.A. TF 13892-I c/ Dirección General Impositiva») — vigente; 346_p1241 (caption «C., J.M. c/ Swiss Medical s/ ley de discapacidad») — reclasificado H219: pertenece al pool del gate L4193 (`apertura_rel is None`: V1 ni corre) → B174, no a este bug.
**Estado de verificación:** `confirmado_cuantificado` (H214, re-medido H219) — `case_name_cuerpo` vacío en **922 de 5894 filas** (era 928; el fix del frente C, parser v42.1, cerró la sub-clase Z_rama_vacio: 6 recuperados — 329_p4944, 330_p3248 Mazzeo, 331_p530, 332_p2159, 334_p214, 340_p1236 Gimbutas). Ramas vigentes por la columna `case_name_cuerpo_fuente` (H218/H219): v1 **3821** · fallback **1151** · vacío **922**.
**Cota ACOTADA (H215) — las 928 no son un bloque homogéneo y B106 no puede reclamarlas todas:** `word_count = 0` en **191** (no hay texto que parsear) · `apertura_tipo` vacío en **236** (`apertura_rel is None`: ni V1 ni el fallback llegaron a correr — no fallaron, no se ejecutaron; sub-clase con ticket propio: B174, pool 6 donde V1 dispararía) · `tipo_entrada` de sumario en **191** (34 `sumario_editorial` + 157 `sumario_con_link`, que no son fallos y correctamente no traen autos completos) · `case_name_indice` vacío en **0** (la carátula está en las 928). La cardinalidad real de B106 —vacío CON «Vistos los autos» presente y apertura localizada— es sustancialmente menor y hay que calcularla con la intersección exacta, no restando a ojo (los conjuntos de arriba se solapan).
**Validador propuesto:** grep de «Vistos los autos» en el bloque de los casos con `case_name_cuerpo` vacío, separando los `apertura_rel is None`. Revisar por qué el caption presente no se captura (¿formato del tomo, ruido OCR en el delimitador?).
**Estado del fix:** no diseñado para el residuo (sub-clase Z_rama_vacio CERRADA por el fix del frente C, parser v42.1).
**Referencias cruzadas:** H098. M19. B014/B093/B094 (carátula), B089/B096 (residuo), B174 (gate L4193). H219. Sin ID histórico.
...
### B117 — Zona `epilogo` absorbe la cola del considerando (cuerpo) a mitad del bloque — CERRADO H184
...
**Constancia H182 — DIAGNÓSTICO EJECUTADO (poc_b117_disparador v0.1, scripts/diagnostico/H182/): la hipótesis H179 se CORRIGE Y ENSANCHA.** Corrida en disco sobre los 299 (wc_epilogo>500): A0 identidad zonificador-vivo == zonas.csv **299/299, 0 desync** (parser v25.0 == sello). El PoC clasifica el primer span global Y el primer span dentro del caso propio (hallazgo del smoke: en `fin_extendido` el 1er span suele ser pie/cola del caso ANTERIOR — 10 casos con 1er span pre-apertura-propia). Resultado (clase_post × guard_fuente): **(1) el mecanismo (b) DOMINA pero es más ancho que `^Recurso`** — recurso_narrativo 240/299 (237 arrancan en minúscula = wrap OCR inequívoco) + rotulo_pie 27 (0/27 con `:` — las ramas Profesionales/Parte actora-demandada/Tribunal de origen SIN ancla de dos-puntos fallan igual; hasta una carátula matchea: 340_p1025 «Profesionales de Parque Nacional Iguazú c/ Misiones»). **(2) El envenenamiento (a) es MINORITARIO:** 213/240 narrativos tienen guard PROPIO (flip post-firma-de-mayoría / entre votos: guard legítimo + regex floja); solo_pre_apertura ~18 + mixta ~11 → sanear anclas del residuo recupera ~29, no el grueso. **(3) pie_genuino 31 → su inflación es frontera INFERIOR = familia B096, frente aparte.** (4) Finos para el fix: 341_p1679 «Recursos ordinarios interpuestos y fundados por:» = pie genuino con gramática variante que RE_PIE_START pierde (gap a ensanchar si esa gramática gatea); 329_p4289 «recurso de apelación interpuesto por la concursada…» = narrativa que SÍ matchea RE_PIE_START vía re.I → discriminador faltante = CASE-SENSITIVITY (el pie real arranca «Recurso/Queja» mayúscula; hipótesis sustentada por 237/240+1, a medir, no asumir). sin_match 1 (337_p1451, leer). El primer span explica mediana 11% del wc inflado (la captura se reparte en múltiples spans por caso). Evidencia: poc_b117_disparador_out300.csv. La corrida gemela sobre la selección de residuo (wc_residuo>300, 793→708 con span) quedó SIN adjudicar (out.csv) — insumo del frente B089/B096.
**Constancia H183 — F1 EJECUTADA Y ADJUDICADA EN VERDE (poc_b117_superficie v0.1→v0.2 + poc_b117_triage v0.1, scripts/diagnostico/H183/, read-only).** Corpus completo: A0 5697/5697 (zonificador vivo == zonas.csv) · regresión out300 299/299 (clase reproducida + k0 entre markers). Universo real: **11.201 epilogo_marker**. V1 (case-sensitive entera + rótulos «:» + «y fundados»): queda 9848 / cae 1353. Triage (inicial mayúscula/minúscula × rama) + lectura una-a-una de las 120 de riesgo: **67 voces TODO-CAPS de sumario** (45 «RECURSO EXTRAORDINARIO» + 9 «RECURSO DE QUEJA» + …; testigo 343_p1218 leído en .md: voces intercaladas con «VIGENCIA DE LA LEY»/«SENTENCIA ARBITRARIA» — sub-clase nueva de B117) · ~15 narrativa con mayúscula (Recursos Hídricos, citas bibliográficas, carátula 340_p1025) · **34 pies genuinos con gramática degradada** (patrones: «por» comido/deformado por OCR ~13 · material entre verbo y «por» ~7 · verbos fuera de set ~5 · rótulos sin «:» 7 · Title Case t347 2) · 4 ambiguos. **V2** = case SOLO en `^(?:Recursos?|Queja)` + resto `(?i:)` · verbos `inte\w*rpu\w+|deducid|presentad|articulad|fundad` · «por» LIBRE (`\bpor\b` tras el verbo) · sin-por (artículo o `(?-i:[A-ZÁÉÍÓÚÑ])`) · `^Recurso[^:\n]{0,60}:` · rótulos Parte/Tribunal de origen/Profesionales case-sensitive con separador `[,.]`/contenido de pie: **queda 9882 / cae 1319 · recupera EXACTAMENTE los 34 testigos · 0 readmitidas no-testigo** (escalera de atribución: gram_v2 24 · rotulo_relaj 7 · case_scope 2 · recurso_dp 1) · fn_candidata_v2 = 58 = las narrativas re.I ya adjudicadas como FP del detector VIGENTE (v2 no pierde nada que el vigente detecte de verdad). Ambiguos leídos en .md (extraer_caso → scripts/diagnostico/H183/): 348_p61 = **pie genuino, verbo partido por soft-hyphen** («…Magistratura, in­⏎terpuesto por…») → FN aceptado; 330_p2877 = títulos de sección DEL DICTAMEN («Recurso Extraordinario de la Municipalidad») → recupero; 330_p3248 (Riveros/Mazzeo, 5446 líneas legítimas, sin índice absorbido) = narrativa de dictamen con wrap, el pie real del final CONSERVA marker → recupero. **FN total corpus-wide = 3 líneas** (348_p61 · 338_p1084 verbo en línea siguiente · 331_p793 sin verbo) = 0,23% de las caídas, clase única «verbo ausente/partido» → **M44**. Efecto por caso (v2): todos_caen 86 · mixto 834 · todos_quedan 3453 → 920 casos recuperan cuerpo. Hallazgo Gate 2: `RE_DATOS_PARTES` tiene un **2º consumidor** (A001 búsqueda inversa de firma, parser.py L1686, validado PoC H045) → el fix va como regex NUEVA (`RE_EPILOGO_MARKER`) solo en el sitio del marker (L2467), `RE_DATOS_PARTES` intacta. **DECISIÓN: F2 = A pura, sin lookahead** (el lookahead/deshifenado de frontera va a M44 con PoC propio, post-F2). Evidencia: poc_b117_superficie_out.csv (corrida v0.2; la de v0.1 fue pisada salvo renombre) + poc_b117_triage_riesgo.csv + los 4 .md extraídos.
**Constancia H184 — F2 EJECUTADA Y CERRADA (parser 25.0→26.0→26.1, MAJOR; golden re-congelado + manifest [CLEAN] 64).** (1) **26.0:** `RE_EPILOGO_MARKER` = los seis cuerpos de la candidata v2 unidos con `|` verbatim (único cambio sintáctico: el `re.I` global de los rótulos-`:` escopeado como `(?i:)`); swap SOLO en el sitio del marker (Pasada 1); `RE_DATOS_PARTES` intacta para A001 (verificado: exactamente 2 consumidores `.match`, L2030 y el sitio). Regresión del fix: poc superficie post-fix = **queda 9882 exactos, 34/34 testigos, cae 0, fn 0** (producción == escalera demostrado). (2) **Hallazgo del dump (dump_diff_h184.py):** la regex vieja no anclaba `Queja` y exigía `Causa:` pegado → la nueva admite **104 markers FUERA del universo F1** (partición en script: 9986 = 9882 + 104). Lectura completa: **104/104 espurios** — 103 narrativa «causa “Casal” (Fallos: …» (el `:` lo regala la cita de Fallos, rama `Causa[^:\n]*:` re.I) + 1 «Queja contra el Gobierno de Argentina presentada por el Sindicato» (340_p437, leída en .md = cita de queja OIT caso 2240/informe 332 en el cuerpo). (3) **26.1:** rama `Causa` fuera de la lista de rótulos → alternativa propia `(?i:^Causa\s*:)` (verbatim regex vieja); 0 pérdidas del universo F1 por construcción (todo marker «Causa» viejo tenía el `:` adyacente); verificado 87/87 líneas únicas mueren + 21 controles sin diff. **FP residual ACEPTADO y documentado: 340_p437** (entra por gram_v2 `presentad…por`; discriminador candidato «contra pre-verbo» → sub-clase M44, PoC propio, NO parche ad-hoc). Regresión 26.1: markers **9883 = 9882 + 1** (el FP, único readmitido no-testigo). (4) **Adjudicación del ciclo (--consciente x2):** casos/textos/votos/editorial **byte-idénticos al golden** → 0 flips de decisión/outcome/mérito por identidad (2965 == gate quieto, M43 no contaminada); materia/recursos **hash-idénticos al pre-fix** (sha256 vs backup) → 0 ripple; único CSV que cambia: zonas (141451→140459, −992 segmentos). (5) **Corrección de constancia:** el «920 casos (86+834)» de F1 era **marker-level**; el efecto real **zona-level = 728 casos** (68+1 caída entera de zona + 655 parcial), conciliado EXACTO en disco (`conciliar_f1_h184.py`: 920 − 19 todos_caen sin zona en v25 [guarda de dictamen: ancla en Pasada 1 que nunca abre zona en Pasada 2] − 173 mixtos con markers caídos redundantes [dentro de zona ya abierta por marker sobreviviente] = 728, **anomalías 0**). Corrección 2: casos.csv NO tiene columnas wc_cuerpo/wc_epilogo/wc_residuo (viven en zonas.csv). (6) **Efecto:** 470.005 wc de cuerpo recuperados / 655.469 wc de epílogo espurio descartado; Mazzeo 330_p3248 epi 19.962→56; contrato negativo cumplido (348_p61 pierde pie [FN aceptado, epi 46→0 sin_zona], 338_p1084 103→31, dictámenes 330_p2877/330_p3248 recuperan, 343_p1218 1248→434). Downstream: epilogo ok 4402→4377 (via_firma 98→100), partes recurrente_ok 3848 (mérito 2652/2965 = 89,4%). (7) **Residuo B089/B096 INDEPENDIENTE demostrado:** selección wc_residuo>300 recomputada = 793; tocados 199, **0 bajan de 300, 0 bajan algo** (mecanismo: el residuo es pre-primera-zona-semántica, Pasada 3; el epílogo no participa de ese borde). Evidencia: `scripts/diagnostico/H184/` (dump_diff_h184.py + _261.csv, conciliar_f1_h184.py, pocs postfix 26.0/26.1) + 340_p437.md.
**Estado del fix:** aplicado (CERRADO H184; parser 26.1, golden + manifest [CLEAN] 64). Costo aceptado y documentado: 3 FN (clase verbo-partido → M44) + 1 FP (340_p437, sub-clase «contra pre-verbo» → M44). Sucesor downstream **EJECUTADO H185**: `RE_PIE_START` de extraer_epilogos (v0.4) = unión escalera v2 + rama local «Nombre de la/el»; via_firma 100→104, diff adjudicado 5/5 por lectura (detalle en el encabezado H185 y en el changelog de extraer_epilogos). Expuso B145 (gramática v1 gemela en `RE_MARK_REC` de derivar_partes).
...
**Comp. 2 CERRADO EN AMBOS LADOS — cabeza H200 (vía D1, parser v33.0) · FIN H201 (vía D2, parser v34.0):** los tiers exact/prefix de `refinar_inicio_por_titulo` exigen forma de carátula y no-firma (B147/D1), y la Pista 1 de `detectar_fin_real` prefiere el primer match CON forma con fallback al primer match a secas (B147/D2 — el `es_caratula` sin estructura que esta entrada documentó en su origen quedó reparado con el discriminador `_es_caratula_v2` en los dos lados; los previos truncados tipo 330_p1135 reparados, flip-set 50). De la matriz de fixes histórica, lo aplicado ≈ opción B (endurecer el test de forma) con el dato que le faltaba, más el fallback que la matriz no previó. Sigue abierto: comp. 1 (= **D3**, exclusión de tokens en `primer_token_de_caratula` con el dato del PoC — el flip-set D1 confirma los basura: «ello» matchea «Por ello» en 331_p427/583, «Autos»/«Unión»-vs-acrónimo en 333_p1401; **dato nuevo H201: «Autos» ancló de casualidad la carátula correcta en 334_p763 — la exclusión D3 debe re-medirse contra el layout v34.0, el token basura ya no rompe el fin pero sigue sin identificar**). **RESUCITADA H199 (patrón B067/H194: bug dormido por falta de discriminador, resucitado cuando llega el dato).** La «señal no medible desde el CSV» de H068 existía: vivía en la gramática de zonas (D-87 de H194, 30 FRONTERA_FPA + 2 MIXTA). Adjudicado por lectura sobre 7 `.md` que los DOS componentes operan en AMBOS lados de la frontera: comp. 2 (match sin verificación de estructura) también vive en `refinar_inicio_por_titulo` tier base (L2470-2478, sin guards — solo el Tier 4 los tiene), y comp. 1 (tokens institucionales no excluidos) alimenta ambos («Banco» 329_p2645, «Mendoza» 330_p1158/1135 — un solo token rompe cabeza del siguiente Y fin del previo, «Nuevo» re.I 329_p2179; top del flip-set: Restitución 12 / Banco 7 / Luis 6 / Asociación 6 / Aires 5 / Cámara 4 / Sala, Recurso, Autos, Civil…). Dimensión corpus-wide: `poc_b147_d_cabeza` v0.1 (E0 5894/5894), flip-set candidato 105 casos solo del lado CABEZA. El fix va partido: comp. 2 lado cabeza = **D1 (H200)**, comp. 2 lado fin = **D2**, comp. 1 = **D3** — plan completo en la entrada B147 (Adjudicación H199).
...
### M16 — Refacción del explorador Streamlit para auditoría a ojo — CERRADO H186
...
**Cierre (H186):** `scripts/explorador/exploradorv8.py` cierra los dos pendientes: (1) tab Auditoría = modo auditoría-de-precisión separado del masivo (banderas precomputadas con conteo y click-para-filtrar, muestreo aleatorio reproducible por seed, marcado TP/FP/dudoso con export a CSV — las marcas viven en la sesión del navegador, descargar antes de cerrar); (2) panel inline del considerando/por_ello/firma COMPLETOS desde `csjn_casos_textos.csv` en el detalle (habilitado por el split H113; respeta M15: texto sin truncar). Además: joins graceful de los 4 sidecars del deriver, métricas `wcz_*`/`nseg_*` por zona con sliders bajo demanda, gramática de zonas con 14 banderas (poblaciones nuevas → M45), detalle en tabs con la fuente a ancho completo, README en `scripts/explorador/`; (3) v8.1 misma sesión: tabla multi-selección (los ticks eligen, no abren — semántica corregida) + export de casos crudos `.md` (anclaje canónico source_file+líneas, lección B102) + reporte de auditoría `.md` (filtros+banderas+muestra+marcas+crudos) como insumo directo de la sesión H siguiente. La duda de escalabilidad (H093) se mitigó (expanders colapsados, sliders solo para zonas elegidas), no se eliminó; si duele, el paso siguiente es el lazy-load de textos (M45, segunda parte).
**Referencias cruzadas:** H093, H096. B100. M15. H186, M45.
...
**Prioridad H212 — READ-ONLY, CERRADA SIN CONCLUSIÓN (parser v38.0 / `derivar_materia` v4.1 / manifest [CLEAN] 65 INTACTOS). La validación del operador de los baselines H208 NO se produjo: sigue PENDIENTE (tercera sesión que la difiere).** Hechos: baselines **recuperados** (estaban en `Downloads`, no en H208/) — muestra sha `590c5a7d11c4`, spot-check 35/35; reconstrucción desde transcript idéntica al byte. Instrumento `dump_gate_kw.py` v0.1 (7/7 rc=0 → `gate_kw_lectura.txt`, insumo listo para cuando la validación se haga). Mediciones: pool kw = **134 `kw(1)` / 4 `kw(2)`**; 77 altas = 44 norma + 29 kw+norma + 4 kw(2); los 22 kw adjudicados son todos `kw(1)`; `kw(1)` 16/22 con los veredictos sellados (penal 7/7 · cc 3/4 · tributario 0/2 · lesa 0/1; pool: penal 96 de 134); 64/134 con señal de contienda en el head (heurística). Decisiones del operador: `aduanero` **dual** (penal en lo delictivo / contencioso en el resto) con **duda declarada sobre si un regex puede dividirlo**; `penal_tributario` es penal Y tributario; `materia` clasifica el pleito y **NO es proxy de la secretaría** (indeducible del corpus; fuente = fieldwork en producción; sec. 6 = salud + laboral + DDHH); **temas y firmas dan información valiosa sin la secretaría**. **CANDIDATA: taxonomía del Anuario de Estadísticas de la CSJN** (temas que no calcan las secretarías). Candidatas para la próxima: **taxonomía del Anuario (decide el marco del frente materia)** · **validación del operador (PENDIENTE — insumo en `gate_kw_lectura.txt`)** · H213 relabel terminal · vocab_faltante (testigos 23.660/23.661/26.682→salud) · M59 paso 2 (ámbito caratula, ≤265) · M61 (DJA) · M62 · M51 · `materia_fuente` legible (residuo de M49; decisión pendiente) · B170 · B168 · B171 · B166 (b1)-B · M60 · infra git-lfs · **M43 ÚLTIMA**.

**Prioridad H211 — M58 APLICADO COMPLETO: parser v38.0 (dictamen_text, 3.434 casos) + extraer_normas v1.1 (ámbito dictamen, 6.206 pares) + derivar_materia v4.1 (tier lectura_dictamen, 77 altas, muestra 24/25 TP) — manifest [CLEAN] 65; cobertura materia 78,5%.** Incidentes de método documentados en BITACORA (3 violaciones de orden; commits b9ceb64 amendado y 15e415f con corrección de constancia) → M62. Candidatas para la próxima: **validación del operador de los baselines H208 (sigue PENDIENTE — decide si el pool kw ~138 se recupera)** · M59 paso 2 (ámbito caratula, flip-set ≤265) · **vocab_faltante (candidatas selladas con testigo: 23.660/23.661/26.682→salud, 26.396; al poblarlas, las altas 340_p1921/344_p3469 re-deciden — constancia en M58)** · M61 (DJA prior + priorizar_normas) · M62 · M51 (tercera evidencia) · B170 · B168 · B171 · B166 (b1)-B · M60 (espera madurez) · infra git-lfs (textos.csv 62 MB) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H210 — CIERRE RETROACTIVO H209 SELLADO EN DISCO (verificación completa: versiones · sha 89eb50fe · baselines 4 ámbitos · atadura v0.2↔v1.0 5973/1393/2643 · gap 265 replicado desde la definición del código · gate [CLEAN] 65; backlog git H207–H209 commiteado y pusheado — los «Commits: 2» de H208 nunca se habían ejecutado, corrección de constancia; parser v37.0 detectado FUERA del primer add y commiteado retroactivo antes del push) + M49 APLICADO (`derivar_materia` v4.0 MAJOR, candado cross-tab 1:1 PASÓ, sha materia 607793de…, manifest [CLEAN] 65 — detalle en la entrada M49).** Yapa no canónica: explorador v8.1→v8.2 (ID como LinkColumn `?caso=ID`, click abre solo ese caso; ticks intactos). Candidatas para la próxima: **M58 (dictamen_text + tier — PROMPT_H211 armado; MAJOR de schema de textos.csv, ciclo blind completo, EXIGE subir parser.py)** · M59 paso 2 (ámbito caratula, flip-set ≤265) · vocab_faltante (4 CSV de _meta/vocab_materia) · **M51 (unidad chica; segunda evidencia H210 del costo del flag)** · B170 · B168 · B171 · M60 (espera madurez) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H209 — M59 PASO 1 CERRADO (`extraer_normas` v1.0 canónico, 4 ámbitos → `csjn_casos_normas.csv` · `derivar_materia` v3.3 byte-idéntico, candado sha 89eb50fef37b PASÓ · `correr_pipeline` v1.1 · `generar_manifiesto` v1.9 · MAPA · manifest [CLEAN] 65, 10 CSV) + DECISIÓN TAXONÓMICA RESUELTA (dos ejes: terminal + `materia_subyacente`/JYC) + M49 RATIFICADO (rename `materia_capa`, esquema sellado en la entrada).** Parser v37.0 INTACTO; materia cobertura 77,0% / sin_ancla 1104 SIN CAMBIO (refactor puro). Ámbitos dispositivo/voto medidos por primera vez (161/78/145 · 1562/580/825), inertes. Sesión sin cierre en el momento (browser); documentación reconstruida el mismo día. Candidatas para la próxima: **M49 = unidad H210, ADJUDICADA (derivar_materia v3.3→v4.0 MAJOR, candado cross-tab 1:1, ciclo completo + explorador + grep análisis + tabla equivalencias CODEBOOK)** · M59 paso 2 (ámbito caratula, flip-set ≤265) · M58 (dictamen_text + tier — taxonomía resuelta, falta validación spot-check) · vocab_faltante (4 CSV de _meta/vocab_materia) · B170/B168 (cola técnica) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H208 — FRENTE MATERIA read-only triple (parser v37.0 y manifest [CLEAN] 64 INTACTOS): veredicto H115 corregido — pool sin_ancla DESCOMPUESTO (41% vocab_faltante · 22% competencia · 19% procesal_propio · 18% remisión) · canal dictamen con rinde 215/748 y ≈80% precisión (norma 13/13 TP, los 4 FP todos kw → diseño tiered) · extraer_normas v0.2 (1393 normas distintas / 2643 casos / gap carátula 265).** Instrumentos en H208/: `diagnostico_sin_ancla.py` v0.1 (E0 0/5894) · `diagnostico_dictamen.py` v0.2 (E1 0/3060; v0.1 frenó por contador no-verbatim, lección H207) · `extraer_normas.py` v0.2 (E2 0/5894) + baselines de adjudicación (`sin_ancla_muestra_adjudicada.csv`, `dictamen_spotcheck_adjudicado.csv` — **validación del operador PENDIENTE**). Candidatas para la próxima: **M59 paso 1 — promoción de extraer_normas a etapa canónica + refactor byte-idéntico de derivar_materia (la más REEL: candado sha 89eb50fef37b)** · **decisión taxonómica del operador** (bloquea ~40% del pool, redefine denominador — puede ir primero, es conversación no código) · **M58** (dictamen_text + tier, rinde medido) · M59 paso 2 (ámbito carátula, flip-set ≤265) · vocab_faltante (esperan los 4 CSV de _meta/vocab_materia — los VERIFICAR de triggers GCBA/ministerios/Estado-Provincial) · B170/B168 (cola técnica H207) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H207 — B166 (b1) sub-clase A CERRADA: fix peek de wrap en las perf-variantes del resolutor (parser v37.0, MAJOR), flip-set 4 = 4 TP, ciclo completo [CLEAN] 64.** Superficie → adjudicación (2779 por extracto) → fix → ciclo, todo en una sesión: casos/textos SOLO {4} · outcome solo 2779 (sin_dispositivo→desestima, recuperación) · zonas/editorial byte-idénticos · blind byte-idéntica · gate M55 pool 29 sin movimiento · recursos re-derivado (433006ff0f68…). Residual H194 cerrado. Regresiones vivas: `poc_b166_b1a_peek --esperar-version 37.0 --post-fix --out propio` (flip-set VACÍO · E0''=={4}) + radar `poc_b166_b_superficie --esperar-version 37.0 --out propio` (pool == 10 · t1 5646 · t2 13 · t3 10 · t3b 1 · t4 12 · none 21; desvío = FRENAR) + `check_m55_quorum --out propio` diffeado (pool 29) + las de H200-H205 **con pin ajustado a 37.0**. Candidatas para la próxima: **B170 superficie** (ivi-FP por cita wrapeada, barata, read-only — el FP daña recall de T1 hoy; el testigo 728 quedó adjudicado) · **B168 fix propio / opción C** (prerequisito cuantificado 1.243/807: adjudicar muestra de los 807 es el primer paso) · **FRENTE MATERIA** (insumo tesis; sin_ancla sigue en 1.104 — medido en la corrida H207 — → el diagnóstico arranca directo en muestreo estratificado) · B167 · M51 (unidad chica) · B166 (b1)-B queda enumerada (vocabulario no escala) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H206 — Sesión READ-ONLY triple (v36.0 sin cambio, canónicos intactos): B169 ADJUDICADA (clase pick = 3, costo sostenido, fix diferido; pool latente no-pick 1.243/807 cuantificado como prerequisito C/M57) · B166 (b) SELLADA (pool expuesto 13 = solo t3-post-techo; 4140 → (b2)/B161; 13/13 por extracto en sub-clases A/B/C; daño de decisión 4/5894) · B168 pool 13 reconfirmado · M57 registrada · B170 nueva (ivi-FP por cita wrapeada).** Instrumentos en H206/: `superficie_b169.py` v0.2 (E0t/E0c/E2 = 0) · `poc_b166_b_superficie.py` v0.1 (E0/Etier = 0/5703; re-corrible como radar del resolutor: pool ≠ 13 o distribución de tiers distinta = señal). Regresiones vivas H200-H205 sin cambio (nada tocado). Candidatas para la próxima: **B166 (b1) sub-clase A — peek de wrap en las perf-variantes del resolutor** (3 testigos coincidentes en valor → flips de texto sin flips de decisión esperados; superficie del peek ANTES, puede re-pickear) · B168 fix propio / opción C (breaks por fin de zona firma — con el prerequisito B169/B170 ya cuantificado) · B170 superficie (ivi-FP, barata) · B167 · M51 (unidad chica) · FRENTE MATERIA (insumo tesis, diagnóstico sin_ancla) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H205 — B166 (c) CERRADO: fix real panel-por-evidencia-de-zonas (parser v36.0, MAJOR), ciclo completo [CLEAN] 64.** Flip-set SELLADO 34 (31 TP + 3 costos → B169 nueva) · adjudicación keyed-by-caso [OK] · 4140 adjudicado por extracto (firma publicada = sentencia embebida en nota al pie, panel propio recuperado) · corrección de constancia a B168 (cura 2/14, el resto era dedup del contador H204) · gate M55 45→29 (alta única 339_p270 adjudicada) · sin_firma 12→8 · votos 27.916 (+66 exactos). Regresiones vivas: réplica `--esperar-version 36.0 --post-fix --out propio` (E0 0/5894 · flip-set VACÍO) + `check_m55_quorum --out propio` diffeado contra baseline (alta ≠ 270 = FRENAR) + las de H200-H203 **con pin ajustado a 36.0**. Candidatas para la próxima: **B166 (b) techo de votos en T3/T3b/T4** (el outcome de la clase sigue expuesto; testigo vigente 4140 nota-al-pie; el residuo quedó dimensionado por (c)) · **B169 superficie** (dimensionar la clase FP-prosa antes de discriminador) · B168 fix propio (pool 12+2063) · B167 · M51 (unidad chica, fatiga del flag reconfirmada H205) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H204 — Unidad B166 (c) ADJUDICADA (read-only, v35.0 sin cambio): gemelos C2 leídos (575 = residual-wrap en_virtud_perf con caso real; 441 = mecánica nueva firma-colectiva-post-voto, pick correcto) + regla panel-por-evidencia-de-zonas validada corpus-wide (5643/5687 = 99,2%) + flip-set candidato ≈41 dimensionado + destape B168 (voto tragado, pub_nj con duplicados, 14 casos).** Instrumentos en H204/: `poc_b166_c_regla` v0.2 (regla c′ post-apertura + panel vía collect desde zona_ini; baseline `poc_b166_c_regla_v02_out.csv`) · `query_firma_raw_clase_a` (clase A 14/14 adjudicada). Regresiones vivas: las de H200-H203 sin cambio (nada tocado). **Candidata ÚNICA para H205: fix real B166 (c)** — cablear el consumidor en el camino del panel (L4124-4144) + PoC-réplica A/B (patrón poc_b166_variante: parser importado, candado E0 0/5894, flip-set SELLADO vs el candidato ≈41; extras = FRENAR o adjudicar) + ciclo completo (consciente/blind/regolden/gate M55 — pool 45 debería bajar fuerte). Después: B166 (b) con el residuo que (c) deje · B167 · B168 · lote C2 restante ya casi absorbido por (c).

**Prioridad H203 — B166 fix parcial VOCABULARIO cerrado (parser v35.0, MAJOR): variante T1 `en_orden_expuesto`, flip-set {344_p1102} exacto, candado de decisión completo (hace_lugar · mixed · nj 5 · full bench).** Mecánica real adjudicada con código+extracto (headers-wrap y último-performativo MUERTAS): fórmula de cierre invisible al catálogo + T3 sin techo de votos + panel forward-only. Blind byte-idéntica · manifest [CLEAN] 64 (sha casos 457cdc863dbb…) · gate M55 pool 45. Regresiones vivas: `poc_b166_variante --esperar-version 35.0 --post-fix --out propio` (E0 0/5894 · flip-set VACÍO) + `check_m55_quorum --casos ... --out propio` diffeado contra baseline 48 (bajas {1847, 3070, 1102}, fila nueva = FRENAR) + las de H200-H202. Candidatas para la próxima: **lote C2 del pool M55 (16 casos nj=1∧hay-votos, gemelos 344_p441/344_p575 al frente — 441 SIN huella de zonas = mecánica a adjudicar por extracto, puede no ser la de 1102)** · B166 (b) techo-de-votos en T3/T3b/T4 (diseño no trivial: T3 existe justamente para buscar más allá del techo — flip-set propio obligatorio) · B166 (c) panel-por-evidencia-de-zonas (la firma_linea temprana del zonificador sin consumidor) · B167 (wrap-FP «por ello» minúscula, pool 76, discriminador candidato case-sensitivity con advertencia B159) · M51 (unidad chica) · D1b + B165 en tándem · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H202 — Tándem M55 + B154-bis CERRADOS (ciclo de dato, tabla 87→91, parser v34.0 sin cambio de código): instrumento de quórum con baseline sellado + gate estrenado (pool 48→46, bajas exactas {1847, 3070}, 0 altas).** Flip-set {1847 nj1→5 mixed · 2752 nj0→4 · 3070 nj1→5} + 3 FP zona-label letrado-Rivera(h) con constancia (daño canónico cero → pool B148/B155); votos 27.846 (+12) · zonas 137.988 (+9, contabilidad completa) · 0 flips decisión · blind byte-idéntica · manifest [CLEAN] 64 (sha casos 265883cb5cd0…). Regresiones vivas: `poc_b154bis_conjueces --esperar-version 34.0 --out propio` (E1 0/0/0 · E0 0 · flip-set VACÍO) + `check_m55_quorum --out propio` diffeado contra `output/validacion/m55_pool_baseline.csv` (fila nueva = FRENAR) + las de H200/H201. **DESTAPE: B166 (nuevo, grave)** — 1102 outcome=el dispositivo de la DISIDENCIA (corrección al candado H201; panel real 5/mixed/hace_lugar-revoca). Candidatas para la próxima: **B166 (mecánica a adjudicar: votos/zonas de 1102 → dimensionar clase; pariente residual multivoto H194)** · pool M55 restante 46 como tablero (338_p1060/B019 · 330_p3801/B149 · 348_p473 re-mirar constancia fantasma H181) · D1b + B165 en tándem (diseños sellados) · **M51 (corpus-drift como dato — operador fatigado del flag, unidad chica)** · D3 · B161 · M54 · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H201 — B147/D2 CERRADO (parser v34.0, MAJOR): guard-con-fallback en Pista 1 de detectar_fin_real; B018 comp. 2 cerrado en AMBOS lados.** Rechazo duro MUERTO con dato (v0.1, 6 extractos); fallback sellado (v0.2, E0 5894/5894 + E1 0); flip-set 50, 50/50 TP-o-mejora; decisión 21/22 TP + 1 regresión de metadata aceptada (1847 → B154-bis; 1442 recupera un FALLO entero: sumario_editorial→fallo, is_merit 0→1); votos 27.834 (+16) · zonas 137.979 (+74) · blind byte-idéntico · manifest [CLEAN] 64. Regresiones vivas: `poc_b147_d2_v02 --esperar-version 34.0 --out propio` (flip-set VACÍO) + `poc_b147_d1_v03` (ajustar pin a 34.0 si se corre; su E0 no consume linea_fin_real) + `poc_m52_literales`. Candidatas para la próxima (ruta del tramo sellada en PROMPT_H201): **D1b + B165 en tándem (H202, chicas, diseños candidatos sellados — D1b ahora también sana el residual 1259 de D2)** · D3 (re-medir contra layout v34.0, dato nuevo «Autos») · B161 (+1928, variante sentencia) · B155 · **B154-bis (conjueces saga art. 110, 5 nombres sellados + 54 causas gemelas — ciclo de dato chico, rinde potencial alto)** · M54 · **M55 (invariante quórum — instrumento barato, calza con B154-bis: la misma unidad mide el pool y limpia los 2 testigos)** · B163/B164 · M53(a) · frente nuevo Pista-3 (catalogo-previos 2731/570/1784, solapes +4/+11) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H200 — B147/D1 CERRADO (parser v33.0, MAJOR): guards de forma en los tiers exact/prefix de refinar_inicio_por_titulo; B018 comp.2-cabeza cerrado con él.** Flip-set 93 sellado (E0/E0'' 5894/5894, v0.2→v0.3), perímetro completo 0-fuera-de-set, 2 flips de decisión TP, 2 regresiones de metadata → B165; blind 1 caso ⊆ set; manifest [CLEAN] 64; zonas 137.905 (−74). Regresiones vivas: `poc_b147_d1_v03 --esperar-version 33.0` (E0'' mismatch = 93 exactos esperado; diffear contra `_baseline.csv`) + `poc_m52_literales` (5.4x). Candidatas para la próxima: **D2 (vista FIN, Pista 1 — repara previos truncados y limpia el costo-bleed de las caídas a catalogo)** · D1b (V-wrap, 6 casos, flip-set propio) · D3 (tokens B018-1, dato confirmado por el flip-set) · B165/D4 (juez-parte, diseño candidato fullname-del-caso exime guard firma) · B161 (notas al pie — sumó 330_p224 y 345_p715 como constancias) · B160 · dimensionar B163+B164 · M53(a) · M54 (auditoría vigencia, pool 9) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H219 — frente C, clase Z CERRADA (parser v42.1, [CLEAN] 7/7 + regolden + blind); B173 gap huérfano adjudicado.** Candidatas para la próxima: **validación del operador H208 (SÉPTIMA sesión diferida — mayor desbloqueador de materia)** · frente C restante (B 15 · F1 10 · S 6 · E 1 = 32) · B174 (pool latente L4193, 6 — superficie propia primero) · título-de-cabeza/Eje A (2578 sin medir) · B175/B176 (superficies baratas) · borrado de `case_name_cuerpo_legacy` (recién ahora que el frente C tiene la Z cerrada, sigue esperando el resto del frente) · M43 ÚLTIMA.

**Prioridad H199 — Sub-frente D de B147/M45 ADJUDICADO (diagnóstico, pipeline intocado): B018 resucitada; fix partido en D1-D5.** Mecanismo leído sobre 7 `.md`: falso-match-temprano del token en `refinar_inicio_por_titulo` (tier base sin guards) + el mismo token truncando el FIN del previo vía Pista 1; correcciones de constancia (329_p3444 = Melchiori, NO Mendoza; el Mendoza real = 330_p1158; 329_p4815 = interleave). Instrumento `poc_b147_d_cabeza` v0.1 E0 5894/5894, flip-set 105, muestra seed-199 adjudicada (guards de D1 dictados por los FP: firma-versales, carátula mixta, rubro-no-carátula). Micro-items H198 saldados (manifest re-sellado 32.1 sha idénticos · `_tmp/` purgado · commits constatados). Candidatas para la próxima: **D1 (fix cabeza, PoC v0.2 con guards → flip-set sellado → ciclo)** · D2 (vista FIN Pista 1) · D3 (exclusión de tokens B018-1, ciclo de dato) · B161 (notas al pie, diseño-primero) · B160 (GATE de lectura) · dimensionar B163+B164 · M53(a) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H198 — M52 CERRADO (parser v32.1, MINOR): prefiltro de literales obligatorios para JUECES_CONOCIDOS; refactor byte-idéntico (triple vía) y etapa parser ~2x (208→107,8 s).** Unión de alternación única muerta con dato (1.0x en negativos); equivalencia sellada en disco (E3: 0 mismatches / 2,2M líneas); regresión propia viva `poc_m52_literales`. Micro-items saldados: H195 unificado en H196/, bak v27.1 rescatado a archivo\, cierre H197 constatado. Candidatas para la próxima: **frontera B147/M45 sub-frente D** (30 FRONTERA_FPA + Mendoza 329_p3444 prioritaria; constancia 329_p4815) · **B161** (notas al pie, 6 casos, diseño-primero) · **B160** (GATE de lectura) · **dimensionar B163 + B164** (dos PoC read-only) · **M53(a)** (RUIDO_FIRMA-vs-dato, Gate 2 pendiente) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H197 — B162 CERRADO (parser v32.0): perf-gate zonificador-solo para {en_consecuencia, atento_a} + red A1; resolutor intacto (0 flips de decisión, verificado con perímetro completo).** Flip-set 423 casos E0 5703/5703; zonas 137.979 (−99); 17 casos wc descontaminados; regresión post-fix VACÍA; sello [CLEAN] 64. Candidatas para la próxima: **M52** (perf REEL, contrato byte-idéntico — abrir H198 con esto rinde en todas las sesiones siguientes) · **frontera B147/M45 sub-frente D** (30 FRONTERA_FPA + Mendoza 329_p3444 prioritaria; constancia nueva 329_p4815 pick en b0) · **B161** (notas al pie, 6 casos, diseño-primero) · **B160** (GATE de lectura) · **B164** (dimensionar, nuevo) · **B163** (dimensionar) · micro-items: decisión H195 housekeeping + `dir archivo\` · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H196 — B159 clase B CERRADO (parser v31.0): RE_DICT_HDR forma-título laxa; el «dictamen sin cierre» era un FP narrativo del detector.** Calibración 3804 matches / 0 pérdidas; flip-set 7/7 en contrato, 0 outcome, radio fin_real VACÍO medido; sello [CLEAN] 64; regresión propia viva (`poc_b159_flipset --esperar-version 31.0` → vacío). Candidatas para la próxima: **B162** (`en_consecuencia` perf-gate, playbook validado 2×, GATE de superficie primero — puede mover outcome vía el resolutor) · **frontera B147/M45 sub-frente D** (30 FRONTERA_FPA + **Mendoza 329_p3444 prioritaria por perfil**) · **B160** (GATE de lectura) · **B161** (notas al pie, 6 casos, diseño-primero) · **M52** (perf, contrato byte-idéntico) · micro-items: leer p1952 L923 (constancia B162) · B163 dimensionar · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H194 — B149 CERRADO (parser v30.0): el zonificador hereda la evidencia del resolutor + B067 resucitada perf-gated; residuo B148 adjudicado COMPLETO.** Flip-set A1 25/25 TP + A2 16+1 TP (testigo original 348_p443 recuperado, is_merit 2967→2968); diff alineado 41/41 dentro del contrato; sello [CLEAN] 64; baselines pre/post conservados. Candidatas para la próxima: **B159** (7 dictamen-sin-cierre: extraer y leer el no-cierre) · **frontera B147/M45 sub-frente D** (30 FRONTERA_FPA + **Mendoza 329_p3444 prioritaria por perfil** + 329_p2179) · **B160** (4 sub-clases del detector con GATE de lectura + colector, cruza con B155) · adjudicar los 46 post-dispositivo de la vista 2 · multivoto ~13 residual · M50 (evidencia masiva H193) · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H193 — B148 CERRADO (parser v29.0): guards del detector de firma, flip-set sellado 3799/1303, ciclo 100% TP, sello [CLEAN] 64.** Espec iterada en disco (v0.1→v0.3: los controles atraparon 521+71+5 firmas reales que el diseño H192 habría matado — clase FRENO v27.0 evitada ANTES del parser). Bonus: 2 fronteras reparadas de rebote; corrección: firma_raw 346_p965 = colector, no detector (→ B149). Residuo del pool 217/398→101/125. Candidata natural para la próxima: **leer los 8 casos no adjudicados del residuo (329_p4140, 339_p270, 339_p323, 348_p1751, 329_p4446, 331_p2363, 331_p364 + re-clasificar el D 87)** — esa evidencia diseña la unidad (protocolo: sin plan B pre-armado) · B149 con la evidencia nueva 346_p965 · encolados M50 (evidencia masiva nueva) / M51 · CODEBOOK/Dataverse espera M43 · **M43 ÚLTIMA**.

**Prioridad H192 — B154 CERRADO (ciclo de dato, tabla 74→87) + diagnóstico B148 COMPLETO con fix A diseñado.** Flip-set 24/+45/0 pérdidas, contrato 1 TP (344_p1557 voting_pattern); ripple +10 zonas = 2 TP + 5 FP-B148 con constancia. Diagnóstico B148: dos causas raíz exactas en código (RE_HEADER_VOTO_DISIDENCIA sin tolerancia a guión/paréntesis; linea_es_header_sumario deja _en_sumario inerte), pool 217/398, adjudicación 34 casos = 15 sumario-atribución + 9 cuerpo-denso + 5 no-B148, firma_raw desacoplado 5/6 (excepción 346_p965). Candidata única para la próxima: **implementar B148 fix A (PROMPT_H193 armado: PoC flip-set a nivel línea → fix v29.0 → ciclo MAJOR con blind a re-verificar)** · después el residuo del pool decide · encolados M50/M51 nuevos · **M43 ÚLTIMA**.

**Prioridad H191 — B153 CERRADO (parser v28.0): set de jueces externalizado a dato + columna jueces_desconocidos viva.** Flip-set 35/+48/0 pérdidas; contrato verificado (0 flips decisión; 2 voting_pattern TP); ripple de frontera recuperó firmas enteras (1303/1305/2830/3221/3235). Candidatos para la próxima: **B154** (Müller + Leal-«M.» + adjudicar Aranguren/Irurzun del top de desconocidos — ahora es UNA FILA de CSV + mini-ciclo) · auditoría derivers M48 como fuente de unidades de materia (sin_ancla 1102 = la bolsa más grande) · CODEBOOK/Dataverse N=5894 · B148 (insumo _clase_linea_frontera listo) · B152 · **M43 ÚLTIMA**.

**Prioridad H190 — B147-1A CERRADO (parser v27.1): retroceso de frontera fina, cardinalidad 1A corregida 51→1866.** Mecanismo re-adjudicado sobre 8 testigos `.md` (el modelo H188 era doble-erróneo: en Pista 1 la carátula NO se absorbe — cuelga el material pre-carátula del siguiente —; en Pistas 3/4 se absorbe carátula+sumario ENTERO por miss de token). Fix `retroceder_frontera` (etapas A/B, guards firma-versales/header-voto/nombre-propio/continuación-de-wrap, todo reuso de detectores existentes) sobre los fines de Pistas 1/3/4; editorial/firma_actual/fallback intactos. FRENO intermedio v27.0 adjudicado (fragmento de firma wrapeada «LORENZETTI.» fuera de `JUECES_CONOCIDOS` → votos −42; guard v27.1 lo cierra y repara 329_p4506 de rebote). Ciclo sellado + regresión propia del PoC = 0 recortes. Candidatos para la próxima: **CODEBOOK + Dataverse a N=5894** (sigue pendiente de H189, ahora con zonas/wc nuevos) · B148 (firma sobre-extendida — la maquinaria `_clase_linea_frontera` de H190 es insumo directo) · B149 · leer gap Hotesur 348:18047-18076 (M46) · B152 · B153 (con la evidencia nueva de 329_p4506) · frente 1C con los 3 candidatos flageados · residuo B089-B096 · M44 · B146 · **M43 (re-κ) ÚLTIMA**.

**Prioridad H189 — arco fantasmas CERRADO (B147-fase-2 + B151 + M46): los 4 ausentes recuperados, N=5894.** Adjudicación completa de los 46 candidatos (43 FP), causa raíz del ancla degradada leída y fixeada en `construir_catalogo` (v1.04), suplemento editorial operativo para omisiones del índice oficial. Colaterales reparados: Tedesco/Parques/Décima descontaminados (F002 cerrado), Bergés 2218 (1→6), Chabán 2221 (quimera resuelta). Candidatos para la próxima: **actualización CODEBOOK + Dataverse a N=5894** (los dos ciclos sellados tocan el dataset publicado) · leer el gap Hotesur 348:18047-18076 → si confirma, fila nueva del suplemento (M46) · B152 (Figueiredo, decisión de schema sub-página) · B153 (conjueces — relevante para el eje de votos) · fixes de frontera fina B147-1A / B148 / B149 (de a uno, cada uno MAJOR con su ciclo) · residuo B089-B096 · M44 · B146 · **M43 (re-κ) ÚLTIMA**.
...
**Constancia H184 — INDEPENDIENTE de B117, demostrado en disco:** el fix de B117 (parser 26.1, bordes de epílogo) NO mueve el residuo: selección wc_residuo>300 recomputada del sello v25 = 793 casos; post-fix **0 bajan de 300 y 0 bajan algo** (conciliar vía dump_diff_h184). Mecanismo: el residuo se define pre-primera-zona-semántica (Pasada 3) y el epílogo no participa de ese borde. La corrida gemela de H182 (out.csv, 793→708 con span) sigue SIN adjudicar y es todo el frente.
...
### B145 — `RE_MARK_REC` de derivar_partes sin tolerancia «y fundado»: gramática v1 gemela de la reemplazada en extraer_epilogos — CERRADO H187

**Componente:** derivar_partes (capa de marcadores, `RE_MARK_REC` L67).
**Origen / fuente del diagnóstico:** H185 (regresión del ensanche RE_PIE_START: partes recurrente_ok 3848→3848 +0 pese a 4 epílogos nuevos; adjudicado a nivel fila).
**Causa raíz:** `RE_MARK_REC` = `^(?:Recursos?|Queja)\b[^\n]*?(?:interpuest\w+|deducid\w+)\s+por\s+` exige `por` ADYACENTE al verbo — la misma gramática v1 que RE_PIE_START v0.3 y RE_DATOS_PARTES pre-B117. «Recurso ordinario interpuesto **y fundado** por el Estado Nacional…» rompe la adyacencia → sin marcador → el caso muere ANTES de parse_parte (no es problema de parte-institucional vs persona: el marcador no mira qué sigue al `por`).
**Diagnóstico / evidencia:** 334_p1876 (mérito) con epílogo capturado por v0.4 y fila de partes `partes_fuente=sin_marcador_recurso`, recurrente vacío — verificado en disco H185. Mérito epilogo_sin_marcador 187→188 (+1 == el testigo). El epílogo es el caso resoluble de la cascada actual: pelado el «y fundado», parse_parte resolvería «el Estado Nacional –Secretaría…–, representado por la Dra. Rey» → Estado Nacional (wrapper de representación + pasada artículo-inicial v0.17 ya existentes).
**Estado de verificación:** `confirmado_caso_testigo` (1 verificado a nivel fila; población corpus-wide de la clase «y fundado»/verbos ensanchados en epílogos capturados NO medida).
**Validador propuesto:** PoC read-only sobre csjn_casos_epilogo.csv: RE_MARK_REC vigente vs tolerante (`(?:\s+y\s+fundad\w+)?`; evaluar además si los verbos ensanchados de gram_v2 aplican al deriver) — contar flips de marcador y adjudicar por lectura antes de tocar v0.17. Patrón poc_h185_ensanche_pie.
**Estado del fix:** **APLICADO Y CERRADO H187 (derivar_partes v0.17→v0.18).** El PoC (poc_b145_mark_rec v0.1, escalera E0→E3 sobre csjn_casos_epilogo.csv v0.4 real, universo ok 4381 == sello H185) redimensionó el bug: E0 match 3722; la clase «y fundado» era 3 casos — **el grueso era el «por:» de tomos viejos (35), el gap que H161 anotó sin medir**; verbos gram_v2 sumaron 12 más. Fix = gramática v2 completa portada (verbos `inte\w*rpu\w+|deducid\w+|presentad\w+|articulad\w+|fundad\w+` + tolerancia «y fundado» + `por[:\s]`) + **guard «contra» pre-verbo** (tempered dot `(?:(?!\bcontra\b)[^\n])*?` entre token y verbo) — divergencia DELIBERADA vs RE_PIE_START: acá el marcador CAPTURA el clause y el FP lo corrompe; superficie del guard medida corpus-wide = **1 línea exacta** (340_p437, cita queja OIT «Queja contra el Gobierno … presentada por el Sindicato», sub-clase M44 «contra pre-verbo»), 0 matches genuinos afectados. «Por libre» (rama 5) DIFERIDO → **B146**. Flip-set del PoC: recupera 50 / pierde 0 / 4 clause_movido adjudicados por lectura (patrón principista: ancla el PRIMER marcador genuino del pie, convención leftmost; incluye el fino H182 341_p1679 TANDANOR). Diff en disco (diff_partes_b145 v0.1, vs backup v0.17): **52 filas = 34 GANA + 8 CAMBIA_FUENTE + 10 MOVIDO + PIERDE 0** — conciliación exacta con el PoC (50 recuperos = 34 sin-rescate + 16 con rescate previo de carátula; los 2 movidos restantes del PoC salieron byte-idénticos porque parse_parte extrajo la misma parte). Eyeball del único MOVIDO con cambio real de entidad (344_p645, extraído contra el `.md`): el pie nombra a **Omint S.A. = sucesora por fusión** de la ejecutada disuelta (dictamen §VI) — el pie es MÁS preciso que la carátula; doctrina de cascada vindicada en el caso más difícil. Efecto: recurrente_ok 3848→**3882** (+34); mérito **2677/2965 = 90,3%** (+25); epilogo_sin_marcador mérito 188→163. Colaterales documentados (no bugs del fix): 3 roles perdidos en flips caratula→epilogo (329_p4659/334_p1804/348_p879 → población M33; 344_p645 fuera del alcance del guard M33, Omint no figura en la carátula — límite conocido) + micro-gap RE_MULTI (341_p1727: la señal plural «Recursos … interpuestos por:» queda en la línea del marcador, FUERA del clause que RE_MULTI evalúa → multi si→no pese a pie multi genuino; anotado en la fila multi-recurrente del Tablero). Parser [CLEAN] 5/5, manifest [CLEAN] 64 (partes.csv sha 6ef71c06167a… v0.18). Unidad atómica: extractor y clasificadores intactos.
**Referencias cruzadas:** B117 (F1/F2: origen de la gramática v2), H161 (gap `por:` ya anotado en el flujo del fallback), H182 (finos 341_p1679/329_p4289), H185, **B146** (sucesor: «a favor de X por» + por-libre), **M33** (roles perdidos), M36 (deriver sin golden — el diff se validó con script de sesión). H187. Sin ID histórico.

### B146 — Pie con parte pre-«por»: «Recurso interpuesto a favor de X por su defensor» pierde la parte (y el «por libre» de gram_v2 diferido)

**Componente:** derivar_partes (`RE_MARK_REC`, gramática del marcador capturante).
**Origen / fuente del diagnóstico:** H187 (nivel E3 del PoC de B145: «por» libre `[^\n]*?\bpor\b` recupera 5 marcadores más, pero la lectura de los 5 mostró que en 3 la PARTE queda ANTES del «por» y la captura se lleva al letrado).
**Causa raíz:** en la variante «Recurso de hecho interpuesto **a favor de X** por su defensor Dr. Y» la parte real (X) vive entre el verbo y el «por»; cualquier captura anclada después del «por» devuelve al defensor (→ solo_letrado en el mejor caso). Es gramática NUEVA (rama que capture «a favor de (X)»), no ensanche del marcador — por eso E3 quedó fuera del fix B145.
**Diagnóstico / evidencia:** 5 casos leídos en el PoC: **3 de la clase a-favor-de** (331_p2285 Scheller, 331_p2462 Azic, 331_p600 Relats — los tres penal, imputado pre-«por») + **2 recuperos limpios que solo el por-libre agarra** (329_p5467 «Recursos de hecho interpuestos: el S.397.XXXVII por Estado Nacional…» multi-expediente; 330_p1036 «Recurso de reconsideración interpuesto a fs. 314/320 por los Dres…»). Hoy los 5 caen a sin_marcador → cascada carátula (los 3 penales probablemente rescatados por PASO 4; verificar en partes.csv antes de diseñar).
**Estado de verificación:** `confirmado_caso_testigo` (5 leídos; población corpus-wide de «a favor de» en epílogos NO medida — el PoC solo la vio dentro del universo por-libre).
**Validador propuesto:** grep «a favor de» en epilogo_text corpus-wide para dimensionar la clase; luego PoC con rama dedicada `interpuest\w+…a\s+favor\s+de(?:l)?\s+(X)` que capture la parte pre-«por» (rol penal esperable) + evaluar el por-libre restringido a los 2 patrones limpios (multi-expediente / «a fs. N por»). Patrón poc_b145.
**Estado del fix:** no diseñado. Bajo ROI individual (≤5 confirmados); vale por completitud de la gramática y porque toca la misma regex compartida (no-regresión sobre los 3882).
**Referencias cruzadas:** B145 (origen, E3 diferido), RE_PIE_START rama 5 (el por-libre del extractor, donde NO captura y por eso no duele), frente parse_parte (Tablero). H187. Sin ID histórico.
...
**Estado del fix:** diseñado, no aplicado. **Población +3 H187 (B145 colateral):** los flips caratula→epilogo del fix B145 perdieron el rol que la carátula sí daba (329_p4659, 334_p1804, 348_p879: actora→∅) — exactamente el gap de esta entrada, ahora con testigos frescos. 344_p645 (Omint, sucesora por fusión) queda FUERA del alcance: el nombre del epílogo no figura en la carátula → el guard de consistencia lo deja (sin rol), conservador correcto — límite conocido a documentar al implementar.
**Referencias cruzadas:** **M29** (la capa de partes que da el nombre), `derivar_de_caratula`/PASO 4 (la maquinaria que se reusa), **M25**/**M32** (consumen el rol), fila "rol por carátula" del Tablero, **B145/H187** (población nueva). H162. Sin ID histórico.
...
**POSTERGADA por decisión H185:** se ejecuta como ÚLTIMA unidad, después de TODAS las refacciones pendientes, como gate de publicación de Dataverse v2 (racional: cada refacción que toque el eje o los insumos invalida la ronda ciega y arriesga contaminar la muestra; una sola ronda, con el pipeline congelado de verdad). NO proponer como unidad de sesión hasta orden explícita de Guillermo.
...
### M44 — Vista normalizada por línea para la Pasada 1 del zonificador (geometría preservada)

**Componente:** parser (`zonificar_bloque`, Pasada 1 — transversal a TODAS las anclas).
**Origen / fuente del diagnóstico:** H183 (adjudicación F1 de B117: los 3 FN aceptados son la clase «verbo del pie ausente o partido por wrap/soft-hyphen en la línea disparadora»). Formulación de Guillermo: leer deshifenado pero conservando las líneas-mapa.
**Causa raíz:** todas las regex de anclas de la Pasada 1 matchean línea por línea sobre el CRUDO; el wrap del OCR parte palabras clave (verbo del pie → B117-FN; firma de juez → B126; banner → familia M21) y una regex de una línea no puede verlas. La deshifenización existente (deuda #3: `extraer_epilogos`/`derivar_partes`) corre DESPUÉS de decididas las zonas — sanea el contenido, no la frontera.
**Diseño propuesto:** vista sombra por línea, geometría intacta: `bloque_norm[k] = bloque[k] + completación tomada del arranque de bloque[k+1]` solo si `bloque[k]` termina en guión/soft-hyphen (variante: lookahead acotado de línea siguiente cuando el arranque `^Recursos?/Queja` no cierra gramática). `len(bloque_norm) == len(bloque)`: índices, spans, `wc_*` y golden siguen computándose sobre el crudo — nada aguas abajo se entera. **NO deshifenar el corpus ni el bloque de verdad** (unir líneas rompe `mapa_paginas` / `linea_inicio` / spans / golden — descartado en H183).
**Superficie conocida:** los 3 FN de B117 (348_p61 soft-hyphen · 338_p1084 verbo en línea siguiente · 331_p793 solo parcial) + recuperos probables en B126 (firmas partidas) + **sub-clase H184 «contra pre-verbo» (dirección FP):** 340_p437 «Queja contra el Gobierno de Argentina presentada por el Sindicato» = cita de queja OIT en el cuerpo que RE_EPILOGO_MARKER 26.1 acepta vía gram_v2 (único FP residual de B117, aceptado documentado); discriminador candidato: «contra» entre el token de arranque y el verbo del pie nunca aparece en los 34 testigos genuinos — medir con PoC, no parchear ad-hoc. CUIDADO: cambia el matching de TODAS las anclas a la vez → diff ancho, hoy sin medir.
**Estado de verificación:** `confirmado_caso_testigo` (los 3 FN leídos en disco, H183).
**Validador propuesto:** PoC read-only con y sin vista sombra sobre los 5697 (patrón `poc_b117_superficie` v0.2, que ya trae la escalera y los testigos): flips de anclas POR TIPO + adjudicación del delta. Recién después, decidir alcance (¿solo epilogo_marker, o todas las anclas?).
**Estado del fix:** no diseñado en código. **NO acoplar a F2 de B117** (decisión H183: F2 va pura para que el diff tenga un solo culpable posible); unidad propia post-F2.
**Referencias cruzadas:** B117 (origen, 3 FN), B126 (beneficiario probable), M21 (banner partido, clase hermana), deuda #3 (deshifenización downstream). H183. Sin ID histórico.

### M45 — Explorador v8: adjudicar poblaciones de gramática de zonas — FASE 1 ADJUDICADA H188 (lazy-load pendiente)

**Componente:** explorador (`scripts/explorador/exploradorv8.py`, herramienta no canónica); el hallazgo escaló a bugs del parser (B147/B148/B149).
**Origen / fuente del diagnóstico:** H186 (calibración del instrumento nuevo «gramática de zonas» sobre los CSVs reales de H185).
**Diagnóstico / evidencia:** dos banderas destapan poblaciones nunca miradas: **epílogo-no-terminal = 154** y **firma-antes-de-dispositivo = 179**.

**Adjudicación FASE 1 (H188) — instrumento validado en disco + ambas banderas resueltas:**
- **Validación del instrumento (pendiente H186/H187 «primer uso en disco»):** ambas banderas reproducen EXACTO (154/179) reimplementando `compute_zona_metrics._flags` sobre `zonas.csv` real. El cómputo de bandera del explorador queda verificado; el explorador NO zonifica (Lección H045, lee la columna `zona` que produce `parser.zonificar_bloque`).
- **Bandera 2 (179) — partición final:** FP-A 64 (firma del caso previo en la cola-residuo, wc≤30 — confirmado 4/4 por lectura del `.md`: **329_p2179, 333_p9, 337_p346, 341_p2019**, en los 4 el bloque abre a mitad del fallo anterior) · residuo-miss 14 (firma del previo que la Pasada 3 no capturó como residuo; testigo **330_p1927**) · **BUG-1 49** (`residuo`+`firma` etiquetan la CABEZA del propio caso — carátula/sumario/dictamen; testigos **334_p1461, 333_p385** [~1100 palabras de dictamen como `firma`]) · **BUG-2 52** (`firma` DENTRO del cuerpo: párrafo denso en nombres de jueces «acéptanse las excusaciones…»/«asumido…los doctores…» disparando `linea_es_firma_de_juez` — **329_p3649, 339_p1371** · miss de dispositivo «Que por lo expuesto…se hace lugar» no detectado por `detectar_apertura_dispositivo` — **329_p2596**). **Mis-zonificación genuina = 101/179 = 56%.** El discriminador es wc + posición del tramo respecto del residuo, NO la forma de secuencia (el pase estructural inicial sobre-estimaba el FP; corregido por lectura en disco). → BUG-1/BUG-2 = **B148** (firma) + **B149** (dispositivo).
- **Bandera 1 (154) — es un detector de bleed de frontera de caso, no de zonificación.** Adjudicada por `diag_frontera_b1.py` v0.3 (disco): 51 carátula-suelta-1A + 27 fantasma-1B-fuerte + 19 fantasma-1B-ambiguo + 21 solape-1B' + 13 apéndice-1C-legítimo + 23 epílogo-previo-FP-de-bandera. → **B147** (bleed de frontera; el subconjunto fantasma = familia B012/B045 dimensionada). Testigos leídos: 329_p1480 (dos fallos en un `caso_id`: Tedesco+Albornoz), 346_p1564 (caso tragado como epilogo), 329_p2614 (carátula suelta), 339_p1530 (apéndice = 2ª resolución del mismo Club de Planeadores).

**Segunda parte (mejora, no bloqueo) — PENDIENTE:** lazy-load de `csjn_casos_textos.csv` por índice de offsets (hoy se cachea entero) — prerequisito de la eventual versión online. Sin urgencia mientras el uso sea local.
**Estado de verificación:** `confirmado_cuantificado` (154/179 en disco; cada clase con ≥1 testigo leído).
**Estado del fix:** N/A (M45 era diagnóstico; los fixes viven en B147/B148/B149 + refinamiento del explorador, unidades propias). Refinamiento del instrumento (baja prioridad): B1 debería separar clases por wc de cola + regla epílogo-previo (ya en `diag_frontera_b1`); B2 debería descontar la firma pegada a residuo + techo de wc.
**Referencias cruzadas:** M16 (cerrado H186). **B147/B148/B149 (findings del parser derivados).** B117/B141/M21 (familias candidatas confirmadas parcialmente). M43 (esta adjudicación = «madurar el pipeline» pre-κ, decisión H186). H186, H188. Sin ID histórico.

### B147 — Bleed de frontera de caso: la cola de un fallo absorbe carátula/fallo del siguiente (o pierde uno sin fila)

**Componente:** parser (`detectar_fin_real` ~2477-2650, upstream de `zonificar_bloque`; y catálogo/cruce para el subconjunto fantasma). Familia B012/B045.
**Origen / fuente del diagnóstico:** H188 (adjudicación de la bandera epílogo-no-terminal de M45).
**Causa raíz:** dos mecanismos distintos. (1) **Carátula-suelta:** las Pistas 3/4 de `detectar_fin_real` anclan en el sumario/DICTAMEN del caso siguiente y devuelven `k-1` (líneas 2585/2588/2599), pero la carátula del siguiente cae 1-2 líneas ANTES del sumario → queda dentro del bloque previo. No pierde datos (el siguiente tiene fila), pero contamina epilogo_text/partes del absorbente. (2) **Fantasma:** cuando el caso siguiente NO está en el catálogo (falta la fila), el rango previo lo absorbe entero; Pista 1 (2538-2566) matchea el token del *próximo caso catalogado* (el de más allá) y devuelve antes de que Pista 3-atrás corte en el sumario del fantasma — INFERIDO del orden del código, no demostrado con PoC. Es la familia «Hotesur real» (gap `348_p461` 18047-18076).
**Diagnóstico / evidencia:** universo bandera epílogo-no-terminal = 154. Partición corpus-wide en disco (`diag_frontera_b1.py` v0.3, discriminador = cobertura del rango absoluto de la cola por otros casos del volumen + wc de cola + señal apertura-en-cola): **51 carátula-suelta-1A** (cola wc≤15) · **27 fantasma-1B-fuerte** (apertura en cola + sin cobertura = fallo nuevo casi seguro) · **19 fantasma-1B-ambiguo** (estructura sin apertura: fantasma con apertura no detectada vs multi-resolución tipo 329_p1350) · **21 solape-1B'** (vecino con fila reclama la cola) · **13 apéndice-1C** (2ª resolución legítima del mismo caso) · **23 epílogo-previo-FP** (el bloque abre con la cola+epílogo del previo; la bandera midió mal). **Dimensión de la integridad: 27-46 fallos candidatos a ausentes del corpus (~0,5-0,8% de 5697), concentrados en tomos 329-331.** Testigos leídos: 329_p1480 (Tedesco absorbe Albornoz completo; Albornoz sin fila, vecino salta a 329_p1487), 346_p1564 (caso tragado como epilogo, fin de tomo), 329_p2614 (carátula Lacustre suelta; Lacustre tiene fila 329_p2620), 339_p1530 (apéndice cautelar mismo Club de Planeadores). Acople downstream verificado (procesar_archivo 3572-3610): un bloque con dos fallos mezcla `considerando`/`outcome`/`por_ello`/votos/partes en una fila.
**Estado de verificación:** `confirmado_cuantificado` (154 particionados en disco; 4 testigos leídos; el mecanismo fantasma es INFERIDO del código, falta PoC del orden de Pistas).
**Validador propuesto:** `scripts/diagnostico/H188/diag_frontera_b1.py` v0.3 (read-only, salida `diag_frontera_b1_out.csv` ordenada por clase). Adjudicación fase 2 = lectura del `.md` de los 27 fuertes + 19 ambiguos → separar fantasma-real de multi-resolución; el conteo de fantasmas confirmados decide si el frente upstream (recuperar filas al catálogo) se abre o se encola.
**Estado del fix:** **subconjunto FANTASMA: CERRADO H189** (adjudicación fase 2 completa + recuperación de los 4 ausentes en la misma sesión — ver B151 y «suplemento editorial»). **FRENTE (1A) carátula-suelta: CERRADO H190 (parser v26.1→27.1).** Mecanismo re-adjudicado sobre 8 testigos `.md` — el modelo de esta entrada era doble-erróneo: (i) en Pista 1 (37/51 del diag) la carátula del siguiente NO se absorbe (Pista 1 corta en carátula−1); lo que cuelga es el material PRE-carátula (rótulos temáticos, banners, apertura del dictamen del siguiente por orden OCR revuelto, strays); (ii) en Pistas 3/4 (13/51) el ancla es el DICTAMEN del siguiente y se absorbe carátula+rótulo+SUMARIO ENTERO (~12-14 líneas) — perfil causado por miss de Pista 1 (token corto «Lumi»<5, parser L2538; divergencia OCR índice/cuerpo «Hurting»/«HURTIG» 334_p405) → el fix es INDEPENDIENTE del token. Fix: `retroceder_frontera` (etapa A recorte de no-sustantivas + etapa B carátula-por-forma con prosa intermedia) sobre los fines de Pistas 1/3/4; guards adjudicados en disco (PoC v0.2→v0.4): firma-versales (`linea_es_firma_de_juez`) + header-voto (`RE_HEADER_VOTO_DISIDENCIA`) + nombre-propio (apéndices) + continuación-de-wrap (v27.1, post-FRENO: el fragmento final de firma wrapeada «LORENZETTI.»/«M. ARGIBAY.» no matchea `JUECES_CONOCIDOS` nombre+apellido — 34 casos, votos −42 en el consciente de v27.0, revertido por guard antes del regolden). **Corrección de cardinalidad: 1A real = 1866 recortes** (1197 solo-banner + 669 con contenido ajeno sustantivo) — la bandera-1 de H188 subestimó >10× (solo veía el bleed caído en zona epilogo) e incluía 3 FP verificados en `.md` (345_p683, 330_p2892, 330_p3126: frontera perfecta, carátula del siguiente en lfr+1). Validación: PoC 91/91 TP en etapa B (79 auto-match nombre del vecino), anclas 8/8, 0 fantasmas nuevos, ciclo consciente adjudicado (0 flips outcome/is_merit/gate), regresión propia post-regolden = 0 recortes corpus-wide. RESIDUALES: under-trim 345_p1219 (carátula ajena mixta wrapeada «N.N. s/…», B ancla en la 2ª línea, deja ≤1 línea — clase acotada, documentada) · conjueces completos siguen en B153. Frente restante de esta entrada: **(1C)** modelar apéndices en la gramática — 3 candidatos flageados por el detector `b_descarto_propio` del fix (339_p1648, 340_p1993, 345_p1138), disponible corpus-wide re-corriendo el PoC. **ADJUDICACIÓN FASE 2 (H189, lectura de los 46 candidatos extraídos por lote + cruce en disco contra catálogo/casos):** 46 → **43 FP + 3 fantasmas embebidos + 1 huérfano** (Alnavi, ni siquiera dentro de un bloque — detectado solo por el lado-índice B151). FP: **24 solape** (frontera extendida a pág compartida; cabeza del vecino con fila) · **13 bleed editorial** (los `fin_por_editorial`: la cola arrastra índice de partes + acordadas del cierre de volumen — 0 fallos) · **6 multi-resolución del mismo caso** (aclaratorias 332_p640/331_p1013/332_p663, incidentes 329_p4140/334_p913, votos conexos 329_p1066). **La cota «27-46 ausentes» de H188 se corrige a 4** (los masivos de cola gigante eran el índice del tomo, no tandas de fallos; las «firmas» del diag eran de acordadas). Los 4 ausentes, por causa: **Albornoz 329:1484 + Alnavi 329:2965 = pipeline** (ancla de índice degradada → B151, fix aplicado) · **Lombardi 333:1966 + Mercante 349:43 = editorial** (el índice oficial NO los lista — verificado contra índice fuente; recuperados vía suplemento editorial, `construir_catalogo` v1.03/v1.04). Corrección de constancia: el mecanismo fantasma NO era el orden de Pistas de `detectar_fin_real` — el borde inter-caso viene del catálogo (`linea_fin` argumento en `procesar_archivo` L3434; `siguiente_caso` = siguiente LOCALIZADO L3966-3979); el parser solo refina contra el siguiente localizado y nunca ve la carátula embebida. Frentes restantes de esta entrada: **(1C)** modelar apéndices en la gramática — sigue abierto y diferido (**(1A) CERRADO H190**, ver arriba; el fix registrado acá en H188 —retroceso `_parece_caratula` sobre k−1— resultó insuficiente para ambos perfiles y fue reemplazado por `retroceder_frontera`).
**Evidencia H194 (frontera engordada, sub-frente D):** la clasificación mecánica del D-87 (`poc_b148_clasificar_d` v0.2, scripts/diagnostico/H194/) aisló **30 casos FRONTERA_FPA** (firma+epílogo del previo en cabeza del bloque, ady_residuo 30/30; spot-checks leídos 330_p2478 y 331_p1596 — en este último lo absorbido es la firma de la DISIDENCIA de Argibay del previo) + 2 MIXTA graves leídas: **329_p3444 (Mendoza/Riachuelo): el bloque arranca ~46 líneas ANTES de la carátula, con DOS colas de casos previos adentro — prioritario por perfil del caso** · 329_p2179 (la disidencia ENTERA del previo dentro del bloque + FP nuevo header-de-voto-wrapeado → B160; sub-clase frontera-por-header-de-voto, hermana de la frontera-por-atribución H193). Además 331_p364 adjudicado en el residuo A/C. Lista completa: `poc_b148_d87_clasif.csv`.
**Cierre D2 (H201, parser v34.0 MAJOR):** espec = `poc_b147_d2_v02.py` VERBATIM (scripts/diagnostico/H201/; baseline sellado `poc_b147_d2_v02_baseline.csv`, regla `--out` propio estrenada). Diseño ADJUDICADO ≠ espejo literal de D1: **guard-con-fallback** — k_plain (primer match con guards viejos = ancla v33.0) + k_forma (no-firma incondicional ∧ `_es_caratula_v2`); ancla = k_forma ∨ k_plain; sin caída de pista inducida. El rechazo duro (v0.1) MUERTO con dato: el fall-through de Pista 1 NO es benigno como el de los tiers de D1 — amputa fallo propio (868/599 vía Pista 4 sobre el «FALLO» propio), voto entero (763), o epílogo (~12, clase →firma_actual); causa: carátulas genuinas sin forma en la línea del token (D1b-wrap / mixtas institucionales). Flip-set 50 (E0 5894/5894 + E1 0), 50/50 TP-o-mejora: 33 epílogo-propio recuperado + 16 prosa (330_p1135 testigo, 341_p2015 mid-dispositivo) + 1 versal (1301 +32); D1b-en-FIN = 3 (1762/6071 bleed cero vía retroceso, 1259 absorbe 1 línea mixta). Decisión 22/22 TP (1442 sumario_editorial→FALLO recuperado entero, espejo inverso de 715; 1847 = regresión de metadata con sucesor B154-bis, conjueces saga art. 110). Votos +16 · zonas +74 · blind byte-idéntico · manifest [CLEAN] 64. Residuales D2: catalogo-previos = **Pista 3** (2731/570/1784 por sumario_siguiente, 570/1784 solape +4/+11 — corrección de constancia al PROMPT_H201, unidad futura propia si se abre) · 330_p5251 (token wrapeado en prosa ajena, sin forma en ventana — el fallback conserva el bleed, con constancia) · 660 = gap-banner benigno (la sub-clase solape<0 del diag H199 mezcla clases — corrección de constancia). Regresión propia: `poc_b147_d2_v02 --esperar-version 34.0 --out propio` → E0 5894/5894 / E1 0 / flip-set VACÍO. **Cierre D1 (H200, parser v33.0 MAJOR):** espec = `poc_b147_d1_v03.py` VERBATIM (scripts/diagnostico/H200/; baseline sellado `poc_b147_d1_v03_baseline.csv` — la corrida de regresión pisa el CSV de salida, usar el baseline para diffear). Guards en tiers exact+prefix: `_es_caratula_v2` (caps-ratio ∨ conector « c/ »/« s/ » re.I fin-de-línea-ok / « V. » case-sensitive) + rechazo `linea_es_firma_de_juez`; fall-through a 5b/T4/vistos/catalogo SIN cambio. Flip-set 93 (E0/E0'' 5894/5894), perímetro completo 0-fuera-de-set en los 4 CSV, 2 flips de decisión TP (4815 otro→rechaza; 715 fallo→sumario_editorial), 2 regresiones de metadata aceptadas → **B165**. **D1b (nuevo, residual medido):** el ancla cae en la línea 2 de una carátula wrapeada y amputa la línea 1 — 6 casos conocidos (340_p1263 [línea 1 «Incidente n° 2 Denunciante:…» falla el discriminador], 329_p1767, 329_p1898, 329_p6072, 330_p1580, 330_p2584 [todos «V. …» = demandado en línea 2]); candidato: retroceso-de-1 con guards (caps ∧ no-firma ∧ no-banner), unidad propia con su flip-set. **Adjudicación H199 (sub-frente D, vista CABEZA) + plan D1-D5:** mecanismo adjudicado por lectura de 7 extractos `.md` (329_p3444, 329_p2179, 329_p4815, 329_p4811+cola80, 330_p1158, 330_p1135+cola20, 329_p2645) + código: el tier base de `refinar_inicio_por_titulo` toma el PRIMER match del token sin guards (falso-match-temprano: «Banco»/«Mendoza»/«Nuevo» matchean en la cola/epílogo/disidencia del previo); el mismo token rompe el FIN del previo vía Pista 1 (330_p1135 truncado a mitad de oración → sub-clase solape<0 = cola huérfana heredada); divergencia índice/cuerpo «Melchiori/MELCHORI» → `_ancla_catalogo` sin recorte. **Corrección de constancia: 329_p3444 = Melchiori c/ Santa Fe, NO Mendoza/Riachuelo** (el Mendoza real del pool = 330_p1158, adjudicado); su cabeza trae la cola de Ferrari 329_p3403, cuyo epílogo quedó fuera de su propio rango. 329_p4815 = interleave OCR (cuerpo del caso en gap de 64 líneas sin dueño; familia B152/329_p4140). Convergencia formal con **B018 comp. 1+2** (resucitada, ver su entrada). Instrumento: `poc_b147_d_cabeza` v0.1 (H199/), **E0 5894/5894**, flip-set candidato 105 (∩D 19/32), muestra seed-199 adjudicada: los 13 D-sin-flip = firma-versales-como-forma («Santiago»/«Juan» = nombres de jueces, FP clase H190-v0.2) + 4 Boggiano homónimos + carátulas mixtas que el detector de forma no ve; ~4/10 flips nuevos = FP del instrumento (ancla en carátula REAL anonimizada, k_forma = rubro caps → un avance ingenuo amputaría la carátula); clase 962 TITULO_PROSA = benigna (constancia: `_parece_caratula` no es discriminador de carátula); 37 CAT_FORMA_SIN_TOKEN = clase Melchiori dimensionada. **PLAN (una unidad = un flip-set = un ciclo):** **D1 (H200)** — fix CABEZA en `refinar_inicio_por_titulo`: discriminador carátula v2 (caps ∨ patrón conector «c/»/«s/»/«V.» para mixtas/anonimizadas) + guard firma-versales (reuso `linea_es_firma_de_juez`, calcado H190) + regla primer-match-CON-forma (nunca saltar carátula válida hacia un rubro); PoC v0.2 con guards → flip-set sellado → ciclo consciente (MAJOR: mueve linea_inicio→bloque→zonas/wc); los 37 sin-token entran SOLO si la adjudicación banca carátula-por-forma-sin-token (mismo riesgo FP que obligó los guards H190). **D2** — vista FIN: mismo discriminador en el es_caratula de Pista 1 (B018 comp. 2); repara previos truncados (330_p1135). **D3** — B018 comp. 1: ampliar exclusión de `primer_token_de_caratula` con el dato del PoC (Banco/Luis/Aires/Cámara/Sala/Recurso/Autos/Civil…), con la validación advertida en H068. **D4** — vecinos homónimos (Boggiano ×4, sagas): el token no discrimina propia/hermana; diseño abierto, el residuo post-D1/D2 decide. **D5** — interleave (4815, 4140): data-quality familia B152, fuera de frontera-fina; el cuerpo huérfano de 4815 documentado.
**Referencias cruzadas:** M45 (origen), B012/B045 (familia, fantasmas — cardinalidad RESUELTA), B151 (fix del subconjunto pipeline), B152 (Figueiredo, colisión de página destapada en la adjudicación), «Hotesur real» (pendiente H181/PROMPT — candidato al suplemento editorial). H188/H189. Sin ID histórico.

### B148 — Detector de firma sobre-extiende: cuerpo/dictamen/cabeza denso en nombres etiquetado como `firma` — CERRADO H193

**Componente:** parser (`linea_es_firma_de_juez`, ancla firma_linea de Pasada 1 de `zonificar_bloque` ~2872; + Pasada 3 residuo ~2939-2956 para la variante cabeza-propia).
**Origen / fuente del diagnóstico:** H188 (adjudicación de la bandera firma-antes-de-dispositivo de M45, sub-clases BUG-1 49 + parte de BUG-2).
**Causa raíz:** `linea_es_firma_de_juez` dispara sobre texto de cuerpo denso en nombres de ministros — párrafos «acéptanse las excusaciones de los doctores Petracchi, Fayt, Maqueda…» / «en razón de haber asumido…los doctores Rosatti y Rosenkrantz». Ya existe el guard `_en_sumario` (B076, 2836-2876) que suprime firma espuria DENTRO de sumario; falta el guard análogo para cuerpo/dictamen. Variante (BUG-1): cuando la carátula+sumario del propio caso no dispara ancla al arranque, la Pasada 3 lo marca `residuo_caso_anterior` incondicionalmente (2951-2956) y la línea de atribución de disidencia queda `firma`.
**Diagnóstico / evidencia:** 49 (BUG-1, cabeza-propia) + ~parte de 52 (BUG-2, cuerpo). Discriminador wc: un tramo `firma` con wc>200 antes del primer dispositivo es cuerpo mal etiquetado (una firma real tiene ~5-20 palabras). Testigos leídos: **333_p385** (dictamen entero de la Procuración ~1100 palabras en 4 segmentos `firma` de wc 70/402/426/235; el caso NO tiene zona `dictamen` — todo absorbido), **334_p1461** (carátula+sumario «ANDREU c/ INTEGRA» como `residuo`, atribución de disidencia como `firma`), **329_p3649**/**339_p1371** (excusaciones/asunción de jueces como `firma`, wc 43/81/209). Acople: la zona `firma` entra a `_ZONAS_FALLO` (visible a votos M09); `lineas_residuo` (2939) SÍ borra del `considerando_text` y del wc (3583/3607) — la variante cabeza-propia pierde considerando real. Severidad del ripple a `outcome` = TBD (medir por ciclo `--consciente`).
**Estado de verificación:** `confirmado_caso_testigo` (4 testigos leídos; cardinalidad 49+ estimada por wc, no toda leída).
**Validador propuesto:** PoC de flip-set sobre `zonas.csv` — medir tramos `firma` wc>200 pre-dispositivo corpus-wide; adjudicar por lectura una muestra por sub-clase (excusación / asunción-de-jueces / cabeza-propia). El fix (guard en `linea_es_firma_de_juez` o post-guard de zona) NO admite PoC read-only sobre outputs: cambia bordes de zona → ciclo `--consciente` corpus-wide con adjudicación del diff (camino H126/B117).
**Estado del fix:** **APLICADO H193 (ver «Fix aplicado» abajo).** Registro del diseño H192: Diagnóstico H192 (sesión de lectura + medición, pipeline INTOCADO en este frente): `linea_es_firma_de_juez` L1842-1876 leída ENTERA (el agujero motor es `es_corta ≤80`: el OCR wrapea el cuerpo a ~70-85 chars) + Pasadas 1-3 de `zonificar_bloque` (supresión SOLO en sumario L3019; el ancla tiñe hasta la próxima ancla L3078). **Dos causas raíz exactas verificadas en código:** (1) `RE_HEADER_VOTO_DISIDENCIA` L1832 exige `^\s*(disidencia|voto)\b` y las atribuciones editoriales traen guión/paréntesis adelante («-Disidencia del juez X-», «(Voto del Dr. X)») o cola parentética → el guard #2 del detector no dispara; (2) `linea_es_header_sumario` L1806 exige «.»/«:» que las voces caps del sumario no tienen → `_en_sumario` (B076) INERTE donde más se necesita. Pool medido (`poc_b148_cardinalidad` v0.1, post-B154): **217 casos / 398 tramos** firma-pre-primer-dispositivo (A>200wc 48 ≈ los 49 BUG-1; B 21; C 18; D 130 dominado por FP-A M45); LIMITACIÓN: la vista pre-primer-dispositivo subcuenta multi-voto (346_p1564/348_p841 fuera) → el PoC del fix necesita doble vista (+ tramos wc>200 en cualquier posición). Adjudicación por lectura 34 casos / 99 tramos (`dump_b148_tramos.md`): **15 SUMARIO-atribución** (clase dominante) + **9 CUERPO-denso** (excusaciones/recusaciones/cesaciones/designaciones/enumeraciones/citas; incluye los 5 FP-B154) + 5 no-B148 (frontera/FP-A + B149). **firma_raw mayormente DESACOPLADO del daño de zona** (5/6 verificado: fin_real ancla en la firma final real); excepción 346_p965 (firma_raw arranca en el dispositivo — el fix A también lo repara). **Fix A** (causa raíz, en el detector): (i) guard nuevo `RE_ATRIBUCION_SUMARIO` DENTRO de `linea_es_firma_de_juez` — NO ensanchar RE_HEADER_VOTO_DISIDENCIA, tiene segundo consumidor en frontera L2488 sin adjudicar; (ii) predicado residuo-forma-de-nombre para la clase cuerpo (residuo prosa-minúscula → no firma; residuo capitalizado → firma — PELIGRO: no re-romper firmas de conjueces desconocidos, clase B153/v27.1). **Opción B (pasada correctiva por umbral wc) DESCARTADA como parche — no encolada; el residuo post-A decide.** Radio (call-sites grep H192): L1990/2005 (A001) · L2029 (header_sumario_guardado usa el detector como EXCLUSIÓN — achicar el detector puede agrandar sumarios, adjudicar) · L2488 · L2595/2705/2776 (fin_real/extender/retroceder) · L3019 · L3329 · L3412. Contrato H193: textos.csv VA a cambiar → candado blind a re-verificar (build_m20 + git status).
**Fix aplicado (H193, parser v29.0):** tres guards de consumo LOCAL en `linea_es_firma_de_juez` (RE_HEADER_VOTO_DISIDENCIA y RE_CALIFICADOR intactas): (i) `RE_ATRIBUCION_SUMARIO` — prefijo «[-–—(] Disidencia(s)/Voto(s)» + cola «(Disidencia(s)|Voto(s)) [parcial/conjunta] de»; (ii) `_firma_nucleo` — strip de paréntesis balanceados SIN juez + corte de fragmentos parentéticos wrapeados (prefijo hasta «)» colgante / sufijo desde «(» colgante), con la regla juez-debe-sobrevivir-en-el-núcleo; (iii) `_firma_residuo_es_prosa` sobre el núcleo. La espec NO es la de H192 tal cual: la iteración del PoC en disco (v0.1→v0.3) destapó y cerró dos clases de FP que el diseño H192 habría producido (521 firmas con calificador partido por wrap + 71 continuaciones «cia)—…» + 5 calificadores fuera de catálogo «mi voto»/«fundamentos»/soft-hyphen — clase FRENO v27.0). Validación: flip-set sellado 3799/1303 adjudicado por clases; regresión propia 0 kills / viejo=True −3799 exacto; ciclo consciente 100% TP (contrato 0 flips decisión, votos 27818 identidad intacta, zonas 552/552 ⊆ flipset); blind [CLEAN]; sello [CLEAN] 64. Bonus: 2 fronteras reparadas de rebote (337_p505, 340_p1070 — la atribución del caso siguiente era ancla falsa de fin_real). CORRECCIÓN de la expectativa H192: firma_raw 346_p965 NO se reparó (mecanismo del colector, la semilla no menciona juez del set → B149-familia); sus zonas sí. Residuo del pool 217/398→101/125; lo no-adjudicado del residuo (5 A + 3 C) diseña la unidad siguiente. **Residuo ADJUDICADO H194 (unidad completa):** los 7 A/C leídos = 4 FP-detector reales (sub-clases nuevas → **B160**: recusados-wrapeados 348_p1751/339_p270 · carátula-con-juez 339_p323 Boggiano · excusación 329_p4446) + 2 FP-del-pool (329_p4140 = fallo de 2004 transcripto en nota al pie «(*) Dicha sentencia dice así», interleaved por páginas, zonas del caso mezclan DOS fallos — constancia data-quality familia B152; 331_p364 = frontera B147/M45) + 1 B149 (331_p2363, CERRADO H194); D-87 clasificado + muestra 15/15 leída (ver B149/B147/B160); **346_p1564 y 348_p841 YA reparados** (tramos firma wc 8/13 en v29.0 — la vista-1 del PoC ocultaba residuo CERO); vista 2 nueva: 46 casos firma wc>200 SOLO-post-dispositivo sin adjudicar (`poc_b148_pool_v2.csv`) + conciliación 148-vs-142 con la bandera del explorador PENDIENTE (leer su métrica). Observación 2ª vez sin adjudicar: cabeceras de recusación (carátula+rubros+sumarios) zonificadas como `residuo_caso_anterior` en 339_p270/348_p1751.
**Referencias cruzadas:** M45 (origen), B076 (guard precedente), B117 (familia zona-que-come-cuerpo), M44 (vista normalizada de Pasada 1), B149 (hermano, destape 330_p3801), B153/B154 (los 5 FP del ciclo H192 = testigos). H188, H192. Sin ID histórico.

### B149 — Detector de dispositivo se pierde «Que por lo expuesto… se hace lugar/resuelve» — CERRADO H194 (fix A, parser v30.0)

**Componente:** parser (`detectar_apertura_dispositivo`, ancla dispositivo de Pasada 1 de `zonificar_bloque` ~2866).
**Origen / fuente del diagnóstico:** H188 (adjudicación de la bandera firma-antes-de-dispositivo de M45, sub-clase de BUG-2).
**Causa raíz:** el dispositivo de la mayoría que abre «Que por lo expuesto, oído el señor Procurador General, se hace lugar…» no matchea `detectar_apertura_dispositivo` (que espera «Por ello,» u otros performativos) → queda `cuerpo`. El PRIMER dispositivo detectado pasa a ser el de una disidencia/voto posterior que sí abre con «Por ello,», así que la firma de la mayoría queda antes del primer dispositivo (por eso dispara la bandera).
**Diagnóstico / evidencia:** subconjunto de BUG-2 (52). Testigo leído: **329_p2596** (Del'Olio — mayoría «Que por lo expuesto, oído el señor Procurador General, se hace lugar a la queja…» etiquetada `cuerpo`; primer dispositivo detectado = disidencia Argibay «Por ello… se desestima»). NB: en `zonas.csv` esto afecta la etiqueta `zona=dispositivo`, PERO el `outcome`/`por_ello` los decide `resolver_dispositivo`/`_barrer` (3602) por su cuenta sobre `lineas_dictamen`, NO sobre la zona `dispositivo` → severidad probablemente acotada a `zonas.csv`/`wc_*`; el ripple a `outcome` es TBD (medir).
**Estado de verificación:** `confirmado_caso_testigo` (2 testigos leídos; cardinalidad no aislada del resto de BUG-2). **Evidencia nueva H192:** 330_p3801 — firma REAL de mayoría (con calificadores) etiquetada pre-dispositivo en el pool B148 porque el dispositivo de su voto no fue detectado; adjudicada como FP-del-pool-B148 / TP-de-B149 en `dump_b148_tramos.md`. **Evidencia nueva H193:** 346_p965 — firma_raw sigue envenenado POST-fix-B148 (arranca en el dispositivo «1°) Declarar que la Dra. Figueroa cesó…»): la línea semilla no menciona juez del set, o sea el veneno NO era del detector (expectativa H192 corregida) sino del camino del colector/dispositivo → leer el camino de `collect_firma_lines`/`detectar_apertura_dispositivo` para este caso al abrir B149. Además el pool post-B148 (101/125) retiene 330_p3801 y 329_p2596 como testigos vivos.
**Validador propuesto:** grep corpus-wide de «Que por lo expuesto» / «Que en atención» + performativos sin «Por ello» como arranque de párrafo pos-considerando; adjudicar cardinalidad. Fix = ensanche de `detectar_apertura_dispositivo` (o su regex fuente). Cuidado con no pisar el eje de `outcome`: verificar que el detector de zona y `_barrer` no diverjan tras el ensanche.
**Estado del fix:** **APLICADO H194 (ver «Fix aplicado» abajo).** Registro del plan original: acotado a `detectar_apertura_dispositivo`, sin tocar la máquina de estados de Pasada 2; MAJOR (mueve borde de zona). El diseño final resultó DOBLE (el ensanche de anclas era solo la mitad del bug — la otra mitad era arquitectural).
**Fix aplicado (H194, parser v29.0→30.0, MAJOR):** causa raíz LEÍDA (GATE del PROMPT cumplido: `detectar_apertura_dispositivo` L122-148, `collect_firma_lines` L974-1030, `resolver_dispositivo` L3524-3584 con la cascada T1→2→3→3b→4, call-sites del zonificador L3109/3364/3388, secuencia de `procesar_archivo` L3729→3849→4027): **el resolutor de outcome ya conocía T2 mid-línea y T4 «Así/el Tribunal resuelve», pero el zonificador (Pasada 1) anclaba dispositivo SOLO con Tier 1** — por eso outcome resuelto con zona ausente. Espec = `poc_b149_anclas` v0.1 VERBATIM (scripts/diagnostico/H194/; importa el parser real, réplica de la secuencia L3729→3849 con candado E0 por caso = re-zonificación == zonas publicadas, **5703/5703, 0 mismatch**). DOS piezas: **(A1) herencia del ancla** — en `procesar_archivo`, post-resolver/pre-`extraer_segmentos`: si `por_ello_idx≠None ∧ zona[idx]=='cuerpo'`, relabel cuerpo→dispositivo desde el idx hasta la primera zona de cierre (firma/voto_separado/epilogo). Flip-set SELLADO 25/25 TP (cierre en firma 25/25, rangos 1-11 líneas; incluye el testigo histórico **329_p2596** Del'Olio); guard de alcance excluye los 7 zona=dictamen (dictamen-sin-cierre, rangos 111-1213 líneas → **B159**); consumidores previos de `_zonas_linea` inmunes POR CONSTRUCCIÓN (lineas_dictamen/residuo no tocan cuerpo; `_ZONAS_FALLO` contiene cuerpo Y dispositivo). **(A2) `RE_DISPOSITIVO_VARIANTES_PERF`** — 2 variantes T1 condicionales-a-performativo (guard `RE_PERF` fuente única H130, sobre cola de línea + wrap; `detectar_apertura_dispositivo` +1 arg opcional `cola_wrap`, peek SOLO en el zonificador — en el resolutor el performativo debe estar en la misma línea, los wrapeados recuperan ZONA y no outcome, residual documentado): `en_virtud_perf` = **B067 RESUCITADA** (BITACORA:4450 la había pospuesto sobre 5 hits de UN tomo, 60% FP; corpus-wide 268 matches / 204 en dictamen / perf-gated 16 = **16/16 TP** adjudicados, 8 son dispositivos de VOTO que A1 no alcanza, testigo original **348_p443 recuperado** — sin_dispositivo→rechaza) + `que_de_conf_perf` (análogo `que_por_ello`; 29 matches / perf-gated 1 = 344_p776, sin_dispositivo→competencia). **«Por lo tanto» DESCARTADA con constancia** (patrón RE_ART_117_CN: 640 superficie / H039 «70% argumental» confirmado a escala / rinde 2-3 / borde 341_p250 «se declara inconstitucional…» con firma a 50 líneas, indecidible sin extracción; residuales documentados 343_p720, 346_p1068). Guarda-dictamen H052 verificada en sandbox como red extra (variante perf DENTRO de dictamen no ancla). Validación: ciclo consciente adjudicado **100% dentro del contrato** — casos/textos/votos 3/3 TP (341_p739 otro + is_merit 0→1 vía gate «se modifica la sentencia apelada»; sin_dispositivo 27→24; is_merit 2967→2968), **0 re-picks** materializados, zonas 41/41 = 25 A1 + 16 A2 con diff ALINEADO por caso_id (`diff_zonas_b149.py` v0.1: +38 filas exactas / 36×+1 + 5×+2 = +46 tramos dispositivo / 0 fuera de contrato / 0 formas raras / 342_p1393 redundante-inocuo ausente como predicho); candado blind **[CLEAN]** (build_m20 + git limpio, ninguno de los 3 en la clave n300); regolden + manifest **[CLEAN] 64**. Regresión propia post-fix: `poc_b149_anclas --esperar-version 30.0` → e0_mismatch=25 EXACTOS (== flip-set A1: la réplica corta en L3849, pre-relabel — esperado, no FRENO) + A1 flip-set=7 (solo dictamen/B159) + matches 937→936 (el −1 = un match no-perf de «Por lo tanto» dentro de un caso A1 excluido por E0, contabilizado). Baselines conservados: `poc_b149_a1_flipset_v29_baseline.csv` (32) / `_postfix.csv` (7) · `poc_b149_a2_anclas_v29_baseline.csv` (937) / `_postfix.csv` (936); NB: `diff_zonas_b149.py` default apunta al nombre viejo → re-corridas con `--a1 …_v29_baseline.csv`. RESIDUALES → B159 (7 dictamen) · **B160** (colector: los breaks de `collect_firma_lines` no incluyen «Abogado:»/«Tribunal interviniente:» — leído H194, repara la expectativa 346_p965) · ~13 MEDIO-multivoto (dispositivo per-voto con fórmula no cubierta, p.ej. voto Rosatti 344_p1102 — lista en `poc_b148_d87_clasif.csv`) · «Por lo tanto» con constancia arriba.
**Referencias cruzadas:** M45 (origen), B148 (hermano, misma bandera), B124/B130 (regla P de `_barrer`, eje outcome separado). H188. Sin ID histórico.

### B151 — Ancla del índice de nombres degradada por OCR: entradas pegadas → fallos fantasma + polución de `nombres_indice` — CERRADO H189

**Componente:** catálogo (`construir_catalogo.py`, `RE_ANCLA` / `parsear_indice_nombres`).
**Origen / fuente del diagnóstico:** H189 (adjudicación B147 fase 2 → cruce en disco: «Albornoz» solo aparecía como línea de índice pegada al campo `nombres_indice` de otras filas).
**Causa raíz (leída en código):** `RE_ANCLA` exige `':' + 'p.'` juntos; el OCR degrada de a un token — variante A `': 1484.'` (sin «p.»), variante B `'p. 748.'` (sin «:»). `parsear_indice_nombres` joinea el bloque y corta por anclas: la entrada sin ancla queda pegada como prefijo de la carátula siguiente y su página muere. Si TODAS las entradas de una página se degradan → sin fila de catálogo → sin localización → el caso anterior se estira (fin_extendido) → fallo fantasma. Si sobrevive otra entrada → solo polución de nombre (la B150 provisoria de la sesión = esta faceta; misma raíz, misma población, cerrada por el mismo fix).
**Diagnóstico / evidencia:** PoC en disco (`scripts/diagnostico/H189/poc_b151_ancla_degradada.py` v0.1, read-only, 2 pasadas: estricta real + rescate sobre carátulas): **10 rescates / 14.406 entradas (0,07%)** = 2 páginas GHOST (**329:1484 Albornoz** ×4 entradas, **329:2965 Alnavi** ×2 — ambos fallos completos verificados por lectura; Alnavi era huérfano total: cabecera de 329.3 antes de la primera localización, invisible al lado-cuerpo) + 2 POLUCION (329:1385 Rizzo, 330:748). El detector lado-índice es la cuenta autoritativa de drops (diseño H109 L546, implementado acá); complementario del lado-cuerpo (bandera 1 / B147).
**Fix aplicado (H189):** `construir_catalogo` v1.01→**v1.02**: `RE_ANCLA_DEG` en 2ª pasada SOLO sobre carátulas de la estricta (RE_ANCLA intacta); guards: 1-4 dígitos, punto final obligatorio, página en rango del bloque, carátula ≥8 chars con letras, invariante «una estricta nunca se pierde». Sandbox A/B: 0 páginas perdidas, controles byte-idénticos. **Ciclo consciente sellado:** catálogo 6145→6147 · N 5890→**5892** (+329_p1484 desestima/5, +329_p2965 nulidad_concesion/5) · Tedesco 329_p1480 descontaminado (wc_votos 805→137, tipo_voto indeterminado→B) · **reparación colateral de fronteras por token**: Bergés 329_p2218 panel 1→6 (la Pista 1 buscaba el token espurio «Catering»), Chabán-casación 329_p2221 quimera resuelta (outcome procedente→**desestima**: el `procedente` viejo era el dispositivo de Figueiredo mal atribuido, adjudicado contra el lote). check_regresion [FAIL] consciente adjudicado → re-golden → manifest [CLEAN].
**Referencias cruzadas:** B147 (origen), B012/B045 (familia), B115 (precedente exacto: entrada de índice perdida → rango estirado), B152 (Figueiredo, destapado en la adjudicación del flip 2221). H189.

### M46 — Suplemento editorial del catálogo: fe de erratas curada para fallos que el índice oficial omite — IMPLEMENTADO H189

**Componente:** catálogo (`construir_catalogo.py` v1.03/**v1.04** + `_meta/catalogo_suplemento_editorial.csv`).
**Motivación:** Lombardi 333:1966 y Mercante 349:43 son fallos completos que el índice oficial del tomo NO lista (omisión de la Secretaría de Jurisprudencia, verificada contra el índice fuente — el índice existe y no los tiene). Sin entrada no hay rescate posible por regex; un fallback heurístico que escanee el cuerpo comería citas a fallos (descartado por diseño).
**Mecanismo:** CSV curado como DATO (tomo, pagina, caratula, fuente_verificacion), cada fila adjudicada por lectura con procedencia; `cargar_suplemento_editorial` lo mergea al agrupado (tomo, página) del flujo normal → pagina_fin por inferencia global, cruce/parser sin cambios, idempotente. v1.04: path robusto anclado al script (el default CWD-relative de v1.03 fallaba SILENCIOSO desde la raíz del repo — dos ciclos no-op hasta detectarlo por la ausencia de la línea de aviso; ahora avisa SIEMPRE, cargado-con-conteo o «no encontrado»).
**Ciclo sellado (H189):** catálogo 6147→6149 · N 5892→**5894** · +333_p1966 Lombardi (confirma extradición, según_su_voto 7: mayoría 4 + voto Highton/Maqueda/Argibay, dictamen González Warcalde) · +349_p43 Mercante (desestima 280, disidencia Rosatti-Lozano) · **Parques 333_p1959 y Décima 349_p40 descontaminados de rebote** (el residuo 22% de Décima = F002 queda REPARADO; Parques recupera su propio Vistos «Misiones»). Handoff de rangos verificado sin solape ni agujero.
**Candidatos futuros:** el gap 348:18047-18076 («Hotesur real», constancia H181) — si la lectura lo confirma, va al suplemento como fila nueva.
**Referencias cruzadas:** B147 (ausentes editoriales), F002 (BITACORA — cerrado de rebote), B153 (Lozano destapado en Mercante). H189.

### B152 — Colisión de página: dos fallos que arrancan en la misma página colapsan en una fila (Figueiredo c/ Malagoli)

**Componente:** catálogo/schema (granularidad `caso_id = tomo_pPAGINA`).
**Origen / fuente del diagnóstico:** H189 (adjudicación del flip de 329_p2221 en el ciclo B151).
**Causa raíz:** el catálogo agrupa por (tomo, página): si dos fallos arrancan en la misma página (Chabán-casación y Figueiredo c/ Malagoli, ambos en 329:2221), sus entradas de índice caen en UNA fila y solo uno gana el `caso_id`. Pre-existente e invisible mientras la fila era quimera; el fix B151 la limpió a Chabán y dejó a Figueiredo visible como ausente (queja procedente c/ suspensión, Sala E Civil, firma 5 — leído en el lote H189, líneas 2930-2985).
**Estado de verificación:** `confirmado_caso_testigo` (1 testigo; cardinalidad corpus-wide no medida — detector candidato: páginas del índice con >1 carátula no-cruzada).
**Estado del fix:** no diseñado. Requiere decisión de schema (ids sub-página `2221a/2221b` o equivalente) → toca caso_id_canonico corpus-wide = decisión de fondo, NO un parche. Encolado.
**Referencias cruzadas:** B151 (destape), B012 (familia ausentes). H189.

### B153 — Conjueces no reconocidos en la firma: paneles subcontados — CERRADO H191

**Componente:** parser (lista de ministros de `linea_es_firma_de_juez` / extracción de votos).
**Origen / fuente del diagnóstico:** H189 (349_p43 Mercante: panel real con conjuez **Lozano** → fila `mixed, 2`; ídem **Rodríguez Basavilbaso** ya visible en la firma de Tedesco 329_p1480).
**Causa raíz:** la detección de firma/votos reconoce el elenco de ministros; los conjueces (desinsaculados, frecuentes en recusaciones/excusaciones y en la Corte 2025-2026 reducida) no están en la lista → sus líneas de firma no anclan → panel subcontado y `posicion` incompleta.
**Estado de verificación:** `confirmado_caso_testigo` (3 testigos: Lozano, Rodríguez Basavilbaso, y **329_p4506 «LORENZETTI — EMILIO L. FERNÁNDEZ.» destapado en H190** por el flip-set del PoC B147-1A). Cardinalidad no medida — detector candidato: `n_jueces` < mínimo del período + «conjuez» en el texto de firma.
**Fix aplicado (H191, parser v27.1→28.0 MAJOR):** opción dato-curado (patrón M46): `JUECES_CONOCIDOS` externalizado a `_meta/jueces/jueces_csjn.csv` (74 entradas = 56 migradas verbatim [2 con ensanche: Otero `(?:luis\\s+)?`, Leal de Ibarra `(?:mar[íi]al?\\s+)?`] + 18 nuevas [titulares históricos Moliné O'Connor/Bossert — fallos 1998-2003 publicados tardíamente — + 16 conjueces]), loader `_cargar_jueces_conocidos` fail-fast. Columna `jueces_desconocidos` re-conectada en el mismo ciclo (estaba muerta por construcción). Validación: PoC `poc_b153_conjueces` v0.1 con triple candado (E0 0 diffs/5703 · E1 · E2), sandbox + disco; flip-set 35 casos / +48 jueces / 0 pérdidas / 0 FP fuera del pool; ciclo consciente con contrato verificado retroactivo (outcome/is_merit/is_originaria 0 flips; voting_pattern 2 TP; is_full_bench 2 adjudicados: 329_p4178 corrige FP, 330_p224 FP aceptado → B156); ripple de frontera adjudicado TP (collect_firma_lines extiende sobre firmas ahora reconocidas: 1303 nj 1→5, 1305 2→6, votos +61). Corrección de constancia: 349_p43 (Lozano) NO era testigo válido — panel en disco Rosatti|Lorenzetti; los otros 2 testigos estaban parcialmente resueltos pre-H191 (H189/v27.1). Residuales → B154 (Müller/Leal-«M.»/Aranguren), B155 (bleed), B158 (truncamiento). Escala: conjueces de tomos futuros = fila nueva en el CSV + mini-ciclo; el detector permanente es la columna re-conectada. Opciones descartadas en el diseño original: patrón posicional (no evaluado; innecesario con dato curado). **Reparación parcial de rebote (H190):** el guard continuación-de-wrap de parser v27.1 protege el FRAGMENTO wrapeado de estas firmas del retroceso de frontera (329_p4506 ya no pierde la línea), pero el reconocimiento del conjuez para votos/panel sigue abierto. Encolado; relevante para el eje de votos de la tesis (H5).
**Referencias cruzadas:** M46 (destape), B025/B019 (familia firma), B147-1A/H190 (329_p4506). H189.


### B154 — Conjueces/variantes residuales destapados por el ciclo H191 (Müller, Leal-«M.», candidatos del top de desconocidos) — CERRADO H192

**Componente:** dato (`_meta/jueces/jueces_csjn.csv`) + mini-ciclo parser.
**Fix aplicado (H192, ciclo de DATO — parser v28.0 sin cambio de código):** tabla 74→87 entradas: +13 conjueces (Müller [`m[üu]ller`, la Ü rompía el token del colector], Aranguren B.E. [alternancia por orden INVERTIDO en 348_p563], Irurzun, Lozano [el conjuez de H189/Mercante, visible recién post-ripple v28.0], Boldú [`bold[úu]`], Sánchez A.G., Barreiro, Sotelo de Andreau, Bejas, Candisano Mera [invertido 348_p716/755], Llorens [invertido 348_p569], Moltini [invertida ×3], González R.L.) + 2 ensanches (Leal `(?:mar[íi]al?\s+|m\.\s+)?`; Tazza `alejandr?o` por OCR «Alejando» 348_p1357). Adjudicación por lectura de TODOS los contextos (3 iteraciones hasta converger; la bola de nieve destapó el cluster Tabacalera Sarandí t.348 y los paneles conjueces t.349); descartes con constancia: Juan José Aranguren (ministro PEN, bleed, persona distinta) y Barreiro Deymonnaz (letrado, bleed). 331_p1397 adjudicado por `.md` extraído: Irurzun SÍ firmó (panel real 7), pero el ancla de firma es errónea (POR ELLO capturado) → testigo VIVO de B148. Validación: flip-set 24 casos / +45 jueces / 0 pérdidas / 0 full_bench / 0 fuera del pool; E0 del PoC B153 = 24 diffs pre-regolden y 0 post; contrato con diff alineado por caso_id: 1 flip TP (344_p1557 voting_pattern → mixed). Ripple zonas +10 = 2 TP + 5 FP clase-B148 aceptados con constancia (daño canónico cero). Golden re-congelado, manifest [CLEAN] 64, blind intacto por construcción (textos byte-idéntico). Residual cosmético (d) Tyden-wrap sigue (es del colector de desconocidos, no del reconocimiento). «Guillermo Antelo» nombrado conjuez suplente en 348_p841 SIN firma vista → constancia, NO fila.
**Origen / fuente del diagnóstico:** H191 (ciclo consciente de B153: la firma recuperada por el ripple de frontera expone nombres que el PoC —corrido sobre el firma_raw viejo— no podía ver).
**Causa raíz:** (a) **Müller, Carlos Antonio** (variantes MULLER/MÜLLER) firma en 329_p1303/1305/3221/3235 y no está en la tabla → esos paneles siguen subcontados (1303: cuenta 5, panel real 7). (b) **Leal de Ibarra «JAVIER M. LEAL DE IBARRA»**: la inicial «M.» no entra en el ensanche `mar[íi]al?` (en 3221/3235 «MARÍA» completo sí matchea) → miss en 1303/1305. (c) Candidatos del top de `jueces_desconocidos` (columna re-conectada): **Beatriz Estela Aranguren (4)** — adjudicar conjuez vs bleed; **Martín Irurzun (3)** — huele a cita/bleed, adjudicar. (d) Cosmético: «MIRTA D. TYDEN DE» (4) = fragmento de wrap de conjuez YA conocida; miss del filtro `nombres_jueces` del colector (chequea substring del nombre completo), no del reconocimiento.
**Estado de verificación:** `confirmado_caso_testigo` (a y b leídos en el diff del consciente H191; c sin adjudicar).
**Validador propuesto:** re-correr `poc_b153_conjueces.py` post-fila-nueva; el flip-set esperado son exactamente los casos testigo.
**Estado del fix:** diseñado (filas nuevas en el CSV + mini-ciclo consciente/regolden). Barato por construcción post-B153.
**Referencias cruzadas:** B153 (CERRADO H191), B155. H191.


**Constancia H201 (evidencia nueva, ciclo de dato candidato «B154-bis»):** el ciclo D2 destapó un CLUSTER NUEVO de conjueces fuera de tabla — 342_p1847 = CSJ 7/2007 (43-A) «Álvarez, Gladys Stella y otros c/ EN – CSJN – Consejo de la Magistratura – art. 110» (5/11/2019), saga art. 110/intangibilidad resuelta por TRIBUNAL DE CONJUECES ÍNTEGRO (titulares excusados): Julio César Rivera, Diego Botana, Verónica Nidia Torres, María Rosa Caballero (por su voto), Alberto Manuel García Lema (en disidencia) — panel real 5 / voting mixed. ADJUDICADO CONTRA EL PDF OFICIAL (leído H201; corrección de la adjudicación intra-sesión que lo había mandado a B155: García Lema es CONJUEZ, no letrado). El parser v34.0 publica n_jueces=1/unanime (García Lema vía colector de desconocidos — la columna `jueces_desconocidos` re-conectada en H191 detectando como se diseñó); golden previo decía 0/sin_firma: ambos incorrectos → regresión de metadata aceptada con constancia y sucesor (patrón H178/B165). RINDE POTENCIAL: el propio fallo registra 54 causas gemelas de la saga (592 accionantes) ante el mismo tribunal de conjueces → puede haber más paneles idénticos en t.342+ esperando la tabla. Sucesor: ciclo de dato patrón B153/B154 — sumar los 5 a `_meta/jueces/jueces_csjn.csv` + leer el `.md` de 1847 para adjudicar por qué la firma de mayoría (Rivera/Botana/Torres/Caballero) no entró al conteo + grep de la saga en el corpus.

**B154-bis CERRADO H202 (ciclo de dato, tabla 87→91, parser v34.0 sin cambio de código).** CORRECCIÓN DE CONSTANCIA al párrafo anterior: García Lema fue contado vía su FILA propia (migrada verbatim <=27.1) sobre la firma de cierre de su disidencia, NO «vía colector de desconocidos» (la columna publicaba VACÍA en 1847); y el ciclo real fue **4 filas nuevas** (Rivera J.C., Botana, Torres V.N., Caballero — todas con alternación invertida sellada contra 344_p2752) **+ 4 ensanches invertidos** (García Lema, Méndez, Montesi, Leal de Ibarra — sellados contra 344_p3070). Hipótesis firma-estampada MUERTA con dato (mundo (a): firma de mayoría tipografiada, leída en el .md). Grep saga: de las 54 gemelas, 2 más llegaron a Fallos con firma de conjueces legible (2752 invertida sin acentos, 3070 invertida con 3 conjueces extra) + 345_p12 interlocutoria de titulares (nj=4 correcto, control) + 329_p1092 ya contado (control). Flip-set del ciclo: {1847 nj 1→5 mixed · 2752 nj 0→4 unanime · 3070 nj 1→5} + 3 FP zona-label letrado-Rivera(h) aceptados con constancia (334_p38/339_p194/344_p782, daño canónico cero → pool B148/B155). Votos +12, zonas +9 (contabilidad completa), 0 flips decisión, blind byte-idéntica, manifest [CLEAN] 64. Regresión viva: `poc_b154bis_conjueces.py --esperar-version 34.0 --out propio` → E1 0/0/0, E0 0 diffs, flip-set VACÍO (baseline `poc_b154bis_flipset_baseline.csv`, H202/). Lo no-publicado de la saga queda fuera por construcción; conjueces futuros de la saga = fila nueva + mini-ciclo (patrón vigente).

### B155 — Bleed de epílogo dentro de `firma_raw` (~157 casos)

**Componente:** parser (`collect_firma_lines`).
**Origen / fuente del diagnóstico:** H191 (PoC B153: clase «mixta_probable_epilogo» = 751 tokens / 157 casos; testigos 337_p481 ASOCIART, 346_p888 DARIMAR, 329_p471/472 Fisco Nacional).
**Causa raíz:** el colector de firma deja pasar líneas de «Recurso interpuesto por…/Traslado contestado por…» → partes y letrados contaminan `firma_raw`. No afecta `n_jueces` cuando los nombres no matchean la tabla, pero puebla `jueces_desconocidos` de ruido ('Estado Nacional' 50, 'Fisco Nacional' 25 en el top) y degrada la señal del instrumento. **Evidencia nueva H192 — el «no afecta n_jueces» tiene EXCEPCIÓN GRAVE:** en 348_p1553 el bleed contiene «con el patrocinio letrado del Dr. Augusto César Belluscio» y el patrón TITULAR de Belluscio matchea → **n_jueces cuenta hoy a un letrado como juez** (el caso más severo documentado del bug). Observación de Guillermo: «dónde termina la firma» hoy lo definen los propios nombres — el stop por marcador de epílogo es el candidato correcto pero con PoC cuidadoso de desbordes.
**Estado de verificación:** `confirmado_cuantificado` (157 casos medidos sobre firma_raw H190; re-medir post-v28.0; FP de conteo confirmado 348_p1553 H192).
**Validador propuesto:** marcador de epílogo (`RE_EPILOGO_MARKER`) como stop del colector — PoC propio con flip-set; posible acople con B148/frontera.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B153, B154, B117 (familia epílogo), B148. H191.

### B156 — `is_full_bench` no es period-aware (semántica `==5` sobre bancos de 7/9)

**Componente:** parser (L3874).
**Origen / fuente del diagnóstico:** H191 (flip-set B153: 330_p224 0→1 = 5 titulares del banco VIEJO disparan full_bench; 329_p4178 1→0 corrigió el FP simétrico).
**Causa raíz:** `is_full_bench = int(n_titulares == 5)` asume la Corte de 5; en fallos viejos publicados tardíamente (banco de 7/9) produce FP/FN. FP aceptado y documentado en el sello H191: 330_p224.
**Estado de verificación:** `confirmado_caso_testigo`.
**Validador propuesto:** con `periodo_desde/hasta` poblados (M47): full_bench = n_titulares == titulares_del_período(date). Incluye adjudicar la política para fallos con fecha de sentencia vieja.
**Estado del fix:** no diseñado (bloqueado por M47).
**Referencias cruzadas:** M47, B157, B153. H191.

### B157 — 330_p224: fecha imposible (date=2007, panel Moliné/Belluscio/Boggiano/López/Vázquez)

**Componente:** parser (fecha) o frontera (firma de fallo embebido).
**Origen / fuente del diagnóstico:** H191 (contextos de Moliné en firma_raw).
**Causa raíz (hipótesis):** «20 de febrero de 2007» es incompatible con ese panel (todos fuera de la Corte para 2005): o la fecha parseada es de otro acto (publicación/acumulación) o la firma pertenece a un fallo viejo embebido/acumulado.
**Estado de verificación:** `hipotesis_no_verificada` — leer el `.md` (extraer_caso).
**Validador propuesto:** lectura del caso; si es clase, detector fecha-vs-período con la tabla M47.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B156, M47, familia B012/B045 si resulta embebido. H191.

### B158 — `firma_raw` truncada residual: fragmento final de firma wrapeada AUSENTE del texto (clase v27.0-espejo)

**Componente:** parser (upstream de parse_firma; frontera/collect).
**Origen / fuente del diagnóstico:** H191 (descartes del PoC B153: 329_p158 y 329_p576 terminan en «RICARDO LUIS» sin «LORENZETTI» → Lorenzetti no contado, nj=5 con panel real 6).
**Causa raíz:** pariente del FRENO v27.0 pero inverso: allí el fragmento estaba y el retroceso lo comía (guard v27.1 lo protege); acá el fragmento final NO está en el texto extraído. NO se arregla por set/tabla.
**Estado de verificación:** `confirmado_caso_testigo` (2 casos; cardinalidad de la clase sin medir — detector candidato: firma_raw que termina en nombre-de-pila de la tabla sin apellido).
**Validador propuesto:** leer los 2 `.md` para ubicar la capa (¿linea_fin_real corta antes? ¿OCR sin el apellido?); después detector corpus-wide.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** H190-b (FRENO v27.0), B019 (firma wrapeada), B153 (descarte con constancia). H191.

### B159 — Zona `dictamen` sin cierre engulle el fallo entero — CERRADO H196 (clase B; clase A escindida → B161)

**Componente:** parser (`RE_DICT_HDR` — la causa resultó estar en el DETECTOR, no en el cierre de la zona).
**Origen / fuente del diagnóstico:** H194 (flip-set A1 de `poc_b149_anclas` v0.1: 7 casos con `por_ello_idx` en zona `dictamen` y cierre en `(borde)`, rangos de relabel-hipotético 111-1213 líneas → excluidos del fix B149-A1 por guard de alcance).
**Corrección de constancia (H196):** la hipótesis H194 («la apertura del fallo posterior al dictamen no dispara ancla de cierre») era el modelo del SÍNTOMA, no la causa. Lectura de los 7 `.md`: en **6/7 no hay dictamen terminal que cerrar** — hay un FP NARRATIVO de `RE_DICT_HDR`: el wrap del OCR deja «dictamen de la/del Procura…» a inicio de línea en una remisión/cita del cuerpo y `re.I` lo matcheaba → `dictamen_inicio` espurio → guarda H052 suprime todo lo posterior. El 7mo (332_p2418) es dictamen GENUINO como nota al pie «(*) Dicho dictamen dice así» intercalado a pie de página — mecanismo distinto → **B161**.
**Fix aplicado (H196, parser v30.0→31.0, MAJOR):** `RE_DICT_HDR` pasa a forma-título LAXA (la línea es SOLO el título: prefijo verbatim + slot de 1 token para «General» que tolera OCR «Geberal»/«Genera» + cola opcional «de la Nación» + remate de puntuación/(*)); `re.I` SE CONSERVA (títulos genuinos en versalitas-OCR de 336.1 ×8 — capitalización à la H139 descartada con dato; prev_abierta descartada: 565 títulos genuinos con prev abierta). Calibración corpus-wide (`poc_b159_superficie` v0.1, scripts/diagnostico/H196/): 3804 matches → quedan 3789 / caen 15 / **0 pérdidas**. Flip-set sellado (`poc_b159_flipset` v0.2, réplica fiel CON relabel A1, E0 5703/5703): **7 casos / 0 flips outcome / 0 por_ello_idx**; los otros 8 FP del dump = no-op adjudicado (línea dentro de dictamen genuino, exenta en Pasada 2). Ciclo: perímetro completo 7/7 ids exactos en casos.csv y zonas.csv · radio fin_real VACÍO (medido) · is_merit_decision 0 flips vs golden v30.0 · blind byte-idéntica · zonas +41 · votos recuperan CONTENIDO sin +filas · manifest [CLEAN] 64. Regresión propia: flipset post-regolden → VACÍO/E0 0; superficie = baseline histórico (regex vieja hardcodeada por diseño).
**Referencias cruzadas:** B149 (origen del flip-set), H052 (guarda dictamen), B161 (clase A escindida), B162/B163 (destapes del ciclo), B117-F2/H184-b y H139 (familia FP-narrativo-por-re.I). H194, H196. Sin ID histórico.

### B160 — Residuo del detector B148 (4 sub-clases nuevas) + colector de firma sin breaks de epílogo

**Componente:** parser (`linea_es_firma_de_juez` — sub-clases 1-4; `collect_firma_lines` — colector).
**Origen / fuente del diagnóstico:** H194 (adjudicación por lectura del residuo del pool B148: 7 A/C + muestra D-87 seed 194; `collect_firma_lines` leída ENTERA por primera vez).
**Sub-clases del detector (FP residuales post-v29.0, localmente duros):** (1) **enumeración-de-recusados-wrapeada** — el wrap del OCR deja una línea de cuerpo con SOLO nombres del catálogo + conectores («Horacio Daniel Rosatti, Carlos Fernando Rosenkrantz y Ricardo Luis» 348_p1751 L63 · «Luis Lorenzetti, Elena I. Highton de Nolasco y Juan Carlos Maqueda» 339_p270 L24): residuo GENUINAMENTE vacío, los guards v29.0 no pueden salvarla sin contexto (línea previa sin puntuación final / separador «—» ausente / posición pre-dispositivo). (2) **carátula-con-juez** — la parte litigante es ex juez del catálogo: «BOGGIANO, ANTONIO c/ ESTADO NACIONAL» (339_p323, L0 dispara + P2 absorbe título+sumarios, 53 líneas). (3) **header-de-voto wrapeado** — «DISIDENCIA DE LA SEÑORA ⏎ MINISTRA DOCTORA DOÑA CARMEN M. ARGIBAY»: la 2ª línea sola dispara firma; `RE_HEADER_VOTO_DISIDENCIA` no la ve partida (329_p2179). (4) **cita-de-disidencia wrapeada en paréntesis** — «(conf. Fallos: 325:2119, disidencia de los jueces Eduardo Moliné ⏎ O'Connor, … Guillermo A. F. López).» (329_p3148): hipótesis RECONSTRUIDA (no leída en el guard): el corte de fragmentos parentéticos wrapeados del guard (ii) v29.0 vacía el residuo — GATE de lectura de `_firma_nucleo` antes de diseñar. Variante excusación 329_p4446 con semilla exacta NO adjudicada (mismo GATE).
**Colector:** los breaks de `collect_firma_lines` (L1018-1023: `RE_PAGE_HEADER`, «Recurso de», headers de voto, «Tribunal de origen», «Tribunal que») NO incluyen «Abogado:», «Tribunal interviniente:», «Parte actora:», «Nombre del actor:» → `firma_raw` absorbe epílogo cuando el pie usa esos rótulos (346_p965 testigo H193 — repara la expectativa: el veneno era del colector; formas leídas en 331_p2363/331_p364 pre-fix). Familia B155 (bleed de epílogo en firma_raw) — probable MISMO fix.
**Estado de verificación:** `confirmado_caso_testigo` (todas las sub-clases con testigo leído en `.md`; cardinalidad corpus-wide sin medir).
**Validador propuesto:** re-correr `poc_b148_cardinalidad` v0.2 (doble vista) post-v30.0 → el pool remanente acota las sub-clases 1-4; para el colector, detector sobre `firma_raw` con rótulos de epílogo adentro (cruza con B155). La vista 2 destapó además **46 casos firma wc>200 SOLO-post-dispositivo** (monstruos de 12 tramos: 329_p6087, 342_p1665) SIN adjudicar — misma familia colector/propagación (`poc_b148_pool_v2.csv`). Conciliación pendiente: 148 tramos v0.2 vs 142 de la bandera del explorador (leer cómo computa esa bandera, no presuponer).
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B148 (CERRADO H193 — este es su residuo adjudicado), B155 (colector/bleed, probable mismo fix), B149 (el colector se leyó en su GATE), B147/M45 (las fronteras del D-87 van a esa entrada). H194. Sin ID histórico.

### M47 — Períodos de jueces en `_meta/jueces/jueces_csjn.csv` (habilita validador de cardinalidad + full_bench period-aware)

**Origen:** H191 (diseño de la tabla: columnas `periodo_desde/hasta` reservadas vacías — no se inventaron datos).
**Contenido:** poblar períodos (titulares: dato público verificable; conjueces: del propio corpus, min/max date por juez). Consumidores: (1) validador «n_jueces < mínimo del período» (detector original del PROMPT B153, hoy sin fuente de dato); (2) full_bench period-aware (cierra B156); (3) detector fecha-vs-período (B157). Un artefacto, tres consumidores.
**Estado:** pendiente. Cambio de DATO → ciclo consciente igual (la tabla está en provenance).
**Referencias cruzadas:** B153, B156, B157. H191.

### M48 — Backlog headroom de derivers post-B147-1A (auditoría H191 Unidad B)

**Origen:** H191 (auditoría read-only pre/post-1A; 0 degradación, dividendo cobrado limpio).
**Clases medidas en disco (5894/H191):** (1) materia `sin_ancla` **1102** (95% del pendiente_capa2 — la bolsa más grande del proyecto; candidato: heurística sobre muestra, leyendo capa2/capa3 primero para no reinventar co-ocurrencia); (2) admisibilidad `sin_marcador` **1323**; (3) `causa_inadmisibilidad` = `INADMISIBLE_SIN_CAUSAL_EXPLICITA` **467/1115 (41,9%)** — el «50% ralo» del tablero ya NO es campo vacío (0% vacío post-M26 v0.5), migró a este catch-all: reclasificarlo es el trabajo; (4) partes: mérito sin recurrente **287** (sin_marcador 164 + sin_epilogo 123, techo B117/frontera); (5) `via_recurso` vacío 2183 — medir sobre mérito antes de tratarlo como gap. **Constancias:** 2 discrepancias PROMPT-vs-disco (split tipo_entrada 5701/160/33 vs 5703/157/34; votos sello «27724» vs golden 27712) — auditar la fuente de los headers de PROMPT al generarlos.
**Decisiones taxonómicas del operador (H212):** (a) **`aduanero` es DUAL** — penal en lo delictivo, contencioso en todo lo que no es penal (clasificación de origen, multas, decomisos → `tributario`). **El operador duda de que un regex/kw pueda dividir esa frontera.** (b) **`penal_tributario` es penal Y tributario** (dual). Para contrabando ninguna etiqueta miente. (c) **`materia` clasifica EL PLEITO y NO es representativa del trabajo de las secretarías** ni proxy de la secretaría de radicación, que es **imposible de derivar del corpus** (el fallo publicado no la registra; solo algunas se adivinan). Fuente = trabajo de campo, en producción; ya resuelto por esa vía: la **secretaría 6 agrupa salud + laboral + derechos humanos**. (d) **Se puede obtener información valiosa sobre temas y firmas sin saber por qué secretaría pasó cada caso.** (e) **CANDIDATA: adoptar la taxonomía del Anuario de Estadísticas de la CSJN**, que tiene varios temas que no se corresponden con las secretarías.

**Estado:** pendiente de triage por ROI. **Actualización H208: clase (1) DESCOMPUESTA con dato** — muestra 73 adjudicada (41% vocab_faltante · 22% competencia · 19% procesal_propio · 18% remisión-dictamen); caminos diseñados en M58 (dictamen, rinde 215/748) y M59 (sidecar de normas, gap carátula 265); decisión taxonómica del operador pendiente. El veredicto de irreducibilidad H115 queda corregido: el irreducible real ≈ los 525 mudos-aun-con-dictamen + parte de las clases estructurales.
**Referencias cruzadas:** B117, B137 (audit sigue pendiente — frente votos), M40. H191.

### M49 — Legibilidad de las capas de materia: rename semántico de `materia_capa` — APLICADO H210

**Origen:** H191 (cierre; propuesta de Guillermo). Los valores actuales (`capa1`, `capa1_refinado`, `capa2`, `pendiente_capa2`) nombran el orden de implementación, no la semántica — le exigen al lector del CODEBOOK conocer la historia de desarrollo del proyecto.
**Esquema candidato (Guillermo):** `capa1`→**`lectura_tribunal`** · `capa1_refinado`→**`lectura_tribunal_refinada`** · `capa2`→**`lectura_texto`** · `pendiente_capa2`→**`sin_clasificar`** (el motivo ya vive en `materia_fuente`). Decisión pendiente para `originaria`: queda como está (no es una lectura sino categoría terminal por ausencia de tribunal, art. 117) o pasa a `lectura_originario` por consistencia de prefijo — adjudicar al aplicar. `sui_generis`/`residual`/`no_aplica` quedan. Mismo criterio aplicable a la nomenclatura interna «TIER 1/3» del deriver (router de partes / co-ocurrencia) en comentarios y docstrings.
**Alcance:** dict de mapeo + bump de `derivar_materia` — pero `materia_capa` es columna PUBLICADA (Dataverse) y el explorador v8 filtra por sus valores → arrastra re-derivar + re-sello + tabla de equivalencias vieja→nueva en CODEBOOK + update del explorador.
**Estado:** RATIFICADO por el operador (H209, conversación post-sesión 2026-07-15) — esquema SELLADO: `capa1`→`lectura_tribunal` · `capa1_refinado`→`lectura_tribunal_refinada` · `capa2`→`lectura_texto` · `pendiente_capa2`→`sin_clasificar`. Decisiones cerradas: `originaria` QUEDA (categoría terminal art. 117, no es una lectura); `lectura_dictamen` RESERVADO para M58 — descartado `lectura_tribunal_dictamen` para el refinamiento porque describiría un mecanismo que la capa no tiene (el override es por parte fiscal AFIP/DGI/DGA/ARBA/AGIP/DGR, H114; el dictamen NO clasifica hoy, ni se exporta). En H209 estuvo por duplicarse como «M60» — NO crear: esta entrada es la única. **Implementación = unidad H210:** `derivar_materia` v3.3→v4.0 (MAJOR: valores publicados) · candado = CSV idéntico salvo `materia_capa` mapeada 1:1 (cross-tab viejo→nuevo exacto contra el dict, cero valores fuera; script de verificación propio) · ciclo consciente/regolden · explorador v8 (filtros por valores) · grep de literales `capa1|capa1_refinado|capa2|pendiente_capa2` en `csjn_analisis_v*.py` · tabla de equivalencias vieja→nueva como texto para el refresh CODEBOOK/Dataverse diferido.
**APLICADO (H210):** `derivar_materia` v3.3→**v4.0** (MAJOR) — 28 sitios de código a constantes (`CAPA_TRIBUNAL`/`CAPA_TRIBUNAL_REFINADA`/`CAPA_TEXTO`/`SIN_CLASIFICAR`); **`MAPEO_M49` como contrato importable** (fuente única del rename — lo consume el verificador, patrón M59/`_norm`); «TIER 1/3» → «router de partes» / «motor de co-ocurrencia» en comentarios/docstrings; nombres de FUNCIÓN (`clasificar_capa1/2`, `refinar_capa1`) y prosa arquitectónica «capa 1/2» quedan (fuera del alcance sellado). **Candado PASÓ** (`verificar_m49.py` v0.1, H210/; el instrumento se smokeó en ambos sentidos: PASS limpio + FAIL con 3 sabotajes detectados): 5894 filas, cross-tab exacta {capa1→lectura_tribunal 2301 · capa2→lectura_texto 1472 · pendiente_capa2→sin_clasificar 1161 · capa1_refinado→lectura_tribunal_refinada 159 + identidades originaria 596 / no_aplica 191 / sui_generis 8 / residual 6}, cero pares fuera, materias/fuentes/orden idénticos. Convergencias independientes: sin_clasificar 1161 = 1104 sin_ancla + 57 conflicto_capa2 (H208/M58) · cobertura 77,0% (3932/5107) reproducida. sha `csjn_casos_materia.csv` 89eb50fe…→**607793de53ed…**; resto de la cadena byte-idéntico; manifest re-sellado **[CLEAN] 65**. Constancias de alcance: explorador v8 **0 literales** (multiselect dinámico L571 — smoke en pantalla: 8 valores nuevos / 0 viejos, 159 en refinada) · `csjn_analisis_v4` **0 literales** (además → M60, reescritura) · `materia_fuente` INTACTA incl. prefijo histórico `conflicto_capa2:` (residuo de nomenclatura DELIBERADO — renombrarlo sería otra unidad con candado propio; decidir si molesta) · tabla de equivalencias vieja→nueva entregada como texto (espera el refresh CODEBOOK/Dataverse, M43/M35). Nota de método: en el ciclo el `--regolden` se corrió ANTES del candado (orden invertido); el candado corrido después PASÓ → sello convalidado post-hoc, pero el orden canónico es candado→sello.
**Referencias cruzadas:** M48 (sin_ancla 1102), M35/Dataverse, M16/explorador, M58 (reserva `lectura_dictamen`), M60. H191, H209, H210.

### M50 — Semántica de `word_count`/`wc_mayoria`/`wc_voto`: la zona firma entra al conteo

**Origen:** H192 (observación de Guillermo sobre el diff B154; demostración empírica: 338_p1433 word_count 356→361 y wc_mayoria 184→189 al reclasificarse una línea de firma).
**Problema (semántica, no bug):** las palabras de la zona firma cuentan en el wc del caso y del voto. Dos fallos idénticos en sustancia difieren en wc según cuántos jueces firmen y cuán largos sean sus nombres («María Juana Leal de Ibarra» ≈ 5 palabras de «opinión»); el sesgo es sistemático y correlaciona wc con tamaño de panel — justo una variable de H5 (tesis). Ídem calificadores «(en disidencia)».
**Diagnóstico / evidencia:** `confirmado_caso_testigo` (338_p1433, diff alineado H192). Cardinalidad = corpus-wide por construcción si se confirma en código. **Evidencia nueva H193 (masiva):** el ciclo B148 movió word_count/wc_mayoria a la BAJA en 94 casos (deltas de cientos a miles de palabras, p.ej. 333_p2445 7859→5124) al reclasificarse sumario etiquetado `firma` → el conteo estaba inflado por zonas firma espurias; confirma que wc suma la zona firma y cuantifica parcialmente el sesgo.
**Decisión pendiente:** si word_count/wc_mayoria/wc_voto deben excluir la zona firma (wc_firma ya existe por separado en las métricas de zona del explorador).
**GATE:** leer cómo se computan `word_count`/`wc_mayoria`/`wc_voto` en el parser ANTES de diseñar — en H192 no se leyeron; lo afirmado viene del diff, no del código.
**Referencias cruzadas:** B148 (la zona firma inflada agrava el sesgo), M45 (wcz_*), H5/tesis. H192.

### M51 — Orquestador: exclusiones de corpus-drift como DATO (`_meta/corpus_exclusiones.csv`)

**Origen:** H192 (decisión de Guillermo: el gate de corpus-drift obliga `--ignorar-corpus-drift` en CADA corrida por la exclusión YA documentada de 335/336 — mala ergonomía que normaliza el bypass: el día que aparezca un quinto `.md` inesperado, pasa desapercibido dentro del mismo flag).
**Diseño candidato (patrón dato-curado M46):** `_meta/corpus_exclusiones.csv` (archivo, motivo, referencia B098/B099) consultado por el pre-flight: `.md` fuera del universo `source_file` Y fuera de la lista → ABORT como hoy; en la lista → `[INFO] N exclusiones declaradas` y sigue sin flag. Auto-limpieza: tomo desparqueado que entra a `source_file` pero sigue listado → aviso de fila stale. El flag queda solo para drift genuinamente excepcional.
**Estado del fix:** no diseñado en detalle (entrada de diseño). Tocarlo implica `correr_pipeline.py` v1.x + posiblemente `generar_manifiesto` (que ya conoce la exclusión) — leer ambos gates antes.
**Referencias cruzadas:** M42 (orquestador), M46 (patrón dato-curado), B098/B099 (335/336 parqueados). H192, H210.

**Constancia H211 (tercera evidencia):** los ciclos de M58 (paso 1 y paso 2, tres corridas del orquestador) volvieron a pagar el ABORT + relanzamiento con `--ignorar-corpus-drift` por los mismos 4 `.md` de 335/336 — verificados idénticos cada vez. El costo ya es rutina; la entrada sigue esperando su unidad chica.

**Constancia H210 (segunda evidencia de ergonomía):** el ciclo `--consciente` de M49 ABORTÓ por corpus-drift en la primera corrida; se verificó el drift (exactamente los 4 `.md` de 335/336, nada nuevo) y se relanzó con `--ignorar-corpus-drift`. El costo se pagó de nuevo — aunque esta vez el freno forzó la verificación explícita antes de declarar la exclusión, que es exactamente el comportamiento que esta entrada quiere hacer BARATO (lista declarada → [INFO] y sigue).

### B161 — Dictamen genuino como NOTA AL PIE intercalado: la nota nunca cierra y engulle el fallo (clase A de B159)

**Componente:** parser (`zonificar_bloque` — el header es VERDADERO, el problema es el intercalado y el no-cierre).
**Origen / fuente del diagnóstico:** H196 (escisión de B159 tras la lectura de los 7 testigos; 332_p2418 leído entero).
**Causa raíz (leída en 332_p2418):** cuando el fallo remite al dictamen de OTRA causa, el tomo lo imprime como nota: «(*) Dicho dictamen dice así:» + header real «Dictamen de la Procuración General», **intercalado a pie de página con el cuerpo del fallo durante varias páginas**. El header genuino ancla `dictamen_inicio`; la nota no tiene cierre línea-inicial (su fecha final va mid-línea) y no hay apertura posterior → la guarda H052 engulle dispositivo («Por ello, se desestima la queja…»), firma de 6 jueces y el voto de Argibay. El fix B159 clase B (forma-título) NO lo alcanza por diseño: el título es real.
**Diagnóstico / evidencia:** 6 casos corpus-wide por el marcador `^(\*)\s*Dicho dictamen dice as[ií]` (`poc_b159_superficie` v0.1, tipo=nota_al_pie): 329_p879, 329_p3089, 329_p4032 (sin «:»), 332_p1963, 332_p2418, 334_p419.
**Estado de verificación:** `confirmado_caso_testigo` (332_p2418 leído entero; los otros 5 solo por marcador — leer antes de diseñar).
**Validador propuesto:** extraer los 6 y leer el patrón de intercalado (¿la nota siempre corre al pie con el cuerpo arriba? ¿hay señal tipográfica de segmento?). El fix probable no es de cierre sino de DES-INTERCALADO o de acotado de la nota — cirugía distinta de todo lo existente; candidata a diseño-primero sin fix el mismo día.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B159 (clase B, cerrada H196 — esta es su clase A escindida), H052 (guarda dictamen), B012/B045-familia (el intercalado a pie de página es pariente del fallo-embebido-en-nota de 329_p4140/B160-diag). H196. Sin ID histórico.

**Constancia H206:** 329_p4140 ADJUDICADO A ESTA ENTRADA — la superficie B166 (b) probó que su pick es t1 DENTRO de la ventana (140 < ivi 146): el dispositivo de la sentencia embebida en la nota «(*) Dicha sentencia dice así» está zonificado como dispositivo válido, invisible a cualquier techo/pertenencia por construcción. El residual outcome/pe de 4140 (constancia H205 en B166) muere con el des-intercalado de esta entrada, no con B166 (b1). El validador ya cubría la variante hermana `Dich[ao] (dictamen|sentencia) dice as[ií]` (constancia H201 abajo).

**Constancia H201:** 329_p1928 — el fin corregido por D2 (el ancla vieja era «Halper» citado en el PROPIO considerando: la remisión de Zaffaroni a su disidencia) absorbe la nota al pie «(*) Dicha sentencia dice así: FALLO DE LA CORTE SUPREMA…». Variante HERMANA del marcador de esta entrada (sentencia remitida, no dictamen): el validador debe cubrir `Dich[ao] (dictamen|sentencia) dice as[ií]`. No-peor (la cita es texto físico de la página de 1928); el des-intercalado de esta entrada lo limpiaría.

### B162 — Variantes T1 FP-narrativas ({en_consecuencia, atento_a}) sobre-extienden `dispositivo` — CERRADO H197

**Fix aplicado (H197, parser v32.0 MAJOR):** ambas variantes movidas a `RE_DISPOSITIVO_VARIANTES_ZONIF_PERF`, perf-gated SOLO en el zonificador (`zonif=True`, único call-site, peek de wrap H194); resolutor incondicional. Corrección de constancia: L923 adjudicado = **atento_a** (no en_consecuencia); la clase se ensanchó con dato y con la constancia madre H041. Gate de resolutor descartado con dato (341_p774 revoca correcto, perf wrapeado). Validación: superficie 4607/16 variantes · flip-set E0 5703/5703, 423 casos, 0 flips outcome · perímetro completo `diff_perimetro_b162` (candado de decisión 0 flips; votos solo wc_mayoria denormalizada) · zonas −99 filas · 17 casos con wc descontaminado de bleed (clase M50) · regresión post-fix VACÍA. Vecino-por-solape 330_p2122 adjudicado TP. Residual documentado: dispositivos genuinos NO-pick sin perf ni wrap (per-voto) pierden zona — enumerable en el baseline `poc_b162_flipset.csv`; solapa con la constancia multivoto ~13 de H194.

*(Entrada original al diagnóstico H196:)*

**Componente:** parser (`RE_DISPOSITIVO_VARIANTES` L112: `^En consecuencia\s*,?` INCONDICIONAL, re.I — matchea el wrap en minúscula; radio doble: ancla del zonificador + candidato del resolutor).
**Origen / fuente del diagnóstico:** H196 (destape del ciclo B159: los votos recuperados de 344_p1952/344_p2123 quedaron con zonas `dispositivo` de cientos de líneas; mecánica PREEXISTENTE, visible recién al destapar el dictamen espurio).
**Causa raíz (adjudicada en 3/4 anclas):** «En consecuencia» conector argumental dentro del voto dispara T1 → la zona `dispositivo` persiste hasta la próxima ancla, que en un voto largo es la firma. Testigos leídos: 344_p1952 L1586 «En consecuencia, los actos de las legislaturas provinciales…» → ~360 líneas hasta la firma de Lorenzetti · 344_p2123 L1237 «en consecuencia, ajeno a la vía extraordinaria…» (wrap a mitad de oración) · 344_p2123 L1596. **Pendiente:** 344_p1952 L923 (segmento dispositivo 923-972 del voto de Highton) — ancla sin identificar, leer `bloque[923]` completo.
**Diagnóstico / evidencia:** `confirmado_caso_testigo` (4 zonas en 2 casos; cardinalidad corpus-wide SIN medir — la variante es incondicional, la superficie puede ser grande).
**Validador propuesto / camino:** espejo EXACTO del tratamiento B067→perf-gated de H194-A2, con playbook validado dos veces (H194, H196): (1) GATE — PoC de superficie corpus-wide de la variante (matches totales / en zona dispositivo hoy / cuántos anclan outcome vía el resolutor: **un perf-gate acá PUEDE mover outcome**, dimensionar ANTES de decidir alcance zonificador-solo vs. resolutor+zonificador); (2) flip-set con réplica fiel (adaptar `poc_b159_flipset`, candado E0); (3) fix + ciclo. Riesgo declarado: si hay muchos T1 legítimos sin performativo en la cola, el gate pierde recall — lo decide la medición.
**Estado del fix:** APLICADO H197 (v32.0).
**Referencias cruzadas:** B067/H130/H194-A2 (patrón perf-gate, constancias madre), H041 (constancia madre del trío), B159 (destape), constancia multivoto ~13 de H194 (solape del residual per-voto), M50 (17 casos wc autocorregidos), B164 (destape del ciclo), B147/M45 (constancia 329_p4815 pick en b0). H196, H197. Sin ID histórico.

### B163 — 334_p109: dictamen genuino zonificado como `sumario` (título no línea-inicial en el OCR)

**Componente:** parser (`zonificar_bloque` / `RE_DICT_HDR` — miss de anclaje, no FP).
**Origen / fuente del diagnóstico:** H196 (constancia del diff del ciclo B159: `dictamen_presente` True→False en 334_p109 destapó que el True previo era el bolsillo FP; el dictamen REAL de Righi — «Suprema Corte:» … «Buenos Aires, 7 de agosto de 2009. Esteban Righi.» — está zonificado como `sumario` desde siempre porque su título no aparece a inicio de línea en el OCR).
**Efecto:** `wc_dictamen` 0 / `dictamen_presente` False para un caso CON dictamen; `lineas_dictamen` no lo excluye de nada (benigno para outcome — el caso resolvió bien), pero el eje dictamen queda falso para ese caso. PREEXISTENTE: no lo crea ni lo agrava el fix B159.
**Estado de verificación:** `confirmado_caso_testigo` (1 caso leído; ¿clase? — «Suprema Corte:» sin header de título es forma conocida de tomos con dictamen embebido: dimensionar con detector «Suprema Corte:» en zona no-dictamen antes de diseñar).
**Estado del fix:** no diseñado.
**Constancia H208 (canal hermano destapado):** en la clase competencia-dirimente del pool sin_ancla, `considerando_text` ARRANCA con la voz temática del sumario editorial («JURISDICCION Y COMPETENCIA: Competencia ordinaria. Por la materia. Cuestiones previsionales/laborales/civiles…» — testigos 329_p1389, 329_p5190, 331_p1900, 330_p2345 y ~10/73 de la muestra H208). Doble lectura: constancia de zonificación (sumario dentro del considerando en layouts de competencia, familia de esta entrada) Y ancla candidata de materia de altísima precisión aparente — la voz NOMBRA la materia subyacente. Dimensionar como parte del diseño post-decisión-taxonómica.
**Referencias cruzadas:** B159 (contexto del destape), H052. H196, H208. Sin ID histórico.

### B164 — 347_p109: bloque con comentario DOCTRINAL parseado como fallo (outcome fabricado de narración)

**Componente:** localización / clase de entrada (no es un problema de anclas: el texto no es un fallo).
**Origen / fuente del diagnóstico:** H197 (lectura del extracto al adjudicar los picks expuestos de B162).
**Causa raíz (leída en 347_p109):** el bloque [4164,4440] contiene texto de análisis doctrinal («En síntesis, podríamos decir, conforme Carrió…»; el único «Por ello» es «Por ello, para la Corte…», narración) — probablemente material editorial del tomo tratado como fallo. El pick actual es «En consecuencia resolvió que no había mérito para revocar…» (narra al a quo) y `outcome=revoca` sale fabricado de esa narración.
**Diagnóstico / evidencia:** `confirmado_caso_testigo` (1 caso leído entero; cardinalidad de la clase SIN medir — ¿hay más bloques doctrinales en los tomos?).
**Validador propuesto:** dimensionar con señales de voz doctrinal («podríamos decir», «conforme <autor>», ausencia de carátula/firma canónicas) antes de diseñar; puede ser clase del catálogo/cruce más que del parser.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B162 (destape), familia B012/B045 (entradas espurias). H197. Sin ID histórico.

### B165 — D1 residual: metadata perdida en 2 flips (carátula del juez-PARTE muerta por el guard firma + salto a «Vistos» sobre apertura)

**Componente:** parser (`refinar_inicio_por_titulo`, guards D1 de v33.0).
**Origen / fuente del diagnóstico:** H200 (perímetro del ciclo consciente D1: clase regresiva del `diff_perimetro_d1`; adjudicación post-hoc estilo H178 «valor conocido-equivocado con unidad sucesora»).
**Causa raíz (leída en el diff de textos):** (a) **329_p1303 (Boggiano, juez-parte):** la carátula real «ANTONIO BOGGIANO» es indistinguible de una firma — `linea_es_firma_de_juez` la rechaza — y una CITA posterior con conector («…causa B.1695.XLI "Boggiano, Antonio s/ juicio político…») pasa `_es_caratula_v2` → ancla Δ32 dentro del cuerpo: `date`/`apertura_tipo` a ∅, `status` ok→ok_sin_marcador_apertura, considerando amputa el 1º). Los hermanos p1305/p3235 NO regresan (ningún match posterior pasa el guard → fall-through a T4 re-ancla idéntico). (b) **329_p1930:** el flip válido cae en «Vistos los autos: "H.87…"» salteando la apertura/fecha → misma pérdida de metadata sin amputación sustantiva (wc 1226→1214).
**Diagnóstico / evidencia:** `confirmado_cuantificado` — clase regresiva completa del perímetro = exactamente estos 2 (los otros 3 status-changes son etiquetado honesto del mecanismo nuevo: ok→ok_ancla_catalogo/vistos, adjudicados TP). 0 flips de decisión asociados.
**Diseño candidato (adjudicado con el dato H200, PoC propio antes de tocar):** (a) eximir el guard firma cuando la línea matchea el NOMBRE COMPLETO del caso (`_build_fullname_variants` ya existe, Pista 5b) — la identidad juez/parte se resuelve por posición y forma, NO por vigencia temporal (gate por período fabricaría FN sobre firmas genuinas publicadas tarde, clase Moliné/Bossert de B153, y sobre transcripciones B161); y/o (b) rechazar candidatos de carátula ENTRE COMILLAS de cita. (b) del caso 1930: exigir que el salto a un match válido no cruce una línea RE_APERTURA/fecha sin anclar antes.
**Estado del fix:** diseñado (candidatos), no implementado — unidad propia con flip-set (absorbe el D4 «vecinos homónimos» del plan B147 en su mitad juez-parte).
**Referencias cruzadas:** B147/D1 (origen), B153/H191 (Boggiano en JUECES_CONOCIDOS; publicación tardía), B161 (transcripciones), H178 (patrón costo-aceptado-con-sucesor). H200. Sin ID histórico.

### B166 — Outcome leído del dispositivo de la DISIDENCIA (mayoría-vs-disidencia): el último performativo del bloque no es el del caso — FIX PARCIAL H203 (vocabulario) + **FIX (c) H205 (panel por evidencia de zonas)** + **FIX (b1)-A H207 (peek de wrap en el resolutor)**; residual (b1)-B enumerado / (b2)→B161 / (d)=B167

**CONSTANCIA H216 — testigo sellado para (d): 343_p1457 (Bertuzzi).** El fix B168 le dio techo de votos (ivi=3411, header de Highton ahora contado) y mató TP el pick del voto (revoca de Highton, rel ~4325); el pick nuevo cae en la cadena de anclas argumentales «por ello» minúscula (~rel 2380, 25+ zonas dispositivo espurias 2380-3407 preexistentes) ANTES del dispositivo genuino de mayoría («Por ello, oído el señor Procurador General de la Nación interino, ⏎ se resuelve:» rel ~3352, numerado 1-10, item 2 = **Revocar** → valor real coincidente con el publicado viejo). Publicado v39: outcome=otro, is_merit=0 — costo aceptado H216 con recuperación esperada al cerrar (d): verificar que 1457 vuelva a revoca CON procedencia de mayoría. El operador sigue viendo mal-zonificados de esta familia al navegar: van acá como testigos, sin abrir unidad (regla PROMPT_H217).

**Componente:** parser (pick de dispositivo / zonificación de votos).
**Origen / fuente del diagnóstico:** H202 (paso 2 de la unidad M55+B154-bis: lectura del extracto .md de 344_p1102 para adjudicar su panel; corrección al candado de decisión H201, que había adjudicado el flip desestima→rechaza como «outcome lee el dispositivo real» — leyó UN dispositivo real, pero el del voto perdidoso).
**Causa raíz (hipótesis con testigo leído, mecánica NO adjudicada en código):** en 344_p1102 la estructura es sumario → dictamen → fallo de mayoría (dispositivo «se hace lugar a la queja… se revoca» + firma de 5 titulares L928-930, Rosenkrantz «(en disidencia)», Rosatti «(según su voto)») → voto Rosatti (concurrente, mismo sentido) → disidencia Rosenkrantz (dispositivo «se rechaza la queja» + firma solitaria al cierre del bloque). El publicado v34.0-post-B154bis: outcome=rechaza (el de la DISIDENCIA), nj=1 (solo la firma final), unanime — los tres incorrectos; el real: hace_lugar/revoca, panel 5, mixed. Candidatos de mecánica a adjudicar leyendo código+zonas: votos no zonificados (headers «Voto del Señor Ministro…» L931 y «Disidencia del Señor Presidente… Carlos Fernando ⏎ Rosenkrantz» L1172-1173 WRAPEADO — el wrap del header como causa candidata) → el resolutor pesca el último performativo; pariente del residual MEDIO-multivoto de H194 (~13 casos, dispositivo per-voto).
**Diagnóstico / evidencia:** `confirmado_caso_testigo` — 344_p1102 leído entero (extracto H202); dispositivo de mayoría, voto concurrente y disidencia citados verbatim en BITACORA H202. NO sana con B154-bis (los 5 son titulares ya en tabla — verificado: quedó en el pool M55 post-ciclo).
**Validador propuesto:** query sobre votos.csv/zonas.csv de 1102 (¿hay filas de voto? ¿zonas voto_separado?) para adjudicar la mecánica; después dimensionar la clase: casos donde por_ello_idx cae DESPUÉS de una firma-de-mayoría intermedia con headers de voto posteriores (instrumento read-only, patrón poc_b149).
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B149/H194 (residual multivoto), M55 (1102 en el pool, adjudicado), B154-bis (destape), B045/B019 (pick de firma al fin del bloque). H201 (candado corregido), H202. Sin ID histórico.

**CORRECCIÓN DE MECÁNICA H203 (adjudicada con código v34.0 leído + votos/zonas publicados + extracto).** Las dos hipótesis del párrafo «Causa raíz» de arriba están MUERTAS con dato: (i) los headers wrapeados SÍ se zonifican (`detectar_votos_disidencias` concatena hasta 3 líneas, L1363-1375; zonas de 1102: voto_separado seg1 wc 8 + seg2 wc 9 con el wrap absorbido; n_votos_svoto/n_disidencias = 1/1 SIEMPRE fueron correctos); (ii) «el resolutor pesca el último performativo» es falso — `_barrer` devuelve el PRIMER performativo-con-firma del rango (L3654). Mecánica real, eslabón por eslabón: **(a) VOCABULARIO** — mayoría y voto Rosatti cierran con «En orden a lo que hasta aquí se ha expuesto» (fórmula idéntica en ambos, leída en extracto rel 900/1146), invisible a T1-T4 → T1/T2 barren (apertura=601, techo=913) y quedan VACÍOS; **(b) T3/T3b/T4 SIN TECHO DE VOTOS** — T3 re-barre (inicio, len(bloque)) (L3781-3785) sin chequeo de pertenencia del pick: encuentra el `por_todo_ello` GENUINO de la disidencia (rel 1388; regla P: firma solitaria 1391 en ventana + «se rechaza» RE_PERF) → outcome = el del voto perdidoso; **(c) PANEL FORWARD-ONLY** — `collect_firma_lines(bloque, por_ello_idx+1)` (L4117) solo mira hacia adelante del pick: toma la firma solitaria e ignora la firma de mayoría 910-912 que el PROPIO zonificador etiquetó `firma` (ancla firma_linea sin consumidor en el pick de panel) → nj=1/unanime; el writer de votos (L4271) itera SOLO los jueces de la firma pickeada → Rosatti sin fila pese a estar en marcadores, Rosenkrantz `posicion=mayoria` (firma sin calificador) con `texto_voto` de disidencia = la contradicción publicada. Colateral: la zona dispositivo 1328-1390 publicada era ancla de Pasada 1 sobre el wrap-FP «por ello desestimar» (→ B167), no del pick.

**FIX PARCIAL APLICADO H203 (parser v35.0, MAJOR) — componente (a) vocabulario:** variante T1 incondicional `en_orden_expuesto` (`^En orden a lo que hasta aqu[íi] se ha expuesto\b`, re.I). Superficie corpus-wide (superficie_b166): forma amplia «^En orden a lo» = 16 / 14 argumental-dictamen → MUERTA; estricta 2/2 TP (ambos en 1102), 0 FP. INCONDICIONAL obligada: el performativo wrapea a la línea siguiente y el resolutor no peekea (constancia H194). Espec = `poc_b166_variante.py` v0.2 (H203/, A/B fuente única con parser importado): E0 0/5894 · flip-set {1102} EXACTO (baseline `poc_b166_flipset.csv`). Validación del ciclo: candado de decisión completo (hace_lugar · mixed · nj 5 · full bench · is_merit 1) + considerando descontaminado (−95 wc, el golden traía dispositivo+firma adentro) + votos +4 + zonas +2 + blind byte-idéntica + manifest [CLEAN] 64 + gate M55 pool 46→45 baja exacta {1102} + regresión post-fix flip-set VACÍO. **Regresión viva:** `poc_b166_variante --esperar-version 35.0 --post-fix --out propio` → E0 0/5894 · flip-set VACÍO.

**RESIDUAL ESTRUCTURAL ABIERTO (los componentes (b) y (c) de la mecánica):** el vocabulario curó a 1102 y a cualquier hermano de fórmula futura, pero la clase mayoría-vs-disidencia sigue armada para cualquier caso cuyo dispositivo de mayoría sea invisible por OTRA razón. Diseños NO triviales: (b) un techo duro en T3 rompería su propósito (existe justamente para buscar más allá del techo, B067) — candidato: chequeo de pertenencia post-pick (pick ≥ inicio_votos_indiv = señal de re-inspección, no rechazo automático) con flip-set propio obligatorio; (c) candidato: consumidor de la firma_linea temprana del zonificador para el pick de panel (la evidencia ya existe, falta cablearla). **Tableros medidos H203 (clase_b166_zonas, baseline `clase_b166_zonas_out.csv` en H203/):** C2 = 16 casos nj=1∧hay-votos, TODOS ⊂ pool M55 (gemelos evidentes 344_p441 y 344_p575; ojo: 441 NO tiene la huella de zonas de 1102 → mecánica posiblemente distinta, adjudicar por extracto) · C1-no-M55 = 8 casos unanime∧nd≥1 con nj≥3 (329_p928, 329_p1930, 329_p6087, 330_p3801, 334_p35, 343_p1800, 344_p1444, 348_p1505 — clase blanda: vp mal, panel quizás bien) · constancia: la huella de zonas firma-pre-voto ∧ dispositivo-post-voto NO discrimina (2096 = layout normal multivoto; cada voto zonifica SU dispositivo — la clase solo es visible con el pick, instrumento PoC-réplica si se abre (b)).

**ADJUDICACIÓN H204 — componente (c) COMPLETO (sesión read-only, v35.0 sin cambio): gemelos leídos, regla validada corpus-wide, flip-set candidato ≈41 dimensionado.** (1) **344_p575** adjudicado = clase 1102 vía el residual-wrap DOCUMENTADO de `en_virtud_perf` (H194-A2): mayoría cierra «En virtud de lo expuesto, de conformidad con lo dictaminado por ⏎ …se hace lugar» (extracto 759-763) — el zonificador la ve (peek de wrap, zonas dispositivo 759-763 + firma 764-765 = huella 1102) y el resolutor no (perf en línea obligatorio) → T3 sin techo pesca el «Por ello» del voto de Maqueda (774-777 == pe publicado verbatim) → panel = «Juan Carlos Maqueda.» (778 == firma_raw 20 chars). Ground truth: panel 4, mixed, Rosatti disidencia parcial. La regla (c) original lo cubre. (2) **344_p441** adjudicado = mecánica NUEVA firma-colectiva-post-voto: pick de outcome CORRECTO (T1 clásico 1091); la firma del panel de 5 (1382-1384, zonificada) está POST voto concurrente de Rosatti → collect(pick+1) come el header del voto → nj=1 «mayoria»; header de disidencia wrapeado (1389-1390) rompe la extracción del juez. La regla original NO lo cubre → **ENMIENDA c′: primera zona firma post-apertura cuyo collect parsea PANEL (nj≥2 ∨ calificador RE_CALIFICADOR), cualquier posición; fallback a la original.** (3) **Regla medida** (`poc_b166_c_regla` v0.1→v0.2, H204/, fuente única): v0.1 mató dos defectos con dato (c′ sin post-apertura pescaba bleed, 11 casos; panel del span verbatim ≠ semántica del consumidor → 134 artefactos wrap clase H190-b); v0.2 (panel vía `collect_firma_lines(bloque, zona_ini)`) → **full match 5643/5687 (99,2%)** · 44 mismatches · ancla 1102 OK · baseline `poc_b166_c_regla_v02_out.csv`. (4) **Flip-set candidato ≈41:** pool c_nj>pub_nj 21 (11 C2 + 330_p3801 [C1] + 330_p3141/331_p1397/332_p5 + sin_firma 330_p2794/345_p378/347_p1084 + conjuez 329_p3649/329_p5317 + 340_p1993 [1C H190, leer]) + clase A 14 (pub inflado por voto tragado → B168) + prosa-con-juez 4 (330_p2799/340_p1510/347_p2154/346_p965=B160) + {441, 1396} vía c′. (5) **Residuo con clase 7:** 329_p3148/4426/4446 (primera-zona-firma-FP en sagas conjuez) · 329_p4140 (dos firmas de 7, sets distintos) · 339_p270 (zona arranca 1 línea tarde, pierde juez wrapeado) · 347_p520 (zona arranca en línea-prosa «…Doctor Don Horacio Rosatti.», guarda `_RE_FIRMA_COMPLETA` corta a nj1 con el panel real una línea abajo) · 348_p708 (única zona firma post-primer-voto, nj1 conjuez — puede ser dato correcto, extracto pendiente). **Estado del fix (c): DISEÑADO** — consumidor c′ en el camino del panel (L4124-4144), collect desde la zona elegida, fallback al comportamiento actual si no hay candidata; **pendiente PoC-réplica A/B** (patrón poc_b166_variante: E0 0/5894, flip-set SELLADO vs ≈41, extras = FRENAR/adjudicar) + ciclo — el PoC H204 compara regla-vs-publicado, NO código-vs-código; segundo orden (posiciones/n_titulares/is_full_bench/vp/votos.csv ±filas) solo lo captura la réplica. = **H205**.

**FIX (c) APLICADO H205 (parser v36.0, MAJOR):** consumidor `panel_cprima` (L1273) cableado en procesar_archivo ANTES de collect(pick+1)/A001 (L4174), independiente de por_ello_idx: primera zona firma (run contiguo de `_zonas_linea` post-relabel A1) con inicio > primer índice de zona apertura (−1 si no hay), cuyo `collect_firma_lines(bloque, zona_ini)` parsea PANEL (nj deduped ≥ `_MIN_JUECES_PANEL`=2 ∨ calificador); sin candidata → comportamiento v35.0 verbatim. Espec `poc_b166_c_replica.py` v0.1 (H205/, E0 0/5894) + adjudicación completa `adjudicar_h205.py` [OK]. Flip-set SELLADO 34 = 31 TP + 3 costos (→ B169): baseline `poc_b166_c_replica_out.csv`. Validación: gate M55 pool 45→29 (17 bajas ⊆ flip-set, 1 alta adjudicada = 339_p270) · sin_firma 12→8 · votos +66 · manifest [CLEAN] 64. Residuales de B166 que (c) NO cubre: **(b)** T3/T3b/T4 sin techo de votos — el OUTCOME de la clase sigue expuesto (testigo vigente: 4140 publica outcome/pe de la sentencia embebida en nota al pie, panel ya correcto) — y **(d)** wrap-FP RE_POR_ELLO minúscula (=B167). Regresión propia: réplica `--esperar-version 36.0 --post-fix` → E0 0/5894 + flip-set VACÍO.

**SUPERFICIE (b) H206 (read-only, sellada):** instrumento `poc_b166_b_superficie.py` v0.1 (H206/; tronco verbatim de `poc_b166_c_replica` + réplica-con-atribución-de-tier de las 5 llamadas de `resolver_dispositivo` L3806-3852; candados E0 pe==publicado 0/5703 y Etier réplica==canónico 0/5703; baseline `poc_b166_b_superficie_out.csv`). Pool expuesto (pick ≥ ivi ∨ zona pre-relabel voto_separado) = **13, TODOS t3-post-techo con pick en zona dispositivo** (t1 5642 · t2 13 · t3 13 · t3b 1 · t4 12 · none 22 — T3b/T4 aportan 0). **El componente (b) se PARTE en dos con dato:** **(b1)** t3-post-techo, pool 13; **(b2)** sentencia-embebida-en-nota: **4140 NO está en el pool — su pick es t1 DENTRO de la ventana (140 < ivi 146)**, el dispositivo de la embebida está zonificado como dispositivo válido → ningún techo/pertenencia puede verlo; su fix es el des-intercalado B161 (corrección de constancia: el «testigo vigente 4140» de esta entrada estaba mal clasificado). **Adjudicación 13/13 por extracto (H206):** sub-clase **A** perf-wrap «En virtud de lo expuesto» ⏎ (344_p575, 340_p1940, 347_p2160) — valor COINCIDENTE con dispositivo de mayoría casi calcado; el residual documentado H194 (perf-variantes del resolutor exigen perf en la MISMA línea) — **candidato de fix más barato: peek de wrap en el guard RE_PERF de las perf-variantes del resolutor** (espejo del peek que el zonificador usa desde H194/H197); flips esperados = texto/pe, 0 flips de decisión en los 3; superficie propia obligatoria antes (el peek puede re-pickear otros casos). Sub-clase **B** fórmula-de-mayoría no catalogada o mid-línea (330_p3141 «; por lo que deberán remitirse» mid-línea · 330_p3801 «Conforme lo señalado en los considerandos precedentes… se re-⏎suelve» · 332_p2237 «Por lo tanto» [variante con constancia de MUERTE H194 — acá era el dispositivo real] · 343_p300 «En atención al estado… se resuelve» · 332_p5 «Conforme los argumentos… se declara» mid-párrafo · 340_p204/344_p757 honorarios «En atención a…») — **todo el daño real vive acá: 2237 outcome INVERTIDO** (publicado desestima; la mayoría DISPONE la consulta al TPR) · **343_p300 INCORRECTO** (otro vs competencia; pe = disidencia truncada en habilitación de feria) · 3141 pe con mandato INVERSO y etiqueta salvada · 332_p5 etiqueta divergente; 204/757/3801 coincidentes → **daño de decisión total = 4/5894 (0,07%), enumerado**; vocabulario NO escala (una variante ya tiene constancia de descarte). Sub-clase **C** pick tardío CORRECTO (348_p728 ivi-ESPURIO por cita wrapeada «disidencia de los señores jueces…» → B170 · 347_p1084 dispositivo CONSOLIDADO impreso DESPUÉS de todos los votos, pick correcto con ivi genuino · 343_p1024 mayoría SIN dispositivo impreso, el del voto es la única fuente) → **constancia dura: techo duro en T3 y chequeo-de-pertenencia naive MUERTOS con dato** — el mismo mecanismo que expone a B rescata a C, indistinguibles por posición (solo por lectura). Estado (b1): pool sellado y adjudicado; fix candidato = sub-clase A (unidad propia con superficie + flip-set); sub-clase B queda enumerada-con-clase (fix no rentable hoy; los 2 graves NO habilitan M56, condición de activación no cumplida). Referencias: B161 (b2), B170 (ivi-FP), H194 (perf-wrap y «Por lo tanto»), H206.

**FIX (b1)-A APLICADO H207 (parser v37.0, MAJOR):** peek de wrap en el resolutor — `_barrer` computa `cola_wrap = bloque[k+1].strip()` (espejo verbatim del call-site del zonificador, Pasada 1) y la pasa a `es_candidato`; los 4 `_cand_*` +1 parámetro con default `""` (t2/t4 lo ignoran; `_cand_estructural`/`_cand_t3b` lo forwardean a `detectar_apertura_dispositivo`). Cero regex nueva; monótono por construcción (solo agrega candidatos). Alcance exacto = `RE_DISPOSITIVO_VARIANTES_PERF` (las `ZONIF_PERF` son incondicionales con `zonif=False`, el peek no las toca — la distinción con el descarte H203 queda limpia). Espec = `poc_b166_b1a_peek.py` v0.1→v0.2 (H207/; réplica verbatim de `_barrer`+cascada porque `es_candidato(stripped)` no acarrea el peek; E0 0/5703 · E0'' 0/5703). Flip-set SELLADO **4 = 4 TP** (baseline `poc_b166_b1a_peek_out.csv`): los 3 testigos A (t3→t1, exposición 1→0, valor coincidente verificado en producción, 0 flips de decisión) + **344_p2779 recuperación** adjudicada por extracto (mayoría en_virtud+perf-wrap, disidencia «Por lo tanto» invisible por diseño → sin_dispositivo/none; pick 191 pre-ivi = mayoría; outcome→desestima). Riesgos vacíos: 0 re-picks intra-T1 · anclas negativas C (728/1084/1024) sin flip. Ciclo: zonas/editorial byte-idénticos (relabel A1 no-op) · blind byte-idéntica · gate M55 pool 29 sin movimiento · manifest [CLEAN] 64 (sha casos 19028de5412d…). Regresión propia: `--esperar-version 37.0 --post-fix` → flip-set VACÍO + E0''=={4}; radar (b) pool 13→10 exacto. **El residual documentado H194 queda CERRADO.** Residual de B166 tras (b1)-A: (b1)-B enumerada (7 casos, daño 4) · (b2)→B161 · (d)=B167.

### B167 — Wrap-FP de RE_POR_ELLO: «por ello» en minúscula a inicio de línea por corte del OCR (familia B159/B162, ahora sobre la variante madre)

**Componente:** parser (detectar_apertura_dispositivo / T1 / ancla de Pasada 1).
**Origen / fuente del diagnóstico:** H203 (adjudicación de B166 en 344_p1102: la zona dispositivo publicada 1328-1390 era ancla de Pasada 1 sobre «…Corresponde ⏎ por ello desestimar la queja interpuesta.» — prosa argumental del considerando 5° de la disidencia, con «por ello» a inicio de línea SOLO por el wrap del OCR).
**Causa raíz:** `RE_POR_ELLO` es re.I y `.match()` sobre línea stripeada; el wrap del OCR deja el conector en minúscula a inicio de línea y el guard `POR_ELLO_ARGUMENTAL` solo filtra verbos conjugados (concluyó/estimó/…) — infinitivos y demás continuaciones («desestimar», «que», «no», «la», «debe»…) pasan. Mismo mecanismo que B159 (RE_DICT_HDR narrativo) y B162 (en_consecuencia): conector wrapeado a inicio de línea.
**Diagnóstico / evidencia:** `confirmado_cuantificado` — superficie corpus-wide medida H203 (`superficie_b166.py`, H203/): **76 líneas** `^por ello` minúscula estricta; distribución de primera-palabra dominada por continuaciones argumentales fuera del set (que/no/la/debe/el…; solo «entendió» ×2 cae en el guard). Efecto hoy: anclas de zona dispositivo espurias (huella tipo B162: en 1102, ~60 líneas argumentales pintadas dispositivo) + candidatos T1 vivos en el resolutor — **riesgo de pick robado y outcome corrupto donde el FP tenga una firma a ≤40 líneas** (en 1102 no la tenía: solo ensució zona). Testigo de zona: 344_p1102 rel 1328 (leído en extracto).
**Validador propuesto / discriminador candidato:** case-sensitivity — el dispositivo genuino «Por ello» abre párrafo capitalizado; el wrap-FP es minúscula. ADVERTENCIA B159 obligatoria antes de sellar: medir si existe ALGÚN dispositivo genuino en minúscula corpus-wide (precedente: el título de dictamen genuino existía en versalitas-OCR y obligó a conservar re.I); el pool de 76 se adjudica por lectura muestreada + cruce con picks publicados (¿alguno de los 76 ES por_ello_idx hoy?).
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B159, B162, B166 (destape y testigo), B067 (perf-gate como patrón alternativo si case-sensitivity falla). H203. Sin ID histórico.

### B168 — Colector de firma traga el VOTO entero: header de voto invisible → firma_raw inflada, n_jueces con duplicados, posiciones PISADAS — **VOCABULARIO CERRADO H216 (v39.0) · DEDUP + PEEK-E CERRADOS H217 (v40.0/v41.0)**; residual = 329_p5741 (sub-clase E+vocab)

**Componente:** parser (`collect_firma_lines` breaks / `RE_VOTO_HDR`; consumo en `n_jueces` L4236 y columna `jueces`).
**Origen / fuente del diagnóstico:** H204 (caracterización de los mismatches del PoC B166 (c) v0.2: clase «pub_nj > regla con set idéntico»; adjudicada leyendo el `firma_raw` publicado de los 24 casos con `query_firma_raw_clase_a`, H204/).
**Causa raíz:** los breaks de `collect_firma_lines` frenan en `RE_VOTO_HDR`/`RE_DISID_HDR`, pero el header en la variante «**Voto la señora ministra doctora doña** Carmen M. Argibay» (orden/género distintos del «Voto del Señor Ministro Doctor Don…») no matchea → el colector sigue y `firma_raw` = firma del panel + VOTO COMPLETO + firma del voto. Aguas abajo: `parse_firma` deduplica por POSICIÓN y no por nombre, y `n_jueces = len(firma_parsed["jueces"])` (L4236) → el juez del voto se cuenta 2-3 veces y la columna `jueces` publica el nombre repetido. Testigo 331_p1028: firma_raw 526 chars con el voto de Argibay adentro; pub nj9 / jueces «…| Argibay | Argibay | Argibay»; panel real 7.
**Diagnóstico / evidencia:** `confirmado_cuantificado` — 14 casos con duplicado de nombre en firma_raw (query H204): 329_p3956, 331_p419, 331_p1028, 331_p1302, 331_p2376, 331_p2470, 332_p460, 337_p966, 338_p539, 340_p1236, 346_p629, 346_p931, 347_p1495, 348_p1751. Sospecha colateral SIN adjudicar: si el colector se tragó el voto, `detectar_votos` puede haber perdido la FILA de ese voto en votos.csv (adjudicar en el ciclo del fix B166 (c), cuyo flip-set los incluye a los 14 como mejoras de panel).
**Validador propuesto:** el propio ciclo del fix B166 (c) muestra el flip de estos 14 (nj baja al panel real); aparte, superficie corpus-wide de headers de voto en formas no cubiertas por RE_VOTO_HDR (grep de variantes «Voto (de )?la señora…», «Voto de los señores…» etc.) para dimensionar si hay más allá de los 14.
**Estado del fix:** no diseñado. Candidatos: (a) ensanchar RE_VOTO_HDR/breaks del colector a las variantes de género/orden (con superficie ANTES, patrón B159); (b) el fix B166 (c) corrige el PANEL de los 14 por otra vía, pero `firma_raw` seguiría inflado si el colector arranca en la misma zona — dimensionar tras (c).
**CONSTANCIA H205 (medida en la réplica del fix B166 (c)):** el flip-set real curó SOLO 2/14 (331_p419 nj9→5, 348_p1751 nj5→3 — c′ arranca en otra zona); en los otros 12, c′ elige la MISMA zona donde collect(pick+1) ya arrancaba → output idéntico, firma_raw sigue inflado y n_jueces sigue con duplicados. El «nj baja al panel real» de la adjudicación H204 era el DEDUP del contador del PoC (nj = len(set de nombres)), no un collect distinto. Ítem nuevo de la clase: 329_p2063 (recuperado sin_firma→panel por c′) publica duplicados posicionales «Fayt | Zaffaroni | Fayt | Zaffaroni» = misma mecánica de conteo. El fix propio de B168 (breaks del colector) sigue pendiente con los 12 + 2063 como pool.
**Referencias cruzadas:** B166 (c) (destape y absorción parcial), B160 (familia colector: breaks incompletos; +4 casos H204 de arranque-en-prosa: 330_p2799, 340_p1510, 347_p2154, 347_p520 — y 346_p965 la expectativa original), B148 (predicados de firma), B155. H204. Sin ID histórico.
**CIERRE H216 (vocabulario) — parser v38.0→v39.0 (MAJOR):** la sospecha colateral se adjudicó CORREGIDA: la fila de voto NO se pierde — se DUPLICA (los duplicados posicionales viajan a votos.csv), y lo que sí se pierde es el TEXTO del voto (10/13 con 0 filas wc_voto>0) más la POSICIÓN real: `posiciones` (dict keyed-by-nombre, last-write-wins) borraba 11 pares en 10 casos, 3 disidencias publicadas como mayoria. Universo sellado por superficie corpus-wide (`superficie_b168.py` v0.3 + extractos): 61 casos, 6 clases; el pool 13 = 10 header-invisible + 2 «Ampliación de fundamentos» + 1 conteo posicional (2063). **Fix aplicado:** RE_VOTO_HDR/RE_DISID_HDR ensanchadas (orden verbal-primero, «Voto Concurrente», femenino Vicepresidenta/Presidenta, «de el», «Ampliación de Fundamentos» lado voto) + rama calificador ampliación→«según su voto» vía RE_CALIFICADOR_FIRMA local + Antelo en jueces_csjn.csv. Validación: PoC A/B/B2 E0 0/5894 · flip-set 40+4 sellado (`poc_b168_vocab_baseline.csv`, H216/) · 1 costo aceptado adjudicado (343_p1457 → testigo B166(d)) · blind byte-idéntica · M55 sin movimiento · votos −14 con aritmética exacta · manifest [CLEAN] 65. **RESIDUAL VIVO de esta entrada:** (i) clase E wrap-truncado — 21 casos sellados en `b168_universo_sellado.csv` («DISIDENCIA PARCIAL DE LA ⏎ SEÑORA…»); fix candidato = peek de wrap en los TRES consumidores, superficie del peek ANTES (unidad 1 de PROMPT_H217; mismo riesgo 1457: los 21 mueven ivi y pueden re-exponer picks); (ii) dedup por NOMBRE en parse_firma — cura 2063 (nj 4→2, re-evaluar vp vía B087) + defensa estructural, superficie de dups post-v39 primero; (iii) headers CONJUNTOS asignan texto solo al primer juez (constancia 345_p869: disidencia Rosatti+Maqueda, texto a Rosatti, Maqueda fila sin texto — coherente con el diseño de marcadores, no bug nuevo); (iv) firmas wrapeadas Tyden/Corchuelo en desconocidos = familia truncamiento v27.0, no de esta entrada.

**CONSTANCIA H206:** el pool queda RECONFIRMADO por instrumento independiente — `superficie_b169.py` (H206) midió `dup_nombres` (nj posicional > nj deduped) sobre los 5.689 picks c′ y dio **13 exactos = los 12 de clase A que (c) no curó + 329_p2063**, con 331_p419 y 348_p1751 correctamente ausentes (curados H205). Segunda vía, mismo pool.

**CIERRE H217 (dedup por nombre) — parser v39.0→v40.0 (MAJOR):** residual (ii) CERRADO. Superficie post-v39 primero (dos instrumentos independientes sobre los canónicos: nombre repetido en columna `jueces` = 1 caso; pares (caso, juez) duplicados en votos.csv = 2, ambos de 2063 — los 12 de clase A quedaron absorbidos por el vocabulario H216): **único dup restante = 329_p2063, confirmando la aritmética H216 («las 2 de 2063, que queda para el dedup»)**. Fix en `parse_firma`: dedup por NOMBRE preservando orden y PRIMERA entrada, aplicado DESPUÉS del loop de calificadores (dedup temprano extendería la ventana [fin, siguiente_inicio] del juez anterior a través del match borrado y le robaría el calificador al duplicado — trampa detectada en diseño, H217) + `has_voto`/`has_disid` recomputados desde la lista final (consistencia posiciones↔vp; edge dup-con-calificador vacío hoy, candado VP 0 flips corpus-wide). `panel_cprima` ya dedupeaba para su gate (L1337) → selección c′ invariante por construcción. Espec = `poc_b168_dedup.py` v0.1 (H217/; candados E0 A==publicado 5894/5894 · E0'' réplica==real 5894/5894 · VP 0 flips; baseline `poc_b168_dedup_baseline.csv`). **FLIP-SET SELLADO {329_p2063}**: nj 4→2 · n_titulares 4→2 · vp sin flip (B087 re-aplica idéntico) · posiciones sin cambio (el dict ya dedupeaba). Ciclo: diff adjudicado contra git 1 fila casos / −2 filas votos (27.902→**27.900**), 0 fuera del set · textos/zonas/editorial byte-idénticos (blind por construcción, clave n300 ausente del git status) · ripple normas.csv ⊆{2063} (extraer_normas consume votos; la fila dup de Zaffaroni con texto se extraía dos veces) · regresión propia post-fix E0 5894/5894 + E0'' == {2063} + flip-set VACÍO · **gate M55 pool 29→30, alta única {2063} demostrada por Compare-Object contra el baseline H216**. Constancia de método: el regolden se corrió ANTES del consciente (bloque de comandos del asistente sin el paso bloqueante — familia H191); el candado anti-H178-2 del orquestador ABORTÓ con «producción stale» y el ciclo se reparó fix-forward. **CONSTANCIA MAYOR del cierre: el dedup cierra el CONTEO, no el caso — y el caso resultó no ser el caso: 329_p2063 está MAL LOCALIZADO (su bloque es la cola de Rusillo/329_p2062) → B173.** El nj=2 publicado es el conteo honesto de un bloque ajeno. **RESIDUAL de esta entrada tras el dedup: SOLO (i) clase E wrap-truncado (21) — CERRADA la MISMA SESIÓN, ver CIERRE H217 (clase E) abajo**; (iii) headers conjuntos y (iv) Tyden/Corchuelo siguen como constancias de otras familias.

**CIERRE H217 (clase E, peek de wrap) — parser v40.0→v41.0 (MAJOR):** residual (i) CERRADO con rinde **20/21**. Superficie primero (`superficie_b168e.py` v0.1, H217/): pool truncado corpus-wide **26 = los 21 E sellados + 5 FP** («VOTO» pelado de índice/voces de sumario 347-1); **discriminador sellado = JOIN contra RE_VOTO_HDR/RE_DISID_HDR reales: los 5 FP no joinean → 0 FP por construcción; 2.889 headers completos intactos (el peek ni corre); join matchea 20/21**. Lectura Gate 2: `detectar_votos` YA extiende `header_completo` con offset-loop (+3 líneas, saltea vacías, frena en Considerando) para extraer el juez — los headers completos de 2 líneas ya vivían así → la clase E no necesitaba mecánica nueva aguas abajo, solo que el match inicial dispare. Fix = helper `_hdr_tipo_wrap(stripped, cola_wrap)` (fuente única; cola estricta k+1, espejo H207) cableado en los TRES consumidores (breaks del colector L1198 · detectar_votos L1455 · zonificador Pasada 1 L3420). Espec = `poc_b168e_peek.py` v0.3 (H217/; brazos A/B = código real instalado vs código real parcheado en memoria — cero réplica de lógica; harness espejo verbatim de main(); candado E0 A==publicado 5894/5894 en 39 columnas). **FLIP-SET SELLADO 20 = 20/20 TP ⊆ E20** (`poc_b168e_peek_baseline.csv`): nd +1 los 20 (Δ+20 exacto, nsv +0) · wc_mayoria/wc_votos SOLO en 11 (ivi None→k: la disidencia tragada era el único voto separado y su texto se contaba como MAYORÍA — la reparación real) · vp 0 flips (la disidencia ya se conocía por el calificador de la firma) · votos ±0 filas (el disidente ya tenía fila) · firma_raw/por_ello/considerando/dictamen 0 diffs en el dict completo → **textos.csv BYTE-IDÉNTICO (sha f2dcf8fa46f3 invariante v40→v41) → blind por construcción, clave n300 ausente del git status** · **riesgo 1457 VACÍO CON DATO: 0 flips de outcome/is_merit/es_queja/is_originaria**. Enmienda de contrato declarada en la adjudicación: la re-partición de texto_voto alcanza a los 20, no solo a los 11 — 1092 pobló el texto de Argibay con aritmética exacta (fila vieja −544 wc == fila nueva +544); la cota real ⊆{20} se sostuvo. Ciclo: diff consciente adjudicado fila a fila contra el baseline + conjunto completo por git (casos 20/0 fuera · votos ⊆20/0 · zonas ⊆20/0, +12 filas netas por splits voto_separado) · regolden [CLEAN] 65 · gate M55 pool 30 SIN movimiento · regresión propia post-fix «parche no aplica → convergencia declarada». Ripple normas ADJUDICADO: 8 casos = 2063 ∪ 7 de los 20 (texto_voto nuevo cita leyes → ámbito voto), 0 fuera — cerró de paso la hipótesis pendiente de U2. **RESIDUAL FINAL DE B168: 329_p5741 ÚNICO** — sub-clase E+vocab («DISIDENCIA DE LOS ⏎ DOCTORES CARLOS S. FAYT Y E. RAÚL ZAFFARONI.»; el join tampoco matchea: el header salta directo a DOCTORES sin cargo del grupo). Ensanche del grupo cargo a «Doctor» MUERTO CON DATO (grep del operador: 16 citas narrativas line-initial «voto/disidencia del doctor X…» que el match ensanchado agarraría; y la ZONA no las saca — el zonificador corre en Pasada 1 pre-zonas y detectar_votos no excluye cuerpo). Constancia lateral 5741: el next2 («Visto el expediente Nº 2.667/2004 del registro de la Administración General…») sugiere resolución administrativa publicada entre fallos — observación, no se abre. Constancia de método: dos iteraciones del PoC quemadas por schema inventado (columna `source_file` inexistente en fallos_localizados.csv; clave de headers por filename en vez de `(tomo, archivo)`) — Gate 1 violado por el asistente, reparado leyendo main() completo + verificando los headers de los CSV en disco (regla REE reafirmada por el operador).

### B169 — Zona firma FALSA POSITIVA de PROSA densa en nombres: sobrevive los guards B148 y c′ la consume (3 costos aceptados del fix B166 (c)) — ADJUDICADA H206: clase = 3, costo sostenido, fix diferido

**Componente:** parser (zonificador Pasada 1 `firma_linea` / `linea_es_firma_de_juez` sobre prosa; consumo nuevo en `panel_cprima`).
**Origen / fuente del diagnóstico:** H205 (adjudicación del flip-set de la réplica B166 (c): los 3 casos donde el fix EMPEORA comparten mecánica).
**Causa raíz:** líneas de PROSA con enumeración de jueces (cita de precedente con panel, recusación/excusación, «ha perdido virtualidad… inoficioso») pasan `linea_es_firma_de_juez` — el residuo-prosa B148 corre por línea y la línea individual parece firma — → zona firma espuria TEMPRANA en el bloque → c′ (primera zona post-apertura que parsea panel) la elige antes que la firma real.
**Diagnóstico / evidencia:** `confirmado_cuantificado` sobre el flip-set H205 — 3 casos con valor conocido-equivocado publicado en v36.0: 339_p270 nj3→2 (prosa de recusación 2.175 chars; «Luis Lorenzetti» wrapeado sin RICARDO no matchea el patrón) · 329_p3148 nj9→3 (cita «(voto de los jueces Moliné O'Connor, Belluscio…)») · 329_p4446 nj7→9 (infla Zaffaroni/Fayt desde prosa de excusación). Costos ACEPTADOS con constancia en el ciclo H205 (31 TP vs 3): baseline `poc_b166_c_replica_out.csv`. 339_p270 quedó visible en el pool M55 post-H205 (la alta única adjudicada).
**Validador propuesto:** superficie corpus-wide de zonas firma cuyo `collect_firma_lines(zona_ini)` devuelve raw largo con cola de prosa (los 3 malos: 428/866/2.175 chars vs firmas reales compactas) ANTES de diseñar el discriminador — dimensionar cuánto más hay de la clase fuera de estos 3.
**Estado del fix:** no diseñado. Candidatos: (a) discriminador sobre el RAW colectado (no la línea — la línea ya pasó B148), p.ej. exigencia de forma-de-firma en el arranque del raw; (b) endurecer la zonificación de firma_linea sobre prosa (riesgo FN, medir primero). NO parche ad-hoc por caso.
**Referencias cruzadas:** B166 (c) (consumidor que la expone), B148 (guards que la dejan pasar), B160/B168 (familia colector), M55 (339_p270 en el pool). H205. Sin ID histórico.

**SUPERFICIE H206 (adjudicada — COSTO SOSTENIDO, fix DIFERIDO):** `superficie_b169.py` v0.2 (H206/, read-only, canónicos como insumo + parser importado como fuente única; candados E0t-tiling 0 · E0c pick-script == `panel_cprima` real 0 · **E2 corpus-wide: pick c′ == `b_firma_raw` del baseline H205 en TODO caso, 0**; baseline `superficie_b169_out.csv`, 9.752 runs). **Clase sobre PICKS = exactamente los 3 costos conocidos, 0 miembros nuevos**: picks con señal (chars≥300 ∧ frac_lower4≥0,30 ∧ sin dup) = 5, y los 2 no-anclas se adjudican con lo ya sellado (348_p1751 = clase-A curada H205, firma real de 3 + cola «en los términos…» → familia colector B160/B168, la zona es genuina; 329_p5317 = enumeración de conjueces sobreviviente por-diseño B148, TP del flip-set). Franja corta 150-299 barrida: 6, todas firmas legítimas cargadas de «según su voto» (piso de frac de firma-con-calificadores, no prosa). Escenario «clase chica, no amerita fix» del PROMPT_H206 confirmado: el discriminador queda diferido SIN unidad propia; los 3 costos siguen con constancia y 339_p270 visible en el pool M55. **DESTAPE (dato del prerequisito para C/M57):** runs de zona firma NO-pick = 4.063 → **1.243 califican panel** (nj≥2 ∨ calificador, medido con funciones del parser) · 807 con raw≥300 chars · ~86% de esos 807 con head clase header-de-voto-wrapeado + «Considerando:» (clasificación heurística del instrumento, SIN adjudicar caso a caso) — pool latente INERTE hoy (c′ ya pickeó una zona anterior) pero letal para cualquier consumidor nuevo de bordes de zona firma (opción C «frená donde termina la zona firma», consumidores M57). H206.

**RATIFICADA H217 (decisión del operador — U3 del PROMPT_H217, read-only):** costo sostenido, discriminador sigue diferido y acoplado a C/M57. Dato fresco de la ratificación: los 3 vigentes e idénticos bajo v40.0/v41.0 (339_p270 nj=2 · 329_p3148 nj=3 · 329_p4446 nj=9 — verificados al abrir la sesión; ni el dedup ni el peek tocan la mecánica por construcción); 339_p270 estable en el pool M55 (30) por cuarta corrida consecutiva del gate — ruido conocido del radar, 0 crecimiento de la clase desde H206.

### B170 — `detectar_votos_disidencias` FP: cita de precedente «disidencia de los señores jueces…» wrapeada a inicio de línea dispara header → `inicio_votos_indiv` espurio

**Componente:** parser (`RE_HEADER_VOTO_DISIDENCIA` / `detectar_votos_disidencias`; consumidores del ivi: techo T1/T2 del resolutor, `wc_mayoria`/`wc_votos`, conteos de votos).
**Origen / fuente del diagnóstico:** H206 (adjudicación por extracto del pool B166 (b1): 348_p728 leído entero).
**Causa raíz (leída en 348_p728):** la cita «…(Fallos: 342:1061, ⏎ disidencia de los señores jueces Maqueda y Rosatti).» wrapea y la segunda línea arranca con la forma de header de disidencia → ivi = 307 ESPURIO. Familia wrap-a-inicio-de-línea (B159/B167, ahora sobre el detector de votos).
**Efecto medido en el testigo:** el techo T1/T2 aplicado con el ivi falso EXCLUYÓ el dispositivo real (L364, la opinión principal Rosatti+conjueces) de la ventana → T1 falló POR el FP y el caso quedó en el pool expuesto (b1); T3-sin-techo lo rescató (outcome `competencia` CORRECTO — el TP que mata el techo duro, constancia sub-clase C). El FP ya daña el recall de T1 hoy y contaminaría cualquier consumidor futuro de pertenencia/techo (advertencia espejo de B169→C/M57: consumidor nuevo de señal con FPs).
**Diagnóstico / evidencia (H220, medido corpus-wide):** superficie `superficie_ivi_b170.py` v0.3 (H220/, read-only; candados E0 réplica==publicado **5737/5737** — universo = 5894 − 157 stubs `sumario_con_link`, que producción no evalúa — · E0'' barrido propio == función real · E1 sin-zonas 191 == `sumario_con_link` 157 ∪ `sumario_editorial` 34; baseline `superficie_ivi_b170_out.csv`): **2879 matches** del candidate-set de producción = prev_cerrada 2231 · **prev_abierta 647** · **prev_fallos 1** — la sub-clase LITERAL del testigo (línea previa con «(Fallos:»/paréntesis colgante) es **cardinalidad 1 corpus-wide**; la clase ancha es prev_abierta (línea previa sin «.»). 648 sospechosas en 612 casos; **el ivi se mueve en 496 casos si la clase cae, y en los 496 el techo T1/T2 estaba ACTIVO** — COTA SUPERIOR de daño, no daño adjudicado: prev_abierta puede contener headers genuinos tras prosa sin punto (OCR). Consumidor adicional leído en código, no declarado arriba: guard de vp L4504 (`unanime ∧ wc_mayoria≤4 ∧ wc_votos>wc_mayoria`) — un ivi espurio temprano puede flipear `voting_pattern`.
**Validador EJECUTADO (H220):** el propuesto, tal cual. Sucesor: adjudicación por lectura de muestra estratificada de `prev_abierta` (647) — decide si el pool es daño real o se desinfla.
**Estado del fix:** no diseñado. NO se diseña discriminador hasta adjudicar `prev_abierta`.
**Referencias cruzadas:** B166 (b1) (consumidor del techo, testigo 728 en su pool), B159/B167 (familia wrap-FP), H190-b (`_es_continuacion_wrap`, patrón espejo), M57 (advertencia de consumidores). H206. Sin ID histórico.

### B171 — Tipología de votos Tipo A: `RE_TIPO_A_REMISION_DICTAMEN` subcuenta remisiones al dictamen

**Componente:** parser (clasificador de tipo de voto, catálogo Tipo A L1683-1711).
**Origen / fuente del diagnóstico:** H208 (smoke del instrumento de dictamen contra testigos de la muestra sin_ancla).
**Causa raíz:** el catálogo no cubre al menos dos fórmulas frecuentes: «a cuyos términos corresponde remitir» (la rama anafórica solo acepta «al que/cual cabe remitir») y «comparte el dictamen» a secas (la rama comparte exige «los fundamentos»). H220 suma DOS mecanismos con dato: la variante «cuyos términos SE DAN POR REPRODUCIDOS» (sin «remitir»; verbatim de 339_p1683) y el **soft hyphen `\xad`** que parte «dic­tamen» y enmascara incluso fórmulas YA cubiertas — `clasificar_tipo_voto` trabaja sobre `texto_voto[:4000]` SIN des-guionar (L1965).
**Diagnóstico / evidencia (H220, medido corpus-wide NIVEL VOTO):** superficie `superficie_remision_b171.py` v0.1 (H220/, read-only; E0 clasificador réplica==publicado **27.900/27.900** — réplica exacta desde votos.csv: cabeza=`texto_voto[:4000]` ⊆ el `[:5000]` publicado; baseline `superficie_remision_b171_out.csv`): **75 matches, COTA INFERIOR** (el soft-hyphen también ciega al instrumento) = **F1 «a cuyos términos» 43** (publicado: `indeterminado` 32 · D 5 · A 6; **riesgo B067 MEDIDO: 16/43 en contexto precedente/ambos** — cita Fallos ±300; un ensanche de A se los robaría a E, que va DESPUÉS en la cascada L2048) + **F2 «comparte el dictamen» 16** (`indeterminado` 12 · A 4; el FP a mirar acá es la NEGACIÓN — la forma se auto-ancla, el guard de contexto no le aplica) + **F3 anafórica-«corresponde» 16 en 15 votos** (`indeterminado` 8 · D 5 · E 1 · B 1 · A 1 — los 5 D piden lectura). **Testigos originales 4/4 adjudicados con texto: NINGUNO tiene votos separados** (todas sus filas de votos.csv = mayoria / wc 0) — sus fórmulas viven en `considerando_text` (mayoría) → **el bug tiene DOS NIVELES: (i) voto separado (tipología H2, medido acá) y (ii) remisión de la MAYORÍA al dictamen (consumidores M58 / conteo de remisiones tesis-H2 / `lectura_dictamen`), SIN medir corpus-wide.** Corrección de cita: 339_p1683 no dice «a cuyos términos corresponde remitir» sino «comparte los fundamentos y conclusiones del dic­tamen … cuyos términos se dan por reproducidos en razón de brevedad». El 438/1104 del pool sin_ancla (H208) es de OTRA población (nivel caso, `dictamen_head`) — no comparable con el 75.
**Estado de verificación:** `confirmado_cuantificado` (nivel voto); nivel mayoría `confirmado_caso_testigo` (los 4 testigos).
**Validador EJECUTADO (H220, nivel voto):** el propuesto, tal cual — y el patrón B067 quedó confirmado con número (16/43 de F1 en contexto precedente). PENDIENTE: superficie hermana nivel mayoría (mismo instrumento sobre `considerando_text`, con des-guionado).
**Estado del fix:** no diseñado. El diseño tiene que contemplar: guard de contexto anti-E (los 16/43) · variante «se dan por reproducidos» · des-guionado de la cabeza · negaciones en F2.
**Referencias cruzadas:** M58 (consumidor de la señal), tipología de votos (insumo H2 de la tesis). H208. Sin ID histórico.

### B172 — `find_case_name` (fallback de `case_name_cuerpo`) sin guards de forma: devuelve citas y fragmentos del dictamen — CERRADO H218

**Componente:** parser (extractor de carátula del cuerpo).
**Origen / fuente del diagnóstico:** H214. Detectado al intentar usar `case_name_cuerpo` como clave de búsqueda para el crosswalk caso→expediente del sistema de gestión (frente circulación). Desprendimiento de **B008**, cuya referencia cruzada «sigue con bug — ver B011» apunta a la entrada EQUIVOCADA (B011 es el bug catalográfico de `344_p344`, sin relación) — por eso este bug nunca tuvo ticket propio ni magnitud medida.
**Causa raíz (leída en código, parser v38.0):** L4105 `case_name_cuerpo = case_name_cuerpo_v1 or case_name_cuerpo_legacy`. La primaria `extraer_caratula_v1` (L313) busca `^Vistos los autos: "X"` HACIA ADELANTE desde `apertura_rel`; cuando el fallo abre con otra fórmula —típicamente `Autos y Vistos:` de las resoluciones de competencia— devuelve `""` y cae a `find_case_name` (L1012), que escanea HACIA ATRÁS desde la apertura —o sea, hacia adentro del dictamen— y devuelve la PRIMERA línea que contenga `c/` (max_back=15) o `c/`|`s/` (max_back_fallback=60), **sin ningún guard de forma**: no exige que la línea parezca carátula, no descarta firmas, no descarta citas.
**Diagnóstico / evidencia (medido en disco sobre `csjn_casos.csv`, 5894 filas; la rama es identificable porque `cuerpo == legacy` ⇔ V1 devolvió vacío):** V1 acierta 3795 (64,4%) con 9 señales de daño (**0,24%**) · **fallback `find_case_name` 1171 (19,9%) con 423 (36,1%)** · vacío 928 (15,7%) → B106. Señales (PISO, no estimación — no atrapan el fragmento con forma válida): arranca en minúscula 366 · arranca con `s/` 60 · arranca con número 40 · contiene identificador clásico `L. NNN. ROMANO` 27 · menciona «sentencia del N» 20. Testigo `339_p1373`: carátula real `Riv S.A. y Otro s/ Infracción ley 24.144` (correcta en `case_name_indice`), pero `case_name_cuerpo` = `1075, L. XLV, "Da Silva, Jorge s/ presunta infracción al régimen penal",` — una `Comp.` CITADA en la sección III del dictamen, devuelta verbatim con la coma final. Otros: `330_p2799` (trae «Vistos los autos: "López, José Florencio c/ ANSeS…"», que es OTRO caso) · `340_p1660` (trae la cita `CSJ 00602/2016/CS1, "Pino, Evangelina Magalí c/ ACA Salud"`) · `329_p4418` · `339_p1388`.
**Corrección de constancia (BITACORA L1622-1624):** analizando `330_p829` se concluyó que «como Fix 1 prima V1 sobre find_case_name, el daño no llega al CSV». Vale SÓLO cuando V1 dispara; en los 1171 casos donde no dispara, el daño llega al CSV. La observación de un caso quedó generalizada a la población equivocada.
**Propagación medida:** `case_name_cuerpo` alimenta DOS derivaciones — `classify_queja(por_ello_text, case_name_cuerpo)` (L4179) y `es_originaria(case_name_cuerpo, …)` (L4243, con ripple a `is_merit` por la rama originaria del gate). NINGUNA variable de votación lo consume (`voting_pattern`, `n_jueces`, `n_disidencias`, `n_votos_svoto`, `jueces`, `posiciones`, `outcome` vienen de otros extractores). **Hallazgo de COBERTURA, más grave que el ruido:** el ancla `RE_CARAT_QUEJA` aparece en el **54,0%** de las filas de V1 y en el **0,0%** de las otras tres ramas — no es «menos», es NUNCA: `find_case_name` devuelve una línea suelta y la fórmula ritual completa no sobrevive. En el 35,6% del corpus donde V1 no dispara, la capa carátula de `es_queja` (los +225 flips de H108) NO PUEDE contribuir y `es_queja` depende sólo del `por_ello`. Punto ciego con forma, no ruido aleatorio: cualquier cruce de `es_queja` contra otra variable va a mostrar una correlación del detector.
**Nota de alcance — la rama NO es muestra aleatoria del corpus:** V1 dispara sobre la fórmula de las sentencias de fondo; las resoluciones cortas de competencia abren `Autos y Vistos:`. Medido: `is_merit_decision` 74,7% en V1 vs 6,7% en fallback y 6,6% en vacío. El daño está concentrado en una población que además es de otra naturaleza → **el subcorpus de MÉRITO, que sostiene el análisis de configuraciones de voto, es el limpio.** Reportar así en el CODEBOOK, partido por rama, NO como tasa global.
**Estado de verificación:** `confirmado_cuantificado` (rama identificada, magnitud medida, mecanismo leído en código, propagación medida).
**Validador propuesto:** re-correr la partición por rama (`cuerpo == legacy` ⇒ fallback) más las cinco señales; post-fix el daño dentro del fallback debe caer y la partición V1/fallback/vacío NO debe moverse (si se mueve, el fix está induciendo vaciados nuevos).
**Estado del fix:** DISEÑADO, no implementado. El discriminador ya existe en el proyecto: `_es_caratula_v2` (L2458) y `linea_es_firma_de_juez` se construyeron en H200 (sub-frente D1) para EXACTAMENTE esta clase de falso-match-hacia-atrás en `refinar_inicio_por_titulo`, y `find_case_name` nunca los recibió. El fix es aplicar el mismo guard en la otra función con **fallback al comportamiento actual** (patrón D2/H201: guard-con-fallback, NO rechazo duro — en D1 el rechazo duro mató 46 pistas). Precedente de costo: en H200 el guard de firma mató la carátula legítima del juez-PARTE (`329_p1303`, Boggiano) → clase conocida, ver B165/D4.
**Contrato esperado del ciclo:** flips de `case_name_cuerpo` ⊆ rama fallback · `case_name_indice` sin cambio · `es_queja`/`is_originaria` pueden moverse ⊆ flip-set y se adjudican · `is_merit` puede moverse por la rama originaria del gate · candado BLIND obligatorio (clave n300) · `derivar_partes` re-deriva (consume `case_name_cuerpo` en el PASO 4).
**Referencias cruzadas:** **B008** (el fix que dejó el fallback con el bug viejo, con el puntero roto) · **B106** (hermano: `case_name_cuerpo` vacío con «Vistos los autos» presente — magnitud medida acá: 928 filas, 15,7%) · B011 (referencia ERRÓNEA de B008, sin relación) · H200/D1 y H201/D2 (guards y patrón guard-con-fallback) · B165/D4 (juez-parte, costo conocido) · M29/`derivar_partes` (consumidor aguas abajo) · H108 (capa carátula de `es_queja`). H214.

**ENMIENDA H215 — el fix se redirige de (A) guard-con-fallback a (B) eliminar el fallback.** Medido sobre el mismo `csjn_casos.csv` (5894 filas) verificado en disco.

*(a) La carátula nunca estuvo perdida.* `case_name_indice` = **0 vacías en 5894**. El identificador está completo en todo el corpus y viene del catálogo. Lo que `case_name_cuerpo` intenta recuperar no es la carátula sino los AUTOS COMPLETOS («Recurso de hecho deducido por X en la causa Y»), que es lo que `RE_CARAT_QUEJA` necesita y lo que el índice —en forma invertida de catálogo— no tiene. Consecuencia: las variantes de fix «repoblar desde el índice» y «repoblar desde el encabezado de página» quedan sin objeto para el problema de identificador.

*(b) El fallback no compra cobertura.* `is_originaria`=1 por rama: **V1 165/3795 (4,3%) · fallback 263/1171 (22,5%) · vacío 168/928 (18,1%)**. Con `case_name_cuerpo` VACÍO la detección de originaria rinde prácticamente lo mismo que con el fallback. Y dentro del propio fallback, las filas dañadas dan **24,5%** contra **20,9%** las sanas: indistinguible. La tasa alta es propiedad de la POBLACIÓN (las resoluciones de competencia y las originarias abren «Autos y Vistos:», que es justo lo que manda al fallback), no del campo. **Corrección de constancia:** en H214 se conjeturó que la rama podía estar sosteniendo `es_originaria` — medido, no lo hace.

*(c) ★ Y sí inyecta dependencias falsas.* **34 filas del fallback tienen la palabra «Originario» dentro del string contaminado** —porque el fragmento capturado es la cita de un precedente originario— y en las 34 `is_originaria=1`: **34/34 = 100%**, contra 20,1% de base en la misma rama. En V1 y en vacío hay **0** filas con esa palabra en el campo. Un 100% contra un 20% no es efecto de población: es dependencia mecánica del flag respecto de una subcadena de OTRO caso. Perfil: `outcome` competencia 23 / otro 8 / sin_dispositivo 1 / desestima 1 / hace_lugar 1; `is_merit_decision=1` en **0**; 16/34 conservan marcador de cita visible. Testigos: `329_p3065` («in re M. 466. XXIV. Originario. "Muñoz, Ana Rosa c/ Buenos Aires»), `330_p4678` (Barreto), `331_p994` (Galiano). **CAVEAT: no son 34 FP declarados** — `330_p2610` (Mendoza c/ Estado Nacional, Riachuelo) es originaria genuina, y en las 23 con `outcome=competencia` declarar la propia competencia originaria es el resultado esperable. La PROCEDENCIA está rota en las 34; la corrección material exige adjudicarlas una por una.

*(d) Tampoco ayuda a `es_queja`.* Por rama: V1 2168 (57,1%) · fallback 94 (8,0%) · vacío 39 (4,2%). Dentro del fallback, dañadas **4,8%** contra sanas **10,4%** — el string contaminado degrada la señal en vez de aportarla, consistente con el 0,0% de `RE_CARAT_QUEJA` ya medido en H214.

*(e) Fix redirigido.* **(B) eliminar el escaneo hacia atrás.** `case_name_cuerpo = case_name_cuerpo_v1` y punto: el campo pasa a significar «autos completos, cuando el fallo los trae con la fórmula ritual», que es honesto y auto-descriptivo. Vacías 15,7% → ~35,6%. El identificador lo sigue dando `case_name_indice`, completo. **Prerrequisito NO NEGOCIABLE:** columna de procedencia propia **`case_name_cuerpo_fuente`** ∈ {`v1`,`fallback`,`vacio`}, porque hoy la partición solo es reconstruible por el accidente `cuerpo == legacy` y L4101 marca `case_name_cuerpo_legacy` como «eliminable en una corrida posterior» — si esa columna se borra antes, las filas dañadas se vuelven invisibles otra vez. **Flip-set de adjudicación del ciclo = las 34.** El ciclo deja de ser MAJOR sobre `find_case_name` (guards, fallbacks, costo del juez-PARTE de B165/D4) y pasa a ser un borrado + una columna.

*(f) Número no comparable, declarado.* El screen de daño usado en H215 (minúscula inicial · `s/`|`c/` inicial · dígito inicial · identificador clásico · romano inicial · «sentencia del N») midió **497/1171 = 42,4%** en el fallback. **NO es el mismo screen que las 423 (36,1%) de H214** — distinta definición de señales. Los dos números conviven; no fusionarlos ni presentarlos como serie. `is_merit_decision` re-medido: V1 74,7% · fallback 6,4% · vacío 6,6% (H214 reportó 6,7%/6,6%; diferencia sin consecuencia).

*(g) Pendiente para cerrar el diseño.* El único consumidor que podría vivir del fallback es **`derivar_partes`** (PASO 4). Falta medir su rinde sobre las 1171 filas de la rama (proporción con actor y demandado / solo una / ninguna). Si predomina «ninguna», (B) queda cerrada sin discusión. H215.

**FIX APLICADO H218 — (B) ELIMINACIÓN del fallback. parser v41.0→v42.0 (MAJOR, schema 39→40).**

*Fix.* Sale el `or` de L4169: `case_name_cuerpo = extraer_caratula_v1` y punto. `find_case_name` QUEDA en el código alimentando `case_name_cuerpo_legacy`, que NO se borra en este ciclo (su borrado es decisión propia — hoy es lo único que deja leer QUÉ agarró el fallback en cada fila). Entra `case_name_cuerpo_fuente` ∈ {`v1`, `fallback`, `vacio`}, adyacente a sus hermanas en el schema.

*(g) respondido — el pendiente que H215 dejó abierto.* Rinde de `derivar_partes` sobre la rama: «ninguna» 75,9% · «solo una» 20,6% · «ambas» 3,5%. Los buckets de capa carátula dan **107 en v1, 1 en fallback, 0 en vacío**, y la causa es ESTRUCTURAL: `RE_MARK_CARATULA` es `.match()` anclado en `^(?:Recursos?|Queja)` case-sensitive — un string de la rama no puede satisfacerlo. La única vía expuesta es `_match_letrado_caratula` (L531), que no exige el marcador ritual y solo pide ` c/ `: superficie 513 filas, disparó **1 vez**, TP (347_p357 Gil Domínguez, carátula genuina en versales). Ese es el costo entero del fix sobre partes.

*Partición CONTAMINADA — corrección de constancia.* La rama se venía identificando por el accidente `cuerpo == legacy` (1171 filas). Corriendo `extraer_caratula_v1` REAL sobre el bloque reconstruido (`superficie_c_v1.py` v0.2, candado E0 V1==""), la partición real es **fallback_real 1160 + v1_coincidente 11**: en esas 11, V1 SÍ disparó y `find_case_name` devolvió la carátula del MISMO caso desde el dictamen. Sin la columna de procedencia la rama no es auditable — ese es el argumento duro que la justifica, más allá del que dio H215.

*Daño re-medido por dos métodos independientes.* Screen de forma **42,6%** (499/1160, con dos señales nuevas: abre con comilla de cita 46 · ley ausente del índice 17) y overlap de tokens contra el índice **44,6% de OTRO caso** (517/1160; 45,8% del propio, 9,7% dudoso). Dos instrumentos sin señales en común aterrizan en el mismo lugar.

*★ CORRECCIÓN DE CONSTANCIA a la enmienda (c) de H215 — era FALSA.* El «34/34 = 100% contra 20,1% de base» se leyó como dependencia mecánica del flag respecto de una subcadena de otro caso. Es **correlación**: las 34 son originarias genuinas y el texto lo dice por su cuenta — las señales 1/2/4 de `es_originaria` disparan solas en **33/34** (la 34ª por la señal 5), verificado cargando las regex verbatim del parser sobre `textos.csv`. La causa mecánica es el ORDEN: en L1633-1639 las señales 1 (`RE_COMPETENCIA_ORIGINARIA`) y 2 (art. 117 CN) se evalúan ANTES que la 3, y en esta población el considerando dice «competencia originaria» o cita el 117. **La carátula nunca llega a decidir.** El «flip-set de adjudicación = las 34» que anunciaba H215 queda sin objeto.

*CORRECCIÓN a la propagación de H214 — eran CINCO consumidores, no tres.* El grep sobre `scripts/{pipeline,validacion,tests}` suma `derivar_materia` (L91 columnas requeridas, L524 `caratula = case_name_cuerpo + " " + case_name_indice`) y `extraer_normas` (L17/L90/L136, ámbito `caratula`, idéntica concatenación). Ninguna variable de votación lo consume — eso sigue en pie.

*Decisión: NO se toca ningún consumidor.* Se evaluó cambiar `extraer_normas` para leer solo el índice y se **descartó con dato**: costaría **106 pares legítimos** de la rama v1 (filas donde el cuerpo trae una ley que el índice no) para ganar 17 contaminados. Se arregla el campo, no el consumidor (fuente única).

*Flip-set del ciclo = VACÍO, medido.* `poc_b172_flipset.py` v0.1 (H218/), brazos A/B con `es_originaria` / `classify_queja` / `es_revision_fondo` reales sobre el bloque; candados E0 orig/queja/merit **1171/1171**. `is_originaria` 0 flips · `es_queja`/`queja_resultado` 0 flips · 0 ripple a **`tribunal_origen_status`** (L4309-4316 — columna que esta entrada NO declaraba), `is_merit_decision` ni `tipo_voto_sep`. La partición vieja habría aportado 0 flips espurios.

*Ciclo.* `--consciente` → `check_regresion` `[FAIL]` SOLO por header (**y ahí corta: nunca compara celdas** — su FAIL no demuestra nada, constancia de instrumento) → adjudicación celda a celda con `verificar_h218.py` (H218/, PRE extraído de `git show HEAD`), **[CLEAN] en los 6 candados**: schema 39→40 con única columna nueva · `fuente` v1 3806 / fallback 1160 / vacio 928 · `case_name_cuerpo` vaciada en 1160 exactas y 0 a otro valor · set == `fallback_real` exacto · **0 diffs en las otras 38 columnas** · las 11 `v1_coincidente` intactas → `--regolden` → invariante golden==producción [OK] en los 5 → manifest **[CLEAN] 65**.

*Sha post-ciclo.* casos `46abe17fe666` (era `d22dd411f633`) · textos `f2dcf8fa46f3` · votos `5df40a20cfd5` · zonas `0a5b1b650d47` · editorial `30a6da652e3a` — los cuatro últimos **idénticos al sello H217**: byte-idéntico demostrado, no declarado. Clave n300 byte-idéntica (`build_m20` corrido, ausente de `git status`) → blind vigente, sin acople con M43. Gate M55 sin movimiento (el fix no toca panel).

*Derivers.* `epilogo.csv` y `recursos.csv` byte-idénticos · `partes.csv` 1 fila · `normas.csv` 13.926→13.908 (−18 pares, todos `ambito=caratula`, INERTE para la cascada por el doble filtro L506-511 de `derivar_materia`) · `materia.csv` 87 filas (`poc_b172_materia.py` v0.1, candado E0 5894/5894, baseline `poc_b172_materia_baseline.csv`) = **61 reparaciones · 4 costos · 19 no-ops · 2 dudosos**; 45 de las 53 que pierden materia tenían string de OTRO caso. Deltas de producción EXACTOS contra las transiciones del PoC: cobertura 78,5%→77,5%, `sin_clasificar` 1084→1133 (+49 = 53−4), `lectura_texto` 1472→1420, `lectura_dictamen` 77→80, `conflicto_capa2` 57→53.

*COSTOS ACEPTADOS 4, nombrados.* `329_p1243` Blomqvist · `330_p1183` Pavicich · `331_p357` Lawny — los tres pierden `objeto(1)` porque el `s/ objeto` vive SOLO en el cuerpo: el índice está en forma invertida de catálogo y no lo trae. `331_p125` Correa — el cuerpo dice «ESTADO NACIONAL» y el índice abrevia «E.N.», el trigger estatal no dispara. **`343_p1386` Alessandria NO es costo sino REPARACIÓN:** el fragmento «PROVINCIA de BUENOS AIRES» sin el «Banco» delante burlaba `_RE_BANCO_PROV` (H114), guard que existe justamente para no rutear a CA — el fix restaura el comportamiento diseñado. **`343_p1237` Hug es bug del índice, no costo del fix** (`Federals/` sin espacio rompe el `\b` de `_RE_SOBRE`) → ticket propio.

*Regresión propia post-fix.* `poc_b172_flipset --esperar-version 42.0` → E0 1171/1171 + flip-set VACÍO (ambos brazos convergen: con el `or` eliminado, el brazo B ES el código nuevo). `superficie_c_v1 --esperar-version 42.0` → `fallback_real` 1160 sin mover clases.

*RESIDUAL con superficie medida — frente (C), ensanche de V1.* `superficie_c_v1.py` v0.2 clasifica las 1160 por CAUSA de que V1 no disparara: **`F2_otra_formula_SIN_caratula` 1089 (93,9%) con `ritual: 0`** — la apertura «Autos y Vistos: / Por los fundamentos y conclusiones del dictamen…» **no nombra los autos en ningún momento**: techo estructural, no hay nada que ensanchar. Recuperable ≈41: `B_formula_sin_dos_puntos` 15 · `F1_otra_formula_CON_caratula` 10 · **`Z_caption_anidado` 9** · `S_formula_singular_u_ocr` 6 · `E_caratula_sin_comillas` 1. La clase Z es un bug del reconstructor de V1, no de su regex de entrada: con caption anidado `Vistos los autos: "'X'; Y"` abre en la comilla doble y encuentra la simple en posición 0 → `acumulado[:0] == ""`; la fórmula matchea perfecto y devuelve nada.

**Referencias cruzadas nuevas:** H218 · `superficie_c_v1.py` / `poc_b172_flipset.py` / `poc_b172_materia.py` / `verificar_h218.py` (todos en `scripts/diagnostico/H218/`) · B114 (`_RE_BANCO_PROV`) · H163 (orden de partes sin ancla `c/`).

### B173 — 329_p2063 (Rojas c/ Liberty ART) MAL LOCALIZADO: el bloque publicado es ÍNTEGRAMENTE la cola de 329_p2062 (Rusillo); el caso propio no lo posee ningún bloque — PARKING; gap huérfano confirmado (H219)

**Componente:** parser (localización/frontera: `refinar_inicio_por_titulo` + `detectar_fin_real`) — familia B012/B045/B147; candidato a clase B152 (colisión de página: la p. 2063 contiene el FIN de Rusillo y el INICIO de Rojas).
**Origen / fuente del diagnóstico:** H217. El dedup U2 (parser v40.0) bajó `n_jueces` de 4 (duplicados) a 2 → alta en el gate M55 → el operador leyó nj=2 como imposible y sospechó mislocalización; adjudicado contra el corpus real (greps + dump L26280–26360 de `LibroVol329.2.md`).
**Causa raíz (VERIFICADO en texto; mecanismo de tiers RECONSTRUIDO, no leído en código):** el bloque publicado [26309, fin_real 26323] arranca en el banner de la p. 2063 con `status_localizacion = ok_sin_marcador_apertura_ancla_catalogo` y **recorte 0** — `refinar_inicio_por_titulo` falló todos los tiers y el inicio quedó clavado al arranque de página, DENTRO del caso anterior. La carátula real de Rojas vive en L26325–26326 con forma actor-primero y conector wrapeado a línea 2 («MARCOS EFRAIN ROJAS ⏎ V. LIBERTY ASEGURADORA DE RIESGOS DEL TRABAJO»), divergente del índice («Liberty Aseguradora … (Rojas, Marcos Efraín c/)», él mismo wrapeado en L61099 «…(Ro-») — geometría vecina de D1b/CAT_FORMA_SIN_TOKEN (H199/H200); por qué ningún tier pescó el token en 26326 exige releer `refinar` (NO hecho — parking). El fin `sumario_siguiente` 26323 corta ANTES de la carátula real: correcto para el bloque equivocado.
**Diagnóstico / evidencia (adjudicado H217 contra el texto real):** el contenido del bloque = header wrapeado L26311–26312 «DISIDENCIA DE LOS SEÑORES MINISTROS ⏎ DOCTORES DON CARLOS S. FAYT Y DON E. RAÚL ZAFFARONI» + considerando de la disidencia + firma «CARLOS S. FAYT — E. RAÚL ZAFFARONI.» + epílogo de **Rusillo** («Recurso de hecho interpuesto por Flavia Rusillo y Marcos Rusillo…», Cámara Federal de Mar del Plata). **TODO lo publicado de 329_p2063 es de Rusillo:** sin fecha (el bloque no tiene «Buenos Aires, …»), `outcome=sin_dispositivo`, nj=2, `vp=segun_su_voto` (artefacto B087 sobre bloque ajeno), wc 121, `jueces_desconocidos` = el fragmento del header wrapeado, y hasta el `tribunal_origen` «Cámara Federal de Apelaciones de Mar del Plata» es el de Rusillo. **El caso Rojas genuino** (p. 2063–2064: «el recurso de queja no es susceptible de ser deducido por medio postal», **desestima la presentación**, 30-may-2006, **firma de 5** Petracchi–Highton–Maqueda–Zaffaroni–Lorenzetti, unánime, Cámara Federal de la Seguridad Social Sala III) tiene valor real conocido y recuperable completo.
**Estado de verificación:** `confirmado_caso_testigo`; geometría del hueco confirmada (H219, dump L26315-26350 de `LibroVol329.2.md` + casos.csv): **GAP HUÉRFANO completo, no bleed** — el Rojas genuino ocupa [26324-26347] (0-based) y ningún bloque lo posee (329_p2063 cierra en fin_real 26323; Vignolo, sucesor en p. 2064, abre en linea_inicio 26348, intacto); el genuino abre con la V1 canónica ritual («Recurso de hecho deducido por los herederos de Marcos Efraín Rojas…»). El bloque publicado de 2063 además DUPLICA la cola de Rusillo que 2062 ya posee — ambos con fin_real 26323 (solape por fin_extendido).
**Validador propuesto:** (i) ejecutado (H219; resultado en Estado de verificación); (ii) al tocar la clase, superficie corpus-wide de `ancla_catalogo ∧ recorte 0 ∧ date vacía` como screen barato de hermanos (hipótesis de instrumento, sin correr).
**Estado del fix:** no diseñado — **PARKING por protocolo (PROMPT_H217):** frente LOCALIZACIÓN, upstream del parser; se registra el testigo y NO se abre unidad. Candidato natural cuando se abra: la clase D1b/CAT_FORMA de B147 (carátula divergente índice/cuerpo) o el schema sub-página de B152.
**Referencias cruzadas:** B012/B045 (contenido fuera de todo bloque), B152 (colisión de página), B147 D1b/CAT_FORMA_SIN_TOKEN (divergencia índice/cuerpo, H199/H200), B168/U2-H217 (el dedup que lo destapó), M55 (la alta H217 = este caso, adjudicada como bloque ajeno y no como panel corto), H205 (constancia previa «la firma recuperada es la de la disidencia Fayt–Zaffaroni» — ahora se sabe que la disidencia ni siquiera era de este caso). H217. Sin ID histórico.

### B174 — Gate `apertura_rel is None` (L4193) suprime `extraer_caratula_v1` en fallos donde la fórmula ritual SÍ está presente — pool latente 6

**Componente:** parser (camino de `case_name_cuerpo`, gate previo a V1).
**Origen / fuente del diagnóstico:** H219 — los exentos del candado E0 de `poc_z_caption.py` v0.2: producción no llama a V1 cuando `apertura_rel is None` (L4193, 45 fallos); el PoC previsualizó qué devolvería V1 corriendo desde el inicio 0 sobre ellos.
**Causa raíz:** con apertura no localizada el parser saltea V1 por diseño; en 6 de los 45 la fórmula («Vistos los autos» / «Recurso de hecho deducido…») SÍ está en el bloque y V1 dispararía desde 0.
**Diagnóstico / evidencia:** [INFO] del PoC con capturas previsualizadas: 329_p988 (Dirección Provincial de Energía de Ctes.), 329_p1930 (Halper — ES el residual B165), 329_p4862 (Luchetta), 331_p1592, 334_p1193 (Gus Mig), 346_p1241 (C., J.M. c/ Swiss Medical — ES el testigo B106).
**Estado de verificación:** `confirmado_cuantificado` (pool exacto 6/45, capturas a la vista en la corrida H219).
**Validador propuesto:** el propio `poc_z_caption.py` v0.2 (H219/) — emite el [INFO] en cada corrida.
**Estado del fix:** no diseñado — llamar a V1 desde 0 sin ancla de apertura requiere SUPERFICIE PROPIA: el riesgo es capturar una V1 ajena (sumario/dictamen del vecino) en bloques con frontera degradada.
**Referencias cruzadas:** B106 (346_p1241 es su testigo; ésta es la sub-clase «no se ejecutó» que la cota H215 ya separaba), B165 (329_p1930), frente C H218/H219. H219. Sin ID histórico.

### B175 — `_RE_SOBRE`: `\b` roto por token pegado del índice («Federals/» sin espacio) — testigo 343_p1237 (Hug)

**Componente:** deriver que consume `case_name_indice` (el módulo exacto donde vive `_RE_SOBRE` NO se releyó en H219 — verificar al abrir la unidad, Gate 8).
**Origen / fuente del diagnóstico:** H218, adjudicación de los derivers del ciclo B172: «343_p1237 Hug = bug del índice (`Federals/` sin espacio rompe el `\b` de `_RE_SOBRE`), ticket propio».
**Causa raíz:** el índice trae el token pegado al separador («Federals/» sin espacio) y el `\b` de `_RE_SOBRE` no matchea → el objeto del pleito no se separa.
**Diagnóstico / evidencia:** testigo único 343_p1237; adjudicado en H218 como bug preexistente del índice, no costo del fix B172.
**Estado de verificación:** `confirmado_caso_testigo`.
**Validador propuesto:** superficie corpus-wide de tokens del índice con `s/`|`c/` pegados (sin espacio previo) antes de tocar la regex.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** B172/H218 (destape), frente C (familia de ruido de índice). Sin ID histórico.

### B176 — `RE_CONECTOR_CARATULA_V`: FP de inicial media — « V. » matchea la inicial de un nombre («Joaquín V. González»)

**Componente:** parser (detector de forma de carátula — el conector « V. » case-sensitive de tomos viejos, H200/D1; call-sites exactos NO releídos en H219, Gate 8).
**Origen / fuente del diagnóstico:** H218 (tickets nuevos del cierre; testigo «Joaquín V. González» — detalle fino en el transcript de esa sesión).
**Causa raíz:** el conector « V. » (versus, tomos viejos) es case-sensitive por diseño, pero una INICIAL MEDIA de nombre propio produce la misma superficie « V. » → una línea con un nombre puede pasar por carátula ante los guards de forma.
**Estado de verificación:** `confirmado_caso_testigo`.
**Validador propuesto:** superficie corpus-wide de matches de « V. » con contexto (inicial media vs conector real) antes de diseñar guard.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** H200/D1 (origen del conector), B147 (familia frontera/carátula), H218. Sin ID histórico.

### M57 — Zonas como fuente única de alcance para los extractores (consumidores pendientes de `_zonas_linea`)

**Componente:** parser (extractores que hoy delimitan su alcance con ventanas/regex ad-hoc en vez de consumir la zonificación).
**Origen:** H205 (conversación post-cierre, registrada H206): constancia estratégica de que el zonificador está SUBUTILIZADO respecto de su propósito de diseño — `panel_cprima`/B166 (c) fue el primer consumidor nuevo de `_zonas_linea` en el camino del panel y rindió 31 TP / 3 costos en un solo ciclo.
**Idea:** cada extractor con alcance propio pasa a consumir las zonas como fuente única. Consumidores pendientes, cada uno unidad propia con superficie + flip-set:
- **(i) Fecha del fallo** — mejora-futura YA escrita en el parser (comentario L4033-4035, v36.0, verificado H206; el PROMPT_H206 la ubicaba en L~3989): «lineas_dictamen se calcula más abajo, pero _zonas_linea ya está disponible. Mejora futura: excluir líneas con zona "dictamen" de la búsqueda de fecha del fallo». Candidata de ARRANQUE más barata del frente.
- **(ii) `find_tribunal_origen`** → escanear solo la zona epílogo.
- **(iii) Breaks del colector de firma** → frenar donde termina la zona firma (el zonificador YA etiqueta el voto_separado que el colector traga) — = opción C del PROMPT_H206; ABSORBE el fix propio de B168 (pool 12 + 2063).
- **(iv) A001** (búsqueda inversa de firma) → guardas de zona reales en vez de heurísticas propias.
- **(v) Frontera (inicio/fin del bloque)** = FUERA de alcance: requiere invertir la zonificación upstream del borde (constancia H131, «divisor de zonas = camino elegante futuro»).
**Prerequisito de calidad:** B169/B163 — cada consumidor nuevo convierte un error de ZONA en un error de DATO (constancia empírica: los 3 costos del ciclo B166 (c) son exactamente eso). Dimensionar la calidad de la zona que el consumidor va a leer ANTES de cablearlo.
**Estado:** registrada (H206), sin diseño por consumidor.
**Constancia H206 (prerequisito CUANTIFICADO):** la superficie B169 midió el pool latente que un consumidor de bordes de zona firma heredaría — 4.063 runs firma no-pick, **1.243 califican panel**, 807 con raw≥300 (~86% clase header-de-voto-wrapeado, heurística sin adjudicar). El consumidor (iii) —breaks del colector— NO puede asumir «fin de zona firma» sin discriminar esa clase. Y B170 agrega la advertencia simétrica del lado de los headers de voto (ivi espurio).
**Referencias cruzadas:** B166 (c)/`panel_cprima` (primer consumidor, precedente del patrón), B168 (consumidor iii lo absorbe), B169/B163 (prerequisito de calidad), H131 (constancia frontera-upstream), M45 (frontera), B159/H052 (guarda dictamen que el consumidor i alivia). H205, H206. Sin ID histórico.

### M58 — Canal DICTAMEN para materia: exportar `dictamen_text` (4ª columna de textos.csv) + tier dictamen en `derivar_materia`

**Componente:** parser (writer de textos — proyección de fieldnames, precedente exacto H113) + derivar_materia.
**Origen:** H208 (observación estructural del operador sobre la clase remisión-al-dictamen; rinde medido por `diagnostico_dictamen.py` v0.2, E1 0/3060).
**Evidencia (medida, no supuesta):** 3434 casos del corpus con zona dictamen (16.924 segmentos); en el pool sin_ancla 748/1104 con zona y `dictamen_presente`==zona 748/748; **rinde 215/748 con vocabulario VIGENTE** (cascada real sobre texto reconstruido; la carátula ya daba sin_ancla → aporte puro del canal); spot-check 35 ≈80% precisión — **canal norma 13/13 TP, los 4 FP todos en kw** (etiquetas hermanas y citas ajenas); 525 mudos aun con dictamen = techo del canal.
**Diseño (dictado por la estructura del error, no sellado):** (1) parser exporta `dictamen_text` — MAJOR de schema de textos.csv, ciclo completo con blind; consumidores además de materia: conflicto_capa2 (57), corroboración corpus-wide, e insumo directo de tesis-H2 (la remisión ES el mecanismo de adhesión a texto ajeno — ~3400 dictámenes estructurados). (2) `derivar_materia`: tier dictamen con prioridad SIEMPRE menor que las señales del caso propio; norma-en-dictamen = ancla fuerte; kw-en-dictamen GATEADA (≥2 votos o kw+norma); `materia_fuente` con procedencia explícita («dictamen:norma:24240») — la L de REEL. (3) Sinergia M59: con extraer_normas como etapa previa el canal norma-dictamen se LEE del sidecar (ámbito `dictamen`), no se re-extrae.
**Estado del fix:** DISEÑADO con rinde y estructura de error medidos; espera (a) validación del spot-check por el operador. **(b) decisión taxonómica RESUELTA (H209):** dos ejes — competencia_dirimente/procesal_propio = categoría TERMINAL (espejo `originaria`) + columna futura `materia_subyacente` vía voces JYC (B163); dos fases, relabel terminal primero. Sinergia M59 ya OPERATIVA: el sidecar de normas es canónico desde H209 — al exportar `dictamen_text`, el ámbito `dictamen` entra al extractor con cambio mínimo y el canal norma-dictamen se LEE del sidecar. Nomenclatura reservada: la capa se llamará `lectura_dictamen` (M49/H209).
**APLICADO (H211, los dos pasos):** **(1)** parser v37.0→**v38.0** — `dictamen_text` 4ª columna de textos.csv (join de `lineas_dictamen` stripeadas, convención verbatim de `extraer_considerando`, sin `_unhyphenate` — espejo del persistido de considerando); columna al FINAL; candados C1 (4 CSV byte-idénticos) / C2 (drop de la col nueva == golden H210 `dc724eeb…`, 5.894 filas) / C3 (cruce EXACTO texto≠''⇔`dictamen_presente` = 3.434, clase all-blank = 0) — corridos POST-HOC (convalidación estilo H210; el verificador v0.1/v0.2 falló dos veces —paths, field_size_limit— y el sello salió antes: ver M62 y BITACORA). Blind PASS (build_m20 proyecta las 3 columnas viejas; clave n300 intacta). **(2)** `extraer_normas` v1.1 (5º ámbito `dictamen` AL FINAL de AMBITOS: 6.206 pares / 1.378 normas / 2.515 casos; candado N1: subset sin dictamen == sellado `8e838c49…`) + `derivar_materia` v4.1 (MINOR, precedente H172/H181) — `clasificar_dictamen` corre SOLO sobre el residuo sin_ancla (conflicto_capa2 NO entra: evidencia H208 medida sobre sin_ancla; desempate por dictamen = unidad futura); canales norma+kw solamente; gates dictados por el spot-check H208: norma = ancla fuerte · kw sin norma exige ≥2 votos · empate→None (sin co-ocurrencia: H115 calibrada sobre texto del caso propio); `materia_fuente` con procedencia `dictamen:norma:NUM(n)` / `dictamen:kw+norma:…` / `dictamen:kw(n)`. Candado M1 PASS: **flip-set 77**, todas altas legales, 0 clasificados tocados, conflicto 57 intacto; canales: norma 44 / kw+norma 29 / kw 4. Cobertura 77,0%→**78,5%**; sin_ancla 1.104→1.027.
**Adjudicación de la muestra (operador, 25 seed 211, contra dictamen_text real): 24 TP / 1 FP (96%).** El FP: 329_p5108 (`dictamen:norma:24241`) — pleito «ticket canasta» Jujuy (laboral/empleo público); la 24.241 es la ÚNICA norma del dictamen, citada de rebote (impacto en aportes). **Primer FP del canal norma** (el 13/13 de H208 era muestra chica); mecanismo identificado: norma-ancla como argumento derivado sin competencia de otras señales. Se DEJA documentado sin parche (espejo FP-F5/H172); guard anti-rebote = unidad futura con superficie medida.
**Constancia rinde (WARN 77 vs 215):** el 215/748 de H208 midió la cascada completa sobre el texto, ANTES del diseño tiered; los gates recortan exactamente la región de los 4 FP (kw-solitaria, empates, canales partes/objeto/provincia). 73/77 altas tienen ancla de norma. Pool recuperable ~138 SI la validación del operador del spot-check (pendiente desde H208) habilita aflojar el gate kw — unidad propia con flip-set.
**Constancia acople relabel-terminal:** ~9/25 de la muestra son contiendas de competencia — la clase que la decisión taxonómica (H209, dos ejes) va a relabelear `competencia_dirimente` terminal + `materia_subyacente`. El tier les asigna hoy la materia subyacente (lo que la columna futura quiere), pero el flip-set del relabel terminal futuro INTERSECTA estas altas — declararlo en esa unidad.
**Constancia vocab_faltante (testigos leídos):** en 340_p1921 el extractor ve la 23.661 ×9 y la 23.660 ×5 y NINGUNA vota (ausentes del índice de anclas — solo 34 anclas / 2.056 normas vistas); en 344_p3469 ídem con la 26.682 (la ley del caso según el propio dictamen). Candidatas selladas: 23.660/23.661/26.682→salud (+26.396 a evaluar); al poblarlas, esas altas re-deciden (salud o empate→None).
**Constancia H212 (mediciones del gate kw — SIN veredicto: la validación del operador sigue pendiente):** el WARN «rinde 77 vs pin 215» tiene aritmética exacta: 215 = norma 44 + kw+norma 29 + **kw 138** + coocur 4, y las 77 altas = 44 + 29 + **los 4 `kw(2)`**. El pool kw se reparte **134 `kw(1)` / 4 `kw(2)`**. En el spot-check H208, **los 22 casos del canal kw adjudicados son todos `kw(1)`** (ningún `kw(2)` fue adjudicado nunca). Precisión de `kw(1)` con los veredictos como fueron sellados: **16/22**. Desagregado por materia asignada: `penal` 7/7 · `civil_comercial` 3/4 · `tributario` 0/2 · `lesa_humanidad` 0/1. Reparto del pool `kw(1)` por materia: penal 96 · civil_comercial 14 · laboral 12 · tributario 5 · lesa_humanidad 3 · ambiental 2 · consumo 2. **Acople con H213 (medido, heurístico):** 64 de los 134 `kw(1)` tienen señal de contienda de competencia en `dictamen_head` (en `penal`, 57 de 96), contra 9 de las 77 altas vigentes — el cruce firme es contra `outcome` de casos.csv, no hecho. **Límite del canal declarado por el operador:** `aduanero` es dual (penal en lo delictivo, contencioso en el resto) y el operador duda de que un regex/kw pueda dividirlo — los dos FP de `tributario` (330_p5020, 333_p1812) son casos de esa dualidad. **Sesgo del baseline:** la adjudicación H208 se hizo sobre `dictamen_head` (700 chars); leídos 7 casos contra texto completo, Claude propuso cambiar 2 veredictos (329_p4234, 333_p1857 → `laboral`), **no validados**. Insumo para la validación cuando se haga: `gate_kw_lectura.txt` (H212/). H212.

**Delta CODEBOOK redactado (H211, espera M43):** columna `dictamen_text`, valor de ámbito `dictamen`, capa `lectura_dictamen` + fuentes `dictamen:*` — texto listo en el cierre H211 (patrón tabla-equivalencias M49/H210); se ACUMULA con los deltas futuros del frente materia, no dispara refresh.
**Referencias cruzadas:** M59 (sidecar de normas; ámbito dictamen HABILITADO en esta unidad), B171 (señal de remisión), B163 (voces JYC — canal hermano), H113 (precedente del split de textos), M48 clase (1), M61 (DJA), M62 (candados-gate), decisión taxonómica H209 (acople relabel terminal). H208, H211. Sin ID histórico.

### M59 — `extraer_normas` como etapa canónica: sidecar caso × norma × ámbito (el hecho separado de su interpretación)

**Componente:** deriver nuevo (`scripts/pipeline/extraer_normas.py`) + refactor de derivar_materia + MAPA/orquestador/manifiesto (etapa nueva = los tres se mueven juntos, regla del MAPA).
**Origen:** H208 (propuesta del operador + tercero; v0.1→v0.2 diagnóstica corrida en H208/).
**Números v0.2 (medidos, candado E2 = 0/5894):** 5973 pares caso×norma×ámbito · **1393 normas distintas** · **2643/5894 casos con ≥1 norma** · ámbito caratula 411 pares / 396 casos · **gap carátula = 265 casos** (norma en carátula NO citada en su considerando = invisible a la cascada vigente). Alcance v0.2: solo «ley N» — decreto-ley entra de rebote (substring sin \b, explica 1285/58 top 323); 3251 casos sin norma alguna = headroom de decretos + ámbito dictamen.
**Diseño de la promoción (DOS pasos con candados separados; corrección del operador H208 post-cierre: extraer TODO lo ya exportado, habilitar por unidad):** **(1)** etapa canónica emite `csjn_casos_normas.csv` (caso, `tipo`, norma, ambito, n_menciones — `tipo` desde v1 aunque arranque solo `ley`) con **CUATRO ámbitos desde v1: `caratula` + `considerando` + `dispositivo` (por_ello_text, textos.csv — ya exportado, costo marginal ~0; «art. 280»/«ley 48»/«acordada 4» viven ahí) + `voto` (texto_voto, votos.csv — ya exportado; las disidencias citan normas que la mayoría no; granularidad por juez = dato de tesis: citación normativa por ministro)**; y `derivar_materia` v3.3 deja de correr RE_LEY: consume el sidecar filtrando `ambito=considerando` → **candado byte-idéntico de csjn_casos_materia.csv (sha 89eb50fef37b)**, contrato REEL de refactor puro (patrón M52/H198) — los otros tres ámbitos quedan EXTRAÍDOS PERO INERTES para la cascada; orden en el DAG: entre extraer_epilogos y derivar_materia (dependencia nueva espejo de epilogos→partes; suma insumo votos.csv); manifest pasa a 10 CSV. **(2)** habilitar cada ámbito nuevo en la cascada = unidad SEPARADA con flip-set propio (caratula: rinde esperado ≤265, adjudicar muestra — «amparo ley 16.986» en carátula es procesal, no ancla; dispositivo/voto: superficie primero). Ampliaciones futuras CON SUPERFICIE PROPIA: decretos/resoluciones/acordadas (formato NNNN/AA colisiona con expedientes y fechas — medir FP antes de sumar el tipo) · ámbito `dictamen` (una línea cuando M58 exporte el texto) · citas sin número (Código Penal, CN por artículo) = territorio del canal kw, NO de este sidecar.
**Rinde doble:** pipeline (canal norma multi-ámbito con pesos por confiabilidad medida; priorización permanente del vocabulario = query sobre el sidecar) + **tesis** (tabla de citación normativa de la Corte, 5894 casos × 20 tomos — continuidad doctrinaria a través de composiciones, insumo H4/H5).
**Estado del fix:** **PASO 1 APLICADO (H209):** `extraer_normas.py` v1.0 canónico (4 ámbitos desde v1) → `csjn_casos_normas.csv`; `derivar_materia` v3.3 consume el sidecar (`ambito=considerando`) — **candado byte-idéntico PASÓ** (sha 89eb50fef37b idéntico); `correr_pipeline` v1.1 · `generar_manifiesto` v1.9 (10 CSV) · MAPA actualizado · manifest [CLEAN] 65. Baselines v0.2 reproducidos exacto (411 carátula / 5562 considerando / gap 265); primeras mediciones: dispositivo 161 pares / 78 normas / 145 casos · voto 1562 / 580 / 825 — extraídos INERTES para la cascada. Validación: py_compile ×5 + harness de equivalencia sobre vocabulario y carátulas reales + sandbox end-to-end + corrida local del operador. **Paso 2 PARCIAL (H211):** ámbito `dictamen` HABILITADO — extraer_normas v1.1 lo emite (6.206 pares) y derivar_materia v4.1 lo consume vía doble filtro propio (`ambito=dictamen AND tipo=ley`) en el tier M58 (flip-set 77, candados N1/M1 PASS — detalle en M58). Restan `caratula` (flip-set ≤265, adjudicar muestra — «amparo ley 16.986» en carátula es procesal, no ancla) y `dispositivo`/`voto` (superficie primero). v0.2 diagnóstica sellada en H208/ (`csjn_casos_normas_out.csv` + `normas_resumen_out.txt`).
**Referencias cruzadas:** M58 (ámbito dictamen), M48 clase (1), `_meta/vocab_materia/indice_normas.csv` (la interpretación-como-dato que este sidecar alimenta). H208. Sin ID histórico.

### M60 — `csjn_analisis`: reescritura contra el schema vigente, post-madurez del pipeline

**Origen:** H210 (decisión del operador al abrir sesión: «csjn_analisis es muy viejo; cuando esté maduro el pipeline, habrá que hacer uno nuevo»).
**Nota de numeración:** el «M60» que H209 advirtió NO crear era un duplicado de M49; esta entrada es independiente y no lo resucita.
**Problema:** `csjn_analisis_v4.py` (v4.0) precede a buena parte de la cadena actual — no consume los sidecars nuevos (normas M59) ni `materia_capa` (constancia H210: 0 literales, ni viejos ni nuevos — el script ignora la capa); el análisis estadístico de tesis corre sobre un modelo de datos desactualizado.
**Diseño candidato:** reescritura contra el contrato vigente (10 CSV canónicos + `_manifest.json` como spec de entrada), no parche incremental. GATE: esperar estabilidad del frente materia (M58 / M59 paso 2 / vocab_faltante mueven las variables que el análisis consume).
**Estado del fix:** no diseñado (entrada de intención).
**Referencias cruzadas:** M49 (constancia 0 literales), M58, M59, tesis (H2/H5). H210. Sin ID histórico.

### M61 — Digesto Jurídico Argentino (ley 26.939) como PRIOR externo del índice de normas + `priorizar_normas.py`

**Componente:** `_meta/vocab_materia/` (dato curado, patrón M46) + instrumento read-only nuevo.
**Origen:** H211 (propuesta del operador durante la adjudicación M58: «está el digesto, que perdió vigencia pero incluye las normas por temas»).
**Contexto medido:** `indice_normas.csv` tiene 34 anclas; el sidecar M59 ve 2.056 normas distintas (cobertura 1,7%). El índice YA nació por inferencia (columnas `pureza_train`/`n_train`, fuente=poc) — lo que falta es escala y priorización permanente.
**Diseño (tres capas por norma candidata, ninguna automática):** (1) **prior_dja** — anexos I+II de la ley 26.939 (SAIJ, descargables; el anexo II de NO vigentes también viene por categoría y sirve igual: consumimos el juicio TEMÁTICO, no el de vigencia) → `dja_categorias.csv` (numero→categoría, 30 categorías art. 7) + `dja_mapeo.csv` (categoría→materia | routing | excluida, ~30 filas curadas UNA vez: P→laboral, S→penal, Q→ambiental, L→tributario, Y→previsional, ASA→salud…; H/U/V/T → routing/EXCLUIDA). (2) **pureza_corpus** — `priorizar_normas.py` read-only: sidecar × materia.csv, distribución de materia por norma no-mapeada usando SOLO `lectura_tribunal`/`refinada` como train (señal independiente del texto — **regla anti-circularidad: NUNCA `lectura_texto`/`lectura_dictamen`**, que el propio canal norma alimenta); ranking por n_casos × pureza; co-citación con anclas = señal secundaria. (3) **adjudicación del operador** — coincidencia prior×pureza = entrada barata al índice; divergencia = cola de revisión (dato en sí: uso forense ≠ objeto regulatorio). Punto metodológico sellado: **el DJA clasifica el objeto regulatorio de la norma; `materia` clasifica el pleito** — el índice ya lo documenta (notas de 24.240 y 20744); el DJA es prior, nunca verdad; `EXCLUIDA` sigue siendo del operador. Límite: DJA corta ~2013 → las 26.9xx/27.xxx quedan para el método pureza-corpus (complementarios).
**Estado del fix:** no implementado (entrada de diseño; primera unidad = parsing de anexos + `priorizar_normas.py`).
**Referencias cruzadas:** M59 (sidecar), M46 (patrón dato-curado), M58 (constancia vocab_faltante: 23.660/23.661/26.682→salud como primeras candidatas), vocab_faltante (esta entrada la escala). H211. Sin ID histórico.

### M62 — Candados de ciclo como GATE del orquestador (el freno no puede depender de un script aparte)

**Componente:** `correr_pipeline.py` / `check_regresion.py`.
**Origen:** H211 (patrón de incidentes de la sesión: TRES violaciones del orden candado→adjudicación→sello — (a) `--regolden` corrido con el verificador caído por bug de paths; (b) commit b9ceb64 declarando PASS de candados no corridos, detectado y AMENDADO pre-push; (c) commit 15e415f declarando «derivar_materia v4.1 + candados PASS + muestra adjudicada» con el `[FAIL]` de versión impreso en pantalla — PUSHEADO; corrección de constancia en BITACORA H211, mensaje honesto en el commit siguiente). Dos bugs del verificador (REPO_ROOT parents[2], field_size_limit) lo dejaron sin correr dos veces: un candado que «siempre falla» normaliza saltearlo.
**Problema estructural:** `check_regresion` corta la comparación en «HEADER cambiado» — un MAJOR de schema deja las columnas preexistentes SIN verificar celda a celda dentro del ciclo; el candado C2 vivió en un script scratch que ni el orquestador ni el gate conocen. El andamiaje frena máquinas, no manos: el `--regolden` no exige evidencia de adjudicación.
**Diseño candidato (NO sellado):** (a) `check_regresion` ante header nuevo: comparar las columnas VIEJAS proyectadas byte-a-byte y reportar la columna nueva como diff separado (el C2 de H211, integrado); (b) `--regolden` gana un requisito de constancia (p.ej. `--adjudicado <archivo/flag>` que quede logueado en el sello) — fricción mínima, huella de que la adjudicación existió; (c) los verificadores de unidad (candados M) se registran y el orquestador los corre si están declarados. Leer ambos scripts ANTES de diseñar (Gate 2).
**Estado del fix:** no diseñado (entrada de diseño con evidencia de tres incidentes en una sesión).
**Referencias cruzadas:** M42 (orquestador), M51 (la otra ergonomía del pre-flight), skill de apertura (REE: «nunca declarar limpio sin demostrarlo»). H211. Sin ID histórico.

### M63 — `git status` como GATE del cierre (el árbol sucio sobrevivió a dos cierres)

**Componente:** protocolo de cierre (`cierre-sesion-corpus`), no código del pipeline.
**Origen / fuente del diagnóstico:** H215 (cierre retroactivo del backlog H211).
**Causa raíz:** el Paso 4 del protocolo escribe la sección **Estado final** de BITACORA a partir de la cabecera de DEUDA y de la salida del manifest, no del árbol de git. Nada obliga a mirar `git status` antes de declarar el estado, así que un commit declarado y no ejecutado no deja rastro visible en el cierre y la constancia queda describiendo un estado que el repo no tiene.
**Diagnóstico / evidencia:** el backlog de H211 (commits (3) y (4) declarados en la entrada, nunca emitidos) sobrevivió **once días y dos cierres completos** — `d07b0c3` (H212) y `69f9984` (H214) se commitearon y pushearon sobre el árbol sucio sin que ninguno de los dos lo notara. Durante ese período HEAD tuvo un manifest declarando `derivar_materia` 4.0 mientras BITACORA declaraba [CLEAN] 65 con v4.1. Precedente idéntico: H210 tuvo que cerrar retroactivamente el backlog H207-H209 porque los «Commits: 2» declarados en H208 nunca se ejecutaron. **Dos ocurrencias del mismo modo de falla, con el mismo remedio no aplicado.**
**Estado de verificación:** `confirmado_cuantificado` (dos ocurrencias, ambas con log de git como evidencia).
**Validador propuesto:** al cierre, `git status --short` ANTES de escribir el Estado final, y el bloque de commits del Paso 6 se redacta contra esa salida (no contra la lista de intenciones de la sesión). Post-push, `git status --short` debe salir VACÍO y esa constancia va en la entrada.
**Estado del fix:** diseñado (cambio de protocolo, no de código). Hermano de **M62** (candados como gate del orquestador): los dos atacan la misma clase — un paso de verificación que existe pero cuya ejecución no está forzada por nada.
**Referencias cruzadas:** M62 · H208 (primera ocurrencia) · H210 (primer cierre retroactivo) · H211 (declaración de los commits) · H214 (detección del backlog) · H215 (segundo cierre retroactivo). Sin ID histórico.

### M64 — Mapeo Anuario→`materia` como CAPA DE REPORTE (`anuario_mapeo.csv` nunca formalizado)

**Componente:** taxonomía de `materia` — capa de REPORTE, no de clasificación.
**Origen / fuente del diagnóstico:** H213 (19-jul-2026), Frente A. Registrado acá al cerrar H213 retroactivamente (H215), para que el pendiente no se pierda con el cierre de la sesión.
**Decisión sellada (operador, H213): MAPEAR — ni adoptar la taxonomía del Anuario ni ignorarla.** La taxonomía interna se conserva como capa operativa y se define un mapeo **sobreyectivo** interna→Anuario como capa de reporte. El valor del mapeo es la **comparabilidad de vocabulario** con la estadística oficial.
**Estado medido (H213, reconstruido del transcript — `[VERIFICAR]` contra los CSV del Anuario en disco antes de usarlo):** `[VERIFICAR]` 8 de 10 categorías de materia del Anuario mapean 1:1 contra el vocabulario vigente (`_meta/vocab_materia/`); la fusión **«Tributaria-Aduanera-Bancaria»** del Anuario absorbe el problema de doble clasificación de `aduanero` documentado en H212. **Cuatro categorías sin resolver:** `lesa_humanidad`, `constitucional`, `electoral`, y las duales penales. Insumos leídos en H213: `anuario_csjn_por_secretaria.csv`, `anuario_csjn_por_materia.csv`, `anuario_csjn_admitidos_por_materia.csv` (extraídos en H083).
**Tres límites declarados, todos del operador:**
1. **Poblaciones distintas — el Anuario NO valida el corpus.** La oficina de estadística trata TODAS las causas; el corpus tiene solo las PUBLICADAS en la colección Fallos, y casi todo el gatekeeping no llega a publicación. «Dialogan pero no es lo mismo»: sirve por vocabulario, **nunca como contraste de distribuciones**.
2. **Cobertura temporal invertida.** El Anuario cubre solo 2024/5; el propósito del frente materia es justamente EXTENDER la estadística hacia los años en que no existía, vía parser de los fallos. El Anuario es referencia de vocabulario, no serie a replicar.
3. **La vía de resolución de las 4 categorías se cayó.** Se había pensado resolverlas por la secretaría de radicación, y (a) el fallo publicado no la registra (H212), (b) las competencias de las secretarías se reorganizaron, así que mapear radicación histórica al eje actual del Anuario puede no ser metodológicamente válido — testigo del operador: `electoral` tramitaba por la **Secretaría Especial N°5**, que ya no existe — y (c) H214 mostró que la secretaría de un caso es una **TRAYECTORIA** de pases entre vocalías y secretarías, no un valor único. El censo `muestreo_secretaria_H213.csv` (`[VERIFICAR]` 82 filas, incluidos los 7 casos gate-kw de H212) quedó **reencuadrado como material de TESIS** (diseño institucional, adjudicación discrecional del docket) y no como insumo del mapeo; su schema además fue superado por el rediseño a dos granos de H214.
**Estado de verificación:** `confirmado_caso_testigo` — la decisión está sellada y las 4 categorías pendientes están enumeradas, pero los conteos vienen de reconstrucción de transcript, no de disco.
**Validador propuesto:** construir `anuario_mapeo.csv` (columnas: `materia_interna`, `categoria_anuario`, `tipo_mapeo` ∈ {1:1, fusión, sin_correspondencia}, `nota`) y verificar la sobreyectividad: toda categoría interna cae en exactamente una del Anuario, y ninguna categoría del Anuario queda huérfana sin declaración explícita.
**Estado del fix:** no diseñado. **Requisito previo:** decidir qué se hace con las 4 categorías pendientes ahora que la vía «secretaría de radicación» está descartada como criterio.
**Referencias cruzadas:** sección «Referencia: taxonomía oficial CSJN (Anuario Estadístico 2025)» (H077) · **M61** (DJA, el otro prior externo, con el mismo filo: clasifica el objeto regulatorio, no el pleito) · H212 (decisiones taxonómicas del operador) · H213 (Frente A) · H214 (secretaría como trayectoria) · H215 (cierre retroactivo). Sin ID histórico.

### M54 — Auditoría firmas × vigencia de jueces: anomalías temporales como instrumento (publicación tardía / transcripciones / voto-fantasma del juez-parte)

**Componente:** auditoría / `_meta/jueces/jueces_csjn.csv` (dato, NO gate del detector).
**Origen:** H200 (pregunta del operador sobre Boggiano post-destitución; scan preliminar sobre votos.csv publicado).
**Idea:** agregar `vigencia_desde/vigencia_hasta` como COLUMNAS DE DATO en jueces_csjn.csv y un análisis read-only que cruce firmas × fecha del fallo. NUNCA como gate de `linea_es_firma_de_juez` (fabricaría FN sobre firmas genuinas: fallos decididos pre-salida publicados tarde — la clase que B153 tuvo que AGREGAR con Moliné/Bossert — y paneles históricos dentro de transcripciones B161; además es circular: el predicado corre a nivel línea, sin fecha disponible).
**Pool inicial (medido H200 sobre votos.csv v33.0):** 9 anomalías — Boggiano (destituido 2005-09) ×6 en tomo 329 + ×1 en 330 · Bossert (renunció 2002) ×1 en 329 · Nazareno (renunció 2003) ×1 en 330. Tres hipótesis a adjudicar POR LECTURA caso a caso: (i) publicación tardía (lag de Fallos — dato en sí para H4 de la tesis), (ii) transcripción embebida B161, (iii) **voto-fantasma del juez-parte** (si el extractor pescó «Boggiano» en p1303/p1305, cuenta al litigante como votante — sería bug de data-quality nuevo, primo de B165).
**Estado:** idea dimensionada, no diseñada. Rinde doble: data-quality del corpus + insumo directo de la tesis (memoria institucional / composición efectiva del tribunal).
**Referencias cruzadas:** B153/M46 (tabla de jueces), B161, B165. H200. Sin ID histórico.

### M55 — Invariante de quórum: `n_jueces` ∈ {0} ∪ [3, ∞) en todo `tipo_entrada=fallo` (detector de FP y errores silenciosos)

**Componente:** validación / auditoría (read-only sobre `csjn_casos.csv`); candidato a check permanente.
**Origen:** H201 (regla del operador tras cazar el FP de 342_p1847: «ningún fallo de la Corte puede tener 1 juez» — el candado de decisión del ciclo lo listó pero ningún check lo frenó solo).
**Fundamento (period-independent):** las decisiones de la CSJN requieren mayoría absoluta de sus miembros (art. 23 decreto-ley 1285/58); el mínimo de los mínimos sobre cualquier composición del corpus (9→7→5 miembros, tomos 329-349) es **3**. Un `fallo` con `n_jueces` ∈ {1, 2} es jurídicamente imposible → SIEMPRE es defecto del pipeline: firma truncada (clase v27.0/B158), conjueces fuera de tabla (B153/B154/B154-bis), letrado contado (B155), bleed, o miss del colector. `n_jueces=0` queda FUERA del invariante (sin_firma legítimo = clase conocida con su propio tablero). El piso 3 NO reemplaza a M47: es la cota inferior universal que corre HOY sin poblar períodos; M47 la refina per-período cuando exista el dato.
**Testigos (sellados H201, ambos del flip-set D2):** 342_p1847 `n_jueces=1` (García Lema solo — adjudicado contra PDF: panel real 5 conjueces, ver B154-bis en B154) · 344_p1102 `n_jueces=1` («Carlos Fernando Rosenkrantz.» — 1 firma en un fallo es imposible por construcción; adjudicación del panel real pendiente de leer el `.md`).
**Diseño candidato (a decidir en la unidad, NO sellado):** (1) instrumento inmediato: query/script read-only que liste el pool `0 < n_jueces < 3` sobre fallos — PRIMERA CORRIDA MIDE LA POBLACIÓN (no inventar cardinalidad; los 2 testigos son cota inferior); (2) integración: reporte estándar del cierre de ciclo + gate de NO-REGRESIÓN (la población no puede crecer respecto del baseline) — gate duro que frene ciclos queda descartado hasta limpiar la población conocida, si la hay; (3) patrón extensible a otros invariantes de dominio baratos (`voting_pattern=unanime` ∧ `n_jueces<3` = subclase automática del mismo pool).
**Estado:** **INSTRUMENTO IMPLEMENTADO H202** — `scripts/validacion/check_m55_quorum.py` v0.1 (read-only, stdlib, salida ordenada por caso_id LF-determinística, `--esperar-filas` candado, `--out` obligatorio regla H201). **Población medida (v34.0 pre-B154-bis): 48** = 19 nj=1 + 29 nj=2 (subclase unanime∧nj<3 = 47/48; excepción 339_p323 segun_su_voto); distribución cargada en la cola (344×8, 348×7, 347×5) + clases viejas 330-332. Baseline sellado: `output/validacion/m55_pool_baseline.csv`. **Gate de no-regresión ESTRENADO H202 post-B154-bis: pool 48→46, bajas exactas {342_p1847, 344_p3070}, 0 altas** (post: `scripts/diagnostico/H202/m55_pool_post.csv`); regla operativa: re-correr con `--out` propio al cierre de cada ciclo y diffear contra el baseline — fila NUEVA en el pool = FRENAR. Pool restante 46 = tablero de adjudicación (caras conocidas: 338_p1060 residual B019 · 330_p3801 evidencia B149 · 348_p473 fila-fantasma H181, re-mirar esa constancia · **344_p1102 adjudicado B166, no sana con tabla**). Integración como reporte estándar del cierre de ciclo: pendiente (paso 2 del diseño). **Gate corrido H203 post-B166-vocabulario: pool 46→45, baja exacta {344_p1102}, 0 altas** (post: `scripts/diagnostico/H203/m55_pool_post_h203.csv`; diff contra baseline 48 = {1847, 3070, 1102}); nj=1 19→16 · nj=2 sostiene 29 · subclase unánime 44/45 (excepción 339_p323 se mantiene única). **Sub-tablero C2 medido H203 (clase_b166_zonas): los 16 nj=1∧hay-votos del pool son TODOS candidatos B166-clase** — lista priorizada en la entrada B166, gemelos 344_p441/344_p575 al frente. **Gate corrido H205 post-B166(c) (parser v36.0): pool 45→29** — 17 bajas ⊆ flip-set (13 nj=1 + 4 nj=2, C2 prácticamente vaciado) + **1 alta ADJUDICADA: 339_p270** (nj3→2, costo aceptado B169 — el radar detectando el costo declarado, no regresión); nj=1 16→3 · nj=2 26 · subclase unánime 28/29. Post: `scripts/diagnostico/H205/m55_pool_post_h205.csv`. **Gate corrido H217 post-B168-dedup (parser v40.0): pool 29→30** — 0 bajas + **1 alta única DEMOSTRADA {329_p2063}** (Compare-Object contra el baseline H216: una sola línea, lado post); nj=1 3 · nj=2 26→27. La alta se adjudica **NO como panel corto genuino sino como BLOQUE AJENO (→ B173):** el nj=2 es el conteo honesto post-dedup de la disidencia Fayt–Zaffaroni de Rusillo (329_p2062) que el bloque mal localizado contiene — tercer valor del radar: dos costos declarados (270, 2063) y cero regresiones desconocidas en tres corridas. Post: `scripts/diagnostico/H217/m55_pool_post_u2.csv`. **Gate corrido H217 post-B168-E (parser v41.0): pool 30 SIN movimiento** — 0 altas / 0 bajas contra el post-U2 (el peek no toca nj); nj=1 3 · nj=2 27 · subclase unánime 28. Post: `scripts/diagnostico/H217/m55_pool_post_u1.csv`.
**Referencias cruzadas:** M47 (versión period-aware), B153/B154/B155/B158, B154-bis (constancia en B154), B166. H201, H202.

### M56 — Overlay curado en `_meta` para casos patológicos (overrides de valor y de frontera) — **PAUSADA: ÚLTIMO RECURSO, NO ATAJO**

**Componente:** `_meta/` (dato curado con procedencia) + etapa de aplicación propia en el DAG.
**Origen:** H201 (idea del operador post-1847: casos que ninguna regex va a agarrar nunca — firmas facsímil-estampadas estilo 335/336, homónimos Boggiano, interleave — curados a mano para darle precisión a las estadísticas de votos/firmas/resultados). Precedente que la legitima: `_meta/catalogo_suplemento_editorial.csv` (M46/H189) ya es dato curado con procedencia; y el modelo SCDB es codificación manual — la mano adjudicada es el gold standard, no un parche.
**CONDICIÓN DE ACTIVACIÓN (sellada por decisión del operador, H201):** esta entrada NO se abre mientras la refinación del pipeline siga rindiendo. Se activa solo cuando una clase tenga constancia en DEUDA de que el fix general es imposible o no-rentable (límite de fuente adjudicado contra el original, no fatiga). Razón de fondo: cada override absorbe un síntoma que hoy es señal de diagnóstico — el sub-frente D existió porque el dato roto quedó a la vista; curar prematuro tapa los testigos que diseñan los fixes generales. Si la lista creciera, eso es señal de bug general no atacado, no de catálogo a engordar.
**Diseño candidato (bosquejo, NO sellado):** DOS tablas separadas — (A) overrides de VALOR (`caso_id, campo, valor, fuente, motivo, sesion`; whitelist cerrada de campos: n_jueces/jueces/posiciones/voting_pattern/outcome…), aplicados como capa FINAL (`aplicar_overrides.py` tardío en el DAG) con FLAG visible en el publicado (columna/sidecar `override`) — el CSV distingue dato-parseado de dato-curado; (B) overrides de FRONTERA (`caso_id, linea_inicio, linea_fin_real`), upstream como insumo del parser (patrón M46) — killer app: 329_p4815/interleave, el mecanismo que a D5 le falta. Candados: (i) triple requisito de entrada (fuente original leída + constancia de imposibilidad + sesión citada); (ii) NUNCA dentro de la lógica de detección — PoC y candados E0 replican el parser puro, el overlay va en etapa propia versionada en el manifest; (iii) los casos overrideados se excluyen o miden aparte en M43 (inflarían el κ del parser).
**Candidatos conocidos al día de la entrada (NO cargados):** 329_p4815 (frontera/interleave, D5) · 342_p1847 si el mundo-(b) de la hipótesis firma-estampada se confirma en el `.md` (valor; ver B154-bis) · Boggiano-D4 si el residuo post-tabla lo justifica · B161 si el des-intercalado resulta imposible.
**Estado:** **PAUSADA** (regla y condición de activación selladas; diseño y carga diferidos a agotamiento de la refinación).
**Referencias cruzadas:** M46 (precedente), B147/D4-D5, B154-bis, B161, M43, M55. H201.

### M52 — Performance del parser: prefiltro para JUECES_CONOCIDOS (~55% del runtime en juego) — CERRADO H198

**Componente:** parser.
**Fix aplicado (H198, parser v32.0→32.1, MINOR):** prefiltro de LITERALES OBLIGATORIOS — el diseño candidato original (unión de alternación única) MURIÓ con dato en sandbox: 1.0x en el caso negativo dominante (`re` de stdlib no optimiza alternaciones grandes; la unión prueba las 87 ramas en cada posición = mismo O(87·n) que el loop; el ahorro de overhead solo aparece en positivos, 3.1x, que son ~1,2% de las líneas). Diseño enmendado: de cada patrón del CSV se deriva mecánicamente (árbol `re._parser`, fallback `sre_parse`, del MISMO patrón — fuente única, cero drift con la tabla) un literal que cualquier match debe contener (garantía por construcción: solo aportan runs LITERAL no-opcionales; BRANCH exige que todas las ramas aporten); helper `hay_juez_conocido(texto)` = etapa 1 `lit in texto.lower()` (substring en C) → etapa 2 loop existente intacto; patrón sin literal ≥4 cae a chequeo incondicional (hoy 0/87). Reemplaza los 6 sitios de existencia pura (`collect_firma_lines` ×2, `_firma_nucleo`, `linea_es_firma_de_juez` ×2, frontera de firma); los 3 sitios de extracción de nombre (`parse_firma` finditer, `detectar_juez_en_voto_header`, `_firma_residuo_es_prosa` sub) intactos. Literal genérico único documentado: «ctor» (Méndez) dispara etapa 2 en «Doctor/doctora» — costo de velocidad, no de corrección.
**Validación (H198):** equivalencia `hay_juez_conocido` == loop: sandbox 0 mismatches (batería + 9933 fuzz, predicados v32.0 vs v32.1 importados contra el CSV real) + **disco E3 0 mismatches / 2.207.636 líneas / 50 `.md`** (26.356 positivos ≈ 1,2%: la constancia de por qué el negativo dominaba). Contrato byte-idéntico por triple vía: 5/5 sha _tmp del perfil == producción · `check_regresion` [CLEAN] · invariante (c) golden==producción con los mismos sha del sello H197 — manifest [CLEAN] 64 sin re-sello de artefactos. **Performance medida:** `re.Pattern.search` 133,8M→10,6M llamadas (−92%) · `linea_es_firma_de_juez` 162,4→66,1 s cumulative (perfiles v32.0/v32.1, mismo día y máquina) · perfil total 218,8→195,6 s · **etapa parser del orquestador sin profiler: 107,8 s** (vs ~208 s histórico H196/H197).
**Correcciones de constancia:** el «~74 re.search» de la entrada original estaba stale desde H192/B154 (tabla 74→87); el prefiltro se construye de los 87. Margen futuro (no urgente, unidad propia): el genexpr del prefiltro es ahora el costo visible del filtro (109,7M llamadas / 21 s tottime) — micro-opt candidata: loop plano en vez de `any(genexpr)`.
**Regresión propia viva:** `poc_m52_literales.py` (scripts/diagnostico/H198/) `--esperar-version 32.1` → E1 versión / E2 tabla 87→74 literales / E3 equivalencia corpus completo / E4 timing.
**Referencias cruzadas:** M46 (tabla-dato fuente del prefiltro), B154/H192 (tabla 74→87), B148/B153 (historia del predicado), M40. H196 (perfil madre), H198.

### M53 — Consistencia y legibilidad menores: `RUIDO_FIRMA` hardcodeado vs. jueces-como-dato + docstring de cabecera stale

**Origen:** H196 (lectura de parser.py entero para el perfil de performance).
**(a) `RUIDO_FIRMA`** (parser ~L864) hardcodea apellidos de jueces («lorenzetti», «argibay»…) mientras `JUECES_CONOCIDOS` es DATO desde M46/H191 — divergencia latente: un conjuez nuevo entra al CSV pero no a RUIDO_FIRMA. Candidato: derivar el componente-apellidos de RUIDO_FIRMA desde la misma tabla (leer primero TODOS los consumidores de RUIDO_FIRMA — Gate 2).
**(b) Docstring de cabecera** de parser.py clavado en «CSJN Fallos Parser v17 (beta)» con uso `python csjnv17_beta.py` — 14 versiones de atraso respecto de `__version__`. Higiene pura; reescribir en el próximo MAJOR que toque el archivo (0 riesgo, 0 ciclo propio).
**Estado:** (b) CERRADO H197 — header reescrito en el MAJOR v32 (B162), ahora sin número de versión (no puede envejecer). (a) pendiente (leer consumidores de RUIDO_FIRMA primero — Gate 2).
**Referencias cruzadas:** M46, M40. H196, H197.

### M65 — Unificación RE_CALIFICADOR / RE_CALIFICADOR_FIRMA (radio por medir)

**Componente:** parser (`RE_CALIFICADOR` L~1004 y sus consumidores; `RE_CALIFICADOR_FIRMA` H216).
**Origen / fuente del diagnóstico:** H216 (fix B168/F): el calificador «(con) ampliación de fundamentos» se agregó SOLO al consumo de `parse_firma` vía regex hermana LOCAL, porque `RE_CALIFICADOR` tiene otros consumidores (continuación de firma L~1152, frontera L~2326, núcleo B148 L~2087/2122) cuyo radio no se midió — una línea «Lorenzetti (Ampliación de fundamentos).» hoy es calificador para el panel pero NO para esos predicados.
**Causa raíz:** dos catálogos de calificadores que pueden divergir (mismo riesgo de clase que M53(a) RUIDO_FIRMA-vs-tabla).
**Diagnóstico / evidencia:** divergencia deliberada y documentada en el código (comentario junto a RE_CALIFICADOR_FIRMA, v39.0). Semánticamente los otros consumidores probablemente DEBERÍAN reconocer ampliación (una línea de firma con ese calificador es firma-ish), pero es hipótesis.
**Estado de verificación:** `confirmado_caso_testigo` (la divergencia existe por construcción; el daño potencial es hipótesis).
**Validador propuesto:** superficie de líneas que matchean FIRMA pero no CALIFICADOR-global por la rama ampliación, cruzada con los call-sites; medir flips de unificar antes de unificar.
**Estado del fix:** no diseñado (unificar = un solo catálogo con el consumo diferenciado si hiciera falta).
**Referencias cruzadas:** B168 (origen), B148 (núcleo), M53(a) (misma clase de divergencia catálogo-vs-dato). H216. Sin ID histórico.
