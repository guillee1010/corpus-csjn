...
**Última actualización:** 2026-06-26 (H165 — **(B) `derivar_partes` v0.15→v0.17:** ARTÍCULO INICIAL — pasada FINAL que quita el artículo gramatical en MINÚSCULA del nombre de la parte («la Administración Federal de Ingresos Públicos»→«Administración Federal…», «la ANSeS»→«ANSeS»). MAYÚSCULA preservada (nombre propio: «La Nación», «La Serenísima», «La Caja ART»); EXCLUYE el wrapper letrado-por-parte («el apoderado de X») para NO enmascararlo (L184/L179). Aditivo puro (pasada final sobre `salida`, tras `_refinar`/`_registrar`): **1591 celdas** (1172 recurrente + 419 recurrido), **0-regresión a nivel celda** (rol/multi/capa/fuente intactos, 0 flips nombre↔vacío), verificado corriendo el script REAL end-to-end vs `csjn_casos.csv`+`csjn_casos_epilogo.csv`. `csjn_casos_partes.csv` sha `8b5eb721…`→`8c46013f…` v0.17; re-sellar manifest. v0.16 quemada (adjudicación manual revertida H163) → next limpio v0.17. **(B136 NUEVO — hallazgo de alcance, confirmado_cuantificado):** `is_merit` excluye originaria DE FONDO sobre una PREMISA FALSA — SCDB NO excluye la jurisdicción originaria (codebook v2021_01 §17 la lista como modo de acceso; Marbury incluido). 546 originarias, 0 en `is_merit`; **167 de-fondo** (96 hace_lugar + 68 rechaza; 153 con Provincia/Estado en carátula) indebidamente fuera de mérito + golden M20 afectado. Fix = condicionar el guard #1 de B119 a la disposición (1 palanca, los verbos ya están); re-golden + re-κ. NO es de M32 ni del bump. Ver **B136**. // H163 — **(B) `derivar_partes` v0.14→v0.15:** cruce NOMBRE-desde-índice anclado a `c/` — recupera el NOMBRE del recurrente cuando el rol está pero el nombre es rol-pelado ("la actora") o vacío. `_partes_desde_indice` + `_refinar_nombre_desde_caratula` cruzan `case_name_indice` "X c/ Y" (actora=izq, demandada=der; invertido "Y (X c/)"→X). **RESTRICCIÓN DOCTRINA: solo con ancla `c/`** — la convención "primer segmento del índice = actora" se midió corpus-wide = **85,3% (464 mismatch)** → orden sin `c/` NO inferible por patrón (el ojo humano acierta por semántica externa persona-vs-empresa, que el regex no tiene). **42 nombres recuperados** (eyeball 42/42 lado correcto); los `caratula:rol_sin_nombre` con `c/` pasan a `recurrente_ok`. `recurrente_ok` 3845→3846 (mejora de CALIDAD, no de conteo: los bare-rol ya contaban). Baseline v0.14 byte-idéntico; `csjn_casos_partes.csv` sha `8b5eb721…` v0.15; manifest `[CLEAN] 65`; parser v22.0 intacto. **(B) capa de adjudicación manual v0.16 — CONSTRUIDA y REVERTIDA (decisión de diseño):** un sidecar `partes_adjudicacion_manual.csv` (30 overrides: 27 carátulas `|` sin `c/` + 3 del cuerpo "Vistos los autos") llegó a nombrar 71/72, pero se REVIRTIÓ — el override **pisa la salida del regex y TAPA los casos que falla** (veneno para afinar a escala; a ~60k no se corrige de a 30) y como input sellado arrastra deuda de procedencia (codificación humana, no derivada del corpus). El deriver quedó **regex puro (v0.15)**. **Gold congelado FUERA del pipeline:** `partes_gold_nombre.csv` (72 filas: `caratula_c/ (regex OK)` 42 · `pipe_editorial (regex NO)` 27 · `cuerpo_caption (regex NO)` 3) — es VARA, NO input; cobertura del name-recovery vs gold = **42/72**. **Hallazgos (→ DEUDA):** (1) **17 penal-falsos** re-medidos (entidades públicas marcadas `penal` por el cargo "Fiscal de Estado/Procurador" del REPRESENTANTE, no de la parte) — actualiza el bump-2 penal-falso de H162 (estimado en 1). (2) **10 rol+cola** que la detección estricta de target salteó (rol pelado + cola), pre-existentes, 0 regresión: 6 con el nombre YA en el clause (miss de `parse_parte` → L184) + 4 que necesitan carátula. **Próximo (H164):** medir si `csjn_editorial_indice_partes.csv` (11445, sha `cba0dcf5…`) conserva el `c/`/orden explícito que el índice colapsado perdió → cruce vs gold para los 27 `pipe`; caption del cuerpo vía `_nombre_desde_causa` para los 3. **El override no vuelve; el gold no se toca; el deriver queda regex puro.** // H162 — **(B, bump 1) `derivar_partes` v0.13→v0.14:** handler de INVERSIÓN en `parse_parte` para el formato "(art)(parte)? ROL[,] NOMBRE [cola]" (el rol procesal precede al nombre; v0.13 dejaba el prefijo pegado "la demandada Ciccone S.A." o cortaba en `RE_CORTE` perdiendo el nombre "la parte actora, X"→"la"). `RE_INVERSION` ancla el prefijo y captura el rol; `RE_CORTE_INV` (superset de `RE_CORTE`: + `representaci\w+`/representant/apoderad/conjuntamente/"con la representación"/PDP) recorta la cola; si tras el rol va letrado/apoderado recursa a `parse_parte` para resolver la parte representada (`344_p1835`→"la Municipalidad de Puán"). **Decisión de rol: PROCESAL** (actora/demandada/coactora/querellante/codemandada), NO `por_derecho_propio` — 9 PDP triple-validados contra el corpus real (epílogo+carátula+bloque). RESTRINGIDO a recuperación de nombre (rol-only sin nombre cae a v0.13 → 0 cambio de métrica). **146 recurrente + 4 recurrido recuperados + 2 correcciones de representación** (`330_p1034` esposa→Antonini Modet/demandada; `346_p491` hijo representado→actora "M. S. E."). Baseline v0.13 byte-idéntico (`54fe7e27`), diff corpus-wide **0 PIERDE, 0 cambio de métrica** (recurrente_ok 3845/67,5%, mérito 2664/92,8%). `csjn_casos_partes.csv` sha `f2894294…` v0.14; manifest `[CLEAN] 65`; parser v22.0 intacto. **Hallazgos cuantificados (→ M33/M34):** rol-por-carátula (de los 2138 `(sin rol)`, 531 con rol leíble directo de la carátula, ~415 recuperables con guard de consistencia → **M33**) + inflación rol-only (~80 "la actora, representada por X" cuentan como recurrente_ok con nombre "la actora"; mérito-con-nombre honesto **90,7%** → **M34**). // H161 — **3 bumps en 3 capas. (C) `derivar_partes` v0.12:** cleanup deuda #4 (`re.split` posicional→`maxsplit=` keyword, L204+L290; saca el error de Py3.14; CSV byte-idéntico); deuda #5 (carátula "nombre enterrado") CERRADA SIN MATERIAL. **(B-extractor) `extraer_epilogos` v0.3:** fallback `sin_zona` para `fin_por_firma_actual` (pie tras firma, escaneo firma→caso siguiente con guard al footer) **+89 epílogos recuperados** (sin_zona 1352→1263) + deshifen migrada (deuda #3); validado end-to-end vs 30 ventanas (27 mérito recuperan). **(B-gramática) `derivar_partes` v0.13:** anclaje de terminadores `RE_MARK_REC`/`RE_MARK_NOMBRE` a inicio de línea (`^Traslados?`/`^Tribunal(?:es)?`/`^Profesional`/`^Normas?`) — fix colisión nombre-vs-footer ("Norma"/"Tribunal"/"Profesional"): +6 nombres + 14 correcciones (abogado→parte), 0 regresión. **Cobertura mérito `recurrente_ok` 2637→2664 (91,9%→92,8%, +27 huérfanos del pie perdido).** Manifest re-sellado `[CLEAN] 65`; parser v22.0 intacto. Outputs: `csjn_casos_epilogo.csv` sha `56eae57f…` v0.3, `csjn_casos_partes.csv` sha `54fe7e27…` v0.13. // H160 — **capa de partes Capa 2 (parseo) + capa CARÁTULA: `derivar_partes` v0.7→v0.11.** Esquema de rol MP adjudicado (`mp_fiscal`/`mp_defensa`) + representado/penal/`por_derecho_propio` + fix indicador `Dr.`; **cierra el frente consolidado `parse_parte`**. Capa CARÁTULA: `rol_causa` (token actora/demandada elige X/Y de "deducido por la actora EN LA CAUSA X c/ Y") recupera **66 nombres** + name-match letrado↔carátula normal "X c/ Y" + fallback `solo_letrado`→carátula. **`NAME_RECOVERED` 43→106, `solo_letrado` 47→9, `mp_fiscal` 81, `mp_defensa` 16, `por_derecho_propio` 44.** Validado vs 10 fallos enteros + diff [CLEAN salvo bluntness de los 9 sentinel]. `csjn_casos_partes.csv` regenerado (5890 filas). Re-sellar manifest. // H159 — **capa de PARSEO de partes: `derivar_partes` v0.6→v0.7.** BUG1 (`RE_REPDE` over-fire en "por sí y en representación de") + BUG2 ("(" colgado) cerrados: 36 cambios sobre el baseline v0.6, 0 regresión (`NAME_LOST_REAL` 0), **16 misatribuciones de recurrente corregidas — 14/16 casos de menores** (el progenitor recurre "por sí" además de representar a los hijos; el fix devuelve al progenitor, no al menor representado). Ganancia de CORRECTITUD, no de cobertura (cobertura −1 a propósito: `344_p2669` "el actor" → `caratula:rol_sin_nombre`). Hallazgo nuevo cuantificado: `extraer_epilogos.py` pierde el pie en cierres `fin_por_firma_actual` (~30 huérfanos de mérito). `csjn_casos_partes.csv` regenerado (v0.7); **parser v22.0 intacto, golden [CLEAN]**. Manifest: re-sellar SOLO si `csjn_casos_partes.csv`/`derivar_partes.py` están en el set sellado.)
...
| B136 | `is_merit` excluye originaria DE FONDO (premisa SCDB falsa: SCDB la INCLUYE) | **NUEVO H165** — confirmado_cuantificado: 546 originarias, 0 en `is_merit`; **167 de-fondo** (96 hace_lugar + 68 rechaza; 153 con Estado) indebidamente excluidas + golden M20 afectado. Fix = condicionar el guard #1 de B119 a la disposición (1 palanca, los verbos ya están); re-golden + re-κ. → B119/B112/M20/M32 |
...
### B136 — `is_merit` excluye originaria DE FONDO (premisa SCDB falsa) — golden/eje partyWinning afectados
...
**Estado del fix:** no diseñado. **Cambio de UNA palanca** — `is_originaria` ya detecta el proceso y `outcome` ya trae los verbos de fondo (hace_lugar/rechaza); el fix condiciona el guard #1 de B119 a la disposición: originaria-NO-fondo (competencia/cautelar/abstracto) sigue excluida, originaria-DE-fondo (hace_lugar/rechaza/etc.) entra a mérito. NO requiere detección nueva. **NO atómico:** toca el eje más usado (`is_merit`) → re-golden consciente + re-κ + republicar Dataverse. Frente propio, posterior; NO es de M32 ni del bump del artículo (H165).
