...
**Estado de verificación:** `confirmado_caso_testigo` (9 casos; sin recall sobre el corpus completo).
**Validador propuesto:** traer el dispositivo real con `extraer_caso.py`, comparar contra el `por_ello` del CSV, medir incidencia sobre el corpus. Posible heurística: si el `por_ello` no contiene verbo resolutivo conocido, reintentar la cascada o marcar `sin_dispositivo`.
**Estado del fix:** no diseñado.
**Referencias cruzadas:** H098. M19. B083/B084/B085/B086 (cascada de dispositivo). Sin ID histórico.
...
**Pendiente del estudio:** correr `analizar_validacion.py` contra la salida del parser `csjn_casos.csv` (columnas de los 7 campos sin prefijo `cod_`) para el TITULAR — exactitud global + IC de Wilson + precisión/recall POR VALOR; doble codificación / inter-coder reliability; sección «Reliability / known limitations» del CODEBOOK; si dispara correcciones, recalibrar el parser bajo la red M12. Si no toca el parser, candidato a dataset v2.1.
**Referencias cruzadas:** H097, H098. B105, B106, B107. M18 (complementaria: M19 mide exactitud, M18 anota distinciones finas). M12 (red de regresión, para recalibraciones). CODEBOOK, nota de versión Dataverse.
