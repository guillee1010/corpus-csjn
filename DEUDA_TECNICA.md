...
**Estado del fix:** **diseñado + validado en gold (n300) y corpus (5890), NO implementado.** Pendiente H129: (1) auditar los 31 rotos de P con `autopsia_rotos_P.py`; (2) si son correcciones → implementar P en `_barrer`/`resolver_dispositivo` (MAJOR, toca el core) con A/B sobre texto idéntico + `check_regresion` column-aware (`outcome` + `causa_inadmisibilidad` + `considerando`-derivados) + re-golden consciente + bump MAJOR + re-sello manifest + cierre; si hay regresiones reales → ampliar/ajustar `RE_PERF`.
