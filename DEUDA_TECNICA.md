...
**Estado del fix:** no diseñado. Dos frentes: (1A) retroceso `_parece_caratula` (H111, ya existe) sobre el `k-1` de las Pistas de `detectar_fin_real` — chico, autocontenido, MAJOR igual (mueve `linea_fin_real` → re-golden). (1B) NO es fix de parser: es recuperar filas al catálogo (upstream, orden 1-3 del MAPA) → cambia N del corpus, CODEBOOK, Dataverse. (1C) legítimo, modelar en la gramática del explorador.
**Referencias cruzadas:** M45 (origen), B012/B045 (familia, fantasmas), «Hotesur real» (pendiente H181/PROMPT). H188. Sin ID histórico.
