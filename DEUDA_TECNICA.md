...
**Estado de verificación:** `confirmado_caso_testigo` (2 testigos). Cardinalidad no medida — detector candidato: `n_jueces` < mínimo del período + «conjuez» en el texto de firma.
**Estado del fix:** no diseñado. Opciones: lista extendida de conjueces por período (dato curado, análogo M46) o patrón posicional «— Nombre (conjuez)». Encolado; relevante para el eje de votos de la tesis (H5).
**Referencias cruzadas:** M46 (destape), B025/B019 (familia firma). H189.
