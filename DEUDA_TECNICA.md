...
**Última actualización:** 2026-06-23 (H158 — **M25 (detector de inversiones de rol de `parte_ganadora`) DESCARTADO y LOCKEADO.** Probadas en disco las dos rutas determinísticas (cruce `disposicion × recurrente_rol`; marcador de disposición `"con el alcance"`): ambas over-firean. La señal de inversión es de MÉRITO, no patrón de texto → detector determinístico inviable. SIN CAMBIOS CANÓNICOS — parser v22.0 intacto, golden [CLEAN], manifest/CHANGELOG no se disparan.)
