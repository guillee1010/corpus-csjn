...
**Última actualización:** 2026-07-05 (H185 — **Ensanche RE_PIE_START EJECUTADO Y CERRADO (extraer_epilogos 0.3→0.4; parser INTOCADO, golden [CLEAN] 5/5 sin re-congelar; manifest re-sellado [CLEAN] 64)**: `RE_PIE_START` del fallback de firma (H161) = **UNIÓN** de la escalera v2 (7 ramas VERBATIM de `RE_EPILOGO_MARKER`, parser 26.1/B117 F2 — fuente única de la gramática del pie) **+ rama 8 LOCAL** «Nombre de la/el actor(a)/demandado(a):» (la v2 dice «Nombre del» y no la cubre; testigo 329_p4811, pie genuino — único `perdido` del verbatim puro, detectado por el PoC). Cae el `re.I` GLOBAL (clase-FP H182/329_p4289, narrativa en minúscula); 0 pies vigentes dependían de él. **Diff adjudicado por lectura ANTES del edit** (poc_h185_ensanche_pie v0.1, scripts/diagnostico/H185/): universo fallback = 113 (sin zona + fin_por_firma_actual) · **A0 réplica == via_firma sellado = 100 EXACTO** · identico 99 · recuperos 4 = 4 pies GENUINOS (334_p1876 «interpuesto **y fundado** por» = clase-gap H182/341_p1679 · 337_p822 + 338_p1060 rótulos Eje-A · 338_p651 solo-footer) · perdido 0 bajo la unión · start_movido 0. **Regresión del fix EXACTA:** epilogo ok 4377→**4381** · via_firma 100→**104** · sin_zona −4 · ids nuevos {334_p1876, 337_p822, 338_p651, 338_p1060} · materia/recursos **hash-idénticos** (verify pre-sello solo marcó epilogo+partes). Costo documentado: captura parcial 338_p1060 (footer corta en wrap «Juan P. » — techo del ARMADO, preexistente, pariente M44). **Hallazgo → B145 (nuevo):** partes recurrente_ok 3848→3848 (+0); 334_p1876 quedó `sin_marcador_recurso` con recurrente vacío (verificado a nivel fila) — `RE_MARK_REC` de derivar_partes v0.17 (L67) exige `por` ADYACENTE al verbo, sin tolerancia `(?:\s+y\s+fundad\w+)?`: gramática v1 gemela de la reemplazada; mérito epilogo_sin_marcador 187→188. **DECISIÓN DE SESIÓN: M43 POSTERGADA** — se ejecuta como ÚLTIMA unidad, después de todas las refacciones pendientes, como gate de publicación Dataverse v2; no proponer como unidad de sesión hasta orden explícita de Guillermo. Candado blind N/A (clasificador e insumos n300 intactos). PENDIENTE H186: **clasificar los 18 outlier_epi residuales** (selección wc_epilogo>500 verificada = 18: 11 fin_extendido_pag_compartida + 7 fin_por_editorial == los 7 ok_cortado_en_indice; dump de epilogo_text pendiente de correr; adentro 340_p437 y 341_p1679) · **residuo B089-B096** (corrida gemela H182) · **B145** (tolerancia «y fundado» en el deriver) · M44 · audit B137 · B144 · B126 · Hotesur real · housekeeping `output/parser_BAK_H184_pre/` (sin decidir) · **M43 solo a pedido explícito**.)
...
**Anterior:** 2026-07-05 (H183 — **B117 F1 EJECUTADA Y ADJUDICADA EN VERDE (poc_b117_superficie v0.1→v0.2 + poc_b117_triage v0.1, scripts/diagnostico/H183/, read-only, 0 cambios en outputs/versiones)**: universo REAL del marker = 11.201 líneas epilogo_marker sobre los 5697 fallos (A0 corpus-wide 5697/5697 · regresión out300 299/299 clase+k0) · candidata v1 cae 1353, triage por inicial + lectura una-a-una → 67 voces TODO-CAPS de sumario (sub-clase NUEVA de B117: la voz «RECURSO EXTRAORDINARIO» flipea a epilogo; testigo 343_p1218 leído en .md) + ~15 narrativa mayúscula + 34 pies genuinos con gramática degradada por OCR · **candidata v2** (case SOLO en el token de arranque + verbos ensanchados inte\w*rpu\w+/presentad/articulad/fundad + «por» libre + sin-por + Recurso…: + rótulos relajados) cae **1319 / queda 9882** y recupera **EXACTAMENTE los 34 testigos con 0 readmisiones no-testigo** (atribución: gram_v2 24 · rotulo_relaj 7 · case_scope 2 · recurso_dp 1) · 4 ambiguos leídos en .md extraídos: 348_p61 = pie con verbo partido por soft-hyphen (FN aceptado), 330_p2877 y 330_p3248 = dictamen (recupero), 343_p1218 = voces caps · **FN total corpus-wide = 3** (348_p61 · 338_p1084 · 331_p793; clase única verbo-ausente/partido → **M44 NUEVO**: vista normalizada por línea con geometría preservada) · efecto por caso: 920 recuperan cuerpo (86 zona entera espuria + 834 parcial) · **DECISIÓN: F2 = opción A pura** — RE_EPILOGO_MARKER nueva SOLO en el sitio del zonificador, RE_DATOS_PARTES INTACTA (hallazgo Gate 2: 2º consumidor en A001/firma inversa, parser.py L1686, PoC H045); sin lookahead (va a M44 con PoC propio). PENDIENTE H184: **B117 F2** (MAJOR parser 25.0→26.0, ciclo --consciente + dump-adjudicación ~920 casos + --regolden + re-derivar epilogo→partes + re-sello; invariante mérito QUIETO 2965==gate, div 0) · M43 · residuo B089-B096 · audit B137 · B144 · B126 · M44 · Hotesur real.)
...
### B145 — `RE_MARK_REC` de derivar_partes sin tolerancia «y fundado»: gramática v1 gemela de la reemplazada en extraer_epilogos
...
**Estado del fix:** no diseñado en código (la tolerancia es 1 edit; el alcance real lo decide el PoC). Unidad propia — NO se acopló a H185 (un culpable por ciclo; el sello H185 quedó con v0.17).
**Referencias cruzadas:** B117 (F1/F2: origen de la gramática v2), H161 (gap `por:` ya anotado en el flujo del fallback), H182 (finos 341_p1679/329_p4289), H185. Sin ID histórico.
