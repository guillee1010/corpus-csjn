...
**Estado de verificación:** `confirmado_cuantificado` (H214) — la incidencia que pedía el validador está MEDIDA: `case_name_cuerpo` vacío en **928 de 5894 filas (15,7%)**. Es la tercera rama del extractor: V1 acierta 3795 (64,4%) · fallback `find_case_name` 1171 (19,9%, dañado → B172) · vacío 928.
...
### B173 — 329_p2063 (Rojas c/ Liberty ART) MAL LOCALIZADO: el bloque publicado es ÍNTEGRAMENTE la cola de 329_p2062 (Rusillo); el caso propio no lo posee ningún bloque — PARKING, testigo con valor recuperable
...
**Estado de verificación:** `confirmado_caso_testigo` (contenido del bloque y ubicación del caso real adjudicados contra el corpus). **Pendiente de verificar:** si el contenido genuino [26325→p. 2064] cae en GAP huérfano (familia B012/B045) o dentro del bloque del caso siguiente como bleed — chequear `linea_inicio` de Vignolo (p. 2064) contra 26325.
**Validador propuesto:** (i) query `linea_inicio` del sucesor de 2063 vs 26325 (gap o bleed); (ii) al tocar la clase, superficie corpus-wide de `ancla_catalogo ∧ recorte 0 ∧ date vacía` como screen barato de hermanos (hipótesis de instrumento, sin correr).
