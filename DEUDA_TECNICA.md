...
### M45 — Explorador v8: adjudicar poblaciones de gramática de zonas + lazy-load para versión online

**Componente:** explorador (`scripts/explorador/exploradorv8.py`, herramienta no canónica); el hallazgo puede escalar a bugs del parser.
**Origen / fuente del diagnóstico:** H186 (calibración del instrumento nuevo «gramática de zonas» sobre los CSVs reales de H185).
**Diagnóstico / evidencia:** dos banderas de gramática destapan poblaciones nunca miradas: **epílogo-no-terminal = 154** (hay tramos de otra zona después del primer tramo de epílogo) y **firma-antes-de-dispositivo = 179** (el primer tramo firma precede al primer dispositivo). La gramática modela el patrón multi-voto (`…dispositivo>firma>voto_separado>cuerpo>dispositivo>firma>epilogo` es legítimo y no dispara), pero puede haber otros patrones estructurales legítimos no modelados → señal adjudicable, NO bug demostrado. Dato de control: residuo-tardío = 0 corpus-wide (consistente con la Pasada 3 del zonificador).
**Estado de verificación:** `hipotesis_no_verificada` (conteos medidos sobre zonas.csv real; ninguna clase leída todavía).
**Validador propuesto:** muestreo con el propio explorador (N=30, seed fijo, por bandera) + lectura del `.md` pintado en el detalle; clasificar clases. Si emergen clases de mis-zonificación, abrir B0xx propio con testigos (candidatas familiares: B117 residual, B141, M21).
**Segunda parte (mejora, no bloqueo):** lazy-load de `csjn_casos_textos.csv` por índice de offsets (hoy se cachea entero) — prerequisito de la eventual versión online (HF Spaces / Streamlit Cloud; repo de deploy separado que baja los datos del Dataverse + persistencia de marcas multi-usuario). Sin urgencia mientras el uso sea local.
**Estado del fix:** no aplica (adjudicación pendiente; lazy-load no diseñado).
**Referencias cruzadas:** M16 (cerrado H186), B117/B141/M21 (familias candidatas detrás de las banderas), M43 (adjudicar estas poblaciones = «madurar el pipeline» pre-κ, decisión H186). H186. Sin ID histórico.
