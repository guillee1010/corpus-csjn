...
**Última actualización:** 2026-06-28 (H167 — **M35 APLICADO** (las 3 patas reversibles), frente elegido del menú A–D. Democión del fósil `csjn_editorial_indice_partes.csv`: ① fuera del set canónico (`generar_manifiesto.py` v1.7→v1.8, `OUTPUTS` 10→9; `--verify [CLEAN] 64` artefactos; `check_regresion [CLEAN]` 5890); ② preservado **trackeado** en `archivo/fosiles/` (insumo de B115/M32); ③ redundancia **reproducida en disco** (no testimonio H166): crosscheck EXTRA-vs-`catalogo.csv` = **478/478** EXTRA y **358/358** SOSPECHOSOS en catalogo con `nombres_indice` poblado, 0 fuera → catalogo cubre lo del fósil. Productor-muerto confirmado por grep (único writer = `auditoria/H061/crosscheck_indice_partes.py`). **PENDIENTE → M35-④:** de-publicar el fósil de Dataverse (doi:10.7910/DVN/TJTVKW) + subir manifest v1.8 — paso a mano, irreversible; **el dataset publicado quedó un paso atrás del repo (10 outputs / manifest viejo)**, desfasaje conocido, primer ítem de H168. Correcciones de registro: el commit "(1)" dice "mv" pero NO hubo `git mv` (el fósil estaba **untracked**, no gitignored; ya relocado de antes, `LastWriteTime` 23/5) → su contenido real es el edit del generador; `scripts/auditoria/` está **gitignored** → el crosscheck queda como one-shot local (Opción A, no commiteado). Deuda nueva: **M35-④** Dataverse · **D-anchor** (anclaje `Path(__file__).parent` roto en `caracterizar_extra_indice.py` y probable en `H061/crosscheck`; el crosscheck nuevo ya usa `.git`). M36 ahora cubre **4** derivers (el índice fósil salió). B136 SIN tocar (frente B, próxima sesión con ventana fresca).)
...
| M35-④ | de-publicar el fósil de Dataverse + subir `_manifest.json` v1.8 (publicado sella 10 outputs, repo 9) | **ABIERTO H167** — paso a mano (sin CLI). Checklist: draft sobre el dataset publicado → borrar `csjn_editorial_indice_partes.csv` → reemplazar `_manifest.json` por v1.8 → version note "H167/M35 democión fósil, manifest v1.8 9 outputs" → publish versión mayor (la v.actual con el fósil queda inmutable = red de recupero). Primer ítem de H168 |
