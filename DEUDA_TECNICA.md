...
**Última actualización:** 2026-06-23 (H156 — **M29 paso 4 — fallback de recurrente desde la carátula del fallo (`case_name_cuerpo`). SIN CAMBIOS AL PARSER** (capa-deriver; parser v22.0 intacto, golden [CLEAN], sin re-golden). **`derivar_partes.py` v0.4→v0.5:** PASO 4 de la cascada — cuando falla el epílogo (`sin_marcador_recurso`) o no hay zona (`sin_zona`), deriva el recurrente del campo canónico `case_name_cuerpo` ("Recurso … deducido por X en la causa …"), que es la MISMA atribución de recurso del epílogo (**Eje B**, voz de la Corte) en otra ubicación canónica — NO actor/demandado (Eje A). `derivar_de_caratula()` + `RE_MARK_CARATULA` (terminador "en la causa/las causas/los autos") + `parse_parte` verbatim; buckets nuevos `caratula:recurso` (nombre) / `caratula:rol_sin_nombre` (rol conocido, nombre vía Eje A); soft-check de la columna (degrada si falta). **ADITIVO PURO** verificado por diff v0.4↔v0.5 en disco de Guillermo: **169 filas cambian, TODAS desde `sin_zona` (98) o `sin_marcador_recurso` (71)** → carátula; **0 regresión sobre los 3641**. Cobertura: **recurrente_ok 3641→3749 (+108 nombre vía carátula)**, `caratula_rol_sin_nombre` 61, epilogo_sin_marcador 704→633, sin_epilogo 1352→1254. **Mérito (2870): 88,4%→90,5%** (nombre 89,3% · nombre-o-rol 91,8% · sin recurrente 235 = 8,2%). Eyeball de los 108: limpios (Boggiano ×2, Astiz, AFIP demandada, Estado Nacional, Pico y Jessen multi, Vega Giménez penal). Caracteres ├│ en consola = codepage de PowerShell, archivo UTF-8 OK (verificado con `Get-Content -Encoding UTF8`). **Diagnóstico que descartó el plan H155 (frente 2 formato viejo):** de los 354 formato-viejo residual, 97% Eje A puro sin marca de recurso, 82%+ originaria → recuperable Eje B solo 17; frente 2 cerrado SIN código (mapear actor/demandado→recurrente violaría la doctrina de los dos ejes). **Reconciliación del residual sin recurrente (1893):** no-mérito-no-originaria 1133 (art.280/inadmisibles = ausencia esperada) + originaria 525 (sin recurrente por naturaleza) + mérito-no-originaria 235 (gap real = capa-cuerpo tail). `is_originaria`=1 total = **546** (confirma la intuición "~500 max"; el grueso del residual es acceso denegado, NO originaria). **M25 DESBLOQUEADO por M29** — la entrada M25/línea 4615 pedía "el ROL procesal que hoy NO existe derivado"; ahora `recurrente_rol` existe → el detector de las 7 inversiones de rol ya tiene insumo. **`partyWinning` ya existía** (binario κ 0,784, derivado de disposición, no necesita el nombre); M29 habilita partyWinning NOMBRABLE (join recurrente×parte_ganadora) + el detector M25. **SCDB = partyWinning (resultado) ⊕ petitioner/respondent (identidad como TIPO categórico ~200-300 clases):** M29 da nombre + rol procesal, NO el tipo → el tipo requiere clasificador inteligente (capa nueva, no fix; ver **M32**). **Manifest:** re-sellado **[CLEAN] 65** (`csjn_casos_partes.csv` sha `ef91f17fc71c…`, v0.5; los otros 64 intactos). **Tickets nuevos:** **M32** (clasificador de tipo SCDB petitioner/respondent), **B135** (is_originaria subdetecta), + polish `parse_parte` "la defensa de X". Próximo: validar 91,8% (eyeball estratificado ~25-30 casos entre epilogo:recurso / epilogo:nombre_recurrente / caratula:recurso / caratula:rol_sin_nombre vs `.md`, gate REE) → **M25** (detector de inversiones, aprovecha el rol de hoy, autocontenido). // **Previo H155 —** **M29 capa 1 — consolidación de la zona epílogo + fallback formato viejo + filtro de mérito como denominador. SIN CAMBIOS AL PARSER** (capa-deriver; parser v22.0 intacto, golden [CLEAN], sin re-golden). **`derivar_partes.py` v0.2→v0.3→v0.4 (dos commits H155):** **v0.3** = fallback `^Nombre del recurrente:` (formato viejo, Eje B directo) cuando falla `RE_MARK_REC` → `partes_fuente="epilogo:nombre_recurrente"`. **ADITIVO PURO** (0 regresión sobre los 3633, verificado con fillna; corrida en disco de Guillermo idéntica al sandbox). Cobertura: **recurrente_ok 3633→3641 (+8)**, epilogo_sin_marcador 712→704. **v0.4** wirea en `_reporte` el split de cobertura mérito/no-mérito (reporte-only, output CSV byte-idéntico a v0.3). **DECISIÓN METODOLÓGICA (clave, IMPLEMENTADA en v0.4): la cobertura de partes se mide sobre el universo de MÉRITO** (`is_merit_decision`, ya en `casos.csv` — no hay que construirlo), NO sobre todos los fallos = universo SCDB. Releído así: sobre **2870 fallos de mérito**, recurrente_ok **88,4%** (2536), sin_marcador 5,1% (145), sin_epilogo 6,6% (189). De los 1352 sin_epilogo, **1163 (86%) son NO-mérito** (art.280 = 229, desestimaciones, inadmisibles, quejas rechazadas) = ausencia ESPERADA, no gap; solo **189 de mérito sin zona = gap real** (techo del fix de B117). Aun en no-mérito hay 1105 recurrente_ok (footer del peticionante aunque le denieguen el certiorari = cobertura bonus). **B117 CUANTIFICADO** (over-capture epilogo): 478 zonas wc>200 (11% de 4345), 205 >1000, max 19788; **NO corrompe la derivación** — `RE_MARK_REC` line-anchored + case-sensitive aísla el cuerpo (verificado en Mazzeo/Riveros 330_p3248, ALITT 329_p5266, Estado Nacional 340_p257 → 3/3 footer correcto). Elevado a deuda estructural **M31** (la delimitación floja de zonas no muerde hoy pero acopla a futuros consumidores de `zonas`/`epilogo_text`/`wc_*` = escalabilidad/modularidad, REE). Riesgo real = 80 doble-marcador, triados 53 multi-recurrente / 16 multivoto / **11 come-varios-casos** (segmentación, familia B009), de los cuales solo ~4 con partes distintas a recodear (349_p40, 329_p2037, 329_p2179, 347_p1259). **Des-hifenación pre-match DESCARTADA** (medida sobre disco: +1/−2 = neto −1, rompe el ancla `^`). Headers ya excluidos del epilogo_text por diseño (1-2 leaks). Frente Eje A (`Nombre del actor:` 83 · `Parte demandada:` 252 · `Nombre del demandado:` 77) DIFERIDO a la cascada de partes. Script nuevo: `scripts/diagnostico/seleccionar_muestra_zona.py` (PoC muestra reproducible). **Manifest:** `derivar_partes.py` v0.4 (final) + `csjn_casos_partes.csv` hash → re-sello esperado **[CLEAN] 65**, sin re-golden (2 commits: v0.3+CSV, v0.4 reporte). Próximo: muestrear los **189 de mérito-sin-zona** (¿footer recuperable → justifica fix B117 / re-golden, o per curiam genuino → capa completa?); recodear los ~4 come-varios-casos; cascada Eje A (`Nombre del actor:`/`Parte demandada:` → quién demandó, después cruce con el cuerpo para quién apeló). // **Previo H154 —** **M29 capa 1 (partes desde el epílogo) IMPLEMENTADA — recurrente/recurrido + rol procesal del epílogo editorial. SIN CAMBIOS AL PARSER** (capa-deriver nueva; parser v22.0 intacto, golden [CLEAN], sin re-golden). **Dos artefactos canónicos nuevos** (patrón insumo→clasificador, replica `textos.csv`): `extraer_epilogos.py` **v0.2** NUEVO → `csjn_casos_epilogo.csv` (insumo persistido; lee `zonas`+`corpus/*.md`, offset relativo verbatim H055), emite **5697 fallos 1:1** con `epilogo_status` (ok 4345/76,3% · sin_zona 1352 · archivo_no_encontrado **0** = offset OK); `derivar_partes.py` **v0.2** NUEVO → `csjn_casos_partes.csv` (output; lee el CSV de epílogos NO el .md → reproducible Dataverse), deriva recurrente vía marcador flexible anclado a línea `^(Recursos?|Queja)…(interpuest|deducid) por X[, rol]`. **Cobertura capa 1: recurrente_ok 3633 (63,8% fallos / 84% epílogos)**, epilogo_sin_marcador 712, sin_epilogo 1352, no_aplica 193; multi_recurrente (flag) 149. **Rol** (sobre 3633): sin_rol 2383 · penal 444 · demandada 368 · actora 343 · solo_letrado 45 · querellante 21 · codemandada 18 · por_derecho_propio 9 · coactora 2. **Sub-patrón letrado resuelto** (75 letrado-capturado→15): `parse_parte` desambigua `por derecho propio` (letrado=parte), `, por <parte>` tras 2º `por`, `defensor de <parte>`, marca `solo_letrado` (parte fuera del marcador, típico penal); bug `\b` tras `Dra.` resuelto; `re.split` maxsplit keyword (sin DeprecationWarning, CSV byte-idéntico). **HALLAZGO de diseño (validado en disco): DOS ejes de partes, no uno** — Eje A actor/demandado (del índice editorial `X c/ Y`, frente futuro) ≠ **Eje B recurrente/recurrido = petitioner SCDB, SOLO del epílogo** (quién apeló a la Corte); el actor/demandado NO define quién recurrió. La entrada M29 de H153 apuntaba al Eje A (`c/` 59%); el insumo que M25 necesita es el Eje B. **Validación cruzada:** las 7 inversiones de rol de M25 → 7/7 recurrente legible. **Corrida en disco de Guillermo = idéntica al sandbox** (3633 exacto) → gate REE cerrado. **Manifest:** 2 scripts a PIPELINE_SCRIPTS + 2 CSV a OUTPUTS (`generar_manifiesto.py` v1.6→**v1.7**, 8→10 canónicos), re-sello esperado **[CLEAN] 63→65**. **DEUDA:** M29 capa 1 (epílogo) CERRADA; pendientes **frente 2 formato viejo** (`Nombre del actor:`/`Parte demandada:`, ~365 epílogos que listan partes pero NO marcan quién recurrió → 2ª gramática, mayor ganancia) + **frente 3 arrastre de zona** (~103, tocaría parser/re-golden, evaluar ROI). Verificación del gold de M25 (7 inversiones) FUERA de M29 por decisión (no acumular incertidumbres). Próximo: frente 2 (formato viejo) → ¿epilogo en CODEBOOK/Dataverse? (decisión separada); B130/reenvia (coding manual, pausado); M25 detector. // **Previo H153 —** **CODEBOOK v1.3→v2.0 (schema real M26, doc público Dataverse) + Ruta 1 `parte_ganadora` binarizado. SIN CAMBIOS AL PARSER** (capa-deriver; parser v22.0 intacto). **CODEBOOK v2.0:** reescrito al schema real — **8 CSV** (eran 5), diccionario nuevo de `recursos.csv` (ejes M26) y de `textos.csv`, `casos.csv` a **39 cols**, `outcome`→LEGACY/congelada, **tabla κ** (§9, reemplaza la accuracy M19 desintercalada), **crosswalk SCDB** (§10: `parte_ganadora`→partyWinning, `disposicion`→caseDisposition, `es_revision_fondo`→merits, `via_recurso`→avenue, `reenvia`→remand), GAPS (declarationUncon · petitioner/respondent prospectivo · `majOpinAssigner` DESCARTADO = per curiam), invariante M26 (`causa ≠ "" ⟺ inadmite`), 335–336 redactado como exclusión **TEMPORAL**. **Ruta 1 (`parcial` eliminado del OUTPUT):** `clasificador_disposicion` v1.08→**v1.09** — `parte_ganadora_regla` mapea `modifica→gana` (SCDB binario, no-reformatio-in-pejus); los 3 `parcial` del corpus (Olivan/Patoco/Picapau, los tres `modifica`, `multi_recurso=no`) validados a mano → `recurrente_gana`. `recursos.csv` re-derivado: binario **{gana 2335 / pierde 537 / no_aplica 3018}**, 0 parcial, 3 celdas. `disposicion()` INTACTO (κ disp 0,912), **κ-parte 0,784 sin cambio**. Manifest **[CLEAN] 63** (`recursos.csv` sha `e4e90c08d091…`). **DEUDA:** M25 actualizado (output binarizado; detector de texto de las 7 inversiones SIGUE abierto), **M29 NUEVO** (capa de partes petitioner/respondent, PRIORITARIA, insumo SCDB), **M30 NUEVO** (centralizar paths + orquestador downstream). Próximo: refresh Dataverse (8 CSV + corpus + CODEBOOK v2.0 + manifest); B130/reenvía (coding manual, pausado); M25 detector de texto. // **Previo H152 —** **Documentación de repo + higiene in/out. SIN CAMBIOS CANÓNICOS** (parser/golden/outputs/`_manifest.json` intactos; los 4 modificados de H148/H149 —`clasificador_causa.py`/`csjn_casos_recursos.csv`/`_manifest.json`/`CHANGELOG.md`— quedaron SIN STAGEAR en este commit, SALDADOS en un 2º commit esta misma sesión —ver abajo—). **6 READMEs por directorio:** `output/parser/` (corregido del manifest: **8 CSV** con cuentas reales —casos 5890, votos 27697, zonas 141451, editorial 152, indice_partes 11445, materia 5890, recursos 5890, + textos 5890—; el viejo `readme_output_parser.md` documentaba 5 con cuentas stale → a archivar); `scripts/pipeline/` (interfaz: etapas + clasificadores como submódulos de `derivar_recursos`); `scripts/validacion/` (subsistema κ); + 3 techos forenses (`diagnostico`/`auditoria`/`archivo` = archivo no reproducible, fuera de Dataverse). **κ PROMOVIDO** `scripts/diagnostico/kappa/` → **`scripts/validacion/`** (git mv, historia preservada) + path fixes en `build_m20`/`validar_H120` (`HERE.parents[2]→[1]` por el cambio de profundidad 3→2; golds a `golds/`; OUT de `__rebuild` a `golds/`). **Demo en disco:** `kappa_confiabilidad` desde la ubicación nueva reprodujo IDÉNTICO (parte **0,784**, gate 0,940, vía 0,943, disp 0,912, reenvia 0,408) → el move no rompió nada. **`.gitignore` reescrito de cero** (dedup, fix línea malformada, EOL): **intermedios `output/{mapa,catalogo,localizacion}` CONSERVADOS** (insumos del parser sellados en `_manifest.json` como `inputs` — corrección de criterio: regenerables PERO load-bearing); forense (session dirs de `auditoria`/`diagnostico` + `archivo/` + `docs/` stale) sacado del tracking vía `git rm --cached` (queda en disco + historia; solo READMEs-techo + `auditar_fallo.py` persisten); `output/validacion/` (κ resultados) trackeado. **Decisiones:** `corpus/` (46 vols, 329–349 excl. 335–336) va a Dataverse como dataset base; `docs/` stale NO se publica; `auditar_fallo.py` carve-out (tool vs `validacion/`) PENDIENTE. **B130/reenvia sigue PAUSADO** (coding manual). **Leftover H148/H149 SALDADO (2º commit):** triados los 4 = trabajo real (M27 `INTERPOSICION_INCORRECTA` §1.2.2 + H151 recall-gap `remite-dictamen(disp)`); `clasificador_causa.py` v0.4→**v0.5** — `INTERPOSICION_INCORRECTA` reordenado al FINAL de la cola (post remite-dictamen, antes de SIN_CAUSAL) para que cumpla su contrato "solo drena SIN_CAUSAL" (a v0.4 había quedado antes del residual al insertarse la rama `remite-dictamen(disp)`); `recursos.csv` re-derivado; `_manifest.json` re-sellado **`--verify [CLEAN] 63`**. Reorden empíricamente NULO (`INTERPOSICION` sigue en 2, sin overlap). Próximo: reescritura del CODEBOOK; refresh Dataverse pre-M26; B130/reenvia (pausado, coding manual). // **Previo H151 —** **M20/M25: gold de `parte_ganadora` limpiado + harness canónico identificado. SIN CAMBIOS CANÓNICOS** (sesión de validación; parser/golden/outputs/`_manifest.json` intactos; manifest/CHANGELOG no se disparan). **Gold M20 recodificado con la regla binaria SCDB:** `partyWinning` es binario petitioner-centric, NO existe clase `parcial` (la victoria parcial = vacate-and-remand se codea GANA) → los 8 `parcial` colapsados a gana/pierde leyendo el `.md` (`329_p3213`/`337_p373`→pierde; resto→gana; Maza `332_p2208`→gana). **Regla off-axis fijada:** `parte_ganadora` vive SOLO donde `es_revision_fondo=si`; `materia`/`via_recurso` transversales (no blanquear); verbo `disposicion` queda aun en no-fondo (B119). Blanqueado `parte` de 3 no-fondo (`329_p1936`/`329_p2645`/`337_p735`) → `cod_parte_ganadora` no-nulo solo en **134 fondo**, binario (**110 gana / 24 pierde**), robusto a cualquier script de κ. Producto: `planilla_M20_57GOLD_parte_limpia.xlsx`. **Harness canónico identificado: `analizar_validacion_M20.py`** (H119, "validar m20") — NO `kappa_confiabilidad.py` (H139, posterior). Confirmado documentalmente que filtra `parte` por el gate y NO lee filas no-fondo (tabla κ H139 corre parte sobre n=136≠300; log H119: "es_revision_fondo filtra; la estadística de fondo se restringe a ='si'"). Pendiente al correrlo: si filtra por el gate del gold o del parser (relevante donde gold-fondo ≠ parser-fondo; κ-gate 0,946). **`majOpinAssigner` DESCARTADO** como variable de tesis — error de categoría CSJN (sin conferencia, regla de asignación ni orden de circulación registrado; coordinación radial). **κ de `parte_ganadora` RECOMPUTADO (H151): 0,653 → 0,784** [0,632–0,908] (n=134, acuerdo 0,933, "sustancial") — sube por el colapso de parcial; techo = 7 inversiones de rol (banco M25). El n=134 = exactamente el universo fondo (el blanqueo del bycatch alineó con el filtro de no-nulos de `kappa_confiabilidad`). **B130/reenvia REABIERTO:** en la misma corrida `reenvia` dio 0,408 (no reportable) — clase negativa sub-codeada (69 si / 6 no / 225 blanco); fix = codear los 42 del universo de reversión en blanco LEYENDO (work-list extraído a `reenvia_42.md`), blanquear 4 codeos fuera de universo (`329_p1473`/`329_p1936`/`337_p735`; re-leer `329_p5368` confirma→si), válvula `'-'`. **Housekeeping:** subsistema κ consolidado en `scripts/diagnostico/kappa/` (+ `golds/`); outputs κ a `output/validacion/`. Próximo: codear reenvia → recomputar → M25 detector. // **Previo H149 —** **M26 paso 4 PARCIAL — κ recompute v22.0 + cierre de validación; camelCase DESCARTADO. SIN CAMBIOS CANÓNICOS** (cadena sellada intacta, manifest 64 H148 válido; el κ es output de validación, no toca pipeline; re-golden/re-sello NO aplican). **κ recompute v22.0 (in-sample, n300, corrido en disco de Guillermo = idéntico al sandbox):** es_revision_fondo **0,946** [0,907–0,980] (sube vs publicado 0,933 — el gate re-cableado no regresó, mejoró), via_recurso **0,941** [0,842–1,0], disposicion **0,912** [0,847–0,966] (esquema `no_*` nuevo benigno en el overlap: 4 cruces en 140), parte_ganadora **0,653** [0,504–0,783] (8 de 17 desacuerdos = techo estructural `parcial`, M25). reenvía EXCLUIDO (B130). Sellado en `output/confiabilidad/kappa_resultados_v22.csv`. **HALLAZGO CENTRAL — `admisibilidad` es SÍNTESIS, no detección:** 5882/5890 (**99,86%**) del eje se reproduce por puro relabeling de `{outcome, queja_resultado, disposicion, is_originaria}` (campos que detecta el parser); el único detector propio (`RE_ADMITE_REX_TXT`/por_ello) toca **8 casos (0,14%)**. Consecuencias: (a) no hay κ-admisibilidad independiente que medir — su confiabilidad ES la de los inputs colapsada → **confiabilidad COMPOSITIVA** (κ upstream outcome/queja M19 + disposicion 0,912 + is_originaria); (b) **el ciego de admisibilidad DEJA de ser requisito**; (c) HM-01 = aporte de **EJE explícito** (acceso-vs-mérito) sobre detección del parser, NO un detector del derivador. **κ-causa DIFERIDO (método resuelto):** `clasificador_causa` lee considerando+por_ello; **40,8% de las causas (incl. ART_280, la dominante) ancladas SOLO en considerando**; el gold tiene por_ello pero NO considerando → seed por_ello DESCARTADO; ancla honesta = `cod_causa` blind de M19 (texto completo). **camelCase DESCARTADO (Guillermo):** mezcla nombres-inglés/valores-español incoherente, `es_queja` se mantiene; el crosswalk SCDB va en el CODEBOOK contra los nombres en español, SIN renombrar columnas. **κ-admisibilidad seed H146 EXPUESTO en H149** (predicción a la vista → no usable como blind; moot por el hallazgo de síntesis). **B134 NUEVO** (originarias leak a inadmite por precedencia). **B130 ACTUALIZADO** (artefacto de diseño; doctrina deja⟹remand 45/46; path de derivación). **Próximo: M27 vía el tratado como gold EXTERNO** (handoff `PROMPT_H150_m27_tratado_gold.md`). // **Previo H148 —** **M26 paso 3 — RE-CABLEO de `causa_inadmisibilidad` IMPLEMENTADO + B133 CERRADO.** La columna sale del parser (v21.01→**v22.0**, MAJOR: −columna −función −`OUTCOME_A_CAUSA`/`OUTCOMES_GATE_GENERICO`/`RE_CAUSA_*`, 158 líneas; `RE_280_*`/`RE_ACORDADA_4_*`/`_unhyphenate` QUEDAN para `classify_outcome`; voto INTACTO) y se deriva en **`clasificador_causa.py` NUEVO v0.1** (fuente única), GATE `outcome`→`admisibilidad=="inadmite"`. **Detectores (validados en disco):** caducidad §2.6 ancla dispositivo + guard polisemia = **13/13, 0 FP**; desistimiento §2.1.11 con gap que tolera adjetivo intercalado ("la presente queja") + reorden de cascada (dispositivo antes que considerando, fix `340_p251`) = **10/10, 0 FP**; CUESTION_ABSTRACTA = `outcome=="abstracto"` (señal validada) + guard `disposicion∉fondo` (96 puros vs 50 mixtos) + respaldo `RE_MOOT` anclado al **dispositivo** (no al considerando — FP de antecedente cazados a ojo: 329_p4446/344_p2011/345_p423). Cola H092 verbatim + guards B100/B101/B103 sin cambios, abierta a todo `inadmite`. **B133 CERRADO** (`clasificador_admision` v0.1→**v0.2**): `outcome=="abstracto" ∧ disposicion∉fondo → inadmite`, tras la originaria; **aditivo y quirúrgico (87 `sin_marcador→inadmite`, todos abstracto, 0 colaterales)**; `inadmite` 1020→1107. **`derivar_recursos` v0.4→v0.5** (importa clasificador_causa, columna en el mismo pass que admisibilidad). **Validación:** harness verbatim reproduce la causa publicada **5890/5890**; A/B causa **134 deltas** (GANA 68 inadmite-real; PIERDE 64 = 49 abstracto→admite [mixtos] + 7 abstracto→no_aplica [originaria] + **8 contradicciones `admite ∩ causa`→κ**; CAMBIA 2). **Invariante nuevo: `causa ≠ "" ⟺ admisibilidad = inadmite` (1107=1107)** — el gate viejo por `outcome` no lo tenía. **Cierre:** `check_regresion` `csjn_casos.csv` [FAIL] deliberado (columna removida) + textos/votos/zonas/editorial [CLEAN] → re-golden consciente; manifest re-sellado +`clasificador_causa.py` ([CLEAN] **63→64**). Versiones post-H148: parser v22.0, clasificador_admision v0.2, clasificador_causa v0.1 (NUEVO), derivar_recursos v0.5 (disposicion v1.08 / via v0.1 sin cambio). **Residuales a κ-admisibilidad:** 8 contradicciones + `339_p676`. M27 sigue abierto (5 causales del tratado). // **Previo H147 —** **M26 paso 3 — diseño + diagnóstico del re-cableo de `causa_inadmisibilidad` + fundación del vocabulario sobre el tratado de la Secretaría. SIN CAMBIOS CANÓNICOS** (parser intacto v21.01, sin outputs tocados, sin bumps de versión, `_manifest.json` sin tocar; manifest/CHANGELOG no se disparan). **Coverage check `outcome` ↔ canales:** `outcome` NO es rollup determinístico (techo **79,2%** con disp+adm+queja+orig+causa+queja_resultado, 5890). El ~20% NO es pérdida: ~76% del residual es conflación/redundancia/catch-all, ~89 desacuerdos de detector (gate nuevo + preciso), 38 migran a causa; solo **~1,9% del corpus (109, casi todo `rechaza` grueso)** es coarsening discutible. **DECISIÓN (paso 3 step 2): `outcome` → legacy CONGELADA (opción B), valores actuales, NO recompute** (recompute = puente 80% a v2.0; congelada = 100%). B-vs-A reabierto (el "(no romper Dataverse)" del prompt es moot — se republica): **B se mantiene por reproducibilidad de versionado**, no por Dataverse. **Ubicación verificada — el re-cableo TOCA parser:** `clasificar_causa_inadmisibilidad` parser L700 (deriva de `outcome` vía `OUTCOME_A_CAUSA` L616), denorm de `outcome` en votos parser L3739 → commit separado / **mover causa al deriver (lean b)**. **A/B del re-cableo (harness verbatim de la causa, fidelidad 5890/5890):** gate swap `outcome`→`admisibilidad=inadmite`, delta 247/5890 (**4,2%**). **Detectores DEMOSTRADOS:** caducidad (gate inadmite + ancla dispositivo `caducidad de la instancia` + guard `apercibimiento`/`rechazar el planteo`) = **13/13, 0 FP/FN**; desistimiento (`se tiene por desistida la queja`) = **10/10, 0 FP**. Reusan la disciplina de polisemia del 280. **Corrección desistimiento (Guillermo):** 9/10 son **default de depósito** (gatekeeping, NO terminación), 1 voluntario (`340_p251`, parcial); RETRACTADO el "relocalizar" y la supuesta inconsistencia de `clasificador_admision`. **Corrección doctrinal `abstracto` = ADMISIBILIDAD (Guillermo + fuente):** la mootness es decisión de admisibilidad (la actualidad del agravio es requisito; sin caso vivo se rechaza el planteo sin fondo) → `CUESTION_ABSTRACTA` **NO relocaliza**, es causa de admisibilidad legítima, se queda. **B133 NUEVO:** `clasificador_admision` SUB-MARCA la mootness (de 148 abstracto solo 5 `inadmite`; ~96 puros deberían ser `inadmite`). Split: ~96 puros (`no_fondo`, gate=no) → `inadmite`+`CUESTION_ABSTRACTA`; ~52 mixtos (verbo de mérito real) ya están bien (el abstracto es subtema). **M27 NUEVO — vocabulario canónico sobre el tratado de la Secretaría de Jurisprudencia** (`documento__37_.md`, REX 11/06/2026, la misma Secretaría que edita los tomos = validez de constructo). Mapeo: 8 causas actuales ✓; **5 gaps** (interposición incorrecta §1.2.2=tribunal equivocado, salto de instancia §1.2.7.3, tribunal superior de la causa, relación directa, introducción oportuna de la CF); **~95/429** del cajón `INADMISIBLE_SIN_CAUSAL_EXPLICITA` con señal canónica (depósito 43 · tribunal superior 19 · relación directa 15 · CF oportuna 13 · per saltum 6). El tratado VALIDA la polisemia de caducidad (§2.6.1 hace lugar / §2.6.2 rechaza) y confirma el rótulo `CUESTION_ABSTRACTA` (término de la Secretaría, no cambia). **M26 paso 3 scopeado 100%, sin decisiones abiertas, listo para implementación** (cirugía deriver + mover causa fuera del parser + B133 + regression + re-sello). // **Previo H146 —** **M26 Fase 2 — canal admisibilidad construido (`clasificador_admision.py` v0.1) + decisión de schema: eje PURO, vía TRANSVERSAL.** `admisibilidad ∈ {admite, inadmite, no_aplica, sin_marcador}` = SOLO la decisión de acceso; la vía (queja / REX directo / ordinario) NO se fusiona en el valor — vive en `es_queja`/`via_recurso`/`is_originaria`. **Revisa §3/§4 del plan M26** (que tenía `admite_queja`/`inadmite_queja`/`admite_rex`/`inadmite_rex` fusionados): HM-01 (dos vías de acceso, el aporte vs SCDB) se lee como el cruce `admisibilidad × es_queja`, NO como valores compuestos. **Hallazgos en disco (5890):** (1) admitir la queja IMPLICA resolver el REX — de las 1485 `admite_queja`, solo **193 (13%)** enuncian "procedente el REX" explícito, pero **1235/1292** de las restantes resuelven al fondo → la queja es modo de acceso, no disposición; (2) la queja es transversal a la vía-tipo (98,5% REX, 30 ordinario), a la admisibilidad (admite 1598 / inadmite 624) y al mérito (revoca+deja 1412 / confirma 106). **`clasificador_admision.py` v0.1 (NUEVO):** fuente única, reusa `norm`, mismo patrón que `clasificador_via`. Cascada con precedencia queja > REX > texto(object-aware) > originaria > implícito. **Object-aware acotado** (`RE_ADMITE_REX_TXT`): recupera "se declara admisible el recurso extraordinario / el recurso de queja" y "hace lugar al recurso extraordinario" del `sin_marcador` (objeto ∈ {REX, queja}, NO reposición/demanda/cautelar); `\b` antes de `admisible` → **0 falsos** en "inadmisible el REX" (17 casos). **`derivar_recursos.py` v0.3→v0.4:** agrega columna `admisibilidad`. **ADITIVO PURO** verificado en disco: 0 diffs en las 7 columnas existentes vs `recursos.csv` previo. **Distribución (5890):** admite **2896** / sin_marcador **1450** / inadmite **1020** / no_aplica **524**. Reproducido exacto en disco de Guillermo. **Coherencia con gate humano (gold n300):** 135/135 `cod_es_revision_fondo=si` son `admite` (acceso ⟹ fondo); los 16 `admite ∩ gate=no` son acceso-sin-mérito legítimos (abstracto/remand/procesal). **κ de `admisibilidad` — seed construido, VERIFICACIÓN DIFERIDA:** planilla seedeada (`cod_admisibilidad` + columnas `ref_*` con las señales + 14 object-aware marcados en amarillo); review focalizado ≈ 27 filas (~4 decisiones reales: `329_p472` nulidad_concesion, `334_p376` demanda, 2 desistidas). κ pendiente de correr. **B132 NUEVO** (parser): `331_p434` "admisible el recurso de queja" no sale en `queja_resultado` — `_SYN_Q` no cubre "recurso de queja". **Parser INTACTO (v21.01)** — todo capa-deriver; `check_regresion` [CLEAN] 5/5 (no toca derivers, no requiere re-golden); `recursos.csv` +1 columna → **manifest re-sellado**. **Provenance a verificar:** `clasificador_admision.py` es nuevo → confirmar que `generar_manifiesto.py` lo selle (igual que `clasificador_disposicion`/`derivar_recursos`), si no, gap de provenance (cf. L618, derivar_materia). **Pendiente M26:** paso 2 (κ `admisibilidad`, verificación diferida); paso 3 (`outcome`→legacy + re-cablear `causa_inadmisibilidad`/votos); paso 4 (re-golden full + todos los κ + camelCase + republicar Dataverse). // **Previo H145 —** **M26 Fase 2 — rewiring del gate CERRADO (diseño B) + B129 CERRADO.** El gate `es_revision_fondo` deja de ser la copia perezosa de `is_merit_decision` y pasa a derivarse de `caseDisposition` + guards: nuevo `es_revision_fondo(disp, por_ello, is_originaria)` en `clasificador_disposicion` v1.07→v1.08 (competencia/inoficioso DISPOSITIVOS = procedimiento; `RE_DISP_COMPETENCIA` verbatim parser L488, `RE_DISP_INOFICIOSO` verbatim L497 + lookahead B129). **Diseño B (guard en el GATE, no en el verbo):** `disposicion()` INTACTO → `caseDisposition`=verbo, κ disposición 0,912 sin cambio, gold sin tocar (re-map descartado = trampa/contaminación; el error humano de codificación es la señal, no se borra). `derivar_recursos` v0.2→v0.3 (rewiring: borrada `out["es_revision_fondo"] = df["is_merit_decision"].map(...)`). **Re-golden n300 sobre texto canónico:** gate **0,933→0,946** (supera al publicado), disposición **0,912** (sin cambio), parte 0,644; sanity `disposicion(sidecar)==recursos.csv` 0/5890. **Corpus:** `es_revision_fondo` 234 celdas cambian, `is_merit` **2870→2816**. **B129 CERRADO** (lookahead absorbido por el guard, no parser). **Hallazgos del camino (REE):** (1) el `2886/+16` del prompt H144 era A/B **pre-B131 stale** → vivo 2856/−14 → con guards 2816/−54 (el gate nuevo es más chico Y más preciso que el publicado); (2) el gate-gap NO era OBJ-vocab (1/9) sino **guards de competencia/inoficioso faltantes** (6/9 FP) que el de-interleave perdió al mudar el gate de `outcome`→`disposicion`; (3) el port verbatim de inoficioso re-rompía B129 (46 FP corpus) → lookahead rescató 24 asides-dictamen; (4) diseño A (guard dentro de `disposicion`) regresaba disposición a 0,901 → diseño B lo evita. **Parser INTACTO (v21.01)** — todo capa-deriver, `check_regresion` [CLEAN] esperado. **Pendiente M26:** `clasificador_admision.py` (canal admisión); `outcome`→legacy + re-cablear; rename camelCase + republicar Dataverse. // **Previo H143 —** **M26 Fase 2 — B131 CERRADO + esquema de nombres del refactor LOCKEADO + eyeballs §4.2 cerrados en disco.** **B131 CERRADO** (commit `a890d92`, `clasificador_disposicion` v1.06→v1.07): `RE_NULIDAD_CONCESION` reusado VERBATIM del parser L470 (B119) como pre-cascada en `disposicion()` → **31 casos `nulidad_concesion`** (22 nulidad + 8 deja + 1 no_fondo), **30 salen de fondo** (2892→2862), `parte_ganadora` de esos 30 `recurrente_gana`→`no_aplica`. Validado vs gold n300: los 4 tocados tienen `es_revision_fondo=no` → **0 regresión**; **κ-gate del de-interleave: sin B131 0,887 → con B131 0,906 (+0,019)** — LATENTE (B131 cambia `caseDisposition`/`parte`, NO `es_revision_fondo` aún: la copia perezosa sigue hasta el rewiring). Regenerado `csjn_casos_recursos.csv` en disco de Guillermo, números reproducidos exacto. **A/B Fase 1 REPRODUCIDO EN DISCO** (corrido `ab_deinterleave_fase1.py`, que nunca había corrido — el 175/153/+22 del prompt H143 era virtual hasta ahora; confirmado base 175/153/+22, +grant +32, +grant+guard +16, eje coarse fondo 2886/proc 2458/orig 546, admisión exacto §3). **Bug del gate confirmado:** `es_revision_fondo == is_merit_decision` 5890/5890 (copia perezosa). **Eyeballs §4.2 (18 deriver-FN) CERRADOS sobre texto real + clasificador real:** 5 FN genuinos (todos **OBJ-vocab**: «lo decidido»/«reajuste de haberes»/«declaración de inconstitucionalidad»/«aplicación» + 1 de distancia W=55 en 338_p234), ~12 remociones correctas (nulidad procesal de actuaciones, excusación, providencia, acordada, cautelar, competencia), 1 borde (338_p284 nulidad de decreto). Causa = OBJ fuera de vocab (confirma cierre H139), NO «shadowing de admisibilidad» (esa premisa = H132, refutada en H139). **Refinamiento:** OBJ-vocab pesa también en el **gate** (no solo en `parte_ganadora` como decía H139/+0,1pp): es parte del gap residual 0,906 vs 0,933 del de-interleave. **ESQUEMA DE NOMBRES M26 LOCKEADO (camelCase SCDB en la cara de datos, snake_case en código Python, valores en español + crosswalk SCDB al CODEBOOK):** `caseDisposition`←`disposicion` (token SCDB exacto: revoca=reversed, confirma=affirmed, deja_sin_efecto=vacated, reenvía=remanded); `isMerit`←`is_merit_decision`/`es_revision_fondo` (bool total, rollup de caseDisposition∈fondo); `admissibility` (multiclass NUEVO, con valor `no_aplica` explícito); `partyWinning`←`parte_ganadora`. **`isAdmissible` NO va como booleano forzado:** es predicado PARCIAL (1950 casos `sin_marcador` = sin gate, no son ni admisibles ni inadmisibles; un bool los mandaría a false=inadmisible, mezclándolos con los 1020 inadmitidos reales y rompiendo HM-01) → el eje de acceso es el multiclass `admissibility` con `no_aplica`; bool solo NA-aware si se necesita. **Perillas LOCKEADAS: dentro + guard originaria → is_merit 2886 (neto +16).** **Decisión OBJ (A vs B): se hace A (arreglar los 5 OBJ) pero como commit SEPARADO, gold-validado, DESPUÉS del rewiring — no fundido (atomicidad: gate se valida por A/B, detector por κ).** **Aclaración metodológica clave (para el κ): reabrir κ NO obliga a re-codear 300 a mano.** El gold es verdad humana independiente del parser; reabrir κ = recomparar salida nueva vs los MISMOS 300. Gate/disposición/parte ya codeadas → recompute gratis. `admissibility` (faltante) = +1 columna seedeable del `queja_resultado`/`outcome` sobre el `por_ello_text` que YA está en el gold (no re-extraer). B131 split nulidad = re-mapear 2 filas. 300 frescos SOLO si se quiere held-out limpio post-tuning (M20-b), evitable con split dev/test. **Sin cambios al parser.py** (solo `clasificador_disposicion` + `csjn_casos_recursos.csv`); `check_regresion` esperado [CLEAN] (parser intacto); manifest a re-sellar (cambió `clasificador_disposicion` v + recursos.csv hash). **Handoff:** `PROMPT_H144_m26_fase2_cont.md`. **Previo H142 —** **M26 Fase 1 — A/B del de-interleave admisión/mérito CORRIDO EN DISCO (5890), reproducido en el repo canónico.** Sesión diagnóstica — **sin cambios canónicos** (parser v21.01, `clasificador_disposicion` v1.06, golden, `_manifest.json` [CLEAN] 63 sin tocar; manifest/CHANGELOG no se disparan). Script: `scripts/diagnostico/H142/ab_deinterleave_fase1.py` (v2, anclado a la raíz, importa el clasificador de `scripts/pipeline`). **Bug de §9 CONFIRMADO en disco:** `es_revision_fondo == is_merit_decision` en **5890/5890** — el gate es la copia perezosa del flag del parser (`out["es_revision_fondo"] = df["is_merit_decision"].map(...)` en `derivar_recursos`, línea a borrar). **A/B (is_merit nuevo = `disposicion ∈ fondo`):** crosstab viejo×nuevo reproduce **175 gana / 153 pierde / neto +22** (base; idéntico al diagnóstico H141, ahora corrido full y reproducido en disco de Guillermo). Sensibilidad de 2 perillas: +grant_remand → +32; +grant_remand+guard originaria → **+16** (RECOMENDADA, la más conservadora: `es_revision_fondo`=*revisión* y originaria no es revisión). Los **153 perdidos = 88% `hace_lugar`(99)+`procedente`(36)** = admisión pura (la conflación, cuantificada). Los **175 ganados** incluyen 49 `abstracto` (**los 25 de B129 se recuperan ESTRUCTURALMENTE por el de-interleave, sin el parche lookahead**) + 30 `nulidad_concesion` (hotspot B131) + 16 originaria∩fondo. **Doctrina admisión/mérito (LOCKED por Guillermo):** `procedente`/`admisible` = SIEMPRE admisibilidad (canal admisión), NUNCA merit por sí, no indican ganador; el mérito y el ganador = el **VERBO** (`disposicion ∈ {revoca, deja_sin_efecto, confirma, modifica, nulidad}`); `hace lugar al recurso` = recurrente gana (fondo, trae el verbo atrás); `hace lugar a la queja` = admisión. Tests en disco: `procedente` 759 reparte **85% gana / 11% confirma** (admisibilidad orthogonal al ganador, CONFIRMADO); hipótesis `es_queja` para discriminar los 81 `hace_lugar`+`confirma` REFUTADA (ambos 93% queja → el objeto solo se ve en el por_ello). **Hallazgo arquitectónico:** el canal admisión YA está medio construido — `admisible` vive en `QUEJA_RESULTADO_PATTERNS` (parser l.766+, campo `queja_resultado`) = gate de admisión de la queja; el comentario de parser l.392-394 trae la fórmula canónica entera ("admisible la queja, procedente el REX y se deja sin efecto" = capa admisión + verbo de mérito). `clasificador_admision` consolida `queja_resultado` (2080: admite_queja 1485 + inadmite 595) + `procedente`/inadmisibles del outcome (990). **HM-01 materializado:** dos gates (admisión de la queja + procedencia del REX) que el corpus tiene adentro = el aporte vs SCDB; `procedente` 9% queja (REX concedido) vs `hace_lugar` 93% queja (recurso de hecho) = dos vías de acceso (conecta con H1, certiorari criollo). **Eje coarse:** fondo 2886 / procedimiento 2458 / originaria 546; `competencia` bifurca (originaria si `is_originaria`, si no procedimiento — Q2 RESUELTO). **Decisiones de borde:** grant_remand → dentro de fondo (recurrente_gana, lo confirma `parte_ganadora_regla`); guard originaria → se queda ("salvo originaria"); ambas pendientes de lock formal. **Pendientes Fase 2 (requieren `csjn_casos_textos.csv`):** object-split de ~57 bare `no_fondo` + `rechaza` (al recurso vs a la queja/demanda/cautelar) para `RE_GRANT_RECURSO`; ~17 deriver-FN (outcome=verbo pero `disposicion≠fondo` = OBJ-vocab/truncado); 30 `nulidad_concesion`→fondo (B131); re-verificar los 25 de B129; value-set de `admision` (~1047 a clasificar + `competencia` 903 al coarse). **Handoff Fase 2 (cirugía del parser):** `PROMPT_H143_cirugia_parser.md`. **Previo H141 —** **Refactor admisión/mérito DISEÑADO (M26): de-interleave de `outcome` en dos canales canónicos.** Sesión de diseño/diagnóstico — **sin cambios canónicos** (parser v21.01, golden, `_manifest.json` [CLEAN] 63 sin tocar). Arrancó en **B129** (falso inoficioso): caso `334_p1272` (salvataje hipotecario, leyes 25.798/26.167), `RE_DISP_INOFICIOSO` alt-2 (`result\w+ inoficioso`) matchea "resultando inoficioso que dictamine el PGN" → `abstracto` antes de que la cascada vea "se revocan" → `is_merit=0`. Es aside del dictamen, no mootness de fondo; fórmula endémica CSJN. **A/B validado** (`scripts/diagnostico/H141/ab_b129_inoficioso.py`, lookahead `(?![^.]{0,40}(?:dictamin|procurador))`): **25 flips** `abstracto`→`{revoca 5, procedente 16, confirma 2, hace_lugar 1, nulidad 1}`, **0 regresiones**, los 25 ∈ MERIT, `is_originaria=0`. Fix validada pero **diferida** (el bug escaló a refactor); disponible como commit standalone. **Hallazgo que escaló:** el deriver ya tiene la señal — 24/25 con `disposicion=FONDO` ∧ `es_revision_fondo=no`; corpus-wide **175** `disposicion=FONDO`∧`gate=no`, **153** `gate=sí`∧`disposicion≠FONDO`, `procedente` total **759**. Causa: `outcome` es plano y conflaciona admisión (procedente/280/desestima) + mérito (revoca/confirma), e `is_merit` cuelga de él. **Decisión (LOCKED):** separar en dos canales canónicos del parser — `admision` (multiclass) + `disposicion` (multiclass FINO) + eje COARSE `{fondo, procedimiento, originaria}` como rollup que define `is_merit = disposicion ∈ fondo` (fondo = revoca/deja_sin_efecto/confirma/modifica/nulidad sustantiva). Reusa la detección existente (REE, no reinventa). **Taxonomía (LOCKED) — corte de nulidad:** nulidad del **auto de concesión** = procedimiento; nulidad de la **sentencia / de todo lo actuado** = fondo. `disposicion` fino conserva competencia/nulidad_concesion/aclaratoria (competencia "en sí es distinto"); coarse para clasificar, fino para analizar. **B131 NUEVO:** el pattern `nulidad` de `clasificador_disposicion` (OBJ incluye `auto`) podría colar "nulidad del auto de concesión" como fondo → verificar precedencia de `nulidad_concesion`. **Schema:** `disposicion`/`admision` multiclass canónicos (LOCKED); `outcome` → deprecar+reemplazar, opción A (corte duro, rompe Dataverse) vs **B (blanda, derivada legacy, rec.)** ABIERTA; `clasificador_admision.py` a construir (consolida `RE_GRANT` admite + cascade 280/ac4/desestima inadmite, fuente única). **HM-01 NUEVO (metodológico):** el gate de admisibilidad es el aporte específico del corpus frente al SCDB (que trabaja post-gate) — el refactor es su expresión arquitectónica. **Plan multi-sesión (3–5, cortes naturales):** (1) A/B en disco del de-interleave [← PRÓXIMA, gate REE]; (2) cirugía parser + `clasificador_admision` + desambiguar nulidad/nulidad_concesion (B131) + `outcome`→derivado; (3) re-cablear `causa_inadmisibilidad`/votos a canales + simplificar `derivar_recursos` + M24; (4) re-golden consciente + recomputar TODOS los κ + re-sello manifest; (5) CODEBOOK + republicar Dataverse + HM-01. **κ se reabre** (gate 0,933 in-sample / 0,813 held-out; disposición 0,912; parte 0,653). **Handoff:** `PROMPT_H142_refactor_admision_merito.md`. **Pendientes vigentes (H141):** M26 Fase 1 (A/B) al tope; outcome A-vs-B a confirmar; value-set de `admision` vs `causa_inadmisibilidad`; ubicación de `competencia`; + heredados (M19 doble-codificación + CODEBOOK Reliability; M20-b muestra fresca gate; B130 negativos reenvía; M21 Fase 2 persist-sidecar; M24; B126; B111; M25; triar cola larga B014–B099). **Previo H140 —** **κ del gate LIMPIO (held-out de facto) — cierre de la única deuda de leakage del set M20.** El único κ contaminado de H139 era el **gate**: B119 (H121) se tuneó sobre el n300 → 0,933 in-sample. Recomputado con la predicción PRE-B119 (`is_merit_decision` del `csjn_casos.csv` en commit `d856318` "checkpoint pre-B119", recuperado del **blob de git** sin re-correr el parser + regla canónica copiada de `derivar_recursos` `is_merit=='1'→'si'`; sanity **0 mismatches/5890** vs `recursos.csv` actual; harness `kappa_confiabilidad.py`): **κ(gate) = 0,813 [0,741–0,873], acuerdo 0,907, "casi perfecto" (Landis-Koch) — held-out de facto** (el detector pre-B119 nunca vio el n300 como dev). El acuerdo 0,907 reproduce exacto el número limpio pre-B119 del ledger H122 (confirmación por 2ª vía independiente); el κ POST 0,933 reproduce el harness oficial de H139 (check de maquinaria). Delta de leakage **+0,120** (20 flips PRE→hoy = 14 de B119 + ~6 de evolución posterior del gate, M21/B019/disp v1.0x). **Ledger confirmado, único contaminado = gate:** resto del set H139 limpio (disposición/parte/reenvía regex congelada pre-gold; cf/dictamen fix nunca aplicado; materia v3.2 refinamientos nunca aplicados; vía deriva de señales pre-gold + per saltum revertido). **Corrección de premisa:** la sospecha de partida (refacciones de cf/dictamen/materia codeadas a ciego contaminaban su κ) quedó refutada — esos fixes se diagnosticaron pero **nunca se aplicaron**; la codificación a ciego fue genuinamente independiente del parser. **Decisión de reporte: (a)** reportar **0,813 como número honesto/conservador** + ledger de contaminación declarado abiertamente; **(b) DEUDA NUEVA (M20):** el gate FINAL (post-B119, el del dataset publicado) no tiene validación limpia sobre el n300 (fue su dev) → validación in-distribution requiere **muestra fresca a ciego (otros ~300, fuera del gold actual)**, conecta con M19. Script nuevo `scripts/diagnostico/H140/kappa_gate_preB119.py` (diagnóstico, no toca pipeline). **Sin cambios canónicos** (parser v21.01, golden, `_manifest.json` sin tocar; manifest [CLEAN] 63). **Pendientes vigentes (H140):** opción 2 = κ de cf/dictamen/materia (predicción del parser en `csjn_casos.csv`/`csjn_casos_materia.csv`, gold ya normalizado, leakage DESCARTADO → habilitada); recodificar negativos reenvía (B130); muestra fresca para gate in-distribution (M20-b); doble-codificación κ + sección Reliability del CODEBOOK (M19); + heredados (M21 Fase 2 persist-sidecar; M24 check_regresion general; B129; B126; B111; banner del considerando para materia); **M25 detector real de `parte_ganadora`** (hoy derivada de disposición, techo 0,889; banco 15 casos = 8 parcial + 7 inversiones de rol); **TRIAR la cola larga sin triar** (B014–B099 / M0x / VIS / A0x — entradas viejas sin marcador de estado, línea ~177; escalón 2 del reordenamiento de DEUDA) + colas de refactor sueltas (B109: `342_p1017`/`331_p530`/`339_p597`; cola de causales de B100). **Previo H139 —** **κ(parser↔gold) CERRADO en disco (M19 parcial) + B127-bifásico CERRADO (premisa refutada) + `RE_RUNNING_HEAD` case-sensitive (precondición materia, no-op).** **κ — nuevo `scripts/diagnostico/kappa_confiabilidad.py` v1.0** (Cohen's κ + IC bootstrap percentil 5000, parametrizado `--gold/--recursos/--out`). Corrido en disco contra el gold rebuild canónico (`scripts/diagnostico/H120/planilla_M20_LIMPIA_n300__rebuild.xlsx`, 300 casos limpios) + `csjn_casos_recursos.csv`; output `output/validacion/kappa_resultados.csv`. Resultados: **es_revision_fondo 0,933 [0,887–0,967]**, **vía 0,941 [0,842–1,0]**, **disposición 0,912 [0,847–0,966]** (subset fondo n=140), **parte_ganadora 0,653 [0,504–0,783]**, **reenvía 0,408 [0,16–0,635] NO REPORTABLE**. Sandbox = disco (idéntico) → cierre REE. Es κ(parser↔gold) = valida el PIPELINE, **NO** doble-codificación (reproducibilidad del codebook), que sigue pendiente (M19). **Gold rebuild = canónico limpio:** el a-mano (`planilla_M20_codificar-56xlsx.xlsx`) tenía 2 filas de totales (caso_id nulo), `cod_materia` duplicada, mayúsc/typos en materia/cf/dictamen; el rebuild (67472 bytes) limpia eso, agrega `cod_cf_norm`/`cod_dictamen_norm` (deja el crudo), **0/300 cambios de etiqueta** vs a-mano → independiente, NO contaminado. Las copias en `Downloads` (56019 bytes) son viejas, NO usar. **B130 NUEVO — reenvía: clase negativa de codificación ruidosa** (69 si / 6 no; ambiguos en blanco, algunos `no` sin certeza): κ 0,408 no reportable (base-rate + ruido + n=75); recodificar negativos explícitos con válvula `'-'`. **B127-bifásico CERRADO H139 — PREMISA REFUTADA:** leído `disposicion()`, hace `pat.search(pe)` **GLOBAL** sobre todo el `por_ello` (no "matchea el primer verbo"); la admisibilidad no está en vocab DISP → no bloquea, el verbo de fondo se captura igual. Medido: de **1935** bifásicos, **1917 capturados como fondo**; los **13** en `no_fondo` NO caen por el bifásico (truncado/banner, OCR/typo, OBJ fuera de vocab, plural `-es`). No hay salto que diseñar; el recuperable real (~+0,1pp sobre `parte_ganadora`) es OBJ-vocab + M21, NO B127. **`RE_RUNNING_HEAD` case-sensitive:** sacado `re.I` (banner SIEMPRE mayúsculas; "Corte Suprema" en mixta = cuerpo legítimo). Sincronizado `parser.py` (L218) + `clasificador_disposicion.py`. **parser v21.0→v21.01**, **clasificador_disposicion v1.05→v1.06**. Verificado **NO-OP** (disposición 0/5890 cambios; 467/467 banners mayúsc en `por_ello`, 0 escapados). `check_regresion` **[CLEAN]** + manifest **[CLEAN] 63**. COMMITEADO. **No arregla nada actual — es PRECONDICIÓN** para limpiar el banner del considerando para materia (frente futuro: el considerando persistido tiene 13275 banners crudos que `derivar_materia` capa2 traga como ruido). `RE_HEADER` (por_ello_cortado) NO tocado (ya 0). **Pendientes vigentes (H139):** doble-codificación κ + sección Reliability del CODEBOOK (M19); recodificar negativos reenvía (B130); κ de cf/dictamen/materia (falta la predicción del parser de esas 3 — están en otros CSV; gold ya normalizado); + heredados de H138 (M21 Fase 2 persist-sidecar; M24 check_regresion general; B129; B126; B111; materia: limpiar banner del considerando; split dev/test). **Previo H138 —** **M21 Fase 2 corrida y validada en disco (parcial) + B127/banner CERRADO en capa-deriver + manifest sella recursos+secciones.** **parser v21.0 (M21 Fase 2) — CORRIDO Y MEDIDO en disco; recuperación PARCIAL.** El masking de `RE_RUNNING_HEAD` en `_barrer` limpia la vista de *matching* (candidate_text), pero el `por_ello_text` que se PERSISTE al sidecar sale del CRUDO, no del chunk enmascarado. Medido: `validar_m21_fase2.py --casos-old backup\csjn_casos.csv` → **Gate A = 467 banners restantes (igual que pre-fix)**, Gate B = 0 flips `real→otro` → `[SUCIO]`. `check_regresion` **[CLEAN]** los 5 CSV (idénticos al golden) → v21.0 es **no-op sobre todo el output persistido**: masking integrado, pero PERSISTIR-AL-SIDECAR el chunk enmascarado queda PENDIENTE (es la mitad faltante de Fase 2). **Decisión (revierte la recomendación del asistente de revertir a v20.01):** se QUEDA v21.0 (código correcto), se documenta honesto como Fase 2 parcial — no se borra código bueno por estética de git. **B127/banner CERRADO en capa-deriver:** `clasificador_disposicion.py` **v1.03→v1.05** — agregado `RE_RUNNING_HEAD` (VERBATIM del parser L215) en `norm()`, que enmascara el banner ANTES de clasificar. Re-corrido `derivar_recursos.py` (disp v1.05): **`por_ello_cortado` 18→0**, los 4 interpolados recuperan fondo (330_p380/330_p960→`deja_sin_efecto`, 333_p1951→`revoca`, 344_p1444→`deja_sin_efecto`), fondo leída 2886→2892 (+6), **0 regresiones**; los truncados-de-verdad caen a `no_fondo` (correcto, ese es territorio del parser/M21). Solución diseñada en H136–H137: el banner INTERPOLADO se recupera en el clasificador (la línea está, solo hay que enmascararla); el TRUNCADO (verbo nunca persistido) solo en el parser. El relabel `por_ello_cortado` (v1.04) queda moot. **Manifest — recursos + secciones SELLADOS:** `generar_manifiesto.py` **v1.4→v1.5** — `derivar_recursos.py` a PIPELINE_SCRIPTS, `secciones_indices.csv`/`construir_catalogo.py` a INPUTS, `csjn_casos_recursos.csv`/`derivar_recursos.py` a OUTPUTS. `--verify [CLEAN] 63 artefactos` (eran 61). Cierra el único gap de provenance de la capa-deriver. **Re-sello final POST-commit** (árbol limpio, dirty=false) — el manifest de la sesión se generó con árbol DIRTY. **check_regresion — gap identificado (NO cerrado):** es un harness del PARSER (corre `parser.py` a temp, compara 5 CSV byte-vs-golden); NO corre los derivers. El único output de parser que le falta es `csjn_editorial_indice_partes.csv` (determinístico de `parser_editorial`, hoy solo cubierto por el manifest, no por byte-regression). **materia/recursos NO van al harness** (son clasificadores que cambian POR DISEÑO — recursos cambió hoy con v1.05; su régimen es accuracy-gold + manifest, no byte-regression). → **M24 NUEVO.** **Estado de cierre H138:** commits — clasificador v1.05 + generar_manifiesto v1.5 + regen recursos + parser v21.0 (SE COMMITEA, no revert) + diagnósticos H136–H138; `backup\` NO se commitea (temporal). Manifest re-sellado POST-commit. **Pendiente próxima sesión:** (1) completar M21 Fase 2 = persistir el chunk enmascarado al `por_ello_text` del sidecar (para que Gate A baje de 467 y el output canónico cambie) → re-golden consciente + re-sello; (2) M24 (check_regresion general); (3) B127-bifásico (admisibilidad), distinto del banner ya cerrado. **Previo H136–H137** — B127 diagnosticado (alcance capa-deriver) + M21 Fase 2 diagnosticada y parser v21.0 PATCHEADO pero NO corrido/validado (cerró deliberadamente solo-docs). El `RE_RUNNING_HEAD` (terna 'número FALLOS DE LA CORTE SUPREMA número', 0 FP sobre 'Corte Suprema de Justicia de la Nación') se diseñó y verificó en sandbox sobre el sidecar (463 banners / 0 FP); H138 lo corrió en disco. **B129 NUEVO:** 334_p1272 — revocación de fondo (salvataje hipotecario, leyes 25.798/26.167) marcada `es_revision_fondo=no`/`is_merit=0`/`outcome=abstracto` (no diseñado). Lección REE: fuente > parche; reusar > reinventar; pedir el archivo > adivinar; cerrar con dato en disco. **Previo H135** — merge `refactor/m20-disposicion`→main (FF, parser v20.01, harness [CLEAN] 5/5). Persistencia tardía de H132 (commit `964359c`: `derivar_recursos` v0.2 + `clasificador_via` v0.1 + regen recursos). **B128 CERRADO** (commit `0333379`): `extraer_caso.py` v2.1→v2.3. **Previo H134** — diagnóstica: recalibración `tipo_cuestion_federal` + seed `causal_arbitrariedad` (B111, weak-supervision: voz=LF alta precisión / dictamen=LF alto recall). **Previo H132** — capa-deriver disposición/vía construida (parser intacto): `clasificador_disposicion` v1.02, `clasificador_via` v0.1, `derivar_recursos` v0.2; bucket `no_fondo` (1860=31,6%) diagnosticado taxonómico (gold concuerda 88/89); B127 + M23 NUEVOS. **Previo H130** — B124 CERRADO (regla P `RE_PERF v2` en `_barrer`, parser v20.0): outcome +121 recup / 0 regresiones, re-golden [CLEAN], manifest 61 artefactos; B126 ABIERTO (firmas partidas). **Previo H126** — M21 Fase 1 (skip de vacías en `_barrer`) integrada, parser v18.26→v19.0, B122/B118 CERRADOS. **M20 — validación formal CERRADA:** gate 0,953 in-sample / 0,907 held-out pre-B119; blind: disposición 0,857, parte 0,794, reenvía 0,773; cuestión federal 0,676, dictamen 0,930; certiorari criollo queja 95% vs concedido 68% (H1). **Pendientes vigentes:** completar M21 Fase 2 (persist sidecar); M24 (check_regresion general); B127-bifásico; B126 (firmas partidas); B129; escrutinio `otro` residual (B123); B120 (banco=166); `derivar_dictamen.py`; fix B111; split dev/test (solo tuneo futuro); CODEBOOK materia; refinamiento CA; test-retest/kappa (en pausa).)
...
| B126 | extractor de firma dropea nombres partidos por salto de línea del OCR (testigo 332_p663 Salas) | abierto — **próximo frente** (B124 lo expone; necesita scan corpus-wide de firmas partidas antes de fixear) |
...
### M21 — Recuperación del `por_ello` truncado: skip en `_barrer` (Fase 1) + masking del banner (Fase 2) — FASE 1 INTEGRADA H126
...
### M25 — `parte_ganadora`: detector real del texto en vez de derivarla de la disposición
...
### B135 — `is_originaria` SUBDETECTA: señal de demanda originaria disponible pero no usada en el gate
