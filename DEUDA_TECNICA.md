...
**Estado del fix:** **validado en PoC (H125), NO integrado.** Integración = **H126**: skip en `_barrer` primero (dependency-free, +50, no mueve índices ni la ventana de firma), masking después (gated en bug de terna + fuente de offsets + threading crudo-vs-limpio). Banco: los 42. Cierre: A/B + check_regresion column-aware direccional + bump parser MAJOR. Perilla `guion` DROP (redundante, downstream ya deshifena).

### M21 — Recuperación del `por_ello` truncado: skip en `_barrer` + masking del banner — VALIDADO EN PoC H125
...
**Estado del fix:** **validado corpus-wide en PoC (H125, en disco), NO integrado.** Dos mitades: (1) skip en `_barrer` — el lever, dependency-free, +50, no mueve índices/`por_ello_idx`/ventana de firma; (2) masking `normalizar_bloque` — secundario, +14, detección por `mapa_paginas` (no regex), con preguntas abiertas (bug de terna orden tomo/página/frase; fuente de offsets al parsear vía `headers_archivo`; threading crudo-vs-limpio por campo, lección B114). Integración = **H126**, skip primero, masking gated. PoC y módulos en `scripts/diagnostico/h125/`.
